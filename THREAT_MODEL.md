...
> **Status:** Living document. Last reviewed 2026-06-04 against the 1.0.x line.
...
| G6 | **Documentation accuracy.** Two public claims were inaccurate and are corrected as part of the milestone that introduced this document: `SECURITY.md` described skills running in `isolated-vm` (the real mechanism is OS-level `bwrap`/`sandbox-exec`), and the `README` implied "no external SDK dependency" (the agent runtime is built on `@earendil-works/pi-coding-agent`). This file is the corrected source of truth; a CI guard (`security-doc-claims.test.ts`) now prevents regressions. | — | Fixed in the same change set as this doc. |
