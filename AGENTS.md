...
- Reusable task expertise, procedures, examples, and tool-selection guidance belong in an opt-in prompt skill. If the expertise should ship with Comis, add a repository-shipped skill (§6.5 owns which tree actually ships); do not inject it into the engine kernel or default workspace starters.
...
- **PR descriptions**: start from `.github/PULL_REQUEST_TEMPLATE.md` and retain its exact required H2 headings. Generated `Intent`, `What Changed`, `Risk Assessment`, `Testing`, or `Pipeline` reports are optional additions, never replacements for the canonical sections. For `packages/*/src/**` changes, `## RED Test Proof` must contain the pre-patch failing test name and assertion output; commit ordering or a later green run is not a substitute.
