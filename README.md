...
<p align="center">
  ...
  <br />
  <sub>Multiple agents, multiple people, one auditable install, with security enforced by OS isolation, typed boundaries, and tests, not by prompts.</sub>
</p>
...
Verify with `curl http://127.0.0.1:4766/health`, which returns JSON containing `"status":"ok"`, then message your agent on any connected channel.

> **Production runs on Linux.** Default Docker Desktop/dev container settings cannot run Bubblewrap's exec sandbox, so Comis detects this at startup (one-shot smoke test) and logs/auto-disables exec sandboxing. Fine for development, not for production without the documented Linux/security-opt setup. [Platform support](https://docs.comis.ai/operations/docker#platform-support)
...
| Exec sandbox                   | Configured **on by default**; kernel-enforced where available (Bubblewrap / sandbox-exec) | Docker sandbox, opt-in (`off` by default)                     | Host-first by default, containers confine the shell tool, not the agent   |
| Secrets at rest                | AES-256-GCM encrypted store                                                              | Plaintext config/auth profiles supported; opt-in SecretRefs   | Plaintext `.env` + file permissions (Bitwarden opt-in)                    |
...
| Supply-chain integrity         | npm provenance, SHA-pinned actions/images, exact-pinned + bundled deps                   | npm provenance, SHA-pinned images                             | Sigstore-signed releases, exact-pinned deps                               |
...
| 🌐 **Any model**         | 35 catalog providers via pi-ai (Anthropic, OpenAI, Google, Groq, OpenRouter, Bedrock, Azure, and more), local Ollama/LM Studio, and any OpenAI-compatible endpoint. Tool schemas adapt per model, an immutable `ModelProfile` scales both a reliability scaffold and a security lockdown to it, fallback chains and key rotation included |
| 🖥️ **Operations**        | Web dashboard (live ops views, visual pipeline builder, approval queue), JSON-RPC + WebSocket + OpenAI-compatible APIs, ACP server/bridge primitives for IDE integrations, cron + heartbeat monitoring, `comis security audit` (14 checks), trace CLI with forensic bundle export, git-backed config history & rollback |
