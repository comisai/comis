...
| **Memory that learns, not just remembers** | Comis is **outcome-gated**: it knows which turns actually *worked*, then reflects repeated successes into named, reusable playbooks (**Mental Models** — skill / user-profile / topic) that it surfaces and reuses, so the agent gets better at recurring tasks instead of relearning each time. Anti-poison by design — a doc needs **≥2 independent sources** to be learned, learning can **never raise its own trust**, and poisoned memory is **removable**. Under the hood: SQLite + FTS5 + vectors, local storage, [reproducible benchmark manifests](benchmarks/results/). |
...
| **Learning memory** | Long-term memory that improves with use: cross-session fact recall (hybrid vector + FTS), plus an **outcome-gated reflection engine** that distills corroborated successes into reusable **Mental Model** docs (skill / user-profile / topic), surfaces them into the agent's prompt, and promotes them on *measured* reuse. Trust-aware, anti-poison, local SQLite-backed. |
...
| Memory | Outcome-gated reflection + reuse loop, trust-aware, anti-poison, local, benchmarked | No trust levels | No trust levels; learning loop not publicly benchmarked |
