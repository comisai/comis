...
**🏠 Any model — without lowering the security floor.** Every model resolves once into an immutable **Model Capability Profile** ([`model-profile`](packages/agent/src/executor/model-profile.ts)) with two independent axes. Context *capacity* drives the token budget — handling both 8K starvation and the 256K-overfill case, where feeding a huge window to a small model degrades it ([`budget-capacity-cap`](packages/agent/src/context-engine/budget-capacity-cap.ts)). Executive *capability* (`frontier | mid | small | nano`) drives scaffold intensity **and security posture** — a weaker model gets a *more* locked-down platform, never a softer one: a `compact-secure` prompt mode pins security invariants through compaction ([`security-context-pinner`](packages/agent/src/context-engine/security-context-pinner.ts)), and compaction itself routes by capability class ([`compaction-capability-router`](packages/agent/src/context-engine/compaction-capability-router.ts)). Swap in Qwen or Gemma fully on-device: your data never leaves the box, and the guarantees stay properties of the platform — never of the model's goodwill.
