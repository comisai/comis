...
**🧠 Memory — recalls the right thing, and stays right over time.** Comis doesn't just store facts and hope keyword or vector search finds them. An optional relevance reranker can re-read the candidate memories against the actual question and surface the ones that truly answer it — not just the ones that look similar. An optional entity lane connects related memories, so asking about a person, project, or place brings back what's tied to them, however it was worded. A read-time temporal model resolves contradictions by recency — when a fact changes, the agent answers with what's *currently* true and orders timelines by when events happened, **without ever deleting the old record**. Repeated facts can consolidate into corroborated **observations** (carrying an evidence count), so the store gets *smarter*, not just bigger. And every memory keeps its trust level (`system` / `learned` / `external`): a web snippet can never quietly override a verified fact — recall is trust-ranked and hardened against memory-poisoning. It all runs **locally**, on the same on-device model engine Comis already uses — no extra service, no cloud calls, no new dependency. And when recall surprises you, it's inspectable: `comis memory recall-trace` (plus an opt-in JSONL trace) shows exactly *why* each memory was chosen — which lanes matched, the rerank and trust/recency scores, what was filtered out — with the same edge-keeping redaction as the rest of Comis's observability, and no silent fallbacks. [Deep dive →](https://comis.ai/agents/memory)
...
| Trust-ranked, relevance-reranked recall (on-device)      |    ✅    |        ❌        |   ❌   |    ❌    |   ❌   |
