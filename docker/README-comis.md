...
|---------|---------|-------|
| `{version}` | `1.0.49` | Immutable — pin this in production |
| `{major}.{minor}` | `1.0` | Tracks the latest patch |
| `latest` | `latest` | Default variant, latest release |
| `{version}-slim` | `1.0.49-slim` | Slim variant, immutable |
| `{major}.{minor}-slim` | `1.0-slim` | Slim variant, latest patch |
| `latest-slim` | `latest-slim` | Slim variant, latest release |

> **Tip:** Pin to an immutable version tag in production (e.g. `comisai/comis:1.0.49`) rather than `latest`.
