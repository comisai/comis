...
The operator runbook for the memory benchmark **SUITE**. Every tier runs by a
single command:
...
| `recall-learning` | `suite recall-learning` | `learning-iq.bench.test.ts` | `claim{1..4}-*.json` under the resolved report dir | KEYLESS | Drives the SHIPPED usefulness-feedback loop (`recordUsage` → usefulness fold → `usefulnessNorm`) and measures four mechanical claims: per-intent bucket reorders recall, default-off byte-identity with `readUsefulness` never called, citation→FEED accrual, and tenant/agent/intent isolation. **No rank-lift number**: the rank-over-episodes harness was removed when the learning subsystem collapsed into one reflection engine, so a lift figure attributed to this tier would be fabricated. |
| `trust-contradiction` | `suite trust-contradiction` | `contradiction-harness.bench.test.ts` | `benchmarks/results/trust-contradiction/trust-contradiction-report.json` | **answer + judge** | Trust-first-correct rate: the rate at which the OLDER high-trust fact wins over a NEWER low-trust claim, over the shipped trust filter + `compareBoosted` tie-break. Feeds the KG gate. |
...
These tiers run a defensive loader over an external corpus. **Placement:** drop
...
| Dataset (tier) | Source | Filename under `$COMIS_BENCH_DATA` | Loader symbol | Vendored fixture | Keys | Note |
|----------------|--------|------------------------------------|-------------------------|------------------|------|------|
...
- **The redaction report commits ONLY the aggregate leak-rate**, never a planted secret —
  the scorer takes boolean leak flags, not secret strings.
...
- **External tiers are operator-gated:** the keyless structural proof runs by default
  ...
  (pin a dated snapshot; run ≥2 judges).
