...
- Verified files declared in a sub-agent's `expected_outputs` are delivered as governed channel attachments when the task completes, including when the parent rewrite produces no text; synthetic parent turns accept their workspace-policy hash without failing completion delivery.
