...
- Skills can now be imported by reference from operator-configured registries. Registry publisher and security metadata is retained as evidence but never bypasses local vetting or grants trust.
- `comis skills list`, `import`, `remove`, and `info` provide the installed-skill lifecycle and provenance view from the CLI. `comis system-health` now reports community-skill, registry-allowlist, and pending-MCP exposure.
- Skill import successes and security rejections now produce content-free trajectory records for `comis explain`.
