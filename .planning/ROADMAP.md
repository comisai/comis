...
Plans:
- [ ] 02-01-PLAN.md — TDD: Add `keyHex` to `MasterKeyWriteResult` in `@comis/core` (STORE-01 foundation)
- [ ] 02-02-PLAN.md — TDD: Add `seedKeyHex` fallback to `setupSecrets` in `@comis/memory` (STORE-01 same-boot)
- [ ] 02-03-PLAN.md — TDD: Wire opt-out flag + `writeMasterKeyIfAbsent` + `seedKeyHex` threading in daemon boot (STORE-01c + STORE-02)
- [ ] 02-04-PLAN.md — TDD: Add `gateway.status` pre-flight in `env_set` for fail-early store check (STORE-03)
