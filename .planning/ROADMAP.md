...
- [x] **Phase 5: Forced Egress + Secure Credential Home** - Egress spike gate, --unshare-net, ~/.claude bind removal, per-spawn proxy env (design P4) — verified PASSED 16/16 macOS scope (2026-05-30); ⚠ R1 live spike GO/NO-GO OUTSTANDING on Linux prod host (publish-gate for Phase 7 containment claims)
- [x] **Phase 6: End-to-End + Observability** - Drive Claude Code via broker, broker:* events, property-tested non-leakage (design P5) — verified PASSED 7/7 (2026-05-30); sandbox-driven sibling-exec E2E is Linux-gated (with Phase 5 R1)
...
Plans:
**Wave 1** *(independent, can run in parallel)*
- [x] 05-01-PLAN.md — Interface contracts: broker:denied reason union, SandboxOptions.network, ExecToolDeps.brokerSpawnEnv
- [x] 05-02-PLAN.md — R1 spike Linux-gated integration harness (bwrap-egress-integration.test.ts, skip-on-darwin)

**Wave 2** *(blocked on Wave 1 completion)*
- [x] 05-03-PLAN.md — TDD: bwrap secure profile (--unshare-net, secureCredentialHome, credential bind removal) (EGRESS-01/02/03)
- [x] 05-04-PLAN.md — TDD: WS upgrade fail-closed guard + additive startUnixSocket (EGRESS-04, EGRESS-01 seam)

**Wave 3** *(blocked on Wave 2 completion)*
- [x] 05-05-PLAN.md — TDD: buildExecEnv brokerSpawnEnv merge-last + pnpm validate gate (EGRESS-03)
...
**Plans**: 4 plans
**Wave 1** *(independent)*
- [x] 06-01-PLAN.md — broker-events.ts typed emit helpers + broker-events.test.ts (TDD RED/GREEN; extraction prerequisite)

**Wave 2** *(blocked on Wave 1)*
- [x] 06-02-PLAN.md — mitm-broker.ts surgery: extract inline emits + add 4 missing emit sites + agentId gap fixes (TDD; wc -l < 800)

**Wave 3** *(blocked on Wave 2, parallel)*
- [x] 06-03-PLAN.md — OBS-01 taxonomy tests + OBS-02 exhaustive 9-path sentinel property test + E2E-01 in-process (TDD)
- [x] 06-04-PLAN.md — Linux-gated E2E-01 sibling-exec key-recovery (credential-e2e.test.ts, skipIf darwin)
...
Plans:
**Wave 1**
- [ ] 07-01-PLAN.md — credential-broker.mdx (new deep-dive) + docs.json registration (DOCS-01)

**Wave 2** *(parallel — all depend on Wave 1)*
- [ ] 07-02-PLAN.md — Core doc updates: exec-sandbox, secrets, audit, config-yaml (DOCS-01)
- [ ] 07-03-PLAN.md — Secondary doc updates: defense-in-depth, get-started/security, reference/security-model, secret-manager, sandbox (DOCS-01)
- [ ] 07-04-PLAN.md — Operations docs: observability, troubleshooting (DOCS-01)
- [ ] 07-05-PLAN.md — README benefit blocks (4 sections) (DOCS-02)
- [ ] 07-06-PLAN.md — Website homepage components: Security.astro, Features.astro, Comparison.astro (DOCS-03)

**Wave 3** *(blocked on Wave 2)*
- [ ] 07-07-PLAN.md — Website pages: compare.astro, compare/openclaw.astro, security.astro + full build gate checkpoint (DOCS-03)
...
| 5. Forced Egress + Secure Credential Home | 5/5 | Complete    | 2026-05-30 |
| 6. End-to-End + Observability | 4/4 | Complete    | 2026-05-30 |
| 7. Documentation | 0/7 | Not started | - |
