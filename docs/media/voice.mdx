...
`OPENAI_API_KEY` will resolve to `auto` → the keyless **local** whisper engine (it
auto-downloads its model on the first voice note when ffmpeg is present and the host
can reach the model hub), and honest-degrades to "not available" only if neither the
local engine nor a reusable main-provider audio key can serve — **or** set
`provider: openai` explicitly to restore the previous behavior. This is intentional:
voice now works with zero credentials, and an OAuth-only main is never sent an
empty-bearer request.
</Note>
...
Voice turns appear in the existing per-session trajectory — this phase added no
new on-disk artifact or data-directory path.
