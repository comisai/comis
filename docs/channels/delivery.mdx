...
4. An idempotency key (session + a hash of the text **and** media references +
   1-second time bucket) prevents duplicates via `INSERT OR IGNORE` -- hashing
   the media references keeps two different attachments sent in the same second
   with the same caption as two entries
