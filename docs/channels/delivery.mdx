...
| Signal | 4,000 ms | ~5 s | `sendTyping` JSON-RPC call to signal-cli |
| LINE | 15,000 ms | 20 s | Uses `showLoadingAnimation` |
| iMessage | _(none)_ | _(n/a)_ | No typing API in `imsg` -- defaults to `never` |
| IRC | _(none)_ | _(n/a)_ | No typing API -- defaults to `never` |
| Slack | _(none)_ | _(n/a)_ | Slack does not expose a bot typing API -- defaults to `never` |
| Email | _(none)_ | _(n/a)_ | Polled inbox, no presence concept -- defaults to `never` |
...
|---|---|---|
| Discord | Markdown | Bold, italic, code blocks, links, lists, headers, spoilers |
| Telegram | HTML | Bold, italic, underline, strikethrough, code blocks, links, blockquotes, spoilers |
| Slack | mrkdwn | Bold, italic, strikethrough, code blocks, links, lists |
| WhatsApp | Plain text | Formatting stripped (WhatsApp does not support bot formatting) |
| Signal | Signal text styles | Bold, italic, monospace, strikethrough, spoiler |
| iMessage | Plain text | Formatting stripped |
| LINE | Flex Messages | Rich layout via Flex; otherwise plain text |
| IRC | IRC control codes | Bold, italic, color, strikethrough |
| Email | HTML | Full HTML output with linkified URLs |
...
registry.registerHook("after_delivery", (event, ctx) => {
  ...
  // event.mediaUrls: delivered media references -- present when the delivery
  //   was an attachment (`origin: "channel:attachment"`), absent for text-only
...
the delivered text is recorded to the `delivery_mirror` SQLite table. Outbound
**attachments** are mirrored the same way on every channel that implements
attachment sending: the entry carries the attachment caption as its text plus the
delivered media reference, so a caption-less photo or voice note sent during the
agent's own turn is still remembered rather than leaving a silent gap. The turn is
what supplies the conversation the entry belongs to, so an attachment delivered
outside one -- a long video render announced by the background poller after its
originating turn ended -- is not mirrored, and records nothing rather than
guessing a conversation. On the next
inbound user message,
pending mirror entries are injected as synthetic assistant context in the dynamic
preamble so the agent has continuity of what it previously said -- even after a
daemon restart.
...
1. Message is delivered to the channel via `deliverToChannel()`, or an
   attachment via the channel adapter's `sendAttachment()`
...
4. An idempotency key (the durable `conversation_ref` + a hash of the text
   **and** media references + 1-second time bucket) prevents duplicates via
   `INSERT OR IGNORE` -- hashing the media references keeps two different
   attachments sent in the same second with the same caption as two entries
...
2. `assembleExecutionPrompt()` calls `deliveryMirror.pending(authority)` -- the
   exact tenant, agent, and opaque `conversationRef` triple, never a formatted
   session key -- to fetch that conversation's unacknowledged entries
...
A 1:1 chat normally gets no reply anchor: with one conversation partner, a reply obviously answers
the message before it, and quoting would be noise. That reasoning holds only while replies arrive
in request order — and a turn waiting on an approval can sit for the whole
`approvals.defaultTimeoutMs` before it resumes. By then a later turn may already have answered, so
its outcome lands beneath a newer reply and reads as a statement about the newer request.

When a reply is delivered after another turn in the same session has already answered, Comis
anchors it to the message it actually answers, in a DM as well as a group, on any channel that can
thread (`replyToMetaKey`). Replies that arrive in order are unchanged, so this adds no quoting to
ordinary traffic. Ordering is tracked in memory per session; a restart has no in-flight waiting
turns to reason about, so there is nothing to restore.

This is not configurable: an unanchored out-of-order reply is not a preference, it is a message
that contradicts its neighbour.
