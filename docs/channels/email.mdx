...
- Receive and process file attachments
- Send HTML-formatted responses
- Filter senders via allowlist or open mode

Email does **not** support reactions, editing or deleting sent messages, fetching
message history, streaming (live-updating responses), buttons, cards/embeds, or
mentions.
...
IDLE (RFC 2177) for near-instant inbox monitoring. The IDLE connection restarts
every 25 minutes per the RFC specification. If your IMAP server does not support
IDLE, Comis falls back to polling at the interval configured by
`pollingIntervalMs` (default: 60 seconds).
