...
needed) or HTTP Mode (for production behind a reverse proxy).
...
- Reply in threads (native Slack reply-chain threads)
- Send files and images (up to 1 GB per attachment)
- Create interactive buttons for user choices
- Send rich cards and embeds with formatted content
- Use mrkdwn formatting (bold, italic, code, links, lists)
- Stream responses with live editing (updates every 400 ms)
- Mention users and channels
- Fetch message history for conversation context

Slack does **not** natively support typing indicators (Comis skips them for
Slack automatically).
