...
- Keep message replies and file uploads in the selected Slack thread via
  `thread_ts`
...
- Fetch root-channel history with `conversations.history`, or the selected
  thread with `conversations.replies`
