  ...
  <Tab title="Docker (no Node.js required)">
    ...
    <Steps>
      ...
      <Step title="Run the setup wizard">
        ...
        Configuration is written to `/data/config.yaml` and `/data/.env` inside
        the container, which map to the `comis-data` Docker volume. Your keys
        survive `docker rm` and `docker pull` as long as the volume sticks
        around.
      </Step>
