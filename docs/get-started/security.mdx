...
- recall is scoped to the tenant and agent, not to an individual sender or end
  user;
- a correction from an explicitly trusted owner in `single_owner` mode
  immediately stales the attributed procedure; other correction sources retain
  corroboration and trend gates and do not guarantee demotion; and
...
### Approvals require an authorized responder

`security.actionConfirmation` classifies actions, but the approval workflow is
disabled by default in the schema. Guided setup enables the gate when it creates
an administrator sender mapping. Otherwise enable `approvals.enabled` only after
configuring an authorized responder. Test the approval path from the same
channel and tool surface you plan to use.
