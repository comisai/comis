...
`security.actionConfirmation` classifies actions, but the approval schema
defaults off. Guided setup enables the gate when it creates an administrator
sender mapping. Otherwise enable `approvals.enabled` only after configuring an
authorized responder. Test the approval path from the same channel and tool
