...
Catalog discovery and runtime authentication are separate. A provider added by a future pi-ai release appears automatically in model lists and pickers, but a new credential scheme may still require a Comis update before the provider can make requests. Amazon Bedrock uses the host AWS credential chain. For Google Vertex AI, use `GOOGLE_CLOUD_API_KEY` with `GOOGLE_CLOUD_PROJECT` and `GOOGLE_CLOUD_LOCATION`; ADC-only Vertex authentication is not currently bridged through the daemon's scrubbed environment.
