...
reply binds when exactly one open request is unambiguous. Text that names a
different managed run remains ordinary chat, while a matching run handle can
disambiguate concurrent runs. Use `/attention <ID> <response>` when a run has
