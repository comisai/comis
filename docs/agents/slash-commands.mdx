...
| `/budget [tokens]` | Set per-execution token budget for the current session | `/budget 500000` |
...
The `/budget` command sets a per-execution token budget for the current session. This overrides the default budget limit for the next agent execution only.

| Usage | Effect |
|-------|--------|
| `/budget 500000` | Set the next execution to use at most 500,000 tokens |
| `/budget` | Show the current budget setting |
