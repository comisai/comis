...
sessions. Configuration is optional; the memory system is enabled by default.

Your agent does not just forget everything after each conversation. Comis stores
information in long-term memory -- facts, conversation summaries, instructions,
and more. This memory persists across sessions and can influence future
responses. Memory is backed by a single SQLite database (SQLite + FTS5 full-text
index + a vector index when an embedding provider is configured); see
[Search](/agents/search) and [Embeddings](/agents/embeddings).
...
Each memory has a trust level that controls eligibility and ranking during
recall. A trust level is a provenance and policy label, not proof that the
content is correct.

**system** -- Created by Comis itself (compaction summaries, internal data).
This is the highest policy tier, but generated summaries can still be wrong.

**learned** -- Things extracted or synthesized from eligible conversations and
memory sources. This tier is included in recall by default. It does not mean the
content was independently verified or is reliable.
...
This means your agent can build a personalised picture of you and your work
without you having to say "remember this." The review runs on a configurable
schedule and de-duplicates by content hash so repeated facts do not clutter the
database. Review output is LLM-generated and can be wrong. The `learned` label
...
- **Clear** -- Remove non-system, non-pinned memories for an explicitly selected tenant or agent
...
Recall is scoped to `(tenant, agent)` -- memory is **agent-scoped**, not
per-user. An agent has one shared memory across every conversation it holds
within a tenant, so the same agent recalls a fact regardless of which chat or
sender it was learned from (the per-memory sender is provenance, not a recall
filter). This is the intended behavior for a single-owner deployment (you talk
to your agent from several chats and share one memory). If you run **one agent
for multiple distinct people** under a single tenant, be aware that a personal
fact learned from one person is recallable when talking to another. Give each
person their own agent (or tenant) if their memories must stay private.
When this shared-agent mode recalls a memory from a different sender, Comis
marks it as `another sender` in the model context and explicitly forbids
attributing personal facts, identity, ownership, preferences, or authorization
from that memory to the current user. This provenance warning reduces mistaken
attribution; it is not a privacy boundary, because the recalled content still
enters the shared agent's context.
...
incoming message into a relevant set of memories. The orchestrator runs these
stages in order:
...
ranking dimension decides their *order* once they qualify. This keeps a
higher-tier record ahead of an equally relevant lower-tier record. It does not
establish that either record is true.
...
- **non-destructive** -- when a correction is accepted into a refreshed profile
  or topic document, it **supersedes** the prior body with a bi-temporal history
  append (the prior body is kept in `history`, never hard-deleted).
...
The reflection engine is gated by `learning.enabled` under the master
`memory.enabled` switch. For skill sources it selects trusted-origin
trajectories whose configured outcome resolver reports `success`. That fused
label is not independent proof of task success: mixed signals can classify a
session that failed overall as successful. In the same pass the engine can
distil **advisory procedure docs** from `orchestrate` runs resolved as successful.
A `kind:"skill"` doc records the run's deterministic tool footprint and can
surface into `<available_skills>` within a per-agent budget. Read-only
`candidate` docs may surface before promotion so they can receive reuse
feedback; `active` is the higher proof tier, not a prerequisite for first use.
The model reads a surfaced doc and **re-authors** the script through its own
permissioned tools. It is never a stored or replayed template and adds **no new
execution path**.

The static learned-document validator blocks critical secret or poison patterns,
but warning-level patterns, including some jailbreak-like language, can pass
validation. An enabled correction signal from the explicitly trusted owner in
`single_owner` mode immediately stales the prior turn's attributed skill.
Corrections from other sources must clear the confidence and corroboration gates
and produce a weakening trend before the state changes.
...
The DAG (LCD) context store preserves the full conversation structure and
tool-call/result pairing after the mandatory secret-persistence projection. The
[Data Directory reference](/operations/data-directory#lcd-store--durable-conversation-history)
owns the durability and on-disk security contract. It backs the **default**
lossless DAG engine (`contextEngine.version` defaults to `"dag"` — full details
