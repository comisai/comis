...
## DAG context store (opt-in; recall tools land later)
...
the **opt-in** lossless DAG engine (`contextEngine.version: "dag"` — full details
in [Context Management](/agents/context-management#dag-mode-opt-in)). In the
current release the engine reconstructs that history verbatim each turn; the
dedicated `ctx_*` recall tools (`ctx_search`, `ctx_inspect`, `ctx_recall`) that
let an agent query the store on demand, and the compaction that creates summary
nodes, land in a later phase — they are **not** present yet. The default context
engine remains **pipeline** mode (`contextEngine.version: "pipeline"`); in
pipeline mode, the `session_search` tool searches the raw session history. The long-term memory
