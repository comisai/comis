...
1. **SEARCH** -- each retrieval lane runs. The FTS5 keyword lane and the vector
   (meaning) lane are now **two independent, operator-weighted RRF lanes**
   (`rag.lanes.fts.weight`, default `1.0`; `rag.lanes.vector.weight`, default
   `1.5` — the parity defaults reproduce the prior ranking byte-for-byte). On top
...
**Fold-into-existing (proof accrual).** When a fact is **corroborated across
multiple runs**, consolidation **folds the new sources into the existing
observation** — growing its `proof_count` and `source_ids` and appending to its
`history` — instead of always creating a second observation. The fold is:

- **idempotent** — re-running over the same sources never double-counts (the new
  source IDs are diffed against the ones already recorded), so a repeated review
  pass is a no-op;
...
- **decay-refreshing** — it refreshes `confidence` and recency so the half-life
  decay stays meaningful as proof accrues.
...
## DAG context store (being reimplemented — not currently available)

The DAG context store — an additional storage layer that preserved the full
conversation history as a structured graph (every message, tool result, and file
content as a node, with compaction creating summary nodes that link back to their
sources) — was removed in the v2.12 demolition and is being reimplemented for the
"Lossless Context DAG" release. Its dedicated `ctx_*` tables and its recall tools
(`ctx_search`, `ctx_inspect`, `ctx_recall`) are not currently present; they return
with the LCD engine. The default context engine is **pipeline** mode
(`contextEngine.version: "pipeline"` — full details in
[Context Management](/agents/context-management)); in pipeline mode, the
`session_search` tool searches the raw session history. The long-term memory
described above (semantic store, recall, consolidation) is independent of the DAG
