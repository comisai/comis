...
1. **SEARCH** -- each retrieval lane runs. The FTS5 keyword lane and the vector
   ...
   `1.5` — the parity defaults reproduce the v2.6 ranking byte-for-byte). On top
...
The recall **core is always on** (as long as `rag.enabled` is `true`). Layered on
top of it, each **opt-in** and described below: locally-gated reranking; the
entity, temporal-spread, and causal lanes; the recall-utility feedback factor; and
consolidation.
...
- **Zero download out of the box (the supply-chain invariant)** — a fresh
  ...
  from the download** (the v2.7 change vs the old "stay opt-in to avoid the
  download").
...
recall is byte-identical to v2.6 (no read, no emit, no extra factor).
