...
| `graph:repaired` | A weak-model graph is repaired to a canonical template (gated off by default) | `pattern`, `nodeCount`, `capabilityClass` |
| `graph:synthesized_from_intent` | A graph is synthesized from a one-line intent (gated off by default) | `pattern`, `nodeCount` |
