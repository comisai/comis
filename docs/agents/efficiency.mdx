...
- **Daemon-wide** -- `comis system-health` rolls it into an **`orchestrate_efficiency`** finding: the run
...
starting over. This is enabled by default and nested under the durability
posture: with
[`autonomy.durability.orchestrateResume`](/reference/config-yaml#autonomy-agents-autonomy)
on (the default, inside the default-on `autonomy.durability` block), an
`orchestrate` run gains three durability affordances. Set either switch to
`false` to opt out.
...
- **Checkpoint / resume inside the script.** The in-jail SDK exposes
  ...
  it executes.
- **Timeout &rarr; a resumable row.** When a resume-enabled run hits its wall-clock
...
This is the default behavior. With `orchestrateResume` off, a timed-out run is
killed and cleaned, and no resumable durable row is written. For debugging, an
operator can deterministically re-run a durable run's pinned bytes against a
separate replay socket with
[`comis orchestrate replay <runId>`](/reference/cli#comis-orchestrate).
