...
when the terminal result is durably recorded and delivered to the immutable
originating conversation. A parent can also await owned children with
`subagents` action `wait` without polling.
...
flowchart LR
    ...
    G --> H[Exact-origin delivery or wait result]
```
...
runtime, token counts, condensation level, disk path). Its promise is not part
