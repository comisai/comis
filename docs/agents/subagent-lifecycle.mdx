...
Comis builds a spawn packet (next section), runs the sub-agent in its own
session, and returns a condensed result like this:
...
brief. The full unredacted research output is on disk if Atlas needs to look
back at sources later.
