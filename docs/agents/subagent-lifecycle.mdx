...
calls `sessions_spawn`. The call returns an internal run handle immediately; the lifecycle ends
...
```mermaid
flowchart LR
    A[Parent calls sessions_spawn] --> B[Internal run handle returned]
...
The handle stays inside the orchestration flow: it is used by `subagents`
operations and is not included in the user-facing launch acknowledgement.
...
security:
  agentToAgent:
    ...
    subagentContext:
      ...
      maxRunTimeoutMs: 1500000     # absolute watchdog ceiling (backstop; stuckKillThresholdMs catches hangs sooner)
...
- **`steer`** sends a high-priority steering message to the child. *How* that message is delivered is gated behind [`security.agentToAgent.steerInject`](/reference/config-yaml#security) (default `false`):
  - **Flag off (default)** -- while the target remains running, `steer` falls back to **kill + respawn**: the child is killed and a fresh run is spawned with the steering message as its new task. The prior transcript and progress are discarded, and the run gets a new `runId`.
...
**`steer` targets a *running* child.** If a `completed` result wins the race before either steering mode can act, the operation succeeds as a no-op with `{ status: "already_terminal", runId, terminalStatus: "completed" }`; the completed result remains authoritative and is not respawned. A `failed` or still-`queued` target continues to fail fast rather than attempting an inject against a child that has no live session.
...
### Unrouted children are cancelled with their parent

When a run reaches a terminal state **abnormally** -- any end reason other than
`completed` -- each child it spawned that is still `running` or `queued` is
checked for an independently authenticated completion route. A child with both
announcement-route coordinates continues and can still deliver its result; the
parent trajectory records that preservation as bounded identifiers plus the
closed reason `announcement_route`. A child without that route is killed with
`killedBy: "system"`, and the failure notification names the cause
(`its parent run ended (<endReason>) with no reader left for this result`).

The cascade therefore stops only work whose parent was its last reader. It
walks the spawn tree once per node, bounded by `maxSpawnDepth`, while routed
children remain independently deliverable.
...
**A child that returns with its own work still running does not end
`completed`.** When a sub-agent answers while one or more of its
[`exec`](/skills/built-in-tools) calls are still auto-backgrounded and were never
polled to a terminal state, the run ends `failed` -- the child reported an
outcome it could not have known. Only *unpolled* handoffs invalidate the run: a
non-zero exit the child did observe stays its own authoritative answer and rides
the completion record as a diagnostic count. The daemon then reaps the background
processes owned by that child's session -- as it also does on a `killed` run --
so a torn-down sub-agent leaves nothing running behind it.
[`comis explain`](/reference/cli#comis-explain) on that child session root-causes
the run as `subagent_background_processes_abandoned` and names how many process
sessions were left unpolled, so a rejected completion is never mistaken for an
ordinary child failure. Have the child poll each session with the `process`
tool's `status` action until it is terminal before returning.
