...
- **Procedure-doc surface budget (`learning.reflect.maxProcedureDocsSurfaced`, default `10`)** —
  ...
  the budget caps that subset only (oldest-first, so the most-corroborated procedures keep their
  slot). User-intent **skill** docs and **topic** docs are **unaffected** — they keep a separate,
  uncapped path.
