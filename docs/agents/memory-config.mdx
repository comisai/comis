...
- **Promotion, correction, and demotion** -- attributed reuse labeled `success`
  ...
  `promoteAtProofCount`. When the correction detector classifies a follow-up from
  the explicitly trusted owner in `single_owner` mode, the prior turn's
  attributed skill moves immediately to `stale`, and continuation attribution on
  the correction turn cannot add proof to it. Corrections from other sources
  require confidence, failure corroboration, and a weakening trend. When an
  accepted profile or topic update supersedes an
