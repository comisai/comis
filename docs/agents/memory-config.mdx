...
<Note>
  **Renamed in v2.31.** The master kill-switch is now `memory.enabled` (was the old top-level
  cost-features switch), and the three recall model knobs nest under `memory.recall.*`.
  The whole per-agent learning layer collapsed into **one** `agents.<id>.learning` block (one
  `learning.enabled` flag + `learning.reflect.*` + `learning.forget.*`) — see
  [Removed in v2.31](#removed-in-v2-31) for the deleted keys and the one-line config update.
</Note>
...
agents:
  my-agent:
    ...
    memoryReview:                    # session → memory review + dedup (accumulate tier)
      ...
      schedule: "0 2 * * *"
      minMessages: 5
      maxSessionsPerRun: 50            # best-out-of-box: broader consolidation coverage per run (was 10)
      maxReviewTokens: 16384           # best-out-of-box: richer consolidation per session (was 4096)
    ...
    learning:
      ...
      reflect:                       # the one __REFLECT__ cron (skill + profile + topic in one pass)
        schedule: "0 */3 * * *"      # best-out-of-box: every 3 hours (near-real-time learning; was daily 03:00)
        ...
        maxDocsPerRun: 100           # finite DoS bound — best-out-of-box raised (don't throttle learning; was 25)
      forget:                        # the wrongness-based soft-eviction policy (the __LIFECYCLE__ sweep reads these)
        maxDormantDays: 365          # best-out-of-box: remember ~a year (forget pure-disuse less; was 90)
...
## The one learning-reflection engine (v2.31)

Comis's learning layer is **one outcome-gated reflection engine**. Where earlier versions had
five separate cron loops (skill synthesis, ranking tuning, user-representation, consolidation,
reasoning), a single `__REFLECT__` cron now maintains named **Mental Model** docs
...
and default, the [`mental_models`](/operations/data-directory#learned-skills-verified-learning-v2-26)
and [`memory_usefulness` / `evicted_at`](/operations/data-directory#ranking--forgetting-verified-learning-v2-26)
...
The per-agent learning config collapsed into one `learning` block, and the master kill-switch
moved to the top level. Because the schema is **strict** (`z.strictObject`), a `config.yaml`
that still carries a removed key is **rejected at boot** with an "unrecognized key" parse error
— the explicit operator-update path (there is no compatibility shim). If your config sets any
of the removed blocks below, delete them and adopt the replacement:

| Removed config block | Replaced by |
|---|---|
| the old top-level master cost switch | `memory.enabled` (the master kill-switch) |
| the flat recall model keys on `memory` (embedding model, embedding dimensions, reranker model) | `memory.recall.embeddingModel` / `embeddingDimensions` / `rerankerModel` |
| the per-agent skill-synthesis block | `agents.<id>.learning.enabled` + `learning.reflect.*` |
| the per-agent ranking-tuning block | folded into `agents.<id>.learning.enabled` (the outcome-reward write stays wired) |
| the per-agent wrongness-eviction block (incl. its eviction floor) | `agents.<id>.learning.forget.*` |
| the per-agent usefulness-judge block | **removed** (the dormant usefulness-judge cron was deleted) |
| the per-agent triple-extraction block | **removed** (the scaffold triple-extraction cron was deleted) |
| the per-agent social-modeling block | **removed** (the social-modeling subsystem was deleted) |
| the standalone user-representation / reasoning / consolidation blocks | **removed** — the work folds into the one reflection cron (`kind:"profile"` / `kind:"topic"` docs) |
| the dormant `memoryLifecycle` FadeMem constants (`thetaPromote`, `thetaDemote`, `durableCap`, `ephemeralCap`, `epsilonPrune`, `maxDormantDays`) | `memoryLifecycle` is trimmed to `{ enabled, schedule }`; the dormancy window moved to `learning.forget.maxDormantDays` |

A **bare config** (`memory:`/`agents:` with no learning keys) loads fully-defaulted — the
removal only affects a config that still names a deleted key. The daemon's boot error names
the exact unrecognized key, so an operator updating an old config is pointed straight at the
line to delete.
...
1. **Some capabilities need built derived state, not just the flag:**
   ...
   - **FORGET** decay (`rag.forget`) shows up at recall; **eviction** is the `memoryLifecycle`
     ...
     [the one learning-reflection engine](#the-one-learning-reflection-engine-v2-31).
