...
| `standard` _(default)_ | The eight floor capabilities (read, web, write, analyze, spawn, graph, cron, skill) | Budget ceiling, spawn-rate limit, concurrency cap, message quota -- all on | The default for almost everyone |
...
The `standard` profile resolves these eight floor capabilities:
...
`maxConcurrentSelfAgents`, `maxSelfSpawnRatePerMin`, `cronSelfMax`) and the
`message` block (`channels`, `maxPerHour`) are all individually overridable. See
the full field list in the [config reference](/reference/config-yaml#autonomy-agents-autonomy).
