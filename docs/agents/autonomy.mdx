...
`standard`'s capability set and attaches an "available in a later milestone"
notice (logged at boot). Comis **never silently grants** a capability whose
...
that the extra surface (sandbox auto-allow) arrives in a later milestone.
