...
surfaces -- a coordinator role and sandbox auto-allow -- are not in this release.
Selecting either resolves to `standard`'s capability set and attaches an
"available in a later milestone" notice (logged at boot). Comis **never silently
grants** a capability whose enforcement floor is not yet built, so you are told
rather than over-granted. (**Durable runs** are no longer one of the deferred
surfaces -- they ship as an opt-in knob orthogonal to the profile; see
...
- **Exactly-once outward sends -- a crash never re-DMs or re-posts.** Every
  ...
  announcement is never delivered twice.
...
<Note>
**M1 resume scope.** A resumed run is re-anchored under the bounded-autonomy
