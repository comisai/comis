...
### 2. Queue assigns priority

Messages enter a queue that controls the order of processing. Direct messages
typically get higher priority than group chat messages. The queue also handles
situations where multiple messages arrive at once -- it makes sure your agent
processes them in a sensible order without getting overwhelmed.
