...
```mermaid
flowchart TD
    A["Message arrives"] --> B["Queue serializes per session"]
...
Messages enter a queue that controls the order of processing. The queue
serializes work through a concurrency gate so your agent processes
messages in a sensible order without getting overwhelmed.

See [Message Queue](/agents/queue) for details on per-session lanes and queue
modes.
...
# 2. Queue serializes per session
DEBUG [queue.command] enqueued { lane: "normal", sessionKey: "default:agent:atlas:user_a:telegram", depth: 0 }
...
INFO  [session.lifecycle] loadOrCreate { sessionKey: "default:agent:atlas:user_a:telegram", isNew: false, messageCount: 12 }
...
|-------|-----------|-------------|
| Queue | `CommandQueue` | `packages/orchestrator/src/queue/command-queue.ts` |
| Routing | `MessageRouter` | `packages/orchestrator/src/routing/message-router.ts` |
