...
<Note>
The DAG (LCD) engine is a **working opt-in** engine — set `contextEngine.version: "dag"` to enable it (the default remains `"pipeline"`; DAG becomes the default in a later release). In this release DAG does **lossless verbatim assembly**: it reconstructs the full faithful conversation history (every message, tool call, and tool result preserved verbatim, paired by id), keeps a verbatim **fresh tail** of the most recent steps, and repairs the transcript so the model always receives a provider-valid `tool_use`↔`tool_result` pairing. This is what fixes the re-read loop that a lossy reconstruction caused. Compaction, summarization, and the on-demand recall tools (`ctx_search`, `ctx_inspect`, `ctx_expand`, `ctx_recall`) land in later phases — they are **not** part of DAG in this release. In pipeline mode, `session_search` searches the raw session history instead.
</Note>
