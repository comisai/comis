...
Comis ships the **pipeline** context engine as the default — `contextEngine.version` defaults to **`"pipeline"`**. Pipeline mode is a fixed chain of layers that drops, masks, and (as a last resort) summarizes old content before each call: lighter and fully deterministic. A second engine — the lossless **DAG** engine (the v2.12 "Lossless Context DAG" release), which keeps the whole conversation losslessly instead of dropping old content — is available as an **opt-in** engine. Set `version: "dag"` to use it. The default stays `"pipeline"`; DAG becomes the default in a later release.
...
<Note>
The DAG (LCD) engine is a **working opt-in** engine — set `contextEngine.version: "dag"` to enable it (the default remains `"pipeline"`; DAG becomes the default in a later release). DAG stores the full faithful conversation history losslessly (every message, tool call, and tool result preserved verbatim, paired by id), keeps a verbatim **fresh tail** of the most recent steps, and repairs the transcript so the model always receives a provider-valid `tool_use`↔`tool_result` pairing. This is what fixes the re-read loop that a lossy reconstruction caused. **Leaf summarization and budget eviction are now part of DAG**: when context utilization crosses `contextThreshold` (default 0.75 × the model's window), the engine summarizes the oldest out-of-tail chunk into a leaf summary and assembles the context under the token budget — always keeping the verbatim fresh tail, and never deleting anything from the store (compressed detail stays recoverable). Still landing in later phases — and **not** yet part of DAG: the multi-tier *condensed* summary hierarchy and the on-demand recall tools (`ctx_search`, `ctx_inspect`, `ctx_expand`, `ctx_recall`). In pipeline mode, `session_search` searches the raw session history instead. For the leaf-summarization mechanics see [Compaction](/agents/compaction).
</Note>
...
agents:
  default:
    contextEngine:
      enabled: true
      version: pipeline       # DEFAULT — set "dag" to opt into the lossless LCD engine
...
Pipeline is the default. `contextEngine.version` accepts `"pipeline"` and `"dag"`; `"dag"` opts into the lossless LCD engine described above (the default stays `"pipeline"` until a later release):
