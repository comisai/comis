...
| 8 | LLM compaction | When the prompt is past ~85% of the turn's budget window (the effective window after served-window and capability caps — not the raw model window), calls the model to summarize older turns into one assistant message. Cooldown of 5 turns prevents thrashing. See [Compaction](/agents/compaction). |
