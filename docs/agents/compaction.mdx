...
condensed summary creation on every turn (the hierarchical summary nodes and the
on-demand recall tools described in the rest of this section) lands in a **later
phase**; until then the budget pass drops the oldest raw history rather than
summarizing it. Nothing is ever deleted from the store — eviction trims the
assembled context at read time only, so evicted messages remain recoverable when
recall lands.
</Warning>
...
fresh tail of the most recent steps. When the assembled history would exceed the
model's token budget, the **oldest evictable history is dropped** to fit (whole
tool-use/tool-result steps only — a pair is never split), while the fresh tail is
always protected. Any leaf summaries present in the store are resolved into the
context in their original position. Eviction is **read-time only**: the underlying
messages stay in the lossless store. In a later phase the DAG will additionally
compress old content into hierarchical summaries automatically on each turn and
expose on-demand recall tools (the design described below); in the current
release it does budget-bounded assembly with read-time eviction.
