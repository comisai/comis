...
| `contextEngine.leafChunkTokens` | number | `20000` | **DAG mode (live):** token cap for the oldest out-of-tail chunk summarized into one leaf summary (1K-100K); clamped at runtime to the resolved summarizer model's window (minus the summary target, template overhead, and previous-summary size), so a small compaction summarizer is never fed an over-window chunk — a single message larger than the clamped cap is replaced by a bounded deterministic extraction (no LLM call) |
