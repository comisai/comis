...
<Note>
The **DAG** engine (a lossless directed acyclic graph with recall tools) is being
reimplemented for the v2.12 "Lossless Context DAG" release and is **not currently
available**. Setting `contextEngine.version: "dag"` is accepted but falls back to
the pipeline engine with a logged warning until DAG is re-enabled in a later
release. New conversations run on the pipeline engine.
</Note>
