...
conversations. The default **DAG** mode preserves your full conversation history
in a lossless directed acyclic graph with recall tools, so nothing is thrown away
— `contextEngine.version` defaults to `"dag"`. The opt-out **pipeline** mode
(`version: "pipeline"`) instead processes every conversation through eight steps
before each AI call, dropping and masking old content as it goes. The result:
your agent never runs out of room to think.
...
## The eight steps (pipeline mode, the opt-out engine)

These eight steps run only in **pipeline** mode — the opt-out engine you reach by
setting `contextEngine.version: "pipeline"`. (The default DAG engine handles long
conversations differently; see [DAG mode](#dag-mode) below.) Here is what happens
...
DAG mode is the **default** context engine: it stores every message and tool
...
Because `contextEngine.version` defaults to `"dag"`, this is what your agent runs
out of the box — no configuration required. To opt out and run the eight-step
pipeline instead, set `version: "pipeline"`. See [Switching modes](#switching-modes)
below.
...
DAG mode replaces the pipeline's `session_search` tool with four recall tools:
...
- **`ctx_inspect`** -- View the full content of a specific summary or stored
  file.
- **`ctx_expand`** -- Expand a compressed summary back into the underlying
  messages it was condensed from.
...
DAG is the default, so you only set `contextEngine.version` explicitly when you
want to **opt out** to the pipeline engine — or switch back:
...
Switching either direction is operator-safe: it is applied on a config reload (no
daemon restart) and takes effect at the next turn boundary — never mid-turn. The
switch is reversible and non-destructive in both directions: going to DAG
reconciles existing JSONL history into the DAG automatically (no manual
migration), and switching back to pipeline re-reads the full conversation while
leaving the DAG's stored content intact for a later switch-back. `contextEngine.version`
is operator-only — an agent cannot switch its own engine mode via the `config.patch`
RPC. For the full switching semantics (including the `context:mode_switched` event
and the security boundary), see
[Switching context modes](/context-management#switching-context-modes-operator-safe).
...
| `contextEngine.version` | string | `"dag"` | Context engine mode: `"dag"` (default) or `"pipeline"` (opt-out) |
