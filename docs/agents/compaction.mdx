...
<Note>
The lossless **DAG** (LCD) engine is
...
*condensed* summary hierarchy is part of DAG too: when enough same-depth
...
`contextEngine.version` defaults to `"dag"` (set `"pipeline"` to
...
is exceeded — always keeping the verbatim fresh tail. **Leaf
summarization is automatic**: when context utilization crosses `contextThreshold`
...
of summaries) builds on top: when enough same-depth summaries accumulate
...
[Honest presentation](#honest-presentation-of-summaries) below). Nothing is ever
deleted from the store — both eviction and
summarization operate on the assembled context only, so the underlying messages
remain recoverable via the on-demand `ctx_*` recall tools described later in
this section.
</Warning>
...
[Honest presentation of summaries](#honest-presentation-of-summaries)), and the
on-demand `ctx_*` recall tools (described below) let the agent drill back into
any compressed region.
...
— deterministic and useful for tests, at the cost of
...
| `contextEngine.summarizerSpend.maxTokensPerTenantPerHour` | number | `500000` | **DAG mode:** per-tenant rolling-hour ceiling on summarizer (input+output) tokens; over the cap the summarizer is bypassed and assembly degrades to truncation-only. `0` disables the hourly cap |
| `contextEngine.summarizerSpend.maxTokensPerTenantPerDay` | number | `5000000` | **DAG mode:** per-tenant rolling-day summarizer token ceiling. `0` disables the daily cap |
| `contextEngine.summarizerBreaker.failureThreshold` | number | `5` | **DAG mode:** consecutive summarizer failures before the circuit breaker opens → truncation-only assembly |
| `contextEngine.summarizerBreaker.resetTimeoutMs` | number | `60000` | **DAG mode:** how long the summarizer breaker stays open before a half-open trial |
| `contextEngine.summarizerBreaker.halfOpenTimeoutMs` | number | `30000` | **DAG mode:** half-open trial window for the summarizer breaker |
| `contextEngine.compaction.summarizerFallbackProviders` | string[] | `[]` | **DAG mode:** ordered list of fallback summarizer provider/model ids tried in turn when the primary summarizer fails; the per-tenant breaker + spend caps still bind, and only an all-providers-exhausted failure trips the breaker. `[]` (default) = no failover — the primary summarizer is the only one tried |
