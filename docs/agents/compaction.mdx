...
(`version: "pipeline"`) instead processes every conversation through ten layers
...
## The ten layers (pipeline mode, the opt-out engine)

These ten layers run only in **pipeline** mode — the opt-out engine you reach by
...
out of the box — no configuration required. To opt out and run the ten-layer
...
[Switching context modes](/agents/context-management#switching-context-modes-operator-safe).
