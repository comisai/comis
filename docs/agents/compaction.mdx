...
<Warning>
The DAG engine is being reimplemented for the v2.12 "Lossless Context DAG"
release and is **not currently available**. `contextEngine.version` defaults to
`"pipeline"`; setting it to `"dag"` is accepted but falls back to the pipeline
engine with a logged warning until DAG is re-enabled in a later release. The
sections below describe the DAG engine's intended design for reference.
</Warning>

### What is DAG mode

DAG mode stores every message and tool result in a directed acyclic graph (DAG).
Instead of discarding old content, the DAG compresses it into hierarchical
summaries. Your agent can recall any past content through search and inspection
tools -- nothing is ever truly lost. Until DAG is re-enabled, selecting
`version: "dag"` runs the pipeline engine instead.
...
but `"dag"` currently falls back to pipeline (DAG is being reimplemented), so the
pipeline engine runs either way:
...
| `contextEngine.version` | string | `"pipeline"` | Context engine mode: `"pipeline"` (default) or `"dag"` (accepted but currently falls back to pipeline; being reimplemented) |
