...
stays `"pipeline"`; DAG becomes the default in a later release). In the current
release DAG does **lossless verbatim assembly** (described below): it keeps the
full conversation losslessly and reconstructs it verbatim each turn. The
**compaction** behavior described in the rest of this section (hierarchical
summary nodes, on-demand recall tools) lands in a **later phase** — DAG today
does **not** compact or summarize; it keeps everything verbatim.
</Warning>

### What is DAG mode

DAG mode stores every message and tool result losslessly and reconstructs the
conversation verbatim each turn — every message, tool call, and tool result
preserved and paired by id, plus a verbatim fresh tail of the most recent steps.
Nothing is dropped or masked. In a later phase the DAG will additionally compress
old content into hierarchical summaries with on-demand recall tools (the design
described below); in the current release it does lossless verbatim assembly only.
