...
handle efficiently. The default **pipeline** engine processes every conversation
through ten layers before each AI call, dropping and masking old content as it
goes — `contextEngine.version` defaults to `"pipeline"`. The result: your agent
never runs out of room to think.

<Note>
The lossless **DAG** engine (the v2.12 "Lossless Context DAG" release) is a
**working opt-in** engine — set `contextEngine.version: "dag"` to enable it (the
default stays `"pipeline"`; DAG becomes the default in a later release). DAG
...
0.75 × the model's window) at the end of a turn: it collapses the oldest chunk
...
footer, body taint-wrapped). Only the on-demand `ctx_*` recall tools land in a
later phase. See [DAG mode](#dag-mode-opt-in) below for the mechanics and
...
These ten layers run in **pipeline** mode — the default engine
(`contextEngine.version` defaults to `"pipeline"`). Here is what happens during
each step, in order:
...
<Warning>
The DAG (LCD) engine is a **working opt-in** engine (the v2.12 "Lossless Context
DAG" release) — set `contextEngine.version: "dag"` to enable it (the default
stays `"pipeline"`; DAG becomes the default in a later release). The current
release stores every message losslessly and assembles the context **under the
model's token budget**: it resolves the ordered context view (raw messages plus
...
(default 0.75 × the model's window) at the end of a turn, the oldest out-of-tail
...
When DAG mode is re-enabled it will replace the pipeline's `session_search` tool
with a set of recall tools (full-text/regex search across messages and summaries,
summary/file inspection, summary expansion, and a recall sub-agent). Those `ctx_*`
recall tools were removed pending the v2.12 reimplementation and are not currently
available; they return with the LCD engine. Until then, the default pipeline mode
uses `session_search` over the raw session history.

See [Built-in Tools](/skills/built-in-tools) and
[Platform Tools](/skills/platform-tools) for the current (pipeline-mode) tool
reference documentation.

### Switching modes

Pipeline is the default. `contextEngine.version` accepts `"pipeline"` and `"dag"`;
set `"dag"` to opt into the lossless LCD engine (the default stays `"pipeline"`
...
agents:
  default:
    contextEngine:
      version: "dag"        # opt into the lossless LCD engine; omit (or "pipeline") for the default
```
...
| `contextEngine.version` | string | `"pipeline"` | Context engine mode: `"pipeline"` (default) or `"dag"` (opt-in lossless LCD engine) |
...
| `contextEngine.contextThreshold` | number | `0.75` | **DAG mode (live):** context-utilization fraction (of the model window) that triggers a leaf-summarization pass at the end of a turn (0.1-0.95) |
...
| `contextEngine.leafChunkTokens` | number | `20000` | **DAG mode (live):** token cap for the oldest out-of-tail chunk summarized into one leaf summary (1K-100K) |
