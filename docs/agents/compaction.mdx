...
*condensed* summary hierarchy and the on-demand recall tools land in a later
phase. See [DAG mode](#dag-mode-opt-in) below for the mechanics and
...
of summaries) and the on-demand recall tools described in the rest of this
section still land in a **later phase**. Nothing is ever deleted from the store —
both eviction and leaf summarization operate on the assembled context only, so
the underlying messages remain recoverable when recall lands.
</Warning>
...
messages stay in the lossless store, so compressed detail remains recoverable. In
a later phase the DAG will additionally roll leaf summaries up into a multi-tier
**condensed** hierarchy and expose on-demand recall tools (the design described
below); in the current release it does leaf summarization + budget-bounded
assembly.
