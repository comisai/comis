...
Providers may price cached prompt reads below uncached input, but support,
eligibility, and rates vary by provider and model. Cache reuse only occurs when
the provider recognizes a stable prompt prefix. Comis therefore keeps stable
content in predictable positions and exposes cache diagnostics so you can
measure the result for your own workload.

<Note>
The mechanisms below span the executor, context engine, and provider adapters.
</Note>

---

## Cache mechanisms at a glance
...
| 11 | **Anthropic ephemeral cache** | `cache_control: { type: "ephemeral", ttl: "5m" or "1h" }` markers placed at the right block boundaries | Uses Anthropic's native prompt-cache controls on supported requests |
...
| 16 | **Tool deferral with discovery** | Non-essential tool schemas are removed from the `tools` array and re-injected only when the agent calls `discover_tools` | Reduces repeated schema tokens when an agent has many tools available |
...
- **Defer transient recall to the uncached tail.** The per-turn memory-recall block (`[Relevant context from memory: …]`) is moved off the cache-marked query block onto a trailing **uncached** block — still visible to the model, never cached — so the prefix does not change when that recall rotates out next turn. Once the turn completes, the recall block is also carved out of the canonical replay history itself (provider-independent), so a historical user turn never re-presents it and the replayed prefix stays byte-stable at every index.
...
The explicit cache is **shallow by design** — it covers the system instruction
and tools, while Gemini's implicit prefix caching can cover the conversation
tail. Below the provider's per-model minimum, the manager passes through
uncached. Usage is reported as `cachedContentTokenCount`. Only Google AI Studio
providers use the explicit path (not Vertex AI). Set `geminiCache.enabled:
false` to opt out, for example when the system prompt changes every turn.
...
Sending every tool schema on every request can add substantial repeated input.
Comis can defer non-essential tools using a **BM25-backed discovery tool**
(`discover_tools`).

Deferred tools are removed from the `tools` parameter entirely. Their schemas are stored in a `DiscoveryTracker`. When the agent needs a deferred tool, it calls `discover_tools` with a natural-language query. Exact callable names and exact MCP callable suffixes (the part after `--`) resolve before BM25 relevance search. Comis re-injects only the matched schemas plus relationships declared through explicit `coDiscoverWith` metadata; matching one MCP tool does not activate every tool on that server. A discovered full schema replaces any same-name deferral placeholder in the live callable set, so a tool displayed by discovery is callable in that turn.
...
**If you see high cache-write costs on Haiku or Sonnet with short-lived
sessions,** set `adaptiveCacheRetention: false` and `cacheRetention: "short"`.
This avoids retaining cache entries longer than those sessions need.
...
**For Gemini agents,** `geminiCache` is **on by default**. The CachedContent API
caches the system instruction and tools server-side when the request meets the
provider's eligibility rules. Below the provider's per-model minimum, the
manager passes through uncached. Set `geminiCache.enabled: false` to opt out,
for example if your system prompt changes every turn.
