...
`resources/read` returns the **current CONFIRMED state** of a session, as a
single bounded page of up to the **200 most recent** messages (the same
page-size ceiling the `session.history` RPC enforces), with the unconfirmed ones
filtered out. Re-reading the same URI
may return _additional_ messages as outbound delivery completes (an in-flight
outbound message that was hidden on the first read may appear on a later read
once the channel adapter confirms delivery).
