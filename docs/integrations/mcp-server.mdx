...
[Exposed tools](#exposed-tools)). The conservative defaults shipped in v2.4
...
**v2.4 starting classification** — see the [security-reviewer gate](#security-reviewer-gate-on-the-51-tool-classification)
...
### `permission-gated` (20) — exposed only if token's `mcpClient.allowlist` includes the tool
...
| `ctx_search` | Context-engine read |
| `ctx_inspect` | Context-engine inspect |
| `ctx_expand` | Context-engine expand |
| `ctx_recall` | Context-engine recall |
...
**Total:** 3 safe + 20 permission-gated + 28 never-export = 51 unique
tools, the full platform-tool surface as of v2.4.
...
`transcribe_audio`) are conservatively classified as `permission-gated` in
v2.4. The reviewer decides whether any can be upgraded to `safe` (if the
...
Out of scope for the v2.4 server, deferred to v2.5+:
...
- **Native binary content in `tools/call` results** — image and audio tool
  outputs are returned as text descriptions (JSON-stringified) in v2.4.
