...
**What this is for:** point any OpenAI-API-compatible client at Comis and have it talk to your local agents. **Who it's for:** developers integrating Comis into existing OpenAI-shaped tooling (LangChain, custom GPTs, third-party UIs).
...
The gateway also mounts `POST /v1/responses` (OpenAI Responses API). It is listed in the [HTTP Gateway routes](/reference/http-gateway#openai-compatible-endpoints) reference but is intentionally not described here because the OpenAI Responses spec is separate from the Chat Completions spec.
