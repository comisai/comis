...
- **The daemon's own injection is preserved.** `COMIS_CAP_LEASE` / `COMIS_ORCH_SOCKET` minted by the daemon ride a separate trusted path and **survive** the scrub — blocking them would break the capability socket. Only the untrusted workspace source is blocked; the trusted daemon-injected source is allowed.
