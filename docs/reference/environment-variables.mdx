...
| **Default** | Unset; the daemon reads `~/.comis/config.yaml` and `~/.comis/config.local.yaml` |

Specifies which YAML configuration files to load, as a comma-separated list.
Files are loaded in order and deep-merged, with later files overriding earlier
ones. Non-existent files are silently skipped. The daemon reads this variable
at startup to locate config files. The CLI uses it in `config validate` and
`pm2 setup` commands.
...
Providing this variable explicitly overrides the auto-generated value. When it
is already present in the daemon environment, Comis does not generate or append
a second master key under the data directory. This supports systemd credentials,
container secret mounts, and external secret managers without creating a stale
`<dataDir>/.env` copy.
...
|----------|-------|
| **Used by** | Daemon |
| **Format** | Non-empty random secret |
| **Default** | Auto-generated with encrypted storage when absent from both the active store and environment, then written to `~/.comis/.env` (mode 0600) |

The canary seed makes outbound exfiltration markers unpredictable. When an
external secret manager or the active encrypted store supplies `CANARY_SECRET`,
Comis uses that value and does not duplicate it under the data directory.
...
3. **WARN once on drift.** If the env-var refresh token differs from the stored profile's refresh token, the daemon emits one WARN per (provider, process) with `errorKind: "auth"` and `hint: "env-override-ignored"`. Operators clear the drift by either:
...
### `COMIS_LIVE_LOCAL_MODELS` / `COMIS_LIVE_OLLAMA_URL`

| Property | Value |
|----------|-------|
| **Used by** | The keyless-model legs of the live tier: `test/live/scenarios/local/local-models.test.ts` and the Stage-C channel rig (`test/live/harness/rig-config.ts`) |
| **Format** | Comma-separated ollama tags / an ollama base URL |
| **Default** | `COMIS_LIVE_LOCAL_MODELS` unset → the local tier auto-discovers every `qwen3.6` tag the server has and the Stage-C rig boots `qwen3.6:35b`; `COMIS_LIVE_OLLAMA_URL` unset → `http://localhost:11434` |

Which on-device tags the live tier may use and where its ollama server listens —
no key, on-device, `$0`. `COMIS_LIVE_LOCAL_MODELS` is read two ways: the local-model
scenario treats it as a **filter** over the tags the server reports, while the Stage-C
channel rig boots its **first** entry as the `"keyless"` model. Point it at a tag the
box can actually hold — an oversized model boots the daemon but never authors a reply,
so a Stage-C round-trip exhausts its reply budget instead of failing loudly.

```bash
export COMIS_LIVE_OLLAMA_URL=http://localhost:11434
export COMIS_LIVE_LOCAL_MODELS=llama3.2:3b     # e.g. a CPU-only box
```

The full live-tier template — including the `COMIS_LIVE_EMBED_MODEL_PATH` /
`COMIS_LIVE_RERANKER_MODEL_PATH` GGUF paths for the memory scenarios — is
`test/live/live.env.example`; the Stage-C rig is described in the
[channel emulation harness](/developer-guide/channel-emulation-harness).
...
| `ANTHROPIC_AUTH_TOKEN` | Anthropic-compatible gateways that require `Authorization: Bearer`; used only when no API key or OAuth token is set |
...
| `QWEN_TOKEN_PLAN_CN_API_KEY` | Qwen Token Plan China |
