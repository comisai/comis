...
Cloudflare also requires `CLOUDFLARE_ACCOUNT_ID`; Cloudflare AI Gateway additionally requires `CLOUDFLARE_GATEWAY_ID`. Google Vertex AI static-key authentication requires `GOOGLE_CLOUD_PROJECT` (or `GCLOUD_PROJECT`) and `GOOGLE_CLOUD_LOCATION` alongside `GOOGLE_CLOUD_API_KEY`.

Amazon Bedrock is the exception to the static-key table. It uses the host AWS credential chain (`AWS_PROFILE`, IAM access keys, a Bedrock bearer, ECS credentials, or web identity) and those values remain ambient for the AWS SDK. Google Vertex ADC-only authentication is not currently bridged through the daemon's scrubbed environment; use the explicit Vertex key path above.
