...
| `OPENAI_API_KEY` | OpenAI (GPT family, Whisper, embeddings; also native `image_generate` via the OpenAI Images API for an `openai` main) | `sk-...` |
| `GOOGLE_API_KEY` | Google AI Studio (Gemini family; also native `image_generate` via Gemini for a `google` main) | `AIza...` |
