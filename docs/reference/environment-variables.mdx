...
The AES-256-GCM master encryption key used to encrypt and decrypt secrets stored in `secrets.db`. Starting with v1.1, the key is auto-generated on first boot — you do not need to run `comis secrets init` for a fresh install. The daemon and CLI both resolve this key in the same order: first from the environment, then from `~/.comis/.env`.
...
### `COMIS_DISABLE_ENCRYPTED_SECRETS`

| Property | Value |
|----------|-------|
| **Used by** | Daemon |
| **Format** | `1`, `true`, or `on` to disable; unset to enable (default) |
| **Default** | (unset — encrypted store is active) |

Set to `1`, `true`, or `on` to disable auto-init of the encrypted secrets store and run in envfile-only mode. The daemon emits a startup WARN when this flag is active. Use only if you need to manage secrets manually in `.env` files and do not want the encrypted store.

```bash
export COMIS_DISABLE_ENCRYPTED_SECRETS=1
```
