...
| `GOOGLE_API_KEY` | Google AI Studio (Gemini family; native `image_generate` via Gemini for a `google` main; also `image_analyze` for a Gemini vision-capable main and `describe_video` raw video, or via the vision registry fallback) | `AIza...` |
...
**`FAL_KEY` also enables video generation.** The `video_generate` tool (v2.24) uses the same `FAL_KEY` for the explicit `provider: fal` path (FAL queue API, `fal-ai/veo3.1/fast`) — the wired baseline this release. With the default `integrations.media.videoGeneration.provider: auto`, video generation **follows the agent's main provider and reuses its key**, mirroring image generation: a `google` main resolves to **Veo** (reusing `GOOGLE_API_KEY`) and an `xai` main resolves to **Grok Imagine** (reusing `XAI_API_KEY`) — these native follow-main video adapters are wired in **Phase 190** (under `auto` a `google`/`xai` main currently resolves the selection but reports an honest "unavailable" naming Phase 190; use `provider: fal` + `FAL_KEY` in the meantime). There is **no video-specific env var** beyond reusing these provider keys.
