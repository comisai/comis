---
title: "Hot Reload"
description: "Config and skill hot reload mechanism reference covering SIGUSR2 restart and chokidar file watching"
icon: "rotate"
---
...
Comis has two distinct hot reload mechanisms: **config hot reload** uses RPC-based writes followed by a SIGUSR2-triggered process restart, while **skill hot reload** uses chokidar file watchers for discovery directory monitoring. These are completely separate systems.

## What reloads, and how

| Change | How it applies | Drops connections? |
|--------|---------------|--------------------|
| `config.patch` / `config.apply` (mutable sections) | Validate → write → 200 ms delayed `SIGUSR2` → process restart | Yes — supervisor (pm2/systemd) restarts the daemon. WS clients reconnect with restart-continuation. |
| `agents.create` / `agents.update` / `agents.delete` | Same path, but with a 2-second debounced `SIGUSR2` so batches coalesce | Yes |
...
| `gateway.restart` RPC | Direct `SIGUSR2` (200 ms delay to flush response) | Yes |
| `tooling.*` (entire subtree) | Runtime writes are rejected. Edit the config file, validate it, and restart manually; prompt shapes and install-detour policy are constructed at boot. | Yes |
| `capabilityServices.*` | Runtime writes are rejected. Edit the config file, validate it, and restart manually so linked service topology and authority roots change only at boot. | Yes |
...
The takeaway: **everything except skill files takes effect through a daemon restart.** The restart is graceful (ordered teardown, 45 s hard timeout by default) and active sessions are preserved across the bounce via the restart continuation tracker, so end users typically don't notice.

<Warning>
  Config hot reload is NOT file watching. The daemon does not watch config files for changes. Mutable config changes go through the RPC API, which validates, writes, and then sends SIGUSR2 to restart the daemon.
</Warning>

## Config Hot Reload

Mutable config changes are applied through RPC methods that validate the change, write to the local YAML file, and trigger a SIGUSR2 signal for process restart.

<Info>
  **Source:** `packages/daemon/src/api/config-handlers/` -- 10 RPC methods including `config.patch`, `config.apply`, `config.rollback`, `gateway.restart`.
</Info>
...
13. **200ms delayed SIGUSR2** -- schedule process restart to pick up new config
...
Restores config from git history to a specific commit SHA. Calls `configGitManager.rollback(sha)`, then sends SIGUSR2 for restart.

### persistToConfig (Internal Utility)

Used by management RPC handlers (agents, tokens, channels) to persist config changes. Follows the same validate-write-restart pattern but with a **2-second debounced SIGUSR2** instead of the 200ms delay used by `config.patch`. This allows batch operations (e.g., creating 8 agents in sequence) to coalesce into a single restart.

<Info>
  **Source:** `packages/daemon/src/api/shared/persist-to-config.ts` -- debounced SIGUSR2 for batch config operations.
</Info>
...
This includes the capability map and install-detour policy under `tooling` and
linked capability-service topology under `capabilityServices`. Those settings
define boot-time prompt construction, service activation, credentials, and
filesystem authority, so they are loaded only at boot. See
[Capability Services](/reference/config-yaml#capability-services) for the config
contract.
...
The `gateway.restart` RPC method triggers the same SIGUSR2 mechanism as config hot reload:
...
When SIGUSR2 is received, the daemon initiates an ordered teardown sequence and exits with code 42. A managed pm2 or systemd service then restarts the daemon with the updated config.
...
The daemon operations guide owns the shutdown order and its operator-visible log
shape. See [Shutting Down](/operations/daemon#shutting-down). The load-bearing
capability-service invariant is that new ingress closes and accepted report
mutations drain before the shared memory database closes.

### Hard Timeout

The shutdown handler has a hard timeout of **45 seconds** (default `timeoutMs`). Scheduled work first receives a 30-second drain window and then a 5-second cancellation grace before dependent adapters close. If the complete teardown sequence does not finish within the hard timeout, the process is forcibly terminated. This timeout must be less than the process manager's stop timeout (e.g., systemd `TimeoutStopSec`).
...
The SIGUSR2 handler triggers the same graceful shutdown used for SIGTERM/SIGINT, ensuring all components are properly cleaned up regardless of the restart trigger.
