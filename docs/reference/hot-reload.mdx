...
|--------|---------------|--------------------|
| `config.patch` / `config.apply` (mutable sections) | Validate → write → 200 ms delayed `SIGUSR1` → process restart | Yes — supervisor (pm2/systemd) restarts the daemon. WS clients reconnect with restart-continuation. |
...
This includes linked capability-service topology under `capabilityServices`.
Those settings define service activation, credentials, and filesystem authority,
so they are loaded only at boot. See
