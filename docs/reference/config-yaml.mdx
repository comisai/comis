...
`maxEvidenceBytes` (decoded evidence body bytes; ceiling `1048576`), and
`maxConcurrentRuns` (how many of the service's runs may be non-terminal at once;
ceiling `10000`). Each field is optional and can never exceed its ceiling — a
larger value is rejected at config validation. An absent field falls back to the
linked definition's own `limits`, then to the protocol ceiling. A service that
only ever produces small reports or evidence, or runs a bounded number of
concurrent jobs, can pin a smaller cap so an oversized payload or a runaway
fan-out — from a bug or a compromised service — is refused early rather than at
the global limit. A run refused for `maxConcurrentRuns` is rejected at activation
admission with reason `capacity_exceeded`, before any durable binding.
