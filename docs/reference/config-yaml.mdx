...
> chars per file (SD6), and the active-tool ceiling is set to 40 tools (SD7; overflow tools stay
> reachable via `discover_tools`). `frontier`/`mid` behavior is **byte-identical** to v2.14 (the
> non-regression guarantee). The security guarantee holds: a weaker model class cannot lower the
> platform's security posture; the scaffolding defaults reinforce it.
...
For `capabilityClass: small`, at most **40 tool schemas** are active in each prompt request. The cold long-tail (tools not in the core set and not recently used) is deferred.
...
- **`nano` class is not affected.** Nano already uses aggressive CORE_TOOLS-only deferral; the 40-tool ceiling does not change its behavior.
- **`frontier`/`mid` classes are not affected.** No ceiling applies — behavior is unchanged from v2.14.

#### Summary

| Class | bootstrap.maxChars | Active-tool ceiling | discover_tools reachability |
|-------|--------------------|---------------------|-----------------------------|
| `frontier` / `mid` | `20_000` (unchanged) | No ceiling | N/A |
| `small` | `3_500` (capability default) | 40 tools active | All deferred tools callable |
| `nano` | `3_500` (capability default) | CORE_TOOLS-only (existing) | All deferred tools callable |

> **Relationship to Phase 158 D1 (reliability defaults):** The GoalAnchor, `rag.baseFloor`, and verification critic defaults from Phase 158 follow the same precedence model — explicit per-agent config > capability default > off. The D2 capacity defaults (bootstrap budget, tool ceiling) are additive: all four default-on behaviors activate together for `small`/`nano` agents with no per-agent config required.
