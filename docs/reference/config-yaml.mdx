...
| `reflect.maxProcedureDocsSurfaced` | `number` (positive int) | `10` | Per-agent surface cap on **procedure** docs (the `orchestrate`-derived subset that records a tool footprint) listed in `<available_skills>` — the scaling guard, since there is no ranked top-K at surface time. Caps that subset only (oldest-first, so the most-corroborated procedures keep a slot); user-intent **skill** docs and **topic** docs are unaffected (separate, uncapped path) |
