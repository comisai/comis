...
| `reflect.corroboration.mode` | `"single_owner" \| "distinct_sessions"` | `"single_owner"` | How a topic must **corroborate** before it can seed a learned doc. `single_owner` (**default**, the primary Comis use case): **repetition-as-corroboration** — an **explicitly-trusted** owner (a sender named in `elevatedReply.senderTrustMap`, non-external tier) repeating the same success `≥minObservations` times corroborates it, so a single trusted DM (where distinct sessions is structurally unreachable) can learn. Safe by construction: a promiscuous-`defaultTrustLevel` or unknown sender can ride past the success filter yet never self-corroborate by repetition, and a box with ≥2 explicitly-trusted senders auto-falls-back to the stricter gate. `distinct_sessions`: the anti-domination gate — ≥2 distinct `(session, sender)` observations (one flooding actor stays cardinality 1); set it explicitly to require independent sessions even from a single trusted owner |
| `reflect.corroboration.minObservations` | `number` (int ≥ 2) | `2` | `single_owner` mode ONLY: the minimum distinct successful repetitions by the owner before a topic corroborates. `≥2` — a single success never corroborates. Ignored in `distinct_sessions` mode |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | `boolean` | `true` | Enable task extraction from conversations (on by default — the agent proactively follows up) |
