...
**Skill Import (agents.*.skills.import)**

Remote well-known and registry imports are disabled until their bases are
allowlisted here. The object is strict: unknown keys fail configuration load.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `registries` | `array` | `[]` | Operator-allowlisted remote bases. Each entry is `{ id, base, kind: "wellknown" \| "registry", trust?: "community" \| "operator" }`; trust defaults to `community` |
| `autoConnectBundledMcp` | `boolean` | `false` | Persist and connect bundled MCP declarations from community or agent-authored skills after validation |
| `maxArchiveBytes` | `number` | `8388608` | Maximum compressed archive bytes accepted before preflight (8 MiB) |
| `maxCompressionRatio` | `number` | `100` | Maximum declared expanded-to-compressed ratio for one archive entry |
| `indexCacheTtlMs` | `number` | `3600000` | Validated well-known index cache lifetime in milliseconds |
      ...
      import:
        registries:
          - id: public-index
            ...
            trust: community
          - id: community
            base: https://registry.example.com/api/v1
            kind: registry
            trust: community
        autoConnectBundledMcp: false
```

Registry publisher and security fields are retained as evidence but never
grant trust. Setting `trust: operator` is an explicit operator promotion. See
[Importing Skills](/skills/importing).
