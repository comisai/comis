...
| `registries` | `string[]` | `[]` | Allowlist of registry origins a `source: "wellknown"` import may resolve from (below). Default-empty = no registry imports |

**`registries` -- the well-known registry allowlist.** Unlike the size caps above, `registries` is a **security allowlist**, not a bound: it names the registry origins a `source: "wellknown"` import may resolve from. Entries are normalized origins `https://<host>[:port]` (matched exactly against the requested registry's origin) plus the reserved literal `clawhub` token. It is **default-empty**, so no registry imports are permitted until you add one -- archive and GitHub imports are unaffected either way. Changing it is a deliberate **config edit**, kept out-of-band from the import call: a non-allowlisted registry refuses **flatly** and `confirm` never overrides that refusal. See [Registry import](/skills/importing#registry-import-well-known-index).
