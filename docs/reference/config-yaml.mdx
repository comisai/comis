...
| `multilingual` | `boolean` | `true` | Advisory: declare the embedder multilingual for the `comis system-health` model-health line. Explicit overrides take precedence over model-id inference. Does not gate search — the FTS5 trigram floor carries recall regardless. See [Multilingual](/operations/multilingual). |
