...
agents:
  default:
    ...
    memoryConsolidation:
      ...
      schedule: "30 3 * * *"
    contextEngine:
      version: pipeline             # the default; set "dag" to opt into the lossless LCD engine
...
Context engine configuration. Controls the system that manages what your agent sees each turn. The default is **pipeline** (the sequential layered engine). The **dag** value (the v2.12 lossless LCD engine) is available as an **opt-in** mode — set `version: "dag"` to use it (the default stays `"pipeline"` until a later release). In the current release DAG does lossless verbatim assembly (full faithful history + a verbatim fresh tail of the most recent steps + transcript repair); compaction, summarization, and the on-demand recall tools land in later phases. See [Compaction](/agents/compaction) for a user-friendly explanation.
...
| `version` | `string` | `"pipeline"` | Operating mode: `"pipeline"` (default, the sequential layered system) or `"dag"` (opt-in lossless LCD engine — lossless verbatim assembly; compaction/recall land later) |
...
**DAG mode fields** (`version: "dag"`) — `freshTailTurns` is active in the current release (lossless verbatim assembly); the compaction/recall keys below it still parse but are **reserved and dormant** until those features land in a later phase (DAG today keeps everything verbatim with no compaction):
...
| `contextThreshold` | `number` | `0.75` | Budget utilization ratio that triggers compaction (0.1-0.95) — reserved; no compaction yet |
