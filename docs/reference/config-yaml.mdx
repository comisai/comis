...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `profile` | `enum` | `"standard"` | `assistant`, `standard`, `unattended`, or `max`. `standard` is the zero-config default. `unattended`/`max` resolve to `standard`-equivalent in this release plus an "available later" notice (no silent over-grant). |
...
| `generalize.enabled` | `boolean` | `false` | Verified Learning (v2.26): a second consolidation pass that synthesizes a higher-order "user prefers X in general" `semantic` memory (`observation_kind='generalization'`) from a diverse cluster. Opt-in (default off — consolidation is byte-identical when off); abstains on small/nano models. The synthesized node is non-destructive (sources kept + linked, trust never raised above `learned`) and is supersedable by a later contradicting triple |
| `generalize.minDistinctContexts` | `number` | `3` | Minimum distinct `(session, sender)` contexts a cluster must span before it generalizes — an anti-domination gate so one repeated session cannot mint a "general" preference |

#### Learning Outcome (agents.*.learningOutcome)

The **Verified Learning** outcome signal (v2.26 WS1): a durable record of each finished trajectory's net task-outcome (`success` / `failure` / `corrected` / `unknown`), so learning can gate on a real task-outcome instead of a text-overlap proxy. **Default-OFF** and **gated by the master cost switch** (`memory.costFeatures.enabled` -- when that is `false`, `learningOutcome` is force-disabled regardless of the per-agent flag). The deterministic tool/pipeline signal is the primary source; the cost-gated LLM judge (`judge.enabled`, default **on** / opt-out) is the **fallback** that resolves a CONVERSATIONAL turn — one with no deterministic tool/pipeline signal, which would otherwise fuse to `unknown` and derive no learning. The judge runs ONE cheap-model pass ONLY when the deterministic resolve is `unknown` (so resolved turns skip it) and is force-disabled by the master cost switch. The capture is daemon-side and content-free -- ids / closed enums / counts only, never a message body. When enabled, observations are written to the `outcome_events` ledger (see [data-directory.mdx](/operations/data-directory)) and surfaced on the trajectory for `comis explain` (the `learning` obs signal) and via `comis memory learning`.
...
#### Learning Skills (agents.*.learningSkills)

The **Verified Learning** procedural-skill loop (v2.26 WS2): an offline cron distils a reusable, **sandbox-validated** procedure ("how to do X") from a cluster of successful trajectories and admits it to the [`learned_skills` table](/operations/data-directory) at `trust=learned`. **Default-OFF** and **gated by the master cost switch** (`memory.costFeatures.enabled` -- when that is `false`, `learningSkills` is force-disabled regardless of the per-agent flag); with the default config the synthesis cron registers no job and a default install is byte-identical. When enabled, an admitted procedure that has **promoted to `active`** and is **read-only** is **surfaced** to the agent (v2.26 P3): it is listed in `<available_skills>` (appended after platform skills so the prompt-cache prefix stays stable — see [prompt cache](/agents/cache#learned-skill-listing-stability)) and materialized to a per-agent `.learned-skills/<name>/SKILL.md` listed by its absolute path (the same shape platform skills use, so the `read` tool opens it directly and skill-use attribution matches). Promotion is gated by `promoteAtProofCount` from attributed successful reuse; corroborated, sustained failing reuse demotes (`active` → `stale` → `archived`) and drops it from the surface. The whole surface is default-off — with `learningSkills` off (or no store) the listing is byte-identical to a platform-only install. Every admission is content-free at the trust boundary: the synthesizer wraps the trajectory as untrusted input, and a procedure can **never** exceed `learned` trust (a DB `CHECK` makes `system` structurally impossible — see [data-directory.mdx](/operations/data-directory)).

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | `boolean` | `false` | Enable procedural-skill synthesis for this agent (opt-in; additionally force-disabled when `memory.costFeatures.enabled` is `false`). Unlike the surrounding `memory*` cost features (which default opt-out/ON), this defaults OFF — the byte-identity guarantee depends on it |
| `validation.requireReproduction` | `boolean` | `true` | A candidate carrying embedded scripts must **reproduce** its effect in the sandbox before admission (fail-closed). The dynamic sandbox replay is **Linux/bwrap-only**: on a host without a usable bwrap jail (e.g. macOS, or a kernel that rejects it) validation **degrades to static-only** (`coverage:'static-only'`) and the embedded scripts are **never executed unsandboxed** — a scripted candidate that could not be reproduced is then not admissible |
| `autoAdmitReadOnly` | `boolean` | `true` | A read-only validated candidate (no mutating tools) **auto-admits** without approval. A read-only procedure cannot change host state, so it is admitted directly at `state=candidate` |
| `approval.requireForMutating` | `boolean` | `true` | A **mutating** candidate (any non-read or `mcp__`-prefixed required tool) routes through the **approval gate** and is admitted **only on approval** — it never auto-admits. This is the SEC-01 belt: an unattended mutating procedure cannot enter the store on its own |
| `minConfidence` | `number` (0–1) | `0.7` | The confidence floor a candidate must clear before admission |
| `promoteAtProofCount` | `number` (positive int) | `3` | The verified-success count at which an admitted `candidate` procedure promotes to `active`. The promote loop (v2.26 P3) reinforces on each **attributed successful reuse** and flips `candidate` → `active` once `proof_count` crosses this bar (a single success below it never mints an `active` procedure) |

> **Admission is fail-closed and anti-domination-bounded.** A candidate is admissible only when it is statically clean **and** (reproduced **or** read-only) **and** (read-only auto-admit **or** an approved mutating procedure); a non-reproduced mutating candidate is never admissible and never even reaches the approval gate. Synthesis additionally clusters successes by **distinct `(session, sender)`** (so N near-identical successes from one sender count once) and writes a **low** initial proof count regardless of cluster size — two independent belts so a procedure cannot be minted to a high trust by repeating one success. The whole loop is **observable** (counts/ids only) via [`comis explain`](/reference/cli#comis-explain) (the `learning` signal + the `learned_skill_failing` / `synthesis_abstained_low_capability` verdicts) and the [`comis memory skills`](/reference/cli#comis-memory) admission funnel.

> **Model tier `skillSynthesis`.** Procedural synthesis runs on the **`mid`** model operation tier (`skillSynthesis`) — an offline batch synthesis op (not a fast classify), resolved by name and injected daemon-side, capability-routed like the other operation tiers. On a `small`/`nano` agent below the synthesis capability gate the loop **abstains** (Defer ≠ Retry — a benign skip, never a failure or a retry); it is dormant while `learningSkills.enabled` is `false`.
...
#### User Representation (agents.*.memoryUserRepresentation)

The per-user profile builder. On by default in v1 (`enabled: true`, opt-out — a cost feature, so force-disabled by `memory.costFeatures.enabled: false`). As of **Verified Learning (v2.26)** the builder **revises** the profile instead of rebuilding it: the `user_representation` table is **bi-temporal**, so a contradicting profile fact of **equal-or-higher trust soft-closes** the incumbent belief (`t_valid_end`/`expired_at` — never hard-deleted, still resolvable via `asOf`) while a **lower-trust** contradiction is **recorded-not-believed** (anti-poison) and a restatement bumps confidence in place. The default profile read returns **current truth only**. See the bi-temporal [`user_representation` columns](/operations/data-directory#user-representation-verified-learning-v2-26) for the storage detail; the revision activity surfaces counts-only on `comis explain` (the `learning` signal's `userModelRevised` count).

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | `boolean` | `true` | Build a per-user representation for this agent (opt-out; force-disabled by `memory.costFeatures.enabled: false`) |
| `historyCap` | `number` | `10` | Verified Learning (v2.26): bounded `asOf` history — superseded profile rows for a `(user, entryType)` slot beyond this many are trimmed **oldest-`expired_at`-first** (the one sanctioned delete; anti-unbounded-growth). A recent `asOf` within the cap still resolves |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `repairProducer` | `boolean` | `false` | **AUTHOR-01.** Enable the conservative weak-model graph repair producer. When a weak-tier model emits a schema-invalid pipeline graph, the daemon attempts a deterministic match to a canonical template (`research-fanout` / `debate` / `vote` / `map-reduce`) and repairs it; an ambiguous match returns a structured "did you mean this template?" instead of synthesizing. The calling tier is resolved **server-side** (never tool-supplied). Off → the existing fail-closed validation error throws unchanged. |
| `intentAction` | `boolean` | `false` | **AUTHOR-02.** Enable the `pipeline` tool's `from_intent` action and its deterministic synthesizer — a one-line intent (`{ pattern, agents \| tasks, budget? }`) expands into a validated, **template-shaped** graph (plain agent nodes, not the typed multi-round drivers). Off → the daemon refuses any `from_intent` call before a graph runs. See [Pipelines: from_intent](/agent-tools/pipelines#from-intent). |
| `gbnfConstrain` | `boolean` | `false` | **AUTHOR-03 (best-effort).** Grammar-constrain the raw `pipeline` tool schema for GBNF-eligible local providers (the local/default family — never a cloud provider by name). This is a removal/relaxation-only structural transform; it **never widens field value validation**. The daemon-side schema validation remains the single source of truth. Off → byte-identical schema normalization. |
...
orchestration:
  authoring:
    repairProducer: false   # AUTHOR-01 — flip on once telemetry justifies it
    intentAction: false     # AUTHOR-02
    gbnfConstrain: false    # AUTHOR-03 (best-effort)
```
