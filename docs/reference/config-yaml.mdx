...
agents:
  default:
    ...
    memoryConsolidation:
      ...
      schedule: "30 3 * * *"
    contextEngine:
      version: dag                  # the default; set "pipeline" to opt out
...
Context engine configuration. Controls the system that manages what your agent sees each turn. Supports two operating modes: **dag** (default, the lossless DAG with recall tools) and **pipeline** (opt-out, the 8-step sequential system). See [Compaction](/agents/compaction) for a user-friendly explanation.
...
| `version` | `string` | `"dag"` | Operating mode: `"dag"` (default, lossless DAG with recall tools) or `"pipeline"` (opt-out, the 8-step sequential system) |
...
OAuth credential storage configuration for providers using subscription-based authentication (currently OpenAI Codex). Stored separately from the read-only [`SecretManager`](/reference/secret-manager) so the daemon can refresh tokens at runtime without violating the secret-store's read-only contract.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `storage` | `"file"` \| `"encrypted"` | `"file"` | Storage backend for OAuth profiles. See [OAuth concepts -> Storage backends](/security/oauth#storage-backends) for the full comparison. |

**`storage: "file"`** -- plaintext JSON at `~/.comis/auth-profiles.json` with mode `0o600` (user-only read). Atomic write (tmp -> fsync -> rename) with cross-process file locking via `proper-lockfile`. Hot-reload via chokidar with 100ms debounce -- `comis auth login` writes are picked up by the daemon without restart.

**`storage: "encrypted"`** -- AES-256-GCM-encrypted rows in the `oauth_profiles` table inside the same SQLite database as `secrets.db`. Each row stores the full profile JSON as one ciphertext blob with per-row IV/auth-tag, plus a denormalized `expires_at` column for fast doctor scans.

<Info>The `encrypted` backend requires [`SECRETS_MASTER_KEY`](/reference/environment-variables#secrets_master_key) to be set. The daemon fails to start with a clear hint otherwise. See [Setting up the encrypted store](/security/secrets#setting-up-the-encrypted-store) for key generation.</Info>

<Warning>The `encrypted` backend does **not** support hot-reload from external processes (e.g., `comis auth login` from a different shell). The SQLite WAL journal masks `fs.watch` change events. Restart the daemon to pick up new profiles. The `file` backend has no such limitation.</Warning>

```yaml title="~/.comis/config.yaml"
oauth:
  storage: file        # default -- plaintext, hot-reloadable, mode 0o600
  # storage: encrypted   # production -- AES-256-GCM, requires SECRETS_MASTER_KEY
```
...
The tool-first capability layer (v1.1+). Operator-only — agents cannot self-configure capability routing or detour policy. The entire `tooling` tree is added to `IMMUTABLE_CONFIG_PREFIXES` and rejected by `config.patch` from agent-callable surfaces.

<Warning>
  **Restart required.** Changes to `tooling.capabilityIndex.enabled` and `tooling.installDetours.mode` apply only after the daemon restarts. The `capabilityIndex.enabled` toggle selects between two cached system-prompt shapes (one-line residual vs flat tool dump); in-process reload is not supported in v1.1. Operator config edits go through the standard `config.patch` / `config.apply` path which validates → writes → 200 ms delayed `SIGUSR1` → process restart.
</Warning>
