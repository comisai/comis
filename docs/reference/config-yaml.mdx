...
| `intentAction` | `boolean` | `false` | **AUTHOR-02.** Enable the `pipeline` tool's `from_intent` action and its deterministic synthesizer — a one-line intent (`{ pattern, agents \| tasks, rounds?, budget? }`) expands into a validated graph. Off → the daemon refuses any `from_intent` call before a graph runs. See [Pipelines: from_intent](/agent-tools/pipelines#from-intent). |
