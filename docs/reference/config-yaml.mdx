...
Override the model and timeout for specific internal operation types. Each entry is an `OperationModelEntry` with optional `model` (string `"provider:modelId"`) and `timeoutMs` (number) fields. Omit any type to use the agent's primary model.
...
| `contextThreshold` | `number` | `0.75` | Budget utilization ratio (of the model window) that triggers an end-of-turn leaf-summarization pass (0.1-0.95) |
...
**Prompt timeout for local Ollama.** The default `promptTimeoutMs` (180,000 ms — 3 minutes) is sized for cloud APIs that typically respond in under 60 seconds. A local model with slow inference (e.g., qwen3.6 on a loaded GPU with multiple concurrent agents) can exceed this. For local deployments, set a longer timeout:

```yaml
agents:
  my-agent:
    promptTimeout:
      promptTimeoutMs: 300000   # 5 minutes — recommended for local qwen3.6 under load
```
...
capability: `contextEngine.budget.effectiveContextCapSmall` / `effectiveContextCapNano`
(`0` = uncapped) or a higher `providers.entries.<id>.capabilities.capabilityClass` pin;
configured: `providers.entries.<id>.models[].contextWindow`. When tool schemas dominate the
...
posture) — and because the boot floor and the turn-time preflight share one source module, the
same numbers re-appear in any later turn-time `context_exhausted` for that agent.
