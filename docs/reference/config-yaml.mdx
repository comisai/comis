...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `agents.<id>.goalAnchor.enabled` | `boolean` | automatic for small/nano (no config needed); false for frontier/mid | Enable GoalAnchor tail injection. When `true` (or capability-default-on for small/nano) and `scaffoldLevel=max` (small/nano), the execution objective and uncompleted step checklist is tail-appended each turn. Default `false` for frontier/mid = opt-in, behavior-neutral. |
...
> `frontier`/`mid` behavior is **byte-identical** to v2.14 (the non-regression guarantee).
> The security guarantee holds: a weaker model class cannot lower the platform's security posture;
