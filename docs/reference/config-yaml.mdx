...
| `leafChunkTokens` | `number` | `20000` | Maximum source tokens per leaf summary chunk (1K-100K). Clamped at runtime to the resolved summarizer model's context window minus the summary target and prompt overhead, so a small compaction summarizer (e.g. an 8K operationModels.compaction model) is never fed an over-window chunk; oversized backlogs drain across multiple bounded passes. |
...
<Note>
  **Engine scope — served-window honesty and the viable floor (the recorded pipeline-parity verdict).**
  ...
  summarizer's window and keeps the un-summarized remainder in context (never dropped); the
  85% trigger re-fires on later turns until the backlog drains.
