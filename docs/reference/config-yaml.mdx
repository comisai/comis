...
Comis uses a single `config.yaml` file (or multiple layered files) to configure every aspect of the system. Its top-level sections define agent behavior, channel adapters, security policies, gateway settings, capability services, and more. All schemas use Zod strict validation -- unknown keys are rejected at startup.

Config files are selected with [`COMIS_CONFIG_PATHS`](/reference/environment-variables#comis_config_paths). See the [Configuration Guide](/installation/configuration) for a step-by-step setup walkthrough.
...
Top-level configuration sections at a glance:

| Section | Type | Description |
|---------|------|-------------|
| `tenantId` | `string` | Tenant identifier for multi-tenancy |
| `identity` | `IdentityConfig` | Authenticated platform-subject to principal mappings |
...
| `capabilityServices` | `CapabilityServicesConfig` | Restart-only linked service topology and managed-run bounds |
...
<Accordion title="identity.principalMappings">

Trusted mappings from authenticated platform assertions to stable internal
principals. An unmapped assertion receives a deterministic namespaced principal;
display names and other adapter metadata are not identity evidence. The section
defaults to an empty mapping list and is operator-only at runtime.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `principalMappings` | `PrincipalMapping[]` | `[]` | Exact mappings scoped by tenant, agent, channel type, channel instance, and platform subject |

Each mapping requires `tenantId`, `agentId`, `principalId`, and an `assertion`
with `channelType`, `channelInstanceId`, and `platformSubjectId`.
  ...
  principalMappings:
    - tenantId: my-project
      ...
      assertion:
        ...
        channelInstanceId: primary-bot
        platformSubjectId: "123456789"
      principalId: customer-123
```
...
| `cacheRetention` | `enum` | `"long"` | Anthropic prompt cache retention: `none`, `short`, `long`. This sets the **warm** target, not the rate every write pays. A session starts cold at 5m so its first write is cheap, then escalates to the configured value once the cache is observed being read — immediately on turn 2 when the first write exceeded 20K tokens, otherwise after 3 turns. A session whose cached prefix keeps collapsing is held down at 5m rather than repeatedly paying the 1h write premium. `short` and `none` disable the escalation (there is nothing to step up to). |
...
| `language` | `string` | _(unset)_ | Explicit response locale, governing the LLM's reply language and locale-quality observability. It does **not** by itself translate the deterministic platform replies (the canned timeout / context-exhausted / output-starved / loop-halted notices) — those come from a string table, and the runtime ships only English. Supply the strings for your language in [`localePacks`](#locale-packs) alongside this key. Must be a canonical BCP-47 tag (`"he"`, `"pt-BR"`) — validation rejects anything else. When omitted, Comis resolves clear natural-language prose in the current message to a script-only locale such as `und-Hebr` or `und-Latn`, then falls back to the channel-supplied request locale (e.g. the Telegram client's `language_code`) for messages too short or identifier-heavy to carry a reliable script signal, else leaves it unset. The explicit key and high-confidence current-message script tier enforce the reply script through the bounded repair and tool-failure-preservation rules documented in [Multilingual](/operations/multilingual). The device-locale fallback remains advisory. Clear current-message prose outranks the client UI locale and earlier conversation turns. Set this key when you need a fixed reply language regardless of any individual message. |
| `localePacks` | `object` | _(unset)_ | <a id="locale-packs"></a>Operator strings for the deterministic platform replies, keyed locale tag → message id → text: `localePacks: { he: { pipeline_timeout: "..." } }`. The runtime holds no human language but English, so this is the only way those replies match the language your users are answered in. Locale keys are canonical BCP-47 (same validation as `language`); a pack is selected by exact tag, else by base language (`he-IL` → `he`), else — for a **script-only** reply locale such as `und-Hebr`, which is what the [script tier](/operations/multilingual#configuration-keys) resolves when no `language` is set — by the base language that script most likely implies (`und-Hebr` → `he`, `und-Hans` → `zh`), so a script-detected reply still gets your strings instead of an English notice appended to a non-English answer. That last step is a likely-subtags guess (`und-Cyrl` maximizes to `ru`, not `uk`), so it runs last and can only ever select a pack you supplied — it never invents a language. Any id you do not supply falls back to English. Valid ids (43): `context_exhausted`, `cause_oversized_input`, `cause_oversized_history_message`, `cause_fixed_overhead_exceeds_window`, `advice_default`, `advice_history`, `advice_fixed_overhead`, `output_starved`, `loop_detected`, `tool_failure_notice`, `tool_failure_notice_unnamed`, `prompt_timeout`, `tool_invocation_stall_missing_configuration`, `tool_invocation_stall_no_receipt`, `execution_failed`, `background_task_failed_notice`, `delegation_evidence_missing`, `delegation_evidence_started`, `persistent_action_evidence_missing`, `outbound_audio_evidence_missing`, `outbound_image_evidence_missing`, `outbound_delivery_status_evidence_missing`, `destructive_action_not_verified`, `provider_requires_model`, `agent_update_noop`, `ongoing_work_evidence_missing`, `runtime_self_report_evidence_missing`, `runtime_self_report_evidence_unsupported`, `scheduler_state_evidence_missing`, `pending_scheduler_confirmation`, `completion_evidence_missing`, `sender_authority_overclaim`, `vision_unavailable`, `response_locale_unavailable`, `pipeline_timeout`, `background_pending_running`, `background_pending_ready`, `background_pending_updates`, `activity_card_approval_required`, `activity_card_detail_server`, `activity_card_detail_credential`, `activity_card_detail_command`, `activity_card_detail_secret`. The last five cover the **activity/approval card** — the prompt a user taps to approve a gated tool call, plus the words labelling its details. `activity_card_approval_required` takes `{operation}` (the tool call being authorized) and `{details}` (its redacted specifics); both are substituted verbatim, so a pack can translate the surrounding words but never rename what is being approved, and a template that omits `{details}` drops them rather than having them appended. `tool_invocation_stall_missing_configuration` — the reply for a turn a tool stalled because its configuration is missing — takes `{tool}` (the stalled tool) and `{configKey}` (the exact key to set); both are substituted verbatim, and a template that omits either still ends with the identifiers, because the reply is unactionable without them. Without a pack entry the card renders exactly as before, in English. An unrecognized id is ignored and logged at WARN naming the exact path, so a typo is never silently dead config. Three deterministic replies are deliberately **outside** this table and always answer in English: the input guard's pre-dispatch refusals — the over-cap size notice, the opaque-payload clarification ask (the `requestClarification` section in [`obs.explain`](/reference/json-rpc#obs-explain)), and the blocked-by-security-policy notice — carry no message id, so neither `language` nor a pack translates them. |
...
| `enabled` | `boolean` | _(from profile)_ | Whether autonomy surfaces are on. Defaults from the profile (`standard` -> true, `assistant` -> false); an explicit value overrides it. ⚠ **Omitting the whole `autonomy` block resolves to enabled, but adding any sub-key without also setting `enabled: true` can resolve to DISABLED** (via the profile default) — so writing e.g. only `autonomy.durability` may turn the autonomy surfaces off. With autonomy disabled for **every** agent, cron jobs and the heartbeat are **not armed**; the daemon boots and logs an ERROR naming this key. |
...
| `subagent` | `180000` (3 min) | Sub-agent task delegation | Deliberately **above** `integrations.mcp.callToolTimeoutMs` (`120000`) so a delegated turn whose first action is a slow MCP call sees the tool's own timeout instead of being killed at the same instant. Matches the `promptTimeoutMs` default, so a delegated turn gets no less room than a direct one. |
...
Trust labels are free-form for `trustModelRoutes` / `trustPromptOverrides`, but the
control-plane gate reads only `admin` / `user` / `guest` -- see
[Deny-by-origin on the control plane](/security/capability-model#deny-by-origin-on-the-control-plane)
for how a label (including the `external` default) maps to authority. Both keys are
operator-file-only: no runtime RPC can set them.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `freshTailTurns` | `number` | `8` | Minimum number of recent **steps** (assistant + tool round-trips, not user-turns) kept verbatim (1-50). The effective tail also reaches back to the store's persisted horizon so every unpersisted message from the current turn, including its originating request, remains covered. A coarse guard may reduce the configured step floor on a genuinely small context window (below roughly 11K tokens); residual token bounding can then discard completed tool segments but pins the originating request. `obs.explain.contextBudget` reports the configured/effective counts and whether that request survived. |
...
| `correction.enabled` | `boolean` | `true` | Enable the cost-gated correction detector. A classified follow-up emits a `corrected` soft-failure for the prior trajectory. When the sender is explicitly named at a non-external trust tier and `learning.reflect.corroboration.mode` is `single_owner`, the correction immediately stales an attributed learned skill and the correction turn cannot add proof to it. Other sources retain the confidence, corroboration, and trend gates. Force-disabled by `memory.enabled: false` |
...
> - **Reactor trust-weighting.** The confidence is scaled by the reactor's channel-sender trust (`elevatedReply.senderTrustMap[reactorId]`, falling back to `elevatedReply.defaultTrustLevel`). An **unknown / external** reactor is capped near zero (~0.05) — a stranger's 👍 barely moves the needle. The `defaultTrustLevel` fallback is inherited only by the **conversation participant** — the sender whose message produced the reply being reacted to. Any other unmapped reactor (a bystander in a group) is treated as external, so weighting a reply they did not prompt requires an explicit `senderTrustMap` entry.
...
Corroborated failure plus a weakening trend can demote it. A classified
correction from the explicitly trusted owner in `single_owner` mode immediately
stales the attributed skill. Other correction sources retain the corroboration
and trend gates and do not guarantee demotion.
...
**Terminal writable overlays (agents.*.skills.terminal.allow[].scope)**

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `credentialPaths` | `string[]` | `[]` | Host credential paths exposed read-only inside the terminal jail. Home-relative values beginning with `~/` are expanded against the daemon account's home. |
| `ephemeralWritablePaths` | `string[]` | `[]` | Existing host directories replaced inside the jail by empty, non-persistent tmpfs mounts. Every target must be absolute or home-relative and must already exist as a real directory. A missing, unreadable, symlinked, or non-directory target rejects terminal creation before bubblewrap starts and identifies `agents.*.skills.terminal.allow[].scope.ephemeralWritablePaths` as the configuration field to fix. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | `boolean` | _(unset)_ | Override heartbeat enabled state |
| `intervalMs` | `number` | _(unset)_ | Override heartbeat interval in ms |
| `showOk` | `boolean` | _(unset)_ | Override show OK status |
| `showAlerts` | `boolean` | _(unset)_ | Override show alerts |
| `target` | `ChannelEndpoint` | _(unset)_ | Exact delivery endpoint (`channelType`, `channelInstanceId`, `conversationId`, optional `threadId`, and `conversationKind`) |
| `prompt` | `string` | _(unset)_ | Custom heartbeat prompt |
| `allowDm` | `boolean` | _(unset)_ | Allow heartbeat delivery to direct conversations |
...
| `alertThreshold` | `number` | _(unset)_ | Consecutive failure threshold for alerting |
| `alertCooldownMs` | `number` | _(unset)_ | Alert cooldown period in ms |
| `staleMs` | `number` | _(unset)_ | Stuck detection timeout in ms |
| `toolPolicy` | `ToolPolicy` | _(unset)_ | Additional heartbeat restriction applied after the agent policy; it cannot restore a denied tool or capability |
...
| `multilingual` | `boolean` | _(inferred)_ | Advisory override for the `comis system-health` model-health line. When omitted, Comis infers the posture from the selected model id; explicit values take precedence. Does not gate search — the FTS5 trigram floor carries recall regardless. See [Multilingual](/operations/multilingual). |

**Local (embedding.local)**

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `modelUri` | `string` | `"hf:gpustack/bge-m3-GGUF:bge-m3-Q8_0.gguf"` | HuggingFace model URI or path to local GGUF file |
| `modelsDir` | `string` | `"models"` | Directory to store downloaded models |
| `gpu` | `enum` | `"auto"` | GPU acceleration: `auto`, `metal`, `cuda`, `vulkan`, `false` |
| `contextSize` | `number` | `2048` | Context size for embedding model inputs (tokens). |
...
| `secret` | `string \| SecretRef` | _(unset)_ | Secret value (min 32 chars, enforced after environment or SecretRef resolution; auto-generated only when no value is configured) |
...
| `defaultDebounceMs` | `number` | `0` | Delay before collected pending messages run; applies only in `collect` mode, after the first message starts immediately |
...
Bounds safe durable recovery attempts for sub-agent completion delivery. See [Resilience: Self-healing delivery retries](/agents/resilience#self-healing-delivery-retries).

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `maxRetries` | `number` | `3` | Maximum automatic recovery attempts for a ledgerless completion delivery whose platform call definitively failed. Range `0`–`10`; `0` quarantines the retained entry without a recovery attempt. Governed, in-flight, unresolved, validation-blocked, and receipt-ambiguous outcomes are parked immediately and never replayed automatically. |
...
| `defaultMode` | `enum` | `"require"` | Decision for actions no rule matches. `"auto"` makes the rule list a denylist: unmatched actions proceed unprompted |
| `rules` | `ApprovalRule[]` | `[]` | Ordered rules; the first whose `actionPattern` matches the action wins |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `actionPattern` | `string` | _(required)_ | Action pattern; `*` matches any run of characters, every other character is literal, and the match is anchored |
| `mode` | `enum` | `"auto"` | Decision when the pattern matches: `auto`, `require`, `deny` |
| `timeoutMs` | `number` | _(unset)_ | Timeout for the prompt a `require` rule produces. Omit to keep `defaultTimeoutMs`; must be positive when set |
| `minTrustLevel` | `enum` | `"admin"` | Trust a requester must reach for an `auto` rule to approve: `guest`, `user`, `admin`. Below the floor the action is put to a human |

<Warning>
  Rules narrow what reaches a human; they are not a universal action-policy
  engine. A rule is consulted only where an execution path already calls
  `ApprovalGate.requestApproval()`, so a rule cannot add a gate to an unguarded
  path — and `security.actionConfirmation` is a separate setting. See
  [Approvals](/security/approvals) for the currently guarded paths.
</Warning>
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `callToolTimeoutMs` | `number` | `120000` | Default timeout for MCP tool calls in ms. On expiry the tool error **names this key and its current value** and states that an identical retry re-expires the same deadline, so the caller narrows the request (smaller page / date window / fewer entities) instead. This key is an **immutable config path** — an agent cannot patch it via the `gateway` tool, so the error says so explicitly and directs the change to an operator. Raising it means editing this file and restarting the daemon. Slow report and media tools may need well over two minutes. This is an **absolute** ceiling: a server that streams progress notifications resets the idle portion of the deadline but can never exceed this total, so one chatty tool call cannot consume a whole turn's budget. The deadline covers the **whole** call as the caller experiences it — lazy reconnect and the wait for a per-server concurrency slot are charged against it, not just the request — so parallel callers fanning out onto one server can no longer overrun it by serializing behind each other. A call whose queue wait alone consumes the deadline fails **without being sent**, with an error that names contention (not request scope) as the cause and points at that server's `maxConcurrency`; unlike a deadline expiry it is not deterministic, so retrying it is reasonable. **Keep it strictly below `agents.<id>.operationModels.subagent.timeout`** (`180000` by default, chosen to clear this deadline): when the two are equal, the enclosing stall budget fires first and a sub-agent whose first action is a slow MCP call aborts with `0` steps before the tool can report its own timeout. The shipped pair is already safe — this only bites if you raise this value or lower that one. `comis system-health` flags that pairing as `config_posture:tool_deadline_collision`. |
...
| `transport` | `enum` | _(inferred)_ | `stdio`, `http`, or legacy `sse`; omitted values infer `stdio` from `command` or `http` from `url` |
| `command` | `string` | _(unset)_ | Command for stdio transport |
| `args` | `string[]` | _(unset)_ | Arguments for stdio command |
| `url` | `string (URL)` | _(unset)_ | URL for HTTP or SSE transport |
...
| `maxConcurrency` | `number` | transport default | Positive integer overriding the per-server tool-call concurrency. The transport defaults are 1 for stdio and 4 for HTTP/SSE. |
| `supportsParallelToolCalls` | `boolean` | `false` | For stdio only, opt into an implicit concurrency of 4 when `maxConcurrency` is omitted. Ignored for HTTP/SSE, whose default is already 4. |
...
| `defaultProvider` | `string` | _(unset)_ | Explicit vision provider. When set (non-empty), it **overrides** the follow-main behavior and the registry serves this provider directly — a hard pin, not a preference: if the pinned provider is unconfigured or cannot serve the requested media kind, the request **fails closed** with an honest "unavailable" instead of falling back down the `providers` chain. When unset, `image_analyze` follows the agent's main vision-capable model first (see the precedence Note below). |
...
1. **Explicit `defaultProvider`** -- when set (non-empty), the vision registry serves that provider and **only** that provider; this wins over everything, including the fallback chain below.
...
3. **The vision registry chain** (`providers`, default `openai -> anthropic -> google`, first with a configured key) -- when the main model is not vision-capable, or as the fallback if a main-vision attempt fails at runtime. The chain applies **only** while `defaultProvider` is unset; an explicit pin replaces it.
4. **Honest "unavailable"** -- when no tier can serve, with an error kind and a hint naming the knob to set.

`describe_video` (raw video) is **Gemini-only** and does not follow the main provider -- it routes straight to the registry's Google provider (no frame extraction this release). It honours the same pin, so a `defaultProvider` that cannot serve video (e.g. `openai`) makes video description unavailable even when `GOOGLE_API_KEY` is set -- leave `defaultProvider` unset, or pin `google`, when the deployment needs video. The existing `providers` / `defaultProvider` / `scopeRules` keys are unchanged; the follow-main behavior adds no new vision config key.
</Note>
...
`capabilityServices` configures linked external capability-service instances and
the durable managed-run limits enforced by the daemon. With the default empty
`instances` list, the platform is inert. The whole section is restart-only and
operator-controlled: edit the config file, validate it, and restart the daemon;
runtime `config.patch` and `config.apply` calls cannot change it.

```yaml
capabilityServices:
  privateContentDirectory: managed-runs/private
  reportRetentionMs: 2592000000
  maxObservedClockSkewMs: 300000
  recoveryBatchSize: 256
  requestDeadlineMs: 5000
  instances:
    - serviceInstanceId: example-service-1
      ...
      control:
        ...
        credentialRef: secret://capability-services/example-service-1
      allowedAgents: [assistant]
      allowedWorkspaceRoots: [/srv/comis-workspaces]
      allowedRuntimeRoots: [/srv/example-service/runtime]
      limits:
        maxReportBytes: 8192
...
An instance may declare `limits` to **tighten** the protocol ceilings for itself:
`maxReportBytes` (combined report summary + details bytes; ceiling `16384`),
`maxEvidenceBytes` (decoded evidence body bytes; ceiling `1048576`),
`maxConcurrentRuns` (how many of the service's runs may be non-terminal at once;
ceiling `10000`), and `maxReportsPerMinute` (how many reports one run may send
inside a rolling minute; ceiling `10000`). Each field is optional and can never
exceed its ceiling — a larger value is rejected at config validation. An absent
field falls back to the linked definition's own `limits`, then to the protocol
ceiling. A service that only ever produces small reports or evidence, runs a
bounded number of concurrent jobs, or emits at a bounded rate can pin a smaller
cap so an oversized payload, a runaway fan-out, or a report flood — from a bug or
a compromised service — is refused early rather than at the global limit. A run
refused for `maxConcurrentRuns` is rejected at activation admission with reason
`capacity_exceeded`, before any durable binding; a report refused for
`maxReportsPerMinute` is rejected at report ingress with the retryable
`rate_limited` error, before any private body is written.

The `serviceDefinitionId` and `mcpServerName` must match a linked contribution.
The control transport is Unix-socket only; `socketPath` must be normalized,
absolute, beneath the daemon's canonical owner-only data directory, and at most
103 UTF-8 bytes. The daemon creates the control socket with mode `0600`.
`credentialRef` must be a `secret://` reference. Agent IDs must be unique.
Workspace and runtime roots must be unique, normalized absolute paths narrower
than a filesystem root.

A linked contribution declares the scopes its instances may request. Each scope
grants one narrow authority and nothing else, so a service that needs records,
evidence, or attention never inherits workspace, terminal, or grouping authority:

| Scope | Grants |
|-------|--------|
| `health` | Bounded health handshake, diagnostic codes, and run liveness |
| `report` | Appending bounded reports to a run the instance owns |
| `evidence` | Storing typed immutable evidence |
| `attention_response` | Collecting the human response to its own attention report |
| `workspace_lease` | Holding and releasing an exclusive workspace directory |
| `terminal_events` | Receiving content-free terminal transitions |
| `execution_attachment` | A run-scoped attachment beneath an approved runtime root |
| `managed_run_group` | Reading the host roll-up for a group of runs it already owns |
| `approval_receipt` | Consuming a one-shot receipt for the exact destructive managed MCP operation Comis approved |

`managed_run_group` needs no filesystem roots. A group is a projection of member
runs the instance can already read, so it adds a view, never new reach. The
roll-up reports member state counts and how many members await a human; it holds
no component names, dependency edges, integration order, or acceptance rules,
because Comis is not a workflow engine and a group that carried those would make
it one.

`approval_receipt` grants no general approval lookup and cannot create or
resolve an approval. After a destructive managed MCP tool is approved, Comis
binds a short-lived grant to the service instance, owned managed run, MCP
operation, exact tool arguments, and resolving principal. The authenticated
service may consume that grant once with `managedRuns.consumeApproval`; an
identical operation replay returns the original receipt, while an altered,
expired, cross-run, or cross-instance request is refused. Grants live only in
daemon memory for 15 minutes. A restart before consumption requires a new
approval and never restores authority from private MCP metadata.

Linked contributions also declare the managed MCP tool bindings enforced by the
host. Run commands resolve their model-visible handle within the exact owner
scope. Comis checks the active authority set first. Only a destructive binding
may fall back to the exact release-reserved record, allowing an approved cleanup
or other terminal operation to replay idempotently without restoring its lease,
attachment, terminal, or worker authority. Read and mutate commands remain
active-only. Destructive commands remain available after a terminal outcome so
an authorized service can release retained resources through the normal
approval and security path.
When the handle is missing from that owner scope, Comis refuses the call before
MCP transport, returns the exact bounded authority reason to the agent, and
records a `precondition` failure with the `managed_mcp_authority` rule and a
hint naming `managedToolBindings[].runHandleArgument`.

| Key | Type | Default | Validation |
|-----|------|---------|------------|
| `instances` | `CapabilityServiceInstanceConfig[]` | `[]` | At most 256 configured instances |
| `privateContentDirectory` | `string` | `"managed-runs/private"` | Normalized relative path using safe directory segments |
| `reportRetentionMs` | `integer` | `2592000000` (30 days) | Positive; at most 365 days |
| `maxObservedClockSkewMs` | `integer` | `300000` | Non-negative; at most 24 hours |
| `recoveryBatchSize` | `integer` | `256` | 1–10000; page size for complete boot recovery scans |
| `requestDeadlineMs` | `integer` | `5000` | 100–60000 |

Each instance requires these fields:

| Key | Type | Description |
|-----|------|-------------|
| `serviceInstanceId` | `string` | Opaque instance identifier |
| `serviceDefinitionId` | `string` | Definition exported by a linked contribution |
| `enabled` | `boolean` | Whether the daemon activates the instance at boot |
| `mcpServerName` | `string` | MCP server owned by the linked definition |
| `control` | `object` | Unix socket and secret-backed bearer credential |
| `allowedAgents` | `string[]` | Non-empty set of agents allowed to use the instance |
| `allowedWorkspaceRoots` | `string[]` | Workspace-lease authority roots; at most 64. Required when — and only when — the definition requests `workspace_lease` |
| `allowedRuntimeRoots` | `string[]` | Execution-attachment authority roots; at most 64. Required when — and only when — the definition requests `execution_attachment` |

Each executor root and the scope that consumes it must agree in both directions,
and a mismatch fails startup rather than activating. A root configured for a
scope the service never requested is authority nothing can use and nothing
revokes, while a scope with no approved root publishes a capability that fails
on first use instead of at configuration time. A service that requests neither
scope — a tool-only or record-only integration — leaves both lists empty.
...
<Warning>
  **Restart required.** Changes to `tooling.capabilityIndex.enabled` and `tooling.installDetours.mode` apply only after the daemon restarts. The `capabilityIndex.enabled` toggle selects between two cached system-prompt shapes (one-line residual vs flat tool dump); in-process reload is not supported. Runtime `config.patch` and `config.apply` calls reject the operator-only `tooling` tree, so edit the config file, validate it, and restart the daemon manually.
</Warning>
...
| `samplingParams` | `Record<string, unknown>` | _(unset)_ | Sampling knobs merged into the request body as-is, after the named request fields (so keys here win). For OpenAI-compatible servers that accept parameters the SDK does not model — `top_p`, `top_k`, `min_p`, `repetition_penalty`. Merged per key over any catalog defaults for the same model; ignored by non-OpenAI-compatible APIs. |
...
Per-recall ranking trace written as bounded, sanitized JSONL for "why did recall pick X?" debugging. It is enabled from first boot so an incident can be explained after it happens.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | `boolean` | `true` | Enable the recall-trace writer. Set `false`, or use the `COMIS_DISABLE_RECALL_TRACE` env hard-off, to disable it. |
...
diagnostics:
  recallTrace:
    enabled: true                   # default -- full-sanitized, bounded JSONL
```
