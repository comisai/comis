...
| `baseFloor` | `number` (0–1) | `0` | Minimum BASE relevance score (pre-boost) for memory injection. Boosts cannot resurrect a memory whose base score falls below this threshold. Gates on `ScoreBreakdown.base` (un-boosted cosine/RRF score), applied after `scoreWithBreakdown()`. Default `0` = no floor (exact prior behavior). |
...
Injects the current execution objective and uncompleted step checklist at the context tail every turn for scaffolded (`small`/`nano`) tiers. Helps weak models stay on task across multi-turn executions. Gated on `scaffoldLevel === "max"` — frontier/mid agents never receive injection even if `enabled=true`.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `agents.<id>.goalAnchor.enabled` | `boolean` | `false` | Enable GoalAnchor tail injection. When `true` and `scaffoldLevel=max` (small/nano), the execution objective and uncompleted step checklist is tail-appended each turn. Default `false` = opt-in, behavior-neutral for frontier/mid. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `agents.<id>.verification.enabled` | `boolean` | `false` | Enable pre-delivery verification critic. Fires only when a completion-claiming response meets `minResponseChars`. Default `false` = opt-in. |
  ...
  default:
    contextEngine:
      ...
      compactPrompt:
        ...
        targetTokens: 3000
    goalAnchor:
      enabled: true          # Tail-inject objective checklist each turn (scaffoldLevel=max)
      maxChars: 500
    verification:
      enabled: true          # Pre-delivery critic (honest unmet-list on failure)
      minResponseChars: 200
    honesty:
      maxCriticRetries: 2
    rag:
      baseFloor: 0.3         # Memory relevance floor (0 = no floor, the default)
```
