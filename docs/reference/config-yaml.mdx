...
| `aggregateBudgetUsd` | `number` | `100` | Hard $ ceiling across the whole spawn tree, per root run. Alias of `budget.aggregateUsd` (either form resolves to the same bound). A generous runaway backstop, not a normal-use limit — lower it per-agent for a tighter guard. |
...
| `budget.aggregateUsd` | `number` | `100` | Per-root-run priced $ ceiling across the whole spawn tree (the `aggregateBudgetUsd` alias resolves here). A generous runaway backstop, not a normal-use limit. Positive. |
| `budget.tokens` | `number` | `100000000` | Per-root-run token ceiling (100M). Bites even on an unknown-priced (subscription / $0) model that the $ limb counts as free, so a runaway loop is bounded regardless of pricing knowledge. Positive integer. |
| `budget.wallClockMs` | `number` | `86400000` | Per-root-run wall-clock ceiling in milliseconds (24 h). Backstops a stuck/looping tree that burns neither $ nor tokens fast. Positive integer. |
