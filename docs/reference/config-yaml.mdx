...
absolute, and beneath the daemon's canonical owner-only data directory. The
daemon creates the control socket with mode `0600`. `credentialRef` must be a
`secret://` reference. Agent IDs must be unique. Workspace and runtime roots
must be unique, normalized absolute paths narrower than a filesystem root.
