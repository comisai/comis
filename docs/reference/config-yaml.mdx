...
| `maxCostPerHourUsd` | `number` | _(unset)_ | Optional per-agent hourly cost ceiling. The field is accepted now; **enforcement lands in a later phase** — `maxPerHour` is the active cost control today. |

In the common `auto` case, two follow-main image paths are wired: the built-in **OpenRouter** path (a main on the OpenRouter proxy, or explicit `provider: openrouter` + `OPENROUTER_API_KEY`) and the **Codex (ChatGPT-login)** path. When the agent's main provider is `openai-codex` (or you set `provider: openai-codex`), `image_generate` reuses the agent's **ChatGPT/Codex OAuth credentials** — there is **no image-specific API key** to set; the OAuth bearer is resolved per call and refreshes automatically (bootstrapped from `OAUTH_OPENAI_CODEX` or `comis auth login --provider openai-codex`). If the agent is not logged in, the tool returns an honest "unavailable" with a `comis auth login --provider openai-codex` hint. The native OpenAI and Google image-API paths (key-auth) arrive in a later phase.
...
integrations:
  media:
    imageGeneration:
      ...
      maxCostPerHourUsd: 5           # accepted now; enforced in a later phase
```
