...
capabilityServices:
  ...
  instances:
    - serviceInstanceId: example-service-1
      ...
      control:
        transport: unix
        socketPath: /run/comis/example-service.sock
...
The control transport is Unix-socket only; `socketPath` must be normalized and
absolute, and `credentialRef` must be a `secret://` reference. Agent IDs must be
unique. Workspace and runtime roots must be unique, normalized absolute paths
narrower than a filesystem root.
