...
agents:
  default:
    ...
    memoryConsolidation:
      ...
      schedule: "30 3 * * *"
    contextEngine:
      version: pipeline             # the default; "dag" is accepted but falls back to pipeline until re-enabled
...
Context engine configuration. Controls the system that manages what your agent sees each turn. The default is **pipeline** (the sequential layered engine). The **dag** value (the lossless DAG with recall tools) is being reimplemented for v2.12 and is not currently available — it is accepted but falls back to pipeline with a logged warning until re-enabled. See [Compaction](/agents/compaction) for a user-friendly explanation.
...
| `version` | `string` | `"pipeline"` | Operating mode: `"pipeline"` (default, the 8-step sequential system) or `"dag"` (the lossless DAG; accepted but currently falls back to pipeline — being reimplemented) |
...
**DAG mode fields** (`version: "dag"`) — these keys still parse but are dormant while DAG falls back to pipeline (being reimplemented for v2.12):
