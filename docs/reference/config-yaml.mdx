...
Comis uses a single `config.yaml` file (or multiple layered files) to configure every aspect of the system. The configuration contains 44 top-level sections defining agent behavior, channel adapters, security policies, gateway settings, capability services, and more. All schemas use Zod strict validation -- unknown keys are rejected at startup.
...
All 44 top-level configuration sections at a glance:
