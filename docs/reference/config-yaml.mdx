...
and restarting the daemon. The config schema is `z.strictObject`, so a mismatch between
the old v1.4 config shape and the new v1.5 layout fails boot with a precise migration error.
**Back up `~/.comis/config.yaml` before editing.**
</Warning>

<Info>
**Migration from v1.4:** The legacy top-level `oauth:` block, `security.secrets.enabled`,
and `COMIS_DISABLE_ENCRYPTED_SECRETS` keys were removed in v1.5. Replace with
`security: storage: encrypted|file|env`. See the [v1.5 Changelog](#) for the migration guide.
</Info>
