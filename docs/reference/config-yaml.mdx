...
`maxReportBytes` (combined report summary + details bytes; ceiling `16384`) and
`maxEvidenceBytes` (decoded evidence body bytes; ceiling `1048576`). Each field
is optional and can never exceed its ceiling — a larger value is rejected at
config validation. An absent field falls back to the linked definition's own
`limits`, then to the protocol ceiling. A service that only ever produces small
reports or evidence can pin a smaller cap so an oversized payload — from a bug or
a compromised service — is refused early rather than at the global limit.
