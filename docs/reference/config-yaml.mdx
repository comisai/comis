...
durable background-task manager. The manager is shared across agents, so
`maxTotal` is a daemon-wide ceiling while `maxPerAgent` is enforced separately
for each agent.
...
| `maxPerAgent` | `number` | `5` | Maximum concurrent background tasks for one agent. |
| `maxTotal` | `number` | `20` | Maximum concurrent background tasks across the shared manager. |
| `maxBackgroundDurationMs` | `number` | `300000` | Hard runtime limit for a background task. When it expires, the task is aborted and finalized as failed. |
| `excludeTools` | `string[]` | `[]` | Additional tool names that must never be auto-promoted. |
| `maxBackgroundHops` | `number` | `3` | Maximum continuation re-entry depth, preventing a completion turn from creating an unbounded background chain. |
