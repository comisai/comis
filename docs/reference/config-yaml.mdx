...
Bounded-unpack caps for the staged [skill-import pipeline](/skills/importing). They bound what an untrusted archive can expand to before any file is written. The defaults are deliberately conservative; raise them only if you knowingly import large skills. Every key is required-with-a-default and the block is closed -- a typo'd cap key rejects at config load. Shared-scope imports read the default agent's block.
