...
host. Run commands resolve their model-visible handle only within the active
owner scope. Read and mutate commands are unavailable after a managed run
reaches a terminal outcome; destructive commands remain available there so an
authorized service can release retained workspaces, attachments, or other
run-owned resources through the normal approval and security path.
