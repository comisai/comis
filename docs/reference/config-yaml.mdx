...
The **Verified Learning** outcome-reward write (v2.26 WS3): the per-agent on/off switch for recording a **real task outcome** against each recalled memory's recall-utility counters. When on, a recalled memory that rides a `success` trajectory gets a `recordUsage` write on the `memory_usefulness` store, and a `failure`/`corrected` outcome gets a `recordFailure` (the outcome-attributed **`failure_count`** that **Learning Forgetting** reads). **Default-ON (opt-out)** and **gated by the master cost switch** (`memory.costFeatures.enabled` -- when that is `false`, `learningTuning` is force-disabled regardless of the per-agent flag). With it off the reward write is skipped and `failure_count` stays at its default. It does **not** tune any recall weight — recall ranking is the fixed `rag.scoring` fusion (deterministic RRF + the cross-encoder reranker). The v2.26 per-query-intent tuned-alpha ranking bandit — its `learner` / `perIntent` / `exploration` sub-knobs, its nightly tuning cron, and its backing weight table — was **removed in v2.31** (it never validated a recall lift). If an existing `config.yaml` set any of those removed `learningTuning` sub-keys, drop them: the block now carries only `enabled` (strict-parse rejects the others). See the `memory_usefulness.failure_count` column in [data-directory.mdx](/operations/data-directory#ranking--forgetting-verified-learning-v2-26).

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | `boolean` | `true` | Enable the outcome-reward write for this agent (default-on, opt-out; force-disabled when `memory.costFeatures.enabled` is `false`). Gates only the `recordUsage`/`recordFailure` write — it tunes no recall weight |

> **Gate composition.** `learningTuning.enabled` is a single switch on the outcome-reward write; it **composes with** — does not replace — `memoryUsefulnessJudge` (the other recall-utility signal source) and `learningForgetting` (the `failure_count` consumer), all under the master `memory.costFeatures.enabled`. With it off the reward write is skipped and recall is byte-identical (recall ranking is always the fixed `rag.scoring` fusion).
