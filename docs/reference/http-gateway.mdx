...
**What this is for:** every HTTP route the daemon exposes — JSON-RPC, REST API
for the dashboard, experimental OpenAI-shaped endpoints, webhooks, media, and
SSE. **Who it's for:** anyone integrating an external system with Comis or
building tools on top of the daemon.
...
The gateway is implemented with [Hono](https://hono.dev) and lives in
...
WebSocket clients that cannot set custom headers may pass the token as a query parameter.
SSE endpoints require the `Authorization` header and reject `?token=` credentials.
...
<Warning>
Query parameter tokens can appear in server access logs. Prefer a WebSocket client that supports
...
Server-Sent Events (SSE) endpoints provide real-time streaming. Both endpoints require an
`Authorization: Bearer ...` header and at least the `rpc` scope. Query-string credentials are
...
| POST | `/api/chat/stream` | Bearer (`rpc`) | Streaming chat via SSE. Requires an application/json body with `message` and optional `agentId`. |

```bash
# System event stream
curl -N -H "Authorization: Bearer your-token" \
  http://localhost:4766/api/events

# Streaming chat without placing the prompt in the URL
curl -N -X POST \
  -H "Authorization: Bearer your-token" \
  -H "Content-Type: application/json" \
  --data '{"message":"Hello","agentId":"default"}' \
  http://localhost:4766/api/chat/stream
```
...
The `/api/chat/stream` endpoint accepts `{ "message": string, "agentId"?: string }` as its JSON
request body. It sends incremental token deltas as `token` events, followed by a final `done`
event with the complete response. The same configured HTTP body-size ceiling used by other chat
...
These routes implement selected request and response shapes associated with
OpenAI endpoints. They are experimental and are **not** a general OpenAI client,
SDK, or drop-in compatibility guarantee. Validate the exact fields and behavior
your integration needs against the endpoint reference.
...
<Info>
For the implemented subset, see [Experimental OpenAI-shaped API](/reference/openai-api).
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Experimental OpenAI-Shaped API" icon="sparkles" href="/reference/openai-api">
