...
| `locale` | `string` | No | Request-tier response locale as a **canonical BCP-47 tag** (`"he"`, `"pt-BR"`). A non-canonical or invalid tag is rejected with `400`. When the message text is written in a script that contradicts this locale, the conversation's script wins. |
