...
The REST API is mounted at `/api` and provides HTTP endpoints for the web dashboard. These endpoints are available when `gateway.webDashboard` is enabled (or equivalent web dashboard configuration is provided).
