...
Comis ships **50+ leaf commands grouped into 20 command groups** (verified against `packages/cli/src/cli.ts` at version 1.0.37). The CLI talks to the running daemon over JSON-RPC; offline-only commands such as `secrets` operate directly on local files. Run any command with `--help` to see its full flag list — every flag below is also documented inline in the binary.
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--provider <id>` | `string` | _required_ | OAuth provider id. Must be `openai-codex` (other providers ship in later phases). |
...
| `config sync-tooling` | Discover MCPs/skills and sync the `tooling:` config block (Phase 25) |
| `config tooling-fill [hint]` | Fill `description` + `replacesPackages` on a tooling hint via the live agent (Phase 26) |
...
Model management commands for browsing the model catalog and updating agent model assignments. The catalog is sourced live from the [`@mariozechner/pi-ai`](https://www.npmjs.com/package/@mariozechner/pi-ai) SDK — adding a new provider/model to pi-ai automatically surfaces it here with no Comis update required.
