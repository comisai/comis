...
Procedural-learning telemetry for the v2.26 Verified Learning skill loop: the **learned-skill admission funnel** — the total count and the per-state breakdown (`candidate` / `active` / `stale` / `archived`), broken down per agent, plus the per-skill `name·state·proof-count`. Read **offline** from the local [`learned_skills` table](/operations/data-directory) in `~/.comis/memory.db` (no gateway required). **Counts and ids only** — never a procedure body, script, or description (the same closed-graph firewall as the trajectory signals).
