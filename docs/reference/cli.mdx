...
Comis currently ships **36 top-level command groups**. Some commands call the
...
<Steps>
  <Step title="Initialize the workspace">
    `comis init` walks through storage, model authentication, the first agent,
    optional channels, gateway, workspace, tools, media, recall, review, and
    daemon startup. Use `--non-interactive` with explicit flags for automation.
    ```bash
    comis init
    ```
  </Step>
  ...
  <Step title="Wire a channel">
    Re-run `comis init` to add a channel through the supported wizard, or follow
    the platform guide and validate a manual config change. Store channel
    credentials in the active secret backend.
    ```bash
    comis init
    ```
  </Step>
...
| `agent set-oauth-profile <agentId> <profileId>` | Set an agent's preferred OAuth profile |
...
Derives the provider from a profile ID such as
`openai-codex:user@example.com` and stores that agent preference. Use `comis
auth list` to obtain the exact profile ID.
...
A cache **write** is billed by its TTL -- on Anthropic a 1-hour write costs 2x base
input and a 5-minute write 1.25x, against 0.1x to read a cache hit. On a cached
workload writes therefore dominate the bill, and *which* TTL they used is the number
that moves it. When the window contains rows that recorded a split, the report adds:

| Field | Meaning |
|-------|---------|
| `cacheWrite1hTokens` | Write tokens billed at the extended (1h) TTL -- the expensive half |
| `cacheWrite5mTokens` | Write tokens billed at the short (5m) TTL |

Both lines are omitted when no row in the window recorded a split -- rows written
before the split was stored, or a provider that prices every write the same. They are
omitted rather than shown as zero because a flat `0` would read as "no writes
happened", which is a different claim: `cacheCreationTokens` remains the authoritative
write total, and the split never has to add up to it.

---

### `comis cron`

Inspect persisted cron jobs, force a named job to run, and read immutable execution history through the running daemon's strict JSON-RPC contracts. The CLI does not parse scheduler files and has no offline mode; an unreachable daemon is an error.

#### Subcommands

| Subcommand | Description |
|------------|-------------|
| `cron list` | List persisted jobs for one agent, or all agents with admin trust |
| `cron run <jobName>` | Force one unambiguous named job to run |
| `cron runs <jobName>` | Show immutable execution groups for one named job |
| `cron status` | Show whether one agent's scheduler is active and its job count |

All subcommands accept `--agent <id>` and `--format table|json`. `cron list` additionally accepts `--all`; it sends the explicit all-agent selector and requires an admin operator token. `--all` and `--agent` cannot be combined. `cron runs` accepts `--limit <n>` from 1 through 10,000 and otherwise uses the daemon's bounded default.

```bash
# Inspect one agent's schedules without displaying job payload text
comis cron list --agent default

# Admin inventory across every active agent scheduler
comis cron list --all --format json

# Force one named job and retain its execution id
comis cron run "Daily status" --agent default --format json

# Inspect terminal status and delivery outcome for recent fires
comis cron runs "Daily status" --agent default --limit 20

# Confirm scheduler activation and persisted job count
comis cron status --agent default
```

Cross-agent selection is authorized by the daemon, not trusted from CLI arguments. Agent-origin RPC calls remain self-scoped. Table inventory deliberately omits payload bodies; JSON is the complete typed operator projection.

---

### `comis tasks`

Inspect, cancel, and recover inferred follow-up tasks through the running daemon's strict admin JSON-RPC contracts. Like `comis cron`, the CLI never parses scheduler files and has no offline mode; an unreachable daemon is an error. Records are content-free: identifiers, states, and timestamps only, never the task text.

#### Subcommands

| Subcommand | Description |
|------------|-------------|
| `tasks status` | Show content-free task authority status, digest, quarantine, and counts |
| `tasks list` | List content-free task records |
| `tasks cancel [taskId]` | Cancel one pending task, or every pending task for the selected agent |
| `tasks reset` | Archive and replace a disabled task authority after digest review |

All subcommands accept `--agent <id>` and `--format table|json`. `tasks list` additionally accepts `--status <state>` (an exact task state: `pending`, `checking`, `delivering`, `delivered`, `delivery_partial`, `dismissed`, `delivery_unknown`, `expired`, `cancelled`) and `--limit <n>` from 1 through 256. `tasks cancel` takes either a `taskId` or `--all-pending`, never both; the bulk form also requires `--confirm`. `tasks reset` requires `--expected-digest <sha256>` (a lowercase SHA-256 of the current raw authority, as reported by `tasks status`) plus `--confirm`, and replaces the authority irreversibly after archiving it.

```bash
# Authority state, file digest, quarantine, and pending/active/terminal counts
comis tasks status --agent default

# Content-free inventory, newest-bounded
comis tasks list --agent default --limit 50

# Only the records still awaiting a due check
comis tasks list --status pending --format json

# Cancel one task by id
comis tasks cancel task_01hx8n4k --agent default

# Cancel every pending task for one agent
comis tasks cancel --all-pending --agent default --confirm

# Recover a quarantined authority after reviewing the digest from `tasks status`
comis tasks reset --agent default --expected-digest <sha256> --confirm
```

Cancellation reports whether the scheduler must rescan, so a terminal state is never inferred from CLI output alone. A reset that does not match `--expected-digest` fails instead of overwriting an authority you have not reviewed.
...
| Subcommand | Description |
|------------|-------------|
| `config audit` | Inspect or scrub the daemon-wide config-audit log |
...
`config audit show` lists recent config-audit records. `config audit scrub`
re-runs the redactor over the stored audit log. Run either nested command with
`--help` for its filters and output options.
...
Secret-posture findings distinguish a confirmed missing secret from a store that the local CLI
cannot inspect. When the configured encrypted store credential is service-scoped, an
`unavailable` warning does not mean `CANARY_SECRET` is absent; verify the selected store with
`comis secrets list` or rerun the check from the service credential context before changing it.
...
| `--api-key-stdin` | `boolean` | `false` | Read the main provider key from stdin in non-interactive mode |
...
| `--msteams-app-id <id>` | `string` | - | Microsoft Teams bot application ID |
| `--msteams-app-password <pw>` | `string` | - | Microsoft Teams client secret |
| `--msteams-tenant-id <id>` | `string` | - | Microsoft Teams tenant ID |
| `--msteams-auth-mode <mode>` | `secret` \| `certificate` \| `managedIdentity` | - | Microsoft Teams authentication mode |
...
| `--stt-provider <id>` | `auto` \| `local` \| `openai` \| `groq` \| `deepgram` | - | Voice transcription provider |
| `--stt-api-key <key>` | `string` | - | Transcription provider credential (e.g. `DEEPGRAM_API_KEY`) |
| `--tts-provider <id>` | `edge` \| `openai` \| `elevenlabs` \| `local` | - | Text-to-speech provider |
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--embedding-multilingual` | `boolean` | `false` | Select multilingual semantic recall |
| `--embedding-provider <id>` | `local` \| `openai` | - | Use local BGE-M3 or OpenAI embeddings |
| `--embedding-api-key <key>` | `string` | - | Dedicated OpenAI embedding credential when the main provider cannot be reused |

#### Paths and Behavior

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--data-dir <path>` | `string` | - | Workspace/data directory override |
| `--config-dir <dir>` | `string` | `~/.comis` | Directory for configuration, credential storage, and daemon launcher files |
| `--storage <mode>` | `encrypted` \| `file` | `encrypted` | Credential storage mode for the generated config |
...
<Warning>
Prefer the interactive prompt or `--api-key-stdin`. An expanded value supplied
to `--api-key` (or any other secret-valued flag) can be visible through process
inspection, shell history, or CI logs. Never paste literal credentials into
...
2. Bootstrap with a valid API-key or local provider, then run [`comis auth
   login --method device-code`](#comis-auth), switch the agent to
   `openai-codex`, and attach the returned profile ID.
...
# Valid bootstrap path — local Ollama must already be running
comis init --non-interactive --accept-risk --provider ollama --model llama3 --start-daemon
comis auth login --provider openai-codex --method device-code
comis auth list --provider openai-codex
comis agent configure comis-agent --provider openai-codex --model gpt-5.1
comis agent set-oauth-profile comis-agent openai-codex:user@example.com
```
...
| `memory clear` | Clear non-system, non-pinned memory for a tenant or agent |
...
| `--agent <agentId>` | `string` | - | Agent scope within the selected or configured tenant |
| `-y, --yes` | `boolean` | `false` | Skip confirmation prompt |

<Warning>At least one scope (`--tenant` or `--agent`) is required. The command removes every non-system, non-pinned memory in that scope; it does not support content or memory-type filters.</Warning>
...
# Clear non-system, non-pinned memory for a specific agent
comis memory clear --tenant default --agent system --yes
...
<Note>
Recall-trace records exist while `diagnostics.recallTrace.enabled` is on (default `true`). See the [recall runbook](/operations/troubleshooting#memory-recall-issues) for the read workflow and explicit-disable behavior.
</Note>
...
```bash
# List all available models (~1,072 entries across 35 providers as of pi-ai 0.80.10)
comis models list
...
| Subcommand | Description |
|------------|-------------|
| `providers list` | List all available providers with model counts and credential status |
...
|------|------|---------|-------------|
| `--format <format>` | `table` \| `json` | `table` | Output format |
| `--agent <id>` | string | default agent | Resolve credential availability for this agent |

The output includes three columns:

- **Provider** — pi-ai provider id (e.g., `anthropic`, `openrouter`, `groq`)
- **Models** — count of models the provider exposes in the catalog
- **Status** — `configured` when the daemon's runtime credential chain resolves for the selected agent, `not configured` when that authoritative chain has no usable credential, `keyless` for providers such as Ollama / LM Studio, or `unknown` when the daemon is unavailable and the local CLI cannot inspect encrypted credentials

JSON output also includes `credentialSource`, a value-free category such as
`secret_store_canonical`, `oauth_profile`, `env_canonical`, or `none`. It never
includes a credential value. When the daemon is unavailable, the CLI can still
recognize keyless and ambient-environment credentials; every encrypted-store
case is reported as `unknown` with `credentialSource: "daemon_unavailable"`.
...
comis providers list --agent research

# Machine-readable
comis providers list --format json
# → [{"provider":"anthropic","modelCount":14,"status":"configured","credentialSource":"secret_store_canonical"}, ...]
```
...
| `--tenant <tenantId>` | `string` | - | Required with the `sessions` target |
| `--agent <agentId>` | `string` | - | Required with the `sessions` target |

- **sessions** -- Deletes conversation sessions in the required tenant/agent scope through the daemon. If the daemon is unreachable, the command warns and falls back to removing the entire `memory.db`, not just that scope.
...
```bash
# Clear all sessions in one tenant/agent scope
comis reset sessions --tenant default --agent default --yes
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--write` | `boolean` | `false` | Append key to the `.env` of the selected data directory (`dataDir` from config, else `COMIS_DATA_DIR`, else `~/.comis`) instead of printing to stdout |
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--file <path>` | `string` | The selected data directory's `.env` | Source `.env` file path |
...
| `sessions report` | Inspect stored SystemPromptReport records for a session |
...
| `--tenant <tenantId>` | `string` | - | Required tenant scope |
| `--agent <agentId>` | `string` | - | Required agent scope |
...
comis sessions inspect "default:agent:default:user123:telegram"

# Delete a session
comis sessions delete "default:agent:default:user123:telegram" --yes

# Full conversation reset (clears LCD history + session transcript — irreversible, requires --yes)
comis sessions reset "default:agent:default:user123:telegram" --tenant default --agent default --yes

# Also clear RAG memories for this session (the full-forget path — prints deleted count)
comis sessions reset "default:agent:default:user123:telegram" --tenant default --agent default --memory --yes

# Nuclear forget — also purge consolidated observations derived from this session
comis sessions reset "default:agent:default:user123:telegram" --tenant default --agent default --memory --purge-derived --yes
```
...
Use `sessions report show <sessionId>` for the latest SystemPromptReport or
`sessions report list <sessionId>` for recent reports. Run each subcommand with
`--help` for its output and data-directory flags.
...
Assemble an `IncidentReport` for a single agent session — a bounded, causal post-mortem (outcome, cost, per-tool stats, normalized failures, circuit-breaker timeline, large-result offloads, recall health — including degraded/failed recall lanes — session-wide activity-finalize tallies, and a deterministic likely root cause). No LLM is invoked. The authoritative evidence-source and report-field contract is [`obs.explain`](/reference/json-rpc#obs-explain); this command is its CLI surface, backed by `ObsExplainContract`.

The default table view prints every bounded normalized failure under `Failures:`,
including sequence, tool name, machine failure code, `errorKind`, transport
status, classifier, and the report's bounded redacted preview. This is the
operator path for the exact failed tool and actionable validation message; raw
daemon logs are not required. `--format json` emits the same `failures[]`
records structurally.

The positional argument is routed automatically by shape: a **`root-` run id** (an autonomy run's `rootRunId` — the synthetic in-process `root-session-<generation UUID>-<base64url formatted session key>` or a real spawned-run lease id; checked **first** because the reserved `root-` shape identifies a run before the session/trace classifiers) is passed as `rootRunId`; otherwise an **execution-graph id** (either `--graph`, or an id with local terminal metadata at `<dataDir>/graph-runs/<id>/_run-metadata.json`) is passed as `graphId`; otherwise a **session key** (the [current agent-scoped format](/agents/sessions#session-scope-key), containing `:`) is passed as `sessionKey`; otherwise a UUID-shaped **trace ID or sub-agent run ID** is passed through the `traceId` field. The daemon canonicalizes a trace ID, sub-agent run ID, `rootRunId`, *or* `graphId` to its session key first, so every form reports on the same session — the trace, session, and run forms produce the identical report, and the graph form adds the `graph` block below. A `rootRunId` is the **`comis system-health` → `comis explain` drill-down target**: paste the `worstRootRunId` that [`comis system-health`](#comis-system-health)'s autonomy block names straight in, and the report renders that run's `spawnTree`. A failed child shown in that tree can be drilled into by passing its sub-agent run ID directly. An unresolvable identifier surfaces an honest `session_not_found`, never a clean-looking empty report.

When the argument resolved to an execution-graph run, the report carries a `graph` block and the table view renders it as `Graph: <status> · <graphId> · <succeeded>/<total> succeeded · <failed> failed · <skipped> skipped · notification=<disposition>`, followed by one indented line per node (`<nodeId>: <status> · <durationMs> · attempts=<n>`, plus `run=<subAgentRunId>` when the node spawned one). `notification=` is the terminal disposition of the graph's aggregate completion announcement — see [`obs.explain`](/reference/json-rpc#obs-explain) for the block's full shape and the meaning of each disposition. An unresolvable graph id surfaces an honest `graph_not_found` verdict naming the missing run metadata, never a clean-looking empty report.

The live daemon resolves session artifacts across every configured agent
workspace, preferring the workspace owned by the agent encoded in the session
key before checking the default and other registered workspaces. This keeps
named-agent sessions explainable when their files do not live under the default
workspace. `--offline` has no live workspace registry and reads only
`<dataDir>/workspace`; use the live command for custom workspace paths.
...
For background completions, `--format json` also exposes the authoritative
[`spawnTree[].outputValidation`, `outwardDeliveries`, and `restartRecovery`](/reference/json-rpc#obs-explain)
evidence. Together they distinguish declared-file validation and attachment
preparation from the platform's durable delivery receipt, including partial
multi-part delivery, and show whether a durable child was suspended and resumed
across daemon restart. None of these fields exposes the attachment body or
provider output.

When the session prepared any managed ([capability-service](/reference/config-yaml#capability-services)) runs, the report carries a `managedRuns` block — the managed runs this session prepared, linked by the trace ids the session's trajectory ran: `total`, `degraded` (the `failed` + `unknown` runs; a `cancelled` run is an intended outcome), and one `runs[]` row per run with its `managedRunId`, `serviceInstanceId`, and closed `status`/`statusReason` (ids + closed enums + counts only — never a report body, workspace path, objective, or credential, safe to paste). This is the session→managed-run drill-down: paste a `managedRunId` into [`comis managed-runs explain`](#comis-managed-runs) for that external-service run's own root cause, without a raw-log join. Reconstructed from the durable managed-run store, so `--offline` shows it too (it reads the same local `memory.db`). Absent when the session prepared no managed run.

When a scheduled job's [wake-gate](/agent-tools/scheduling#wake-gate-run-the-model-only-when-it-matters) **woke** the model, the report folds a content-free `cronWakeGate` fact for that fire — the job id, the `wake` verdict, the gate's `durationMs`, its tool-call count, and the model turns it saved (ids and counts only, never the gate's gathered finding). A **skipped** fire is different: it runs no model and opens no session, so it never reaches `comis explain`. The argument is a session key, trace ID, or `rootRunId` — **never a cron job id**. To inspect a job's skipped fires read its run history (`cron.runs`), and for the cross-job skip-rate and turns-saved use [`comis system-health`](#comis-system-health).

When the session was a proactive check over [inferred follow-up tasks](/operations/scheduler#task-extraction), the report carries a content-free `taskCheck` block — the attempt's `rootRunId`, `attemptId`, and `correlationId`, its `lifecycle` (`started` / `terminal`), the terminal `outcome`, whether it ran `live` or through `ownership_recovery`, and the `deliveredChunks` / `failedChunks` / `ambiguousChunks` counts. A `dismissed` outcome also carries `suppressionReason`, so **"the check ran but the user got nothing"** is answered by the report instead of inferred from a silent success: `heartbeat_token` (the model declined with the bare `HEARTBEAT_OK` token), `ack_under_threshold` (`HEARTBEAT_OK` plus residual text within `ack_max_chars`), `empty_reply` (an empty reply or a `NO_REPLY` / `[SILENT]` marker), or `visibility_filter` (a non-critical alert withheld because `show_alerts` is off) — see the [heartbeat response rules](/agent-tools/scheduling#heartbeat-manage-agent-heartbeat) for the classifier those names come from. The table view renders the block as `Task check: <outcome, or the lifecycle when no outcome was reached> · attempt=… · correlation=… · root=… · delivered=… failed=… ambiguous=… · suppression=…`, with `suppression=none` when the attempt was not suppressed. Identifiers, labels, and counts only — never the task text, the check-in body, or the originating conversation.
...
| `--graph` | `boolean` | `false` | Treat the identifier as an execution-graph id. Only needed when the run's metadata is not on local disk — otherwise the shape routing above detects it |
...
comis explain "default:agent:default:user123:telegram"

# Explain by trace ID, with the uncapped (full) report
comis explain 6ba7b810-9dad-11d1-80b4-00c04fd430c8 --depth full

# Drill into an autonomy run by its rootRunId (the `comis system-health` worstRootRunId) —
# resolves the run to its session and renders the run's spawn-tree
comis explain "root-session-11111111-1111-4111-8111-111111111111-ZGVmYXVsdDphZ2VudDpkZWZhdWx0OnVzZXI6dGVsZWdyYW06MTcxNzAwMDAwMA"

# Post-mortem an execution graph — resolves to the caller's session and adds the
# graph block (per-node states plus the completion-announcement disposition)
comis explain abc-123-def --graph

# Machine-readable output (includes the spawnTree array for an autonomous run)
comis explain "default:agent:default:user123:telegram" --format json

# No daemon required — assemble from the local files
comis explain "default:agent:default:user123:telegram" --offline
```
...
Assemble a `SystemHealthReport` — a bounded, **cross-session** triage over a recent window: the session count and degradation rate, the **degraded-by-cause breakdown** (degraded sessions bucketed by named `endReason` cause, e.g. `context_exhausted` / `output_starved`), the recurring `WARN` findings (recurring `lcd_divergence` / model / config-posture signals, counts + hints only), the total circuit-breaker trips, the window cost, and a deterministic likely root cause. The trip total counts the **tool-retry** breaker, summed from the per-session rollups. An MCP **per-server** breaker trip is a different mechanism and is not in that total — it appears in `comis explain`'s `breakerTimeline` and in the `comis.breaker_trips` metric, so a `0` here does not by itself mean no server was circuit-broken. Like `comis explain`, the report is derived from log + diagnostics evidence only; no LLM is invoked, so the same window yields the same verdict. Backed by `ObsSystemHealthContract` (`obs.system.health`).
...
The raw `sessions.degraded` count still includes `completed_with_tool_errors`
sessions, where a tool failed but the user received a final answer.
`sessions.deliveredWithToolErrors` identifies that subset;
`sessions.hardDegraded` and `hardDegradedRate` are the remaining count and rate
that drive acute/high-degradation findings. The table prints this split instead
of presenting recovered tool errors as user-visible failures.
...
A `health_signal:cache_prefix_churn` finding fires when a session's prompt-cache prefix **collapsed** on a recurring basis — a cached-region message mutated across turns on 3+ calls within a recent window, wasting the cache write (up to hundreds of thousands of cache-creation tokens over a session). It covers every prompt-caching provider, including Bedrock-hosted models, whose content blocks are keyed rather than typed (`{text}` / `{reasoningContent}` / `{toolUse}` / `{toolResult}`) — the class is resolved across both wire shapes, so a reasoning block dropped on replay is reported as `thinking-cleared` there too rather than as an unattributed block-count change. The finding names the recurring **mutation class** (`structural-shift` = a message index shifted below the cache fence, e.g. from LCD condense/re-admit; `block-count-changed` = a cached message was re-serialized with a different content-block shape; `datetime-preamble` / `thinking-cleared` / `content-cleared` = an in-place edit). Counts + the closed class label only — never message text.
...
A dedicated `config_posture:media_credential_gap` finding fires when a configured media pipeline (`imageGeneration` / `transcription` / `tts` / `videoGeneration`) has a **pinned** provider whose credential is absent — the pipeline authenticates fine for the main completion path but fails at first media use, so it was previously invisible to the system health view (you had to hit it or grep the log). The count is the number of affected pipelines at the **latest** boot (standing state; a login/key + restart clears it). Fix by setting the provider's credential (`OPENAI_API_KEY` / `GOOGLE_API_KEY` / `FAL_KEY` / …), logging in for `openai-codex` (`comis auth login --provider openai-codex`), or switching the pipeline's `provider` to one whose credential is present (or `auto` to follow the main provider). Counts only — never a provider name or credential value.

A dedicated `config_posture:tool_deadline_collision` finding fires when an agent's MCP call deadline is **not strictly below** the sub-agent stall budget that encloses it. Both ship at `120000`, so an operator who pins neither inherits the collision: a sub-agent whose first action is a slow MCP call is killed at the exact moment its tool would have reported failure, so the run aborts with `0` steps, no output, and no diagnosis — the abort is logged *before* the tool's own timeout WARN. Fix by lowering `integrations.mcp.callToolTimeoutMs` below `agents.<id>.operationModels.subagent.timeout`, or by raising that sub-agent timeout above the call deadline. The count is the number of affected agents at the **latest** boot (standing state; a config change + restart clears it). Counts only — never an agent name.
...
The acute-degradation root cause (`system_acute_degradation`) names the **worst degraded session's key** inline (`worst: <sessionKey>`) and in its suggested next step (`comis explain <sessionKey>`), so you paste it straight in rather than hunting for "the worst session".

When the daemon runs **unattended/durable** runs, the report carries an `autonomy` block: the cross-run run count + degraded rate (sourced from the **crash-surviving** `durable_run_checkpoints` table, so it survives a hard crash), the `orphaned` / `resumed` / `revoked` / `killed` counts (`killed` is separable from `revoked` because a hard `run.kill` and a cooperative `lease.revoke` emit distinct events), the autonomy breaker-trip + budget-breach counts, the window cost, and the worst degraded run's `worstRootRunId` (orphaned > killed > revoked) so you can paste it straight into `comis explain` for its spawn-tree. The orphaned/revoked/killed counts also surface as dedicated `durable_orphaned` / `autonomy_revoked` / `autonomy_killed` findings, and a degraded autonomy run drives the top-ranked `system_autonomy_degradation` root cause. Counts + the worst run's id only — never a lease bearer, an orphan-reason body, or a secret; safe to paste. The block is **absent** under `--offline` (the daemon-less CLI has no durable-store edge) or on a non-durability boot — an honest coverage degradation, not a silent zero.

When installed [capability services](/reference/config-yaml#capability-services) ran managed work over the window, the report carries a `capabilityServices` block and a `managed_run_degradation` finding: the managed-run count and **degraded rate** (the `failed` + `unknown` statuses — a `cancelled` run is an intended outcome, not degradation), the affected **service count**, the top closed **status-reason codes** across the degraded runs (e.g. `service_state_unavailable` / `failure_verified` / `required_evidence_stale`), and the most-recently-updated degraded run's `worstManagedRunId` so you can paste it into `comis managed-runs explain` for its likely root cause and repair knob. Sourced from the durable managed-run index (managed runs are content-free durable rows by construction), so it needs no contentful global telemetry — counts, closed reason codes, and one opaque host-minted run id only; never a report body, workspace path, objective, or service credential, safe to paste. The block is **absent** when the window held no managed-run activity (a daemon that never used a capability service carries no empty block) or under `--offline` (the daemon-less CLI has no managed-run-store edge) — an honest coverage degradation, not a silent zero.
...
```bash
# System-health triage over the last 24h (default window)
comis system-health
#   …
#   Breaker:    3 trips
...
comis system-health --since 72

# Machine-readable output
comis system-health --since 24 --format json

# No daemon required — assemble from the local files
comis system-health --since 24 --offline
```
...
### `comis capability-services`

The operator view over installed capability services and the external work they
own. A capability service is a separately installed process that contributes
agent tools and durable external-work lifecycle; these commands show the **host**
half of that relationship — whether authority bound, which host records a run
holds, and whether the service is still reporting. The product that owns a run
explains what its task means; that is its own CLI's job.

| Subcommand | Options | Purpose |
|------------|---------|---------|
| `list` | `--format table\|json` | List every configured instance and its current host state |
| `get <serviceInstanceId>` | `--format table\|json` | Show one instance's declared scopes, approved roots, and liveness obligation |

```bash
# Which services are installed, and are they usable?
comis capability-services list
comis capability-services get example-service-1

# What external work exists, across every conversation this daemon holds?
comis managed-runs list
comis managed-runs list --service example-service-1 --state unknown --limit 50

# Why is this one stuck?
comis managed-runs explain <managedRunId>

# Stop it on host authority
comis managed-runs cancel <managedRunId>

# Which runs are waiting on a person rather than on the machine?
comis managed-attention list --run <managedRunId>
```

`explain` returns a deterministic verdict over the durable record — no model, so
the same run always produces the same answer — plus the next actions that are
actually reachable for its current state. Causes are ordered so the outer fault
wins: a service instance missing from configuration outranks a stale heartbeat,
which outranks a run that is merely waiting on a human.

Two rules keep the output safe to paste into a review. No command returns a
report body, a question, or an artifact: those live in the confined content
store and reach a human through the owning product. And no instance row carries
a socket path, a credential reference, or a resolved secret.

Capabilities that have not shipped are labelled rather than omitted — `custody`
and `processes` read `not available` with a reason code, because a blank field
would otherwise read as "no operator holds custody" or "no processes are
running".

`cancel` records the host's decision before it notifies the service, and the
decision stands when the service is unreachable. In that case the command says
the service has not confirmed yet and will reconcile on its next handshake; it
does not report the cancel as failed, because the authority record already says
cancelled.

All three groups are admin-scoped and carry no agent route, so an agent turn —
including a prompt-injected one — can never reach them. They need a reachable
daemon: there is no offline projection of another process's authority record
that would be safe to present as current.

---

### `comis managed-runs`

Inspects, explains, and cancels the external work bound to this daemon.

| Subcommand | Options | Purpose |
|------------|---------|---------|
| `list` | `--service <id>`, `--agent <id>`, `--state <status>`, `--limit <count>`, `--format table\|json` | List durable runs, optionally filtered; default limit `100`, maximum `500` |
| `get <managedRunId>` | `--format table\|json` | Show one run's durable authority record |
| `explain <managedRunId>` | `--format table\|json` | Return the deterministic likely cause and reachable next actions |
| `cancel <managedRunId>` | `--operation <operationId>`, `--format table\|json` | Record host-authority cancellation; the default operation ID is stable for the run |

`--state` accepts `preparing`, `active`, `waiting`, `paused`,
`candidate_complete`, `succeeded`, `failed`, `cancelled`, or `unknown`.

See
[`comis capability-services`](#comis-capability-services) above for the full
usage, the content rules, and the cancellation semantics.

---

### `comis managed-attention`

Lists the durable questions managed runs are waiting on — identifiers and
status only, never the question itself, which belongs to the principal it was
routed to.

| Subcommand | Options | Purpose |
|------------|---------|---------|
| `list` | `--run <managedRunId>`, `--limit <count>`, `--format table\|json` | List attention records, optionally for one run; default limit `100`, maximum `500` |

See [`comis capability-services`](#comis-capability-services) above.

---

### `comis messages`

Extract the **inbound messages users typed**, per channel, from the local session logs — the conversation-export lens. It reads the raw session `.jsonl` message logs under every agent workspace tree (`<dataDir>/workspace[-<agentId>]/sessions/...`). Immediately after authorization and session-key resolution, current sessions commit one immutable, model-context-free provenance plan to an append-only `~ledger~inbound.jsonl` sidecar, before preprocessing or executor lookup. The exact same plan and timestamp are then carried through queueing into every SDK custom-entry mirror. Each batch contains every physical inbound represented by a turn, including each message id, channel/chat, sender, timestamp, and initial body. Rapid messages therefore remain separate even when ingress debounce or queue collection combines them into one model turn, and a missing executor or first-turn setup failure remains recoverable. For session records without structured provenance, the command parses the stored inbound envelope (`[<channelType>] <senderId> (<time>):`) only at the exact producer-owned wrapper boundary.

The command runs **fully offline and has no RPC sibling — on purpose**. Unlike `comis explain` / `comis system-health` (bounded digests, counts + hints only), its output is **content-bearing**: message bodies are the payload. The obs network surfaces stay content-free, so this read never crosses the gateway — it only reads local files the operator already owns.

Internal dispatch turns are excluded by default and **counted, not hidden**: cron, sub-agent, heartbeat, cross-session, background-task, and restart-continuation turns plus the reserved `system` sender are agent-/scheduler-authored prompts, not humans — a summary note reports how many were excluded, and `--include-internal` shows them tagged `origin: "internal"`. Durable compaction summaries are counted separately. Provider-hidden and headerless records are returned as full messages only when the session identity and real trajectory pointer supply unique, matching sender and channel evidence; unresolved or conflicting records remain fail-closed. Coverage also records missing data/workspace/session roots, unreadable files and directories, corrupt or oversized records, malformed provenance, transcript/sidecar mirror differences, same-source identity conflicts, expected or missing sidecars, unparsed records, ambiguous physical boundaries, and every scan/output cap. Unparsed evidence is bounded, credential-redacted, digest-addressed, and classified as authoritative, inferred, or unresolved by following the real trajectory pointer or using successfully parsed records from the same session; channel names are never inferred from filesystem paths.

The reader scans JSONL backward in 64 KiB chunks, keeps the latest 5,000 logical nonblank records plus up to 32 predecessor records so one maximum-size chunk sequence can cross the boundary, caps any one record at 1 MiB, reads at most 16 MiB from one file and 256 MiB per command, and inspects at most 5,000 session files. Coverage reports the actual bytes read and every cap that made source coverage incomplete. Matches are retained in a bounded global latest-message heap while scanning; structured batches use their persisted provenance-record timestamp for filtering and ordering, while fallback messages use the SDK session-record timestamp. When `--limit` cuts the result, the latest N by that authoritative session-record time are kept and the truncation is announced without accumulating every historical match in memory.

Every user-controlled displayed field — including evidence previews — passes through one canonical, overlap-safe output redactor **before any table, text, JSON, or JSONL renderer sees it**. It covers authorization schemes regardless of case, Telegram bot tokens, provider keys, GitHub OAuth tokens, database URLs, and credentials spanning internal scan boundaries. Each record's `redactions` value counts matches, `coverage.secretRedactions` aggregates them, and human formats print a redaction note.

`<when>` values for `--since` / `--until` accept **safe-integer epoch ms** (`1783900800000`), **relative-ago** (`30m`, `24h`, `7d`), or an **ISO date/datetime** (`2026-07-12`, `2026-07-12T10:00:00Z`). Non-finite/unsafe epochs and invalid calendar dates are errors — never a silent widen-to-everything. `--date` is strict one-UTC-day sugar (impossible dates are rejected) and cannot be combined with `--since`/`--until`. `--limit` must be a positive safe integer no greater than 10,000. `--format json` emits the versioned `comis-offline-channel-messages-report` schema version 2 envelope with `messages`, `coverage`, `completeness`, and `redaction`; `jsonl` remains one redacted message per line.
...
| `--limit <n>` | `number` | `500` | Max messages returned, 1–10,000 (the latest N are kept) |
| `--include-internal` | `boolean` | `false` | Include cron/sub-agent/heartbeat/cross-session/background/`system`/restart-continuation dispatch messages |
| `--format <format>` | `table` \| `text` \| `json` \| `jsonl` | `table` | `table` previews the first line; `text` is the full chat log; `json` is the versioned completeness report; `jsonl` emits one full message per line |
...
Assemble a **paste-ready support bundle** — a single directory to hand to a maintainer or attach to a bug report. Each run writes up to nine content-free files (plus a `trace-exports/` directory with `--deep`) under `<dataDir>/support-bundles/comis-support-<ts>/`: a machine-readable triage verdict, the full health-check findings, a human-readable issue summary, an AI-fillable issue draft, a cross-session system health digest, a config-posture digest (config section membership only), a window-scoped audit-summary digest, and a manifest carrying the redaction fingerprint. Focusing on one session with `--session` adds a per-session `explain.json` incident digest; `--deep` additionally embeds that session's redacted trace bundle under `trace-exports/`. The system digest, config-posture digest, and audit-summary digest are each omitted when their source cannot be read — a thrown system assembly, an unparseable config, or an absent observability store — and the manifest records the gap. The verdict is derived from the local health checks only — no LLM is invoked, so the same install state always yields the same verdict.
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--since <hours>` | `number` | `24` | Diagnostic window in hours (same unit as `comis system-health`) |
| `--format <format>` | `table` \| `json` | `table` | Output format |
...
| `system-health.json` | Cross-session system health digest — degraded rate, top error kinds, and findings — over the `--since` window |
...
| `explain.json` | _With `--session`._ The per-session incident digest — the same content-free `IncidentReport` as [`comis explain`](#comis-explain), embedded verbatim |
| `trace-exports/` | _With `--deep`._ A directory holding the redacted per-session trace bundle (the [`comis trace export`](/operations/incident-bundle) output), embedded in place instead of written under the workspace |
...
Requires a gateway token (the `rpc` scope — any valid token works; no admin scope needed, unlike `comis explain` / `comis system-health`). There is no offline fallback — the daemon must be running.
...
| `--remove-user` | `boolean` | `false` | Linux+root only: fully remove installer-owned Comis artifacts and the dedicated user (implies `--purge`) |
| `--yes` | `boolean` | `false` | Skip confirmation prompt |
...
# Full dedicated-user removal with no prompts
sudo comis uninstall --remove-user --yes
```

The default uninstaller removes the managed service and CLI. `--purge` also
removes Comis data, configuration, logs, the CloakBrowser cache and wrapper, and
the `COMIS_EGRESS` egress-logging iptables chain. `--remove-user` performs the
full dedicated-user cleanup: it removes the `comis.service` and
`comis-xvfb.service` units, `/etc/tmpfiles.d/comis-x11.conf`, `/run/comis-x11`,
the managed sudoers rule and AppArmor profile, Comis data and package files,
the installer-created user and group, and the ownership receipt. Account
deletion is fail-closed: the root-owned receipt, account identity marker, home
ownership, and receipt-bound account identity must match. The receipt records
account removal as a one-way state transition, so an interrupted retry never
deletes a recreated user or group. An unowned account or modified managed file
stops the uninstall instead of being deleted.

Shared host dependencies are deliberately preserved because other software may
use them. These include Node.js, uv, Rust, browser/Xvfb packages, build tools,
and package repositories. Use `--dry-run` to preview every applicable action.
