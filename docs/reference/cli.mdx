...
The positional argument is routed automatically: a **session key** (`tenant:user:channel:ts`, contains `:`) is passed as `sessionKey`; a **trace ID** (a UUID, no `:`) is passed as `traceId`. The daemon canonicalizes a trace ID to its session key first, so both forms produce the identical report.
...
### `comis proxy validate`

Validate that the configured egress proxy is reachable, that the loopback gateway address (`localhost:4766`) is correctly bypassed, and list transports not covered by the global dispatcher. For the full proxy configuration guide and per-transport coverage map, see [Network Proxy](/security/network-proxy).

**OFFLINE** — does not require a running daemon. Reads proxy settings from `config.yaml` and the environment (`HTTPS_PROXY` / `HTTP_PROXY` / `NO_PROXY`).

Invoke via (CLI not on PATH — see [Quick Start](/get-started/quickstart)):
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--target <url>` | URL | `https://api.telegram.org` | Target URL to probe through the proxy |
| `--timeout-ms <ms>` | number | `5000` | Probe timeout in milliseconds |
| `--json` | flag | off | Output machine-readable JSON including `uncoveredTransports` |

```bash
# Basic validation — table output
node packages/cli/dist/cli.js proxy validate

# Custom target and shorter timeout
node packages/cli/dist/cli.js proxy validate --target https://api.openai.com --timeout-ms 3000

# JSON output (includes uncoveredTransports[])
node packages/cli/dist/cli.js proxy validate --json
```

**What it checks:**

1. **SSRF guard** — rejects any `--target` that resolves (via DNS) to a private, loopback, or metadata IP before making any network connection.
2. **Proxy probe** — sends a `HEAD` request to `--target` through a one-off `EnvHttpProxyAgent` or `ProxyAgent` dispatcher (never the global dispatcher). A successful CONNECT tunnel returns `probeOk: true`.
3. **Loopback canary** — asserts that `localhost:4766` (the gateway) is matched by the effective `NO_PROXY`, proving the gateway is NOT routed through the proxy.
4. **Failure classification** — maps probe failures to `errorKind` with an actionable `hint`:
   - `proxy_unreachable` — `HTTPS_PROXY` / `HTTP_PROXY` server cannot be reached; check the proxy URL and network path.
   - `proxy_timeout` — connection timed out; check the network path or increase `--timeout-ms`.
   - `proxy_tls_error` — TLS verification failed; set `proxy.tls.caFile` in `config.yaml` to the proxy CA certificate.
5. **Uncovered transports** (`--json`) — lists transports not routed through the global `EnvHttpProxyAgent` dispatcher: IRC (raw TCP), Discord WS gateway, WhatsApp/Baileys WS. `signal-cli` is env-covered (inherits `process.env`) and marked as such.

**Exit codes:**

| Code | Meaning |
|------|---------|
| `0` | Probe succeeded AND loopback canary bypassed |
| `1` | Probe failed OR loopback canary failed (proxy may be misconfigured) |
...
```json
{
  ...
  "proxyUrlMasked": "http://proxy.corp:8080",
  "loopbackCanaryBypassed": true,
  "uncoveredTransports": ["IRC", "Discord WS", "WhatsApp (Baileys)"],
  "message": "Probe succeeded — HTTP 200"
}
```
