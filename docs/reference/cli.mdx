...
**LCD Store checks** — when `memory.db` is present, six read-only scan classes run automatically against the lossless context store: orphaned summaries, dangling `context_items` references, fallback-marker summaries (quality debt), `lcd_ingest_cursor` inconsistencies, FTS row-count drift, and R4 scope anomalies (NULL tenant/agent keys). Output is content-free: findings carry only counts and row ids, never message text. Silently skipped on new installs where `memory.db` does not exist yet.
