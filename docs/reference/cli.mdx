...
Manage installed prompt skills through the typed daemon RPC surface.

```bash
comis skills list [--agent <id>] [--format table|json]
comis skills info <name> [--agent <id>] [--format table|json]
comis skills import <reference> [--source github|archive|wellknown|registry]
  [--registry <id>] [--scope local|shared] [--agent <id>] [--confirm] [--force]
  [--format table|json]
comis skills remove <name> [--scope local|shared] [--agent <id>]
```

The import command infers GitHub URLs, `wellknown:` references,
`registry:<configured-id>:<slug[@version]>` references, archive URLs ending in
`.skill`/`.zip`, and local archive paths. Use `--source` when the reference is
ambiguous. `--confirm` and `--force` both acknowledge a caution verdict or
same-tier replacement; `--force` also permits an explicitly reviewed bundled
MCP-name replacement. Neither skips scanning or overrides a blocked bundle.

`list` shows scope, install source, trust, verdict, and a content-hash prefix.
`info` prints the full content-free provenance record. All mutating operations
retain the `orch:skill` capability and approval gates. See
[Importing Skills](/skills/importing) for source configuration and policy.

---

### `comis system-health`

Assemble a `SystemHealthReport` — a bounded, **cross-session** triage over a recent window: the session count and degradation rate, the **degraded-by-cause breakdown** (degraded sessions bucketed by named `endReason` cause, e.g. `context_exhausted` / `output_starved`), the recurring `WARN` findings (recurring `lcd_divergence` / model / config-posture signals, counts + hints only), the total circuit-breaker trips, installed-skill exposure, the window cost, and a deterministic likely root cause. Like `comis explain`, the report is derived from log + diagnostics evidence only; no LLM is invoked, so the same window yields the same verdict. Backed by `ObsSystemHealthContract` (`obs.system.health`).

The `skillExposure` block counts remote imports, community-tier records,
registry imports, registry records absent from the current per-agent allowlist,
and skills with pending MCP declarations. It reads only
`installed-skills.json`, never skill bodies. Offline mode marks
`registryAllowlistKnown: false` because the active immutable agent config is
not available.
...
A `health_signal:cache_prefix_churn` finding fires when a session's Anthropic prompt-cache prefix **collapsed** on a recurring basis — a cached-region message mutated across turns on 3+ calls within a recent window, wasting the cache write (up to hundreds of thousands of cache-creation tokens over a session). The finding names the recurring **mutation class** (`structural-shift` = a message index shifted below the cache fence, e.g. from LCD condense/re-admit; `datetime-preamble` / `thinking-cleared` / `content-cleared` = an in-place edit). This closes a system-blindness gap where the churn was visible only as a daemon.log `Unstable prefix detected` WARN. Counts + the closed class label only — never message text.
