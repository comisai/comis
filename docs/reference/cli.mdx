...
`comis skills import <ref>` installs a skill from a GitHub directory URL or a packaged archive through the daemon's staged pipeline. The import is scanned and MCP-checked **before** anything is written, and a successful import is stamped the `imported` trust tier and pinned in the provenance store. See [Importing Skills](/skills/importing) for the full model.
