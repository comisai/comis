---
title: "CLI Reference"
description: "Complete reference for all 17 command groups, subcommands, and flags"
icon: "terminal"
---

Complete reference for the Comis command-line interface. The CLI connects to the Comis daemon via JSON-RPC over WebSocket. All commands are available via `comis <command>` (if globally installed) or `node packages/cli/dist/cli.js <command>`.
...
The other pm2 subcommands (`setup`, `start`, `stop`, `restart`, `status`) have no additional flags.
...
Security audit and auto-remediation tools. Runs 14 check categories covering configuration validation, file permissions, secret exposure, gateway hardening, and more.
