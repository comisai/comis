...
When `--repair` is passed, actionable findings are automatically repaired. Three LCD finding categories support repair:

| Finding | Repair action |
|---------|---------------|
| Fallback-marker summaries (`fallback=1`) | Re-summarized via the compaction queue seam (spend-governed, per-tenant breaker applies). If the circuit breaker is open, repair is skipped and the fallback marker remains. |
...
| FTS row-count drift | The `lcd_messages_fts` and `lcd_summaries_fts` full-text search indexes are rebuilt from the base tables using the FTS5 `rebuild` command. |
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--memory` | `boolean` | `false` | **Reserved — not yet implemented.** Accepted without error, but RAG memory is NOT cleared; only LCD history and session transcript are cleared. The CLI prints a warning when this flag is passed. A future phase will wire the full conversation-scoped RAG-memory clear. |
...
# Pass --memory (reserved, not yet implemented — prints warning; only LCD + session are cleared)
