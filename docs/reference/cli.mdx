...
comis skills import <reference> [--source github|archive|wellknown|registry]
  [--registry <id>] [--scope local|shared] [--agent <id>] [--confirm]
  [--format text|json]
...
ambiguous. `--confirm` acknowledges a caution verdict or same-tier replacement;
it never skips scanning and cannot override a blocked bundle.
