...
Comis ships **50+ leaf commands grouped into 18 command groups** (verified against `packages/cli/src/cli.ts` at version 1.0.22). The CLI talks to the running daemon over JSON-RPC; offline-only commands such as `secrets` operate directly on local files. Run any command with `--help` to see its full flag list — every flag below is also documented inline in the binary.
