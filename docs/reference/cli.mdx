...
The interactive **advanced** flow includes **Image Generation** and **Video Generation** steps that offer every supported provider for [`integrations.media.imageGeneration.provider`](/reference/config-yaml) and [`integrations.media.videoGeneration.provider`](/reference/config-yaml). The same selections are available non-interactively via the flags below.
...
Credentials follow the provider-following model: `auto` reuses the agent's main provider key; `openai`/`google`/`openrouter`/`xai` reuse `OPENAI_API_KEY`/`GOOGLE_API_KEY`/`OPENROUTER_API_KEY`/`XAI_API_KEY` when the main provider already supplies them; and `openai-codex` (image) uses your Codex OAuth login rather than a static key. So `--image-api-key`/`--video-api-key` are only needed for `fal` or a cross-provider static-key choice. See [Environment Variables](/reference/environment-variables#llm-provider-keys) for the key reuse rules.
