...
| `sessions reset-lcd <sessionKey>` | Clear the LCD durable conversation history for a session |
...
#### `sessions reset-lcd <sessionKey>`

Clear the LCD durable conversation history for a session. **This operation is irreversible.** Admin-gated — requires an admin token. After a reset, the model starts with a clean slate for this session.

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--memory` | `boolean` | `false` | **Reserved — not yet implemented.** Accepted without error, but RAG memory is NOT cleared; only LCD history is cleared. The CLI prints a `⚠` warning when this flag is passed. A future phase will wire the full conversation-scoped RAG-memory clear. |
| `--yes` | `boolean` | `false` | Required — skip the confirmation prompt (the command exits with an error if omitted) |

<Warning>
The LCD store holds the durable conversation history — deleting the session JSONL file does **not** clear it. Use `sessions reset-lcd` when you want the model to explicitly forget this conversation. See [Data Directory](/operations/data-directory) for the full durability model.
</Warning>
...
# Clear LCD history for a session (irreversible, requires --yes)
comis sessions reset-lcd "default:user123:telegram" --yes

# Pass --memory (reserved, not yet implemented — prints ⚠ warning; only LCD is cleared)
comis sessions reset-lcd "default:user123:telegram" --memory --yes
```
...
Requires an admin-scoped gateway token (same as `comis explain` / `comis trace`).
