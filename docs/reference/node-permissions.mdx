...
The Node.js permission model available in the supported Node.js runtime can
restrict what a process accesses. Comis requires Node.js 22.19.0 or newer, but
the permission model remains opt-in and is not enabled by Comis automatically.
...
1. Launch the daemon under Node.js 22.19.0 or newer with `node --permission --allow-fs-read=... --allow-fs-write=... --allow-net=... packages/daemon/dist/daemon-entrypoint.js`.
...
- **Node.js 22.19.0 or newer required:** This is also Comis's package-engine requirement.
