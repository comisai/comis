...
- Admin-only handlers form the derived **deny-by-origin** control-plane set, so the capability socket cannot turn a lease claim into control-plane access.

An admin attempt is therefore **denied identically to the `orchestrate(script)` path** -- the same closed door, the same denial. This is proved by `comis-agent-no-admin.test.ts`, which asserts the CLI table's intersection with both the derived admin-only method set and the denylisted-method set is empty.
