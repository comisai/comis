...
| **Returns** | An `IncidentReport`: `{ schemaVersion, sessionKey, traceId, agentId, channel, outcome: { endReason, degraded, severity }, cost: { costUsd, totalTokens, cacheReadRatio }, timing: { durationMs, turnCount }, toolStats, failures[], breakerTimeline[], offloads[], summary, likelyRootCause, suggestedNextSteps[], truncations[], coverage? }` |
...
```json Response
{
  ...
  "result": {
    ...
    "truncations": [],
    "coverage": { "trajectory": { "found": true, "records": 25 }, "rollup": { "present": true }, "offloads": { "pointersResolved": 1, "pointersTotal": 1 } }
  },
  "id": 1
}
```
...
Assemble a bounded, redaction-safe **cross-session** fleet-health triage (a `FleetHealthReport`) over a recent time window: how many sessions ran and how many degraded, the merged top error kinds, the total circuit-breaker trips, a per-tool ok/failed rollup, the window cost, the recent activity (agents, channels, exit reasons), the recurring `findings[]` (counts + short codes + hints only -- no raw WARN bodies), and a deterministic `likelyRootCause`. Like `obs.explain`, the report is derived from log + diagnostics evidence only -- no LLM is invoked -- so the same window reproduces the same verdict.
...
| **Returns** | A `FleetHealthReport`: `{ schemaVersion, windowHours, sessions: { total, degraded, degradedRate }, topErrorKinds[], breakerTripTotal, toolStats, cost: { costUsd, totalTokens }, activity: { activeAgents[], activeChannels[], exitReasons, turnTotal, tokenTotal }, findings[], likelyRootCause, suggestedNextSteps[], truncations[], coverage? }` |
...
<Accordion title="context (3 methods)">

LCD lossless-store RPCs. The two `rpc`-scoped browse methods back the dashboard's Context DAG browser; the `admin`-scoped `context.reset_lcd` is the explicit forget command for operators.
...
| `context.reset_lcd` | `admin` | Clear the LCD durable history for a session (explicit forget) |
...
Clear the LCD durable conversation history for a session. **Destructive and irreversible.** Admin-scoped — requires an admin token. The operation is serialized against live ingest (safe to call while the daemon is running). Returns row counts only — no message content is returned or logged.

| Property | Value |
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ session_key: string, memory?: boolean }` — `memory` defaults to `false`; when `true`, the request is accepted without error but **RAG memory is not yet cleared** (the `--memory` grain is reserved and deferred — see note below). Only LCD history is deleted regardless of this flag. |
| **Returns** | `{ sessionKey: string, lcdRowsDeleted: number, memoriesDeleted?: number }` — `memoriesDeleted` is **omitted** (`undefined`) in the current implementation, signalling that RAG memory clearing is not yet implemented. When the full RAG clear is wired in a future phase, `memoriesDeleted` will be set to the number of deleted memory entries. |
...
<Note>
**`memory: true` is reserved and not yet implemented.** Passing it is accepted without error, but RAG memory is not cleared — only the LCD history is deleted. The response will omit `memoriesDeleted` (`undefined`), signalling the not-implemented state. A future phase will wire the full conversation-scoped RAG-memory delete across all memory stores.
</Note>

After a reset, the LCD store is empty for the session. The next conversation turn starts with a clean slate — the model no longer has access to the prior history via the LCD read path. For the full durability model (LCD vs. JSONL vs. RAG memory), see [Data Directory](/operations/data-directory).
