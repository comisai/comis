...
LCD lossless-store **operator-browse** RPCs that back the dashboard's Context DAG browser. Both are `rpc`-scoped and AGENT+TENANT scoped: the calling agent comes from the request context and the tenant from the daemon — neither is a caller-supplied parameter, so a browse can never read another agent's (or tenant's) conversations.
