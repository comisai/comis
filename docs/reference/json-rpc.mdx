...
When the v2.26 **Verified Learning** outcome signal observed the turn the report carries an optional **`learning`** block reconstructed from the trajectory's `learning.outcome_observed` records: `{ outcomeResolved, outcome?, sources, skillsUsed, skillFailures, synthesisAbstained, userModelRevised?, memoriesGeneralized? }` (counts / ids / closed enums only — never a message body, a confidence value, or a recalled/skill id). The `userModelRevised` / `memoriesGeneralized` counts (the P4 user-model revision & generalization deepening) are described in detail below. `outcomeResolved` is `false` when the learning shadow saw a **finished** trajectory but **no** signal tier produced a resolvable outcome; `outcome` is the fused verdict (`success` / `failure` / `corrected` / `unknown`) when one resolved; `sources` are the contributing signal sources (`tool` / `pipeline` / `correction` / `judge` / `reaction` / `explicit`, deduped). This drives a dedicated **`outcome_unresolved`** `likelyRootCause` verdict — a **benign, lowest-priority** diagnostic that fires only when `outcomeResolved` is `false` and **no** acute cause matched (Defer ≠ Retry: an unresolved outcome is not a failure, so every tool-failure cause out-ranks it). It is **distinct** from an explicit `unknown` outcome (which IS a resolution); the verdict means the signal was never resolvable — e.g. a conversational turn with no deterministic tool/pipeline signal AND no judge verdict (the cost-gated LLM judge fallback is off, or it too abstained `unknown`). The **procedural-skill** fields (v2.26 WS2) are populated when the [`learningSkills`](/reference/config-yaml#learning-skills-agents-learningskills) loop is enabled: `skillsUsed` is the ids of the learned procedures whose `<location>` a `read` in the turn matched, `skillFailures` is the used-skill ids in a **failed / corrected** trajectory (a learned procedure implicated in a bad outcome — content-free ids only, derived since the `skill_validated` record carries no id), and `synthesisAbstained` is `true` when the offline synthesizer deferred (the agent's model tier was below the capability gate). They drive two **benign, low-priority** `likelyRootCause` verdicts ranked below every acute cause but above the generic `outcome_unresolved`: **`learned_skill_failing`** (fires on a non-empty `skillFailures`; names the failing skill ids and points at [`comis memory skills`](/reference/cli#comis-memory)) and **`synthesis_abstained_low_capability`** (fires on `synthesisAbstained`; **benign** — Defer ≠ Retry, an abstain is never a failure or a retry). The `learned_skill_failing` hint's promise — "the procedure will demote on continued failure" — is now actionable: a learned procedure that is repeatedly used in failed/corrected reuse is **demoted** (`active` → `stale` → `archived`, corroboration-gated), which drops it from the read-only surface. When the loop is enabled (still per-agent **default-off**), an `active`, read-only procedure is **surfaced** to the agent (v2.26 P3) and its lifecycle emits two **counts-only** trajectory events — **`learning:skill_promoted`** (a candidate crossed the proof bar to `active`) and **`learning:skill_demoted`** (an active procedure weakened to `stale`) — both carrying `{ agentId, count, timestamp }` only (never a body/script/id-list), surfaced through [`comis explain`](/reference/cli#comis-explain) and [`comis fleet`](/reference/cli#comis-fleet). These fields stay empty/`false` for a default install and on any agent with `learningSkills` off. The block is absent for sessions with no learning records (the signal is per-agent default-off); the per-agent outcome coverage is aggregated by [`comis memory learning`](/reference/cli#comis-memory) and the admission funnel + the promotion/demotion roll-up by [`comis memory skills`](/reference/cli#comis-memory).

The v2.26 **ranking & forgetting** loops (WS3/WS4) emit their own trajectory signals, surfaced through the same `comis explain` / `comis fleet` lenses and **counts only** — never an alpha value or a memory body: **`memory:online_tuning_applied`** (one per outcome-rewarded tuning run, carrying the per-query-**intent** dimension + counts — the visible proof that ranking is learning), and **`learning:memory_demoted`** / **`learning:memory_evicted`** (the per-sweep soft-eviction counts). They are bridged onto the trajectory so a tuning run or an eviction sweep is reconstructable per session; like the outcome signal they are per-agent **default-off** (`agents.<id>.learningTuning.enabled` / `learningForgetting.enabled`, force-disabled by `memory.costFeatures.enabled: false`). The per-memory recall **score breakdown** (the `recall` records) additionally carries an OBS-02 `usefulnessOutcomeShare` annotation — the outcome-attributed component of a memory's usefulness factor, distinct from its lexical relevance base (a derived, normalized share — never the raw tuned alpha).

The v2.26 **user-model revision & generalization** deepening (P4) emits two further **counts-only** trajectory events, surfaced through the same lenses and bridged so a revision build or a generalization run is reconstructable per session — both carry **counts only**, never a profile body, an `entry_type`, a synthesized memory's content, or a source id:

- **`learning:user_model_revised`** — `{ agentId, superseded, corroborated, inserted, durationMs, timestamp }`: the per-build counts of incumbent beliefs soft-closed (`superseded`), confidences bumped (`corroborated`), and new current-truth slots inserted (`inserted`) by the bi-temporal user-representation revision. Per-agent **default-off** (rides `agents.<id>.memoryUserRepresentation.enabled`).
- **`learning:memory_generalized`** — `{ agentId, generalized, clustersConsidered, durationMs, timestamp }`: the per-run counts of higher-order `semantic` memories created (`generalized`) and diversity-passing clusters considered (`clustersConsidered`) by the consolidation generalization pass. Per-agent **default-off** (rides `agents.<id>.memoryConsolidation.generalize.enabled`).

Accordingly the `learning` block carries two further optional counts — **`userModelRevised`** and **`memoriesGeneralized`** (folded from the bridged records; omitted when zero) — and a **benign, lowest-priority** `user_model_revised` `likelyRootCause` verdict fires when `userModelRevised > 0` (it ranks below `outcome_unresolved` and every acute cause: Defer ≠ Retry, a profile revision is never a failure or a retry, and it carries no profile body).

Accepts ONE of `sessionKey` / `traceId` / `rootRunId` (at least one is required). A `traceId` *or* a `rootRunId` is canonicalized to its `sessionKey` first, so by-trace, by-session, and by-run produce the identical report. `rootRunId` (FLEET-05) is an autonomy run's id — the synthetic in-process `root-session-<sessionKey>` (resolved by a pure prefix-strip) or a real spawned/socket root (resolved by scanning the day-keyed session-index for the run's `capability.audited` record). It is the [`obs.fleet.health`](#obs-fleet-health) → `obs.explain` drill-down ref: pass the fleet report's `autonomy.worstRootRunId` to render that run's `spawnTree`. An unresolvable `rootRunId` (or `traceId`) yields the honest `session_not_found` marker naming the ref that missed — never a clean-looking empty report.
...
| **Returns** | A `FleetHealthReport`: `{ schemaVersion, windowHours, sessions: { total, degraded, degradedRate }, degradedByCause, topErrorKinds[], breakerTripTotal, toolStats, cost: { costUsd, totalTokens }, activity: { activeAgents[], activeChannels[], exitReasons, turnTotal, tokenTotal }, findings[], likelyRootCause, pipelineAuthoringGate?: { buildAuthor, reason }, autonomy?: { runs: { total, degraded, degradedRate }, orphaned, resumed, revoked, killed, breakerTrips, budgetBreaches, costUsd, worstRootRunId? }, suggestedNextSteps[], truncations[], coverage? }` |
...
```json Response
{
  ...
  "result": {
    ...
    "findings": [
      ...
      { "code": "pipeline_authoring", "detail": "3/12 small-tier pipeline authorings invalid (25.0%)", "count": 3, "hint": "small-model pipeline-authoring failure rate; gates Phase 174 (small-model-authorable DAGs) -- see pipelineAuthoringGate" }
    ],
    "likelyRootCause": {
    ...
    "autonomy": { "runs": { "total": 20, "degraded": 3, "degradedRate": 0.15 }, "orphaned": 2, "resumed": 4, "revoked": 1, "killed": 0, "breakerTrips": 1, "budgetBreaches": 0, "costUsd": 0.42, "worstRootRunId": "root-2f9c1a" },
...
Operator-facing live control of the [bounded-autonomy](/agents/autonomy) control plane: stop, hard-stop, or demote a runaway or misbehaving spawn tree. All three methods are **admin-scoped and deny-by-origin** — they reject any agent-origin call, so a jailed agent can never revoke, un-kill, or un-evict itself (the bound is external to and non-bypassable by the agent). The bound mechanisms (spawn ceiling, rate limit, outward quota, per-root budget) are otherwise automatic from the resolved autonomy profile; these methods are the manual override.
