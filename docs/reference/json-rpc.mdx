...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ sessionKey?: string, traceId?: string, depth?: "summary" \| "full" }` -- at least one of `sessionKey` / `traceId` required |
| **Returns** | An `IncidentReport`: `{ schemaVersion, sessionKey, traceId, agentId, channel, outcome: { endReason, degraded, severity }, cost: { costUsd, totalTokens, cacheReadRatio }, timing: { durationMs, turnCount }, toolStats, failures[], breakerTimeline[], offloads[], summary, likelyRootCause, suggestedNextSteps[], truncations[] }` |
...
<Note>
This reference documents the `obs.explain` JSON-RPC method. The `comis explain` CLI command and the `obs_query` agent action that surface the same report are added in a later release.
</Note>
