...
`outcome.endReason` names the terminal cause. Two degradation causes that previously collapsed into a generic `error` are named here (and drive `likelyRootCause`): **`context_exhausted`** (the context-window pre-flight guard aborted the run before the model could continue) and **`output_starved`** (the final response was truncated at the model's max output tokens; the fix is to raise the agent's `maxTokens` or enable `contextEngine.outputEscalation`). A tool-failure root cause still out-ranks them -- they explain the terminal state, not a tool crash. The same causes are aggregated fleet-wide by [`obs.fleet.health`](#obs-fleet-health)'s `degradedByCause`.

For `context_exhausted` sessions the report carries an optional **`contextBudget`** section -- the terminal per-LLM-call budget equation extracted from the trajectory's `context.budget` records (last record wins): `{ windowTokens, rawContextWindowTokens, windowCapSource, systemTokens, freshTailTokens, budgetedHistoryTokens, keptCount, assembledInputTokens, outputHeadroom, verdict }`. `windowCapSource` names the exact `contextEngine.budget.*` knob that clamped the effective window below the model's declared `contextWindow` (`effectiveContextCapSmall` / `effectiveContextCapNano` / `none`), and `verdict` is the fit-check outcome (`fits` / `downshifted` / `exhausted`). When present, `likelyRootCause` is numbers-backed: it states the assembled-vs-window totals, the cap knob, the system+tools share of the window, and the kept-history count, and its suggested steps name the knob to raise (0 = uncapped) or recommend reducing the active tool surface when tool schemas dominate. The section is absent for sessions recorded before the `context.budget` trajectory event existed; the verdict then falls back to the generic wording.
...
| `session.reset_conversation` | `admin` | Complete cross-mode conversation reset (LCD history + working session transcript) |
...
Complete cross-mode conversation reset. Clears **both** the LCD durable history **and** the daemon sessionStore working transcript. **Destructive and irreversible.** Admin-scoped — requires an admin token.

This is the correct explicit forget command. Unlike `session.delete` (which only removes the session record), this operation ensures the model starts with a clean slate in **both engine modes**: the LCD store is empty (dag mode) and the sessionStore messages are empty (both modes — the JSONL-backed transcript that feeds `state.messages` on the next turn is wiped).
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ session_key: string, memory?: boolean }` — `memory` defaults to `false`; when `true`, the request is accepted without error but **RAG memory is not yet cleared** (reserved and deferred). Only LCD history and session transcript are cleared regardless of this flag. |
| **Returns** | `{ sessionKey: string, lcdRowsDeleted: number, sessionMessagesCleared: number, memoriesDeleted?: number }` — `memoriesDeleted` is **omitted** (`undefined`) in the current implementation, signalling that RAG memory clearing is not yet implemented. |
...
<Note>
**`memory: true` is reserved and not yet implemented.** Passing it is accepted without error, but RAG memory is not cleared — only the LCD history and session transcript are deleted. The response will omit `memoriesDeleted` (`undefined`), signalling the not-implemented state. A future phase will wire the full conversation-scoped RAG-memory delete across all memory stores.
</Note>
