...
running a model turn on it. Length is treated as evidence of opacity only for
scripts that separate words: a long request written in a space-free writing
system is one oversized token and is admitted normally. The size only, never the
payload. The section is absent for every ordinary turn.
