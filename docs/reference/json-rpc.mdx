...
The `provenanceSummary.source` is the acquisition **channel** (`github` / `archive` / `wellknown` / `upload`) -- distinct from the trust-tier `source` (`imported`). A `wellknown` import additionally carries a `registry` field: the origin the skill was resolved from. It is populated only for imported skills; bundled, workspace, local, and learned skills omit it. The summary is content-free: an acquisition channel, an optional registry origin, a hash prefix, and a timestamp -- never a skill body.
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ url?: string, source?: "github" \| "archive" \| "wellknown", registry?: string, archiveUrl?: string, archiveBytes?: string, name?: string, scope?: "local" \| "shared", agentId?: string, confirm?: boolean }` |
