...
The v2.26 **forgetting** loop (WS4) emits its own trajectory signals, surfaced through the same `comis explain` / `comis fleet` lenses and **counts only** — never a memory body: **`learning:memory_demoted`** / **`learning:memory_evicted`** (the per-sweep soft-eviction counts). They are bridged onto the trajectory so an eviction sweep is reconstructable per session; like the outcome signal they are per-agent **default-off** (`agents.<id>.learningForgetting.enabled`, force-disabled by `memory.costFeatures.enabled: false`). The per-query-intent tuned-alpha ranking bandit and its `memory:online_tuning_applied` signal were **removed in v2.31** — recall ranking is the fixed `rag.scoring` fusion. The per-memory recall **score breakdown** (the `recall` records) still carries an OBS-02 `usefulnessOutcomeShare` annotation — the outcome-attributed component of a memory's usefulness factor (the bounded `recordUsage`/`recordFailure` feed), distinct from its lexical relevance base (a derived, normalized share — never a raw learned weight).

The v2.26 **user-model revision & generalization** deepening (P4) defined two further **counts-only** trajectory events. As of **v2.31** the standalone user-representation + consolidation crons that emitted them were **removed** (the work folded into the one reflection cron — see [memory config](/agents/memory-config)); the event **definitions are retained** (dormant — bridged + verdict wiring intact) pending the v2.31 reflection-event rename, so an older daemon's history still resolves them. Both carry **counts only**, never a profile body, an `entry_type`, a synthesized memory's content, or a source id:

- **`learning:user_model_revised`** — `{ agentId, superseded, corroborated, inserted, durationMs, timestamp }`: the per-build counts of incumbent beliefs soft-closed (`superseded`), confidences bumped (`corroborated`), and new current-truth slots inserted (`inserted`). Dormant in v2.31 (no live emitter — the profile is now maintained by the reflection cron's `kind:"profile"` supersede path).
- **`learning:memory_generalized`** — `{ agentId, generalized, clustersConsidered, durationMs, timestamp }`: the per-run counts of higher-order `semantic` memories created (`generalized`) and diversity-passing clusters considered (`clustersConsidered`). Dormant in v2.31 (no live emitter — higher-order observations are now the reflection cron's `kind:"topic"` docs).
