...
A **terminal execution failure out-ranks the recall evidence beside it**: an empty memory store makes *every* recall a zero-hit, so on a session that ended in `error` with **no** tool failure but whose activity surface finalized as a failure, the death is the root cause and the recall miss is incidental (`recall_miss` does not fire on a failed finalize). Three verdicts name it, by the terminal `errorKind`: **`execution_auth_failure`** (an `auth` kind, or an `auth` entry in the session-summary error tally) points at the credential and the provider/model that must match it; **`execution_dependency_failure`** (a `dependency` kind) points at the provider-degradation and circuit-breaker signals in [`comis system-health`](#obs-system-health) and at `agents.<id>.model` / `providers.entries.<name>`; and **`execution_terminal_failure`** names every remaining kind (`internal`, `validation`, `resource`, `precondition`, ... or an unclassified failure) together with the finalize reason. The generic verdict is ranked **below** the specific terminal causes (`terminal_drive_opened_without_task`, `terminal_drive_evicted`, `orchestrate_failed`), which keep their own diagnosis, and **above** the `completed_with_tool_errors` catch-all -- which requires tool failures, so the two never compete.
