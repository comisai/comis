...
> **Folded in v2.31.** The standalone user-representation and consolidation/reasoning crons — their dedicated revision/generalization telemetry events, the `learning` block's former per-build revision and generalization counts, and the matching low-priority verdict — were all **removed**. The work now folds into the one reflection cron as `kind:"profile"` / `kind:"topic"` docs (see [memory config](/agents/memory-config#removed-in-v2-31)). A profile revision surfaces through the `reflect:*` funnel above, not a dedicated event.
