...
Model catalog inspection and connectivity testing. The catalog is sourced live from the [`@mariozechner/pi-ai`](https://www.npmjs.com/package/@mariozechner/pi-ai) SDK — `getProviders()` for the provider list, `getModels(provider)` for per-provider model details. No hardcoded provider/model tables on the comis side.
