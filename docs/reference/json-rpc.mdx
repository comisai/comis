...
<Accordion title="agent (3 methods)">

Agent execution for processing user messages through the LLM pipeline, plus cache statistics inspection.
...
Same parameters as `agent.execute`. Returns streaming token deltas over the WebSocket connection, followed by the final result.
