...
Accepts EITHER `sessionKey` OR `traceId` (at least one is required). A `traceId` is canonicalized to its `sessionKey` first, so by-trace and by-session produce the identical report.
