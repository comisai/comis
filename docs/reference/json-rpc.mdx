...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ session_key: string, memory?: boolean }` — `memory` defaults to `false`; when `true`, also clears RAG memories for this session (the full-forget / GDPR path) |
| **Returns** | `{ sessionKey: string, lcdRowsDeleted: number, memoriesDeleted?: number }` — `memoriesDeleted` is `0` when `memory` is `false` or absent |
