...
In session responses, `ChannelEndpoint` is the complete exact routing identity:
...
```json Response
{
  ...
  "result": {
    "providers": [
      ...
      "opencode-go", "openrouter", "together", "vercel-ai-gateway", "xai",
...
identifies its target with only `channel_type` and `channel_id`; the contract
cannot distinguish channel instances, threads, or direct/shared conversation
kind. A client holding a complete `ChannelEndpoint` must fail closed instead of
discarding those qualifiers.
...
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ channel_type: string, channel_id: string, text: string, buttons?: object[][], cards?: object[], effects?: object[], thread_reply?: boolean }` |
...
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ channel_type: string, channel_id: string, text: string, message_id: string, buttons?: object[][], cards?: object[], effects?: object[], thread_reply?: boolean }` |
| **Returns** | `{ messageId: string, channelId: string }` |
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ channel_type: string, channel_id: string, attachment_url: string, attachment_type?: "image" \| "file" \| "audio" \| "video", mime_type?: string, file_name?: string, caption?: string }` |
