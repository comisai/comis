...
Load a previously saved named graph by its id. The stored entry comes back as-is -- pass its `nodes` to [`graph.execute`](#graph-execute) to run it; loading does not mint a `graphId` or start anything. Legacy `inputFrom` / `inputMapping` fields are stripped from the persisted nodes and edges on load -- use the `{{nodeId.result}}` pattern instead (see [Pipelines](/developer-guide/pipelines)).
