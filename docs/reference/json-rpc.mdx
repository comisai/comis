...
description: "Reference for the JSON-RPC methods exposed by the daemon"
icon: "code"
---
...
Every JSON-RPC method declares one or more accepted scopes. Tokens are configured in `gateway.tokens[]` with a `scopes` array.

| Scope | Access Level | Description |
|-------|-------------|-------------|
| `rpc` | Standard | Client operations: agent execution, memory search, cron management, browser, skills, graphs |
| `admin` | Administrative | Config, agent management, memory management, observability, tokens, channels, capability services, managed runs |
| `*` | Wildcard | Access to all methods (both `rpc` and `admin`) |

A token with `scopes: ["rpc"]` can only call `rpc`-scoped methods. A token with `scopes: ["*"]` has access to all methods. Calling a method without sufficient scope returns error code `-32603` with a message naming the required scope or scopes.

Methods that list multiple scopes accept any one of them. Route admission and
handler trust are separate: after admission, an authenticated admin token keeps
admin trust even when another accepted scope also matched. Internal trust fields
are stripped from client parameters and cannot be forged.
...
Observability methods for diagnostics, billing estimation, channel activity tracking, message delivery tracing, context engine inspection, prompt cache statistics, and data management. Most methods require `admin` scope. `obs.diagnostics` is `rpc`-scoped with no in-handler admin gate. `obs.explain` also uses an `rpc` route, but its handler requires authenticated admin trust.
...
Assemble a self-contained, redaction-safe post-mortem (an `IncidentReport`) for a single agent session: outcome, cost, timing, per-tool stats, the normalized failures (newest-first), the circuit-breaker timeline, large-result offloads, a one-line summary, and a deterministic `likelyRootCause`. The report folds persisted trajectory, rollup, audit, and bounded log evidence; no LLM is invoked, so the same persisted evidence reproduces the same verdict.

Connected-tool recovery is diagnosable from this report without joining raw
logs. The optional `recoveries` block carries `{ total, succeeded, byReason,
groundedResponseBeforeRecoveryCount?, groundedResponsePreservedCount?,
successfulReceiptsOutsideRoute? }`, and `discoveryActivation` carries
`{ displayedCount, activatedCount, replacedCount, skippedCount, failedCount }`.
These are counts only: they prove the presence, handoff, and activation of
runtime evidence, not the factual completeness of an external data source.
`likelyRootCause` uses them to name `recovery_replaced_grounded_response` or
`discovered_tool_not_activated` before downstream completion-correction noise.
When a grounded answer was preserved but the routed workflow still stalled,
the report retains the specific `tool_invocation_stall` diagnosis instead of
overwriting it with the later completion-evidence guard.

Background completion delivery is likewise reconstructable without a raw-log
join. The optional `outwardDeliveries` array carries one bounded durable
completion-send disposition per root: `{ status, rootRunId, transition,
stepIndex?, deliveryKind?, platformMessageId? }`. `deliveryKind` distinguishes text from a verified-file
attachment, and a committed platform receipt is retained as
`platformMessageId`. `status: "partial"` means at least one completion part
committed while another part did not; `blocked`, `in_flight`, `failed`, and
`parked` preserve the corresponding durable-ledger state without claiming the
user received the whole completion.

Direct children in `spawnTree` also carry terminal execution evidence when it
was recorded: `terminalOutcome`, `runtimeMs`, `tokensUsed`, `costUsd`, and an
optional `outputValidation: { expected, verified, attachmentsPrepared }`.
These are content-free counts. They show whether declared files existed and
entered governed attachment preparation, while `outwardDeliveries` supplies the
separate platform-delivery receipt.

Graceful restart recovery is reconstructed in the optional
`restartRecovery: { suspended, resumed, lastStatus, rootRunId, checkpointId }`
block. The assembler folds recovery evidence for the selected session and its
spawned roots, so the same suspension or boot resume is visible when explaining
the parent session or drilling into the affected `rootRunId`. The block carries
counts and identifiers only, never task or provider output.

The same one-call diagnosis applies to bounded execution stalls. A local MCP
queue refusal produces `mcp_queue_contention` with the resolved concurrency and
queue/deadline budgets; a detector-owned repeating-call abort produces
`execution_no_progress_loop` with bounded duplicate and stagnant-result counts;
and a parent timeout overlapping `subagent.wait` produces
`subagent_wait_deadline_overlap` with requested/effective wait budgets and
whether the routed child remained independently deliverable. These specific
causes outrank incidental recall misses and generic terminal summaries.

`outcome.endReason` names the terminal cause. Three degradation causes are named individually rather than collapsing into a generic `error` (and drive `likelyRootCause`): **`context_exhausted`** (the context-window pre-flight guard aborted the run before the model could continue), **`output_starved`** (the final response was truncated at a length limit; the accompanying WARN's `hint` names the remedy that applies, because the cause splits three ways -- output that consumed the whole allowance is fixed by raising the agent's `maxTokens` or enabling `contextEngine.outputEscalation`; output that ended *below* the model's own ceiling (`recoverableLength: true` on the WARN) was truncated early under context pressure, which is fixed by reducing assembled context, and the SDK already compacted and retried that turn once; a turn that stopped at the cap having emitted nothing usually spent its whole output allowance on thinking), and **`timeout`** (the prompt-level stall budget or makespan ceiling killed the turn; `likelyRootCause` code `prompt_timeout` -- when the trajectory carries the enriched `execution.prompt_timeout` record the verdict is numbers-backed: the stall variant names the binding knob, e.g. `agents.<id>.promptTimeout.promptTimeoutMs`, with the configured budget and elapsed time, and the makespan variant names `agents.<id>.promptTimeout.stallCeilingMultiplier` for a streaming runaway; sessions whose trajectory lacks that record fall back to a generic knob suggestion). A tool-failure root cause still out-ranks them -- they explain the terminal state, not a tool crash. The same causes are aggregated deployment-wide by [`obs.system.health`](#obs-system-health)'s `degradedByCause`.

An `outcome.endReason` of **`cancelled`** means the **caller** aborted the
execution (a shutdown or queue-cancellation signal), not the runtime failing:
like `success` it is a **clean** terminal, so it does not count toward the
degraded rate or `degradedByCause` in
[`obs.system.health`](#obs-system-health) and carries no `errorKind`. Do not
read it as a `timeout`.
...
A background task the runtime aborted at its own hard runtime ceiling
root-causes to **`background_hard_timeout`** instead. The verdict names the tool
and the binding `agents.<id>.backgroundTasks.maxBackgroundDurationMs` value
taken from the abort record, and it out-ranks the retry breaker that abort may
have tripped -- the ceiling is the cause, the breaker its downstream symptom.

The report may include content-free request-selection evidence:
`requestRelevantToolNames` lists the bounded tool names selected for the turn;
`requestRelevantPromptSkillNames` lists the bounded prompt-skill names whose
route cleared the [workflow-enforcement bar](/skills/manifest#comis-namespace);
`requestRelevanceHistory` records `{ turnCount, charCount, saturated, recallDisposition? }`
for the bounded prior-user-turn window; and `operatorPolicyToolProjections` records up
to 16 `{ toolName, sectionId, contentHash, projectedChars }` receipts for
immutable operator-policy sections projected into selected tool schemas. These
fields contain names, hashes, sizes, and counts only -- never policy or message
bodies. `recallDisposition` states what the turn's memory recall did with that
window: `search` (the normal path) or `skip_oversized_token` -- the current
request carried an oversized opaque token and no retrieval terms of its own, so
recall was skipped rather than letting older turns supply the query and inject
unrelated memory. An oversized token made mostly of non-ASCII letters is read as
possible space-free prose (the same open Unicode signal the input guard applies
below), so a request written in a writing system that does not separate words
still reports `search` and keeps its recall.

A request that stopped **before** model dispatch to ask the user for an
instruction carries a content-free **`requestClarification`** section --
`{ reason: "opaque_payload_missing_instruction", inputChars }`. It means the
inbound message was a large opaque payload with no task beside it (data, not a
request), so the input guard answered with a clarification ask instead of
running a model turn on it. A long token made mostly of non-ASCII letters is
treated as possible space-free prose and admitted without relying on a closed
locale or script list. The size only, never the payload. The section is absent
...
When the session ran any memory recalls the report carries an optional **`recall`** section -- the memory-recall outcome aggregated over the trajectory's `memory.recalled` records: `{ recalls, zeroHits, lastLanes, lastFinalCount, rerankerAvailable, crossUserRecalls?, lastCrossUserCount? }` (counts and booleans only -- never the query text or any recalled memory body). `recalls` is how many recalls ran, `zeroHits` how many returned **no** injected memories (a recall miss), and the `last*` fields describe the terminal recall (its lane count and final injected-memory count). **`crossUserRecalls`** (present only when `> 0`) is how many recalls injected at least one memory scoped to a **different user** than the conversation -- the cross-sender privacy signal for agent-scoped recall (one shared agent surfacing sender A's memory into sender B's turn), so "was cross-sender data injected into this turn's context?" remains answerable even if the detailed `diagnostics.recallTrace` recorder is disabled; **`lastCrossUserCount`** is the terminal recall's cross-user injected count. Counts only -- never the user-ids or bodies. This drives a dedicated **`recall_miss`** `likelyRootCause` verdict: a degraded session whose recalls **all** missed (`zeroHits === recalls`) and that matched no tool/context/breaker cause is root-caused as a recall miss, pointing the operator at the recall **scope** (agent- vs user-scoped) and, for non-Latin queries, the trigram-twin lanes (see [`comis system-health`](#obs-system-health) `health_signal` / `config_posture` embedder). A zero-hit recall on a **healthy** turn is benign -- the agent simply did not need memory -- and never names a cause. The section is absent for sessions with no recall records.

A **terminal execution failure out-ranks the recall evidence beside it**: an empty memory store makes *every* recall a zero-hit, so on a session that did not end cleanly, carries **no** tool failure, and is not flagged healthy, the death is the root cause and the recall miss is incidental (`recall_miss` does not fire on a failed finalize). The gate follows that suppression rather than the literal `error` end reason -- every end reason except `success` and `background_pending` is in scope when the activity surface finalized as a failure, so a **named** cause carrying the same failed finalize (`provider_degraded`, `narration_stall`, ...) is named rather than left unrooted; the generic `error` end reason also passes the gate on its own, since a hard abort can skip the activity finalize entirely -- with no finalize to read a kind from, only an `auth` tally entry still names a cause there. Three verdicts name it, by the terminal `errorKind` -- the terminal finalize's own kind wins whenever it recorded one, and the session-summary error tally is consulted only as the fallback for a surface that finalized without a kind (a session-wide `auth` entry from an earlier, since-repaired turn must never mask the kind the session actually died of). **`execution_auth_failure`** (an `auth` finalize kind, or an `auth` entry in that fallback tally) points at the credential and the provider/model that must match it; **`execution_dependency_failure`** (a `dependency` kind) points at the provider-degradation and circuit-breaker signals in [`comis system-health`](#obs-system-health) and at `agents.<id>.model` / `providers.entries.<name>`; and **`execution_terminal_failure`** names every remaining kind (`internal`, `validation`, `resource`, `precondition`, ... or an unclassified failure) together with the finalize reason. The auth and dependency verdicts sit directly above `recall_miss`; the generic verdict is ranked **below** the specific terminal causes (`terminal_drive_opened_without_task`, `terminal_drive_evicted`, `orchestrate_failed`), which keep their own diagnosis, and **above** the `completed_with_tool_errors` catch-all -- which requires tool failures, so the two never compete.

When the provider **rejected** any of the session's LLM calls the report carries an optional **`modelErrors`** section aggregated over the trajectory's `model.completed` records: `{ total, byCategory }` -- the count of rejected calls and their per-category tally, ordered dominant-category-first (the closed `ErrorCategory` enum only; the raw provider body is never persisted to a trajectory record). This drives a dedicated **`provider_rejected_request`** `likelyRootCause` verdict, ranked **above** `recall_miss` and the tool-failure catch-all: when the provider refuses the request itself the model never runs, so any recall or tool evidence on that session is downstream of the rejection. It fires only for **deterministic** rejection categories (e.g. `model_capability_unsupported`, `client_request`, `model_not_available`, `auth_invalid`, `credit_exhausted`) -- transient classes (`overloaded`, `rate_limited`, `provider_unreachable`) clear on their own and never name a root cause. The **`model_capability_unsupported`** category means the resolved model refused a request *parameter* (e.g. a thinking/reasoning shape it does not accept); its suggested steps name `agents.<id>.model` and warn that a `providers.entries.*` entry which re-declares built-in catalog models can drop the capability metadata that selects the request shape. The section is absent when no model call errored.
...
When the session prepared any managed ([capability-service](/reference/config-yaml#capability-services)) runs, the report carries an optional **`managedRuns`** block — the managed runs this session prepared, resolved from the durable managed-run store by the trace ids the session's trajectory actually ran: `{ total, degraded, runs: [{ managedRunId, serviceInstanceId, status, statusReason }] }` (**ids + closed status/reason enums + counts ONLY** — never a report body, workspace path, objective, or service credential; safe to paste). `degraded` counts the `failed` + `unknown` runs (a `cancelled` run is an intended outcome, not degradation). This is the session→managed-run linkage: an operator explaining a session sees which external-service runs it spawned without a raw-log join, and pastes a `managedRunId` into [`comis managed-runs explain`](/reference/cli#comis-managed-runs) for that run's own root cause. Content-free by construction (managed runs are durable rows of ids/enums), and it works offline — the daemon-less `comis explain --offline` reads the same local `memory.db`. The section is absent for sessions that prepared no managed run, or when no managed-run store is wired (the block is presence-conditional and the non-strict `.parse()` strips any undeclared key).

When the report was targeted at an [execution graph](/agents/execution-graphs) run (the `graphId` input below) it carries an optional **`graph`** section read from that run's terminal `graph-runs/<graphId>/_run-metadata.json`: `{ graphId, status, cancelReason?, sessionKey?, traceId?, announcementDelivery, startedAt?, completedAt?, durationMs, nodesTotal, nodesSucceeded, nodesFailed, nodesSkipped, nodesRetried, totalCostUsd?, totalTokens?, nodes[] }`, where `nodes` holds up to 20 `{ nodeId, status, durationMs, subAgentRunId, attemptsUsed }` entries. Ids, closed enums, counts, timing, and aggregate usage only — never a node label, task, output, or error body. `sessionKey` is the authenticated **caller** turn the graph was submitted from, so the surrounding report is the launching session's post-mortem rather than an unrelated one: "the graph finished — what did the user who asked for it actually get?" is one call.

**`announcementDelivery`** is the terminal disposition of the graph's [aggregate completion announcement](/developer-guide/pipelines#graph-coordinator), recorded only after delivery settles so one record carries both the execution outcome and the outward one: `not-requested` (the run captured no announcement route), `unavailable` (a route existed but the governed announcement boundary was unwired, so nothing could be sent), `committed` (the channel returned a delivery receipt), `retained` (the send did not commit but the outward operation is durably retained and resumable — inspect it before any retry), or `failed` (the route or caller identity was invalid, or the send settled with no durable evidence). [`comis explain`](/reference/cli#comis-explain) prints it as `notification=<disposition>` on the `Graph:` line.

Accepts ONE of `sessionKey` / `traceId` / `rootRunId` / `graphId` (at least one is required). A `traceId` field may carry either the trace itself or the UUID-shaped sub-agent run ID exposed by a parent report; either form, or a `rootRunId`, is canonicalized to its `sessionKey` first, so by-trace, by-session, and by-run produce the identical report. Child-run resolution requires a structurally parsed session key whose channel is exactly `sub-agent:runtime:<runId>`; unrelated UUIDs elsewhere in a key do not match. `rootRunId` is an autonomy run's id — a synthetic in-process `root-session-<generation UUID>-<base64url formatted session key>` (decoded and validated before resolution) or a real spawned/socket root (resolved by scanning the day-keyed session-index for the run's `capability.audited` record). It is the [`obs.system.health`](#obs-system-health) → `obs.explain` drill-down ref: pass the system report's `autonomy.worstRootRunId` to render that run's `spawnTree`, then pass a failed child's run ID to drill into its own report. `graphId` is a terminal execution-graph run's id, canonicalized through the graph record's own `sessionKey` (else its `traceId`) to the caller session, and rendered with the `graph` section above. An unresolvable `rootRunId` (or `traceId`) yields the honest `session_not_found` marker naming the ref that missed — never a clean-looking empty report; an unresolvable `graphId` yields the sibling `graph_not_found` marker, which names the missing `graph-runs/<graphId>/_run-metadata.json` record and points at [`graph.runs`](#graph-runs) / [`graph.runDetail`](#graph-rundetail) to verify the id, or [`graph.status`](#graph-status) while the graph is still running and its terminal metadata is not yet written. There is **no by-job-id form**: a cron [wake-gate](/agent-tools/scheduling#wake-gate-run-the-model-only-when-it-matters) skip opens no session, so a cron **job id** is not a valid input — read a job's skipped fires from [`cron.runs`](#cron-runs) instead.
...
`coverage.losslessContext` is present **only** when the trajectory yielded no records for the session and the assembler fell back to the DAG (LCD) engine's lossless store to reconstruct tool outcomes -- the case where the on-disk `.trajectory.jsonl` was rotated, deleted, or never written but the conversation itself is still stored. The block (`{ found, messages, toolResults, toolResultsReturned, truncated }`) states exactly how much that fallback recovered: `messages`/`toolResults` count the whole stored session, `toolResultsReturned` is how many the bounded window actually returned, and `truncated` flags the difference. The recovered records are **content-free** -- tool name, call id, and success/failure only, never a tool input, tool output, or message body -- so a report built this way carries provenance without the values. `coverage.trajectory.records` deliberately still reports the **real** trajectory read count (`0`), so the fallback can never disguise a "reader read nothing" bug as a healthy read: `trajectory.records: 0` alongside a populated `losslessContext` is the signature of a recovered report. The block is absent whenever the trajectory was readable, and also absent when the context store is unwired (an honest degradation -- the report is then empty as before).
...
| Property | Value |
|----------|-------|
| **Scope** | `rpc` route plus admin trust. A gateway token needs both `rpc` and `admin` (or `*`); the handler rejects non-admin trust. |
| **Parameters** | `{ sessionKey?: string, traceId?: string, rootRunId?: string, graphId?: string, depth?: "summary" \| "full", includeSynthetic?: boolean }` -- at least one of `sessionKey` / `traceId` / `rootRunId` / `graphId` required. `traceId` accepts a trace ID or a sub-agent run ID and canonicalizes either to the indexed session. `rootRunId` is an autonomy run's id, canonicalized to its session before assembly (a `root-session-` synthetic root carries a generated UUID and a validated base64url session key; a real spawned root resolves by a session-index scan) and renders the run's `spawnTree`. `graphId` is a terminal execution-graph run's id, canonicalized to its caller session through the persisted run metadata and rendered with the `graph` section. `includeSynthetic` is optional; include synthetic/test-harness sessions, which are excluded by default. |
| **Returns** | An `IncidentReport`: `{ schemaVersion, sessionKey, traceId, agentId, channel, outcome: { endReason, degraded, severity }, cost: { costUsd, totalTokens, tokenBasis?, cacheReadRatio }, timing: { durationMs, turnCount }, toolStats, failures[], breakerTimeline[], offloads[], spawnTree?, outwardDeliveries?: Array<{ rootRunId, status, transition, stepIndex?, deliveryKind?, platformMessageId? }>, restartRecovery?: { suspended, resumed, lastStatus, rootRunId, checkpointId }, requestRelevantToolNames?: string[], requestRelevantPromptSkillNames?: string[], requestRelevanceHistory?: { turnCount, charCount, saturated, recallDisposition? }, requestClarification?: { reason, inputChars }, operatorPolicyToolProjections?: Array<{ toolName, sectionId, contentHash, projectedChars }>, contextBudget?, recall?, voice?, learning?, orchestrate?, graph?, cronWakeGate?: { jobId, wake, durationMs, toolCalls, estTurnsSaved }, summary, likelyRootCause, suggestedNextSteps[], truncations[], coverage? }` |
...
```json Response
{
  ...
  "result": {
    ...
    "cost": { "costUsd": 1.320669, "totalTokens": 735800, "tokenBasis": "input+output+cache", "cacheReadRatio": 0 },
...
A governed completion attachment whose preparation or durable send becomes
failed, blocked, or parked contributes the counted
`health_signal:outward_attachment_failed` finding. This is the cross-session
warning surface for attachment-delivery failures; use `obs.explain` on an
affected session or root to inspect its content-free validation and delivery
evidence.
...
When installed [capability services](/reference/config-yaml#capability-services) ran managed work over the window, the report carries an optional **`capabilityServices`** block -- the cross-session capability-service / managed-run health slice: `{ runs: { total, degraded, degradedRate }, services: { total, degraded }, topReasonCodes: [{ code, count }], worstManagedRunId? }` (**counts + closed status-reason enums + one opaque host-minted run id only** -- never a report body, workspace path, objective, or service credential; safe to paste). The run counts come from the durable managed-run index (`ManagedRunStorePort.countByStatus` -- managed runs *are* content-free durable rows by construction, so this needs no session-rollup change and no contentful global telemetry), where `degraded = failed + unknown` (a `cancelled` run is an intended outcome, not degradation). `services` counts the distinct service instances seen in the window and how many carried a degraded run; `topReasonCodes` are the top closed `statusReason` codes across the degraded runs (e.g. `service_state_unavailable` / `failure_verified` / `required_evidence_stale`); `worstManagedRunId` is the most-recently-updated degraded run -- paste it into [`comis managed-runs explain`](/reference/cli#comis-managed-runs) for its likely root cause and repair knob. A degraded population also surfaces as the dedicated `managed_run_degradation` `findings[]` code. The block is **absent** when the window held no managed-run activity (a daemon that never used a capability service carries no empty block) or the managed-run store is unwired (the daemon-less `--offline` CLI) -- an honest coverage degradation, not a silent zero.
...
`sessions.degraded` is the raw count and includes
`completed_with_tool_errors`, where a tool failed but a final answer was still
delivered. `sessions.deliveredWithToolErrors` names that subset;
`sessions.hardDegraded` and `hardDegradedRate` subtract it and drive the acute
and high-degradation findings.
...
```json Response
{
  ...
  "result": {
    "schemaVersion": 1,
    "windowHours": 24,
    "sessions": { "total": 40, "degraded": 22, "degradedRate": 0.55, "deliveredWithToolErrors": 18, "hardDegraded": 4, "hardDegradedRate": 0.1 },
    ...
    "cost": { "costUsd": 1.320669, "totalTokens": 735800, "tokenBasis": "input+output", "offSessionUsd": 0.042 },
    "activity": {
...
<Accordion title="capabilityServices (2 methods)">

Admin-only operator reads over configured capability-service instances. These
methods expose identities, closed state, declared scopes, counts, and approved
roots; they never return a socket path, credential reference, or resolved
secret. See [Capability Services](/reference/config-yaml#capability-services)
for the configuration contract and [`comis capability-services`](/reference/cli#comis-capability-services)
for the CLI surface.

| Method | Scope | Description |
|--------|-------|-------------|
| `capabilityServices.list` | `admin` | List every configured instance and its current host state |
| `capabilityServices.get` | `admin` | Read one instance, including approved roots and its liveness obligation |

### `capabilityServices.list`

| Property | Value |
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | None |
| **Returns** | `{ rows, total }`; each summary row carries `serviceInstanceId`, `serviceDefinitionId`, `enabled`, `state`, closed `reasonCodes`, `requestedScopes`, `allowedAgents`, `mcpServerName`, active/degraded run counts, and `capturedAtMs` |

### `capabilityServices.get`

| Property | Value |
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ serviceInstanceId: string }` |
| **Returns** | `{ instance? }`; the summary fields plus `allowedWorkspaceRoots`, `allowedRuntimeRoots`, and `livenessRequired`. Absence means no configured instance has that ID |

</Accordion>

<Accordion title="managedRuns (4 methods)">

Admin-only operator reads and host-authority cancellation for durable managed
runs. All responses are content-free: no report body, attention question,
objective, artifact, or service credential crosses this surface.

| Method | Scope | Description |
|--------|-------|-------------|
| `managedRuns.list` | `admin` | List durable runs, optionally filtered by service, agent, or status |
| `managedRuns.get` | `admin` | Read one run's durable authority record |
| `managedRuns.explain` | `admin` | Return a deterministic likely cause and reachable next actions |
| `managedRuns.cancel` | `admin` | Commit host-authority cancellation before notifying the service |
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ serviceInstanceId?: string, agentId?: string, status?: ManagedRunStatus, limit?: number }`; `limit` defaults to `100` and is capped at `500` |
| **Returns** | `{ rows, total, truncated }`; rows carry run/service/agent/tenant IDs, closed status and reason, report cursors, attention and terminal counts, lease presence, timestamps, and labelled freshness |
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ managedRunId: string }` |
| **Returns** | `{ run? }`; detail adds owner identity, conversation/root-run IDs, policy and capability hashes, captured capabilities/tools, and bound lease/attachment/terminal/group IDs. `custody` and `processSummary` explicitly report unavailable stages rather than implying empty state |
...
| **Scope** | `admin` |
| **Parameters** | `{ managedRunId: string }` |
| **Returns** | `{ run?, likelyRootCause: { code, hint }, nextSafeActions }`; the verdict is deterministic and the actions are drawn from `cancel`, `inspect_service`, `resolve_attention`, and `wait` |
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ managedRunId: string, operationId: string, reason?: "owner_cancelled" | "authority_revoked" | "budget_exhausted" }` |
| **Returns** | `{ outcome, status?, serviceAcknowledged?, serviceReasonCode? }`, where `outcome` is `cancelled`, `already_terminal`, or `not_found`. `serviceAcknowledged: false` does not undo the committed cancellation |

</Accordion>

<Accordion title="managedAttention (2 methods)">

Admin-only, content-free views of durable questions raised by managed runs. The
question and response bodies remain in owner-private storage.

| Method | Scope | Description |
|--------|-------|-------------|
| `managedAttention.list` | `admin` | List attention records, optionally for one run |
| `managedAttention.get` | `admin` | Read one attention record by ID |

### `managedAttention.list`

| Property | Value |
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ managedRunId?: string, limit?: number }`; `limit` defaults to `100` and is capped at `500` |
| **Returns** | `{ rows, total, truncated }`; rows carry `attentionId`, `managedRunId`, status, optional external key, and created/resolved timestamps |

### `managedAttention.get`

| Property | Value |
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ attentionId: string }` |
| **Returns** | `{ attention? }`; absence means no durable attention record has that ID |
...
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ tenant_id: string, agent_id: string, conversation_ref: string, offset?: number, limit?: number }` — `offset` is a non-negative integer (default `0`, counted from the OLDEST message); `limit` is an integer in `1..200` (default `20`). A `limit` above `200` is rejected as a validation error, not clamped — page through a longer conversation with `offset`, or read its tail with `offset = total - limit`. |
...
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ tenant_id?: string, agent_id?: string, query?: string, scope?: "all" \| "user" \| "assistant" \| "tool", summarize?: boolean, limit?: number }` -- `tenant_id` / `agent_id` default to the authenticated caller's scope and only need to be sent when the caller cannot be resolved from the connection |
...
| `cron.list` | `rpc`, `admin` | List authored and built-in jobs |
...
The request is `{ agentId? }`. The response is `{ resolvedAgentId, jobs }`;
`resolvedAgentId` is the exact selected agent or `"*"` for an all-agent result.
The method accepts either `rpc` or `admin` scope. Non-admin callers are limited
to their resolved agent; selecting another agent or `"*"` requires authenticated
admin trust. Every job contains `id`, `name`, `agentId`, `source`, `schedule`,
`lifecycle`, and `payload`, plus only the optional policy fields that apply:
`maxConsecutiveDependencyErrors`, `sessionPolicy`, `continuationMode`,
`deliveryTarget`, `wakeGate`, `cacheRetention`, and `toolPolicy`.
...
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ nodes: NodeDefinition[], label?: string, onFailure?: string, timeoutMs?: number, budget?: object }` -- dependencies come from each node's own `depends_on`, so there is no edge list to supply (an `edges` array is accepted for symmetry with [`graph.save`](#graph-save) but ignored here) |
| **Returns** | `{ valid: true, nodeCount: number, executionOrder: string[], label?: string, warnings: ValidationIssue[], errors: ValidationIssue[], userVariables: string[] }` -- validation only: nothing runs and **no `graphId` is minted** (ids come from [`graph.execute`](#graph-execute)). `executionOrder` is the topologically sorted node ids, each `ValidationIssue` is `{ nodeId?, type, message, fix }`, and `userVariables` lists the `${VAR}` placeholders found in node tasks |
...
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ nodes: NodeDefinition[], label?: string, onFailure?: string, timeoutMs?: number, budget?: object, variables?: Record<string, string>, node_progress?: boolean }` -- the graph is submitted **inline**, in the same `NodeDefinition` shape [`graph.define`](#graph-define) takes; there is no run-a-saved-graph-by-id form on this method (load it with [`graph.load`](#graph-load) and pass its nodes). `variables` fills `${VAR}` placeholders in node tasks at execute time; an unfilled placeholder comes back as an `unresolved_variable` warning |
| **Returns** | An **async dispatch receipt**: `{ graphId, async: true, nodeCount, label, hint, warnings? }`. Dispatch returns as soon as the coordinator accepts the run -- per-node state and outputs come from [`graph.status`](#graph-status) / [`graph.outputs`](#graph-outputs), and the terminal post-mortem from [`obs.explain`](#obs-explain)'s `graph` section. `hint` is the model-facing instruction for the launching agent, and it branches on the resolved route below: with governed completion delivery it tells the caller to stop duplicating the delegated work; without it, the caller is told to report the `graphId` and **not** promise an automatic notification |

The announcement route -- used for `approval-gate` user interaction and for the
[aggregate completion announcement](/developer-guide/pipelines#graph-coordinator) -- is resolved
server-side, never from an ordinary request parameter: it is taken from the **authenticated caller
turn's endpoint** when the submission carries one, and otherwise from the dispatcher-injected
`_callerChannelType` / `_callerChannelId` internal fields the in-process agent leg supplies. Those
fields are stripped from every external WS/REST caller, so a remote client cannot aim a graph at a
conversation of its choosing; a carried route that disagrees with the turn endpoint is **rejected**
rather than honored, for the same reason. A graph containing `approval-gate` nodes fails when
neither source resolves a route, because the gate would have nowhere to prompt.
...
```json Request
{
  ...
  "params": {
    ...
    "nodes": [
      { "node_id": "research", "task": "Research ${TOPIC}" },
      { "node_id": "summarize", "task": "Summarize the findings", "depends_on": ["research"] }
    ],
    "variables": { "TOPIC": "vector databases" }
  },
  "id": 1
}
```
```json Response
{
  "jsonrpc": "2.0",
  "result": {
    "graphId": "research-pipeline-1a2b3c",
    "async": true,
    "nodeCount": 2,
    "label": "Research pipeline",
    "hint": "Pipeline launched — your job is now DONE. …"
  },
  "id": 1
}
```
...
**Without `graphId`** -- Returns a summary line per recent graph (`graphId`, `label`, `status`, `startedAt`, and `completedAt` once terminal), plus live concurrency statistics. Per-node state and `stats` come only from the with-`graphId` form above. Use the optional `recentMinutes` parameter to filter to graphs active within the specified time window.
...
Cancel a running graph execution. No new nodes are dispatched after the cancellation is processed, and the graph's in-flight children are terminated -- `killed` reports exactly how many.

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ graphId: string }` |
| **Returns** | `{ graphId: string, cancelled: true, killed: number }` -- `killed` counts the in-flight node executions terminated by the cancellation |

### `graph.save`

Persist a graph definition under a human-readable label so you can reload it later with [`graph.load`](#graph-load). The nodes are submitted **inline**, in the same `NodeDefinition` shape [`graph.define`](#graph-define) takes, and are validated before they are stored -- there is no save-an-already-defined-graph-by-id form. The raw nodes and edges are persisted verbatim (a loaded graph is re-validated when it is executed).

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ label: string, nodes: NodeDefinition[], id?: string, agentId?: string, edges?: object[], settings?: object }` -- omit `id` to mint a new entry; passing the `id` of an existing entry overwrites it. `agentId` defaults to the daemon's default agent |
| **Returns** | `{ id: string, saved: true }` -- `id` is the handle to pass to `graph.load` / `graph.delete` |

### `graph.load`

Load a previously saved named graph by its id. The stored entry comes back as-is -- pass its `nodes` to [`graph.execute`](#graph-execute) to run it; loading does not mint a `graphId` or start anything. `inputFrom` / `inputMapping` fields are stripped from the persisted nodes and edges on load -- use the `{{nodeId.result}}` pattern instead (see [Pipelines](/developer-guide/pipelines)).

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ id: string }` |
| **Returns** | The stored entry: `{ id, tenantId, agentId, label, nodes, edges, settings, createdAt, updatedAt }` |
...
```json Request
{
  "jsonrpc": "2.0",
  "method": "graph.load",
  "params": { "id": "8f2c1e40-5a7b-4c3d-9e11-2b6a7d4f0c93" },
  "id": 1
}
```
```json Response
{
  "jsonrpc": "2.0",
  "result": {
    "id": "8f2c1e40-5a7b-4c3d-9e11-2b6a7d4f0c93",
    "tenantId": "default",
    "agentId": "default",
    "label": "research-pipeline",
    "nodes": [
...
List the tenant's saved named graphs, most recent first. Each row is a summary -- the full node/edge payload comes from [`graph.load`](#graph-load).

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ limit?: number, offset?: number }` |
| **Returns** | `{ entries: Array<{ id: string, label: string, nodeCount: number, createdAt: number, updatedAt: number }>, total: number }` -- `total` is the unpaginated count |

### `graph.delete`

Soft-delete a saved named graph definition: the entry stops appearing in `graph.list` and `graph.load` returns "Named graph not found". This only removes the saved definition — it does not affect any in-progress executions or stored run artifacts.
...
Irrecoverably delete one run's directory (`graph-runs/<graphId>/`) and every artifact in it -- the node outputs and the terminal `_run-metadata.json` that [`graph.runDetail`](#graph-rundetail) and `comis explain --graph` read. Saved definitions are untouched.

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ graphId: string }` |
| **Returns** | `{ graphId: string, deleted: true }` |

### `graph.runDetail`

Read one run's per-node outputs and artifacts back from disk, so a run stays inspectable after it has left the coordinator's in-memory retention window.

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | `{ graphId: string }` |
| **Returns** | `{ graphId: string, name: string, status: "running" \| "interrupted" \| "completed" \| "failed" \| "cancelled", date: string, nodes: Array<{ nodeId: string, output: string \| null, artifacts: Array<{ filename: string, content: string }> }> }` -- `output` and each artifact `content` are truncated at 12,000 characters; `date` is the run directory's mtime as an ISO string |

### `graph.runs`

Enumerate the run directories under `graph-runs/`, most recent first. `status` is resolved from the live coordinator when the graph is still tracked, and otherwise from the run's terminal metadata / durable checkpoint on disk -- so a run interrupted by a restart reports `interrupted` rather than a false `completed`.

| Property | Value |
|----------|-------|
| **Scope** | `rpc` |
| **Parameters** | None |
| **Returns** | `{ runs: Array<{ graphId: string, name: string, status: "running" \| "interrupted" \| "completed" \| "failed" \| "cancelled", nodeCount: number, date: string, fileCount: number }> }` |
...
| `subagent.kill` | `rpc`, `admin` | Kill an owned or admin-selected run, or the caller's trusted current spawn tree |
...
|----------|-------|
| **Scope** | `rpc`, `admin` |
| **Parameters** | `{ target: string }` -- `target` is the sub-agent `runId`; an agent caller may use the reserved target `tree` for its trusted current spawn tree |
| **Returns** | `{ killed: boolean, runId: string, count: number }` |

Agent callers may kill an owned direct child. The reserved target `tree` hard-stops every running or queued descendant in the caller's trusted current spawn tree, revokes its leases, and reports the exact `count`; it never accepts a caller-supplied root identifier. Admin callers may target any individual run and use `run.kill` with an explicit root. An individual missing, unknown, or non-running target fails the RPC. A tree with nothing left to stop returns `killed` as false and `count` as 0.
...
{
  "jsonrpc": "2.0",
  "result": { "killed": true, "runId": "run_abc", "count": 1 },
  "id": 1
}
```
</CodeGroup>
...
| **Returns** | `{ status: "steered", oldRunId, newRunId } \| { status: "steered_inject", runId } \| { status: "already_terminal", runId, terminalStatus: "completed" }` |
...
The `status` discriminator is `steered` with `oldRunId` and `newRunId`, `steered_inject` with `runId`, or `already_terminal` with `runId` and `terminalStatus: "completed"`.
...
- **Completed race** -- if the child reaches `completed` before either mode can act, the handler preserves that terminal result and returns `{ status: "already_terminal", runId, terminalStatus: "completed" }`. This is a successful no-op, not a failed steer or a respawn.
