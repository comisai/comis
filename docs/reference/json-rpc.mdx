...
tracked endpoint authority. `destination_endpoint` supplies the complete
routing identity directly and is cross-checked while the handler binds delivery
authority. Resolution fails closed when the endpoint or its active adapter
