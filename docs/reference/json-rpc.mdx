...
Prompt skill management: list, create, upload, multi-source import, update, and delete skills.

| Method | Scope | Description |
|--------|-------|-------------|
| `skills.list` | `rpc` | List prompt skills for an agent |
| `skills.create` | `rpc` | Create a new skill from SKILL.md content (single-file shortcut) |
| `skills.upload` | `rpc` | Upload a skill folder (files + SKILL.md) |
| `skills.import` | `rpc` | Import from GitHub, an archive, a well-known index, or a configured registry |
| `skills.update` | `rpc` | Update an existing skill's SKILL.md content |
| `skills.delete` | `rpc` | Delete a skill |
...
When a community or agent-authored skill declares bundled MCP servers, Comis completes Phase-A validation but does not persist or connect those servers by default. The successful install response includes content-free `pendingMcpServers` entries and an actionable `hint`; the same pending list is retained in `installed-skills.json`. Set the installing agent's `skills.import.autoConnectBundledMcp` to `true` and re-run the install only after reviewing the declarations. Operator-authored and shipped skills keep the normal persist-and-connect behavior.

### `skills.upload`

| Property | Value |
|----------|-------|
| **Scope** | `rpc` plus the in-handler `orch:skill` capability gate |
| **Parameters** | `{ name: string, agentId?: string, scope?: "local" \| "shared", force?: boolean, files: Array<{ path: string, content: string }> }` |
| **Returns** | `{ ok: true, path: string, pendingMcpServers?: Array<{ name, transport, reason }>, hint?: string }` |
...
|----------|-------|
| **Scope** | `rpc` plus the in-handler `orch:skill` capability gate |
| **Parameters** | GitHub `{ source?: "github", url, ... }`; archive `{ source: "archive", archiveBase64 \| archiveUrl, ... }`; well-known `{ source: "wellknown", ref, ... }`; registry `{ source: "registry", registry, ref, ... }`; each accepts `agentId?`, `scope?`, and `force?` |
| **Returns** | `{ ok: true, path, name, fileCount, unchanged?, trust, verdict, contentHash, warnings, pendingMcpServers?, hint? }` |

GitHub URLs use
`https://github.com/{owner}/{repo}/tree/{branch}/{path}`. Archive payloads are
canonical base64 ZIP bytes or a public URL. Well-known references use
`wellknown:<allowlisted-base>#<skill-name>`. Registry imports identify an
allowlisted registry by `registry` id and a `ref` of `slug` or `slug@version`.
Every remote read is SSRF-validated and bounded; every resulting file map goes
through the same pre-write gate. See [Importing Skills](/skills/importing).
...
| **Returns** | `{ ok: true, path: string, name: string, pendingMcpServers?: Array<{ name, transport, reason }>, hint?: string }` |
...
| **Scope** | `rpc` plus the in-handler `orch:skill` capability gate |
| **Parameters** | `{ name: string, content: string, agentId?: string, scope?: "local" \| "shared", force?: boolean }` |
| **Returns** | `{ ok: true, name: string, pendingMcpServers?: Array<{ name, transport, reason }>, hint?: string }` |
