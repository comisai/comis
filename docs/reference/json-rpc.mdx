...
Prompt skill management: list, create, upload, import (from a GitHub directory or an archive), update, and delete skills.
...
| `skills.import` | `admin` | Import a skill from a GitHub directory URL or a packaged archive |
...
The `provenanceSummary.source` is the acquisition **channel** (`github` / `archive` / `upload`) -- distinct from the trust-tier `source` (`imported`). It is populated only for imported skills; bundled, workspace, local, and learned skills omit it. The summary is content-free: an acquisition channel, a hash prefix, and a timestamp -- never a skill body.
...
|----------|-------|
| **Scope** | `admin` |
| **Parameters** | `{ url?: string, source?: "github" \| "archive", archiveUrl?: string, archiveBytes?: string, name?: string, scope?: "local" \| "shared", agentId?: string, confirm?: boolean }` |
| **Returns** | `{ ok: true, path: string, name: string, fileCount: number, source: "imported", resolvedAgentId: string }` |
