...
`outcome.endReason` names the terminal cause. Two degradation causes that previously collapsed into a generic `error` are now named here (and drive `likelyRootCause`): **`context_exhausted`** (the context-window guard aborted the run -- the summarizer hit its token cap, or compaction floored to truncation-only; the fix is to raise `contextEngine.summarizerSpend`) and **`output_starved`** (the final response was truncated at the model's max output tokens; the fix is to raise the agent's `maxTokens` or enable `contextEngine.outputEscalation`). A tool-failure root cause still out-ranks them -- they explain the terminal state, not a tool crash. The same causes are aggregated fleet-wide by [`obs.fleet.health`](#obs-fleet-health)'s `degradedByCause`.
