...
[aggregate completion announcement](/developer-guide/pipelines#graph-coordinator) -- is taken from
the **authenticated caller turn's endpoint** when the submission carries one, and otherwise from an
explicitly supplied `announceChannelType` + `announceChannelId`. An explicit route that disagrees
with that endpoint is **rejected** rather than honored, so a graph cannot be aimed at a
conversation the submitting turn holds no authority over. A graph containing `approval-gate` nodes
fails when neither source resolves a route, because the gate would have nowhere to prompt.
