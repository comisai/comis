---
title: "Sandbox"
description: "Skill sandboxing implementation reference covering content scanning, sanitization, tool policy, and execution limits"
icon: "box"
---

The sandbox is a multi-layer defense system for custom skill execution. It combines content scanning, input sanitization, tool policy enforcement, and execution limits to prevent malicious or runaway skill behavior.
...
<Steps>
  <Step title="Content Scanning">
    Skill body inspected for 18 dangerous patterns across 6 categories at load time.
  </Step>
...
<Info>
  **Source:** `packages/skills/src/prompt/content-scanner.ts` -- 18 patterns across 6 categories. Patterns imported from `@comis/core` injection-patterns module.
</Info>
...
|---------|-------|----------|
| `minimal` | `exec`, `read`, `write` | Basic file operations only |
...
| `supervisor` | `agents_manage`, `obs_query`, `sessions_manage`, `memory_manage`, `channels_manage`, `tokens_manage`, `models_manage` | Administrative agents |
