...
path applies its wired security controls before it executes. Callers that use
the action classifier receive a fail-closed classification for unknown action
identifiers. The architecture never assumes any single check will catch
everything.
...
The execution layer provides action classification and approval-gate
primitives, prevents SSRF, and validates file paths. Classification and
approval take effect only on execution paths that explicitly call them.

### Action Classifier

`classifyAction()` assigns a risk level to each action identifier presented by
a caller. Unknown identifiers default to `"destructive"`.

| Classification | Description | Example Actions |
|----------------|-------------|-----------------|
| `read` | No side effects | `file.read`, `memory.search`, `config.read` |
| `mutate` | Modifiable side effects | `file.write`, `memory.store`, `message.send` |
| `destructive` | Irreversible or high-risk | `file.delete`, `memory.clear`, `system.shutdown` |
...
classification changes by source integrations.

The classifier is a lookup utility, not a universal tool interceptor.
`requiresConfirmation()` returns a boolean but does not create a pending
approval request.
...
When `approvals.enabled` is true, the daemon wires `ApprovalGate` into selected
execution paths. A guarded caller explicitly invokes `requestApproval()`, then
awaits an operator decision before continuing. The gate also enforces timeout,
short-lived approval and denial caches, and pending-request batching.

The gate currently protects selected management actions, memory delete and
flush, channel configuration, pipeline execute and save, orchestrate
pre-flight, install-detour overrides, and configured high-risk terminal session
creation. It is not automatically placed in front of every tool or RPC method.

<Warning>
The schemas accept `security.actionConfirmation`, `approvals.defaultMode`, and
`approvals.rules`, but there is currently no general execution rule evaluator
that applies them across actions. Only an explicit call to
`ApprovalGate.requestApproval()` creates a human approval pause.
</Warning>

<Info>
Source: `createApprovalGate()` in `packages/core/src/approval/approval-gate.ts`
</Info>
...
Admin-only contracts — those exposing an `admin` route without an `rpc` route —
form the deny-by-origin control plane. They reject a non-admin agent-origin call
before capabilities are consulted; an explicitly granted admin-trust turn is
the exception. Multi-scope contracts are excluded from the blanket deny set and
their handlers own caller-vs-operator authorization. See
[Capability Model](/security/capability-model#deny-by-origin-on-the-control-plane)
for the authoritative trust and signal-soundness rules.
...
The output layer scans completed agent responses for leaked-secret patterns,
canary tokens, and prompt-extraction attempts. A streaming transport can emit
deltas before the completed response reaches this guard; those deltas cannot be
retracted. Use non-streaming delivery where this boundary is unacceptable.
...
| `security.actionConfirmation` | `ActionConfirmationConfig` | _(see below)_ | Policy metadata for paths that explicitly consult it |
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `actionConfirmation.requireForDestructive` | `boolean` | `true` | Confirmation signal available to paths that consult it |
| `actionConfirmation.requireForSensitive` | `boolean` | `false` | Sensitive-action signal available to paths that consult it |
| `actionConfirmation.autoApprove` | `string[]` | `[]` | Exemption identifiers available to paths that consult it |
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `approvals.enabled` | `boolean` | `false` | Wire the gate into supported execution paths |
| `approvals.defaultMode` | `"auto"` \| `"require"` \| `"deny"` | `"auto"` | Schema value; no general runtime evaluator currently consumes it |
| `approvals.rules` | `ApprovalRule[]` | `[]` | Schema values; no general runtime evaluator currently consumes them |
| `approvals.defaultTimeoutMs` | `number` | `300000` (5 min) | Pending approval request timeout |
| `approvals.denialCacheTtlMs` | `number` | `60000` | Explicit-denial cache TTL |
| `approvals.batchApprovalTtlMs` | `number` | `30000` | Explicit-approval cache TTL; `0` disables it |

<Warning>
If `approvals.enabled` is `false` but `approvals.rules` has entries, Comis logs
a configuration warning. Enabling approvals wires supported call sites, but it
does not make the rule schema a universal interceptor.
</Warning>
