...
<Warning>
`approvals.defaultMode` and `approvals.rules` are evaluated inside the gate, so
they decide what a guarded action does — they do not place a gate in front of an
unguarded one. Only an explicit call to `ApprovalGate.requestApproval()` reaches
the policy at all, and `security.actionConfirmation` is a separate setting with
no rule evaluator.
</Warning>
...
| `approvals.defaultMode` | `"auto"` \| `"require"` \| `"deny"` | `"require"` | Decision for actions no rule matches |
| `approvals.rules` | `ApprovalRule[]` | `[]` | Ordered rules; first matching `actionPattern` wins |
