...
<Info>
Source: `createInputGuard()` and `PATTERN_WEIGHTS` in `packages/core/src/security/input-guard.ts`
</Info>
...
**Suspicious pattern detection:** Content is scanned against 16 injection
...
**Total: 18 scan rules** across 6 categories.
...
The registry contains 100+ registered actions organized by category. After
...
**13 critical patterns** (blocked and redacted):
...
**2 warning patterns** (detected but not redacted):
...
**2 patterns** (warning severity, detect-only):
...
**16 credential patterns** processed in order (more specific patterns first):
