...
<Info>
  **Source:** `packages/daemon/src/api/config-handlers/` -- token bucket shared between `config.patch` and `config.apply`.
</Info>
...
- **Config churn:** Excessive config writes causing constant daemon restarts (each patch triggers SIGUSR2)
