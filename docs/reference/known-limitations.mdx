...
Two public claims were inaccurate and are corrected as part of the v1.7 milestone that introduced
[`THREAT_MODEL.md`](https://github.com/comisai/comis/blob/main/THREAT_MODEL.md):

1. **`SECURITY.md`** previously described skills as running in `isolated-vm`. The real mechanism is
   OS-level `bwrap` (Linux) or `sandbox-exec` (macOS) — the Node.js `vm` module is not used for
   skill or tool confinement.

2. **`README`** implied "no external SDK dependency" for agent execution. The agent runtime is built
   on `@earendil-works/pi-coding-agent` (exact-pinned, bundled, part of the supply-chain threat model —
   see [`THREAT_MODEL.md §5.8`](https://github.com/comisai/comis/blob/main/THREAT_MODEL.md)).

Both claims are corrected in the current documentation. A CI guard (`test/architecture/security-doc-claims.test.ts`)
