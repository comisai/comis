...
description: "Deploy Comis as a systemd service on a supported Ubuntu VPS"
icon: "server"
---
...
| A model path: **provider API credential**, supported OAuth account, or local provider | Provider account or a model service you operate |
...
The installer reports each system, package, user, and service step. Review any
warning before continuing; duration depends on the host and package mirrors.
...
<Warning>
CloakBrowser does not include a proxy and cannot change a VPS's IP reputation.
It is an optional compatibility runtime, not a guarantee of access. Review its
separate binary license and every target site's terms before enabling it.
</Warning>
...
The interactive wizard configures storage, model access, the agent, optional
channels, gateway, workspace, tools, media, and recall. It shows a review before
writing files or starting the daemon.

```bash
comis init
```

New installations default to encrypted credential storage. Credentials are
written to `~/.comis/secrets.db`; the master key is stored as
`SECRETS_MASTER_KEY` in the owner-readable `~/.comis/.env`. Back up that key in
a secure credential manager. Losing it makes the encrypted store unrecoverable.

<Tip>
When the wizard asks to restart the daemon, choose **Restart (recommended)**.
This ensures the daemon reads the new configuration and credentials.
</Tip>
...
The wizard detects headless hosts using standard environment signals:

- `SSH_CLIENT` or `SSH_TTY` set, **or**
- `DISPLAY` unset

When detected, the picker pre-selects `Device code (phone)`. The wizard prints
a short code and `https://auth.openai.com/codex/device`. Open it on another
device, sign in, enter the code, and return to the terminal when authorization
completes.
...
For the supported Codex path, see [Quickstart: OpenAI Codex OAuth](/get-started/quickstart#openai-codex-oauth).
For the OAuth threat model and storage backends, see [OAuth concepts](/security/oauth).
