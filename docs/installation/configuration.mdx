...
pipeline. Most users do not need to change any context engine settings -- the
defaults work well.
...
<Note>
  The DAG mode (the `dag` engine version) — the v2.12 "Lossless Context DAG"
  lossless engine — is available as an **opt-in** mode: set `version: "dag"` to
  use it (the default stays `"pipeline"` until a later release). In the current
  release DAG does lossless verbatim assembly (full faithful history + a verbatim
  fresh tail + transcript repair); compaction, summarization, and the on-demand
  recall tools land in later phases. See [Compaction](/agents/compaction) for details.
</Note>
