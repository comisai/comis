...
<Note>
  The DAG mode (`version: "dag"`) — a lossless conversation graph with recall
  tools — is being reimplemented for the v2.12 "Lossless Context DAG" release and
  is **not currently available**. Setting `version: "dag"` is accepted but falls
  back to the pipeline engine with a logged warning until DAG is re-enabled. See
