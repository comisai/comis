...
| Custom paths | Set [`COMIS_CONFIG_PATHS`](/reference/environment-variables#comis_config_paths) |
| Reload | Config is loaded once at startup. Restart the daemon to pick up changes. |

<Info>
Set `COMIS_CONFIG_PATHS` to choose one or more files instead of the defaults.
The [environment-variable reference](/reference/environment-variables#comis_config_paths)
owns its syntax; the [Config YAML Reference](/reference/config-yaml) owns merge
precedence and field contracts.
</Info>
...
This guide covers the essential configuration sections. The remaining sections
control memory, security, routing, scheduling, monitoring, plugins, and more.
...
<CardGroup cols={2}>
  ...
  <Card title="Config Reference" icon="book" href="/reference/config-yaml">
    Complete reference for every configuration section.
  </Card>
</CardGroup>
