...
mounts. This breaks the bubblewrap exec sandbox the agent depends on for safe
shell execution -- the container starts and the dashboard works, but every
agent-issued shell command fails at runtime, so the agent loses a major part of
its capability and the credential-isolation guarantee.

- **Development / local testing on a Mac or Windows machine:** fine for exploring
  the dashboard, channels, memory, and LLM features that don't need `exec`.
- **Production:** deploy to a real Linux host (bare metal, VPS, or any Linux cloud
  VM). The bare-metal `install.sh` flow on Linux gets you the same setup with
  the sandbox actually working.
