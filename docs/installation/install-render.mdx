...
This guide deploys Comis to [Render.com](https://render.com) using the published
Docker image. Render provides a managed Linux container, HTTPS endpoint, and
optional persistent storage. Do not assume the exec sandbox is active merely
because the host is Linux: confirm the startup diagnostics and sandbox provider
...
| **Hosted model credential** | Obtain one from a supported provider; interactive OAuth/local providers are better suited to a host you can access directly |
...
| **A Render plan with persistent disk and always-on service** | Check Render's current plan limits before creating the service |

You **don't** need Docker, Node.js, or anything else installed on your machine.
Render pulls the image directly from Docker Hub.

<Warning>
**Plan choice.** Select a current Render plan that provides persistent storage
and the availability your channels require. Without a disk, redeploys can lose
configuration, credentials, memory, and sessions. A sleeping service cannot
respond to channel events while suspended. Render's pricing and limits can
change; verify them in the Render dashboard.
</Warning>
...
<Steps>
  ...
  <Step title="Point at the Comis Docker Hub image">
    ...
    a tag such as `docker.io/comisai/comis:<version>-slim`.
    </Tip>
  ...
  <Step title="Configure the service">
    ...
    | **Instance Type** | A plan with persistent disk and suitable availability |
  ...
  <Step title="Add environment variables">
    ...
    **Optional channel tokens** — add only those you'll use:
    ...
    Back up `SECRETS_MASTER_KEY` separately in a secure credential manager —
    losing it makes `secrets.db` permanently unreadable. To disable the encrypted store, set
  ...
  <Step title="Deploy">
    ...
    image, attaching the disk, and starting the container. Deployment duration
    depends on image-pull and platform capacity.
...
services:
  - type: web
    ...
    runtime: image
    image:
      url: docker.io/comisai/comis:<version>-slim   # replace with an immutable tag
...
If you pinned a version tag, update it deliberately after reviewing the release:
...
<Steps>
  ...
  <Step title="Update the image URL">
    ...
    the existing immutable tag to the new version → **Save Changes**.
  ...
  <Step title="Render redeploys automatically">
    The service may be unavailable while Render pulls the new image, runs the
    health check, and switches traffic over. Your
