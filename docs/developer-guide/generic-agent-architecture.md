...
Translation target is separate from surrounding response locale. An explicit operator locale or a high-confidence script signal from clear current-request prose enables post-generation script enforcement; the channel-supplied device locale remains advisory. Clear prose in the current message outranks the client's UI locale for that turn and may produce an open `und-<Script>` BCP-47 expectation without guessing a human language. Short or identifier-heavy fragments do not qualify for inferred enforcement and fall back to the advisory device locale. A mismatch against an enforced response locale creates a content-free quality finding. It starts at most one tools-disabled repair turn only when every recorded tool failure has a proven later recovery; message delivery recovery additionally requires the same action and content-free exact route/target identity. Otherwise the original response is preserved so a rewrite cannot reverse the observed tool outcome. Repaired output is checked again before delivery. Correct mixed-script content is not rewritten. Streaming consumers receive only the finalized response while enforcement is active, so an invalid draft cannot escape before validation.
