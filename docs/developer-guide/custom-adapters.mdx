...
export interface ChannelPort {
  ...
  /** OPTIONAL — capture inbound reactions. Omit it if your platform exposes none. */
...
### Capturing inbound reactions (optional)

`onReaction?` is an **optional** capability — like `getStatus?`, an adapter may omit it entirely and the system treats it as an honest no-op (the orchestrator calls it as `adapter.onReaction?.(handler)`, which does nothing when absent). Implement it **only** if your platform delivers an inbound reaction-add event that carries the **reactor's identity**. The reference adapters do: Discord (`MessageReactionAdd`), Slack (`reaction_added`), and Telegram (`message_reaction`) all implement it; iMessage, LINE, IRC, Email, and Echo omit it because their inbound APIs expose no reactor id.

When you implement it, convert each inbound reaction-add into a `NormalizedReaction` and pass it to the handler, mirroring how `onMessage` mints a `NormalizedMessage`:

```typescript
import type { NormalizedReaction, ReactionHandler } from "@comis/core";

export interface NormalizedReaction {
  messageId: string;    // the message that was reacted to
  reactorId: string;    // who added the reaction (same identity space as NormalizedMessage.senderId)
  emoji: string;        // the reaction emoji (a Unicode emoji, or a platform short name)
  channelType: string;  // e.g. "discord"
  channelId: string;    // the channel the message lives in
}
```

`NormalizedReaction` is deliberately **trust-free** — it carries no authority field. It is untrusted inbound data; the daemon resolves the reactor's trust downstream (from config) and a reaction can only *weight* a learning outcome, never assert one. Filter out the bot's **own** reactions before dispatching (so the agent does not learn from itself), and keep the handler dispatch non-fatal — a throwing handler must not crash your platform's event loop.
...
Wrap your adapter in a `ChannelPluginPort` to provide channel metadata,
capabilities, and lifecycle methods to the channel bootstrap path. Production
channel lifecycle is wired directly by the daemon; it does not register these
wrappers through the generic `PluginRegistry`.
...
export function createMyPlatformPlugin(config: MyPlatformConfig): ChannelPluginPort {
  ...
  return {
    ...
    register(_registry: PluginRegistryApi): Result<void, Error> {
      // Channel bootstrap owns adapter lifecycle. Register no generic hooks here.
      return ok(undefined);
    },
...
Import the adapter factory in the daemon's channel setup and add the adapter to
the channel map used by `setup-channels-adapters.ts`. The production channel
bootstrap owns activation and shutdown directly. It does not discover channel
packages or pass channel plugins through the generic `PluginRegistry`.

Keep the declared capabilities conformant to `ChannelCapabilitySchema` and pin
them in the adapter's co-located tests.
