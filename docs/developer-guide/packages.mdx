...
Comis is structured as a monorepo with 17 TypeScript packages under `packages/`.
...
| capability-service-sdk | `@comis/capability-service-sdk` | Private schemas and deterministic artifacts for the capability-service protocol | Strict Zod contracts, JSON Schemas, conformance fixtures, bundle manifest |
...
<Note>
The published umbrella package is named `comisai` on npm (the directory is `packages/comis/`). It bundles the runtime `@comis/*` workspace packages, including the private capability-service SDK needed by the daemon and skills packages, and exposes a `comis` CLI binary. Subpath imports such as `comisai/core` or `comisai/agent` are exposed via the umbrella's `exports` map. The SDK remains private and has no standalone npm publication or `comisai` subpath export.
</Note>

<Note>
A capability service written in another language consumes the protocol through **release assets**, not through npm. Every `vX.Y.Z` release carries `comis-capability-service-protocol-vX.Y.Z.tar.gz` — the generated JSON Schemas, conformance fixtures, and bundle manifest — alongside a `comis-capability-service-protocol-vX.Y.Z.manifest.json` sidecar that states the protocol identity and `bundleDigest` without unpacking the archive. Pin the release asset rather than a repository commit: the asset's bytes are fixed for the tag that produced them, and the digest is the value the daemon asserts at handshake.
</Note>
...
- `observability-otel` is the opt-in telemetry extension; `web` is the
  ...
  workspace packages.
- `capability-service-sdk` owns strict protocol schemas and deterministic
  cross-language artifacts. The daemon and skills packages consume its runtime
  contracts, and `comisai` bundles it as an internal dependency.
