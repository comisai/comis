...
| capability-service-sdk | `@comis/capability-service-sdk` | Private schemas and deterministic release artifacts for the capability-service protocol | Strict Zod contracts, JSON Schemas, conformance fixtures, bundle manifest |
...
<Note>
The published umbrella package is named `comisai` on npm (the directory is `packages/comis/`). It bundles the runtime `@comis/*` workspace packages and exposes a `comis` CLI binary. Subpath imports such as `comisai/core` or `comisai/agent` are exposed via the umbrella's `exports` map. The private capability-service SDK is excluded from the npm umbrella and distributed as pinned release artifacts.
</Note>
...
- `capability-service-sdk` owns strict protocol schemas and deterministic
  cross-language artifacts without entering the runtime package graph.
