---
title: "Plugins"
description: "13 lifecycle hooks and 3 extension registrations for customizing Comis behavior"
icon: "puzzle-piece"
---

Plugins extend Comis by hooking into lifecycle events, registering custom tools, adding HTTP routes, and defining config schemas. Every plugin implements the `PluginPort` interface and uses the `PluginRegistryApi` to register its capabilities. The hook system runs at 13 lifecycle points -- 5 modifying hooks (can change behavior) and 8 void hooks (observe only).
...
Subscribe to one of the 13 lifecycle hooks. The `hookName` must be a valid hook name from the `HookName` type. Options include `priority` (number, default `0`, higher runs first, range `-100` to `100`).
...
The hook system is the primary extension mechanism for customizing Comis behavior. All 13 hooks are organized into five domains: Agent, Tool, Compaction, Delivery, Session, and Gateway.
