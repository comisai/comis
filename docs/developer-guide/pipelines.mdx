...
- **Aggregate announcement** -- A single completion message summarizing all node results is sent to the originating channel when the graph finishes. It requires an authenticated originating route captured at submission, and **retained** delivery (a completion that survives a daemon restart) additionally requires `agents.<id>.autonomy.durability.enabled`. When either is missing, `graph.execute` tells the launching agent to report the `graphId` and *not* promise an automatic notification, so the caller never claims a follow-up the runtime cannot deliver.
