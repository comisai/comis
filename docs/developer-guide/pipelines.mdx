...
- **Aggregate announcement** -- A single completion message summarizing all node results is sent to the originating channel when the graph finishes. It requires an authenticated originating route captured at submission, and **retained** delivery (a completion that survives a daemon restart) additionally requires `autonomy.durability.enabled` on the **autonomy-bearing** agent. That store is daemon-wide -- it is built from the first agent whose `autonomy` is enabled, not from whichever agent launched the pipeline -- so setting the key on a non-autonomy agent does not turn retained delivery on. When either is missing, `graph.execute` tells the launching agent to report the `graphId` and *not* promise an automatic notification, so the caller never claims a follow-up the runtime cannot deliver.
...
<Tip>
`inputFrom` and `inputMapping` are not part of the graph schema -- the `graph.load` RPC strips those fields from persisted graph definitions on load. Use the `{{nodeId.result}}` pattern in task text to flow outputs downstream.
</Tip>
