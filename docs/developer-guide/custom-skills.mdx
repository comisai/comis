...
Custom skills extend what Comis agents can do. Beyond the built-in tools, you can create prompt-based skills with SKILL.md manifests or connect external MCP servers. This page covers the developer internals -- the `SkillPort` interface, skill loading, content scanning, and the MCP integration layer.
...
| `comis` | object | Comis-only namespace block -- see the [`comis:` field table](/skills/manifest#comis-namespace) for its fields |

<Info>
Comis-platform-only fields live exclusively under the `comis:` namespace block in frontmatter, enumerated in [Skill Manifest Reference](/skills/manifest#comis-namespace). Other pi-coding-agent hosts ignore this block. The full schema is `SkillManifestSchema` in `packages/skills/src/skills/manifest/schema.ts`.
</Info>
...
## Adding Code-Backed Tools

The plugin registry cannot register agent tools. Its public API exposes
`registerHook` only, and Comis does not currently provide a stable third-party
code-plugin loader.

Adding a built-in, code-backed tool requires implementing it in the skills
package and wiring it into the runtime tool assembly. This is an application
source change, not a plugin registration. For integrations that should remain
outside the Comis process, use an MCP server. For instruction-only extensions,
use a prompt skill.
...
<CardGroup cols={2}>
  ...
  <Card title="Built-in Tools" icon="toolbox" href="/skills/built-in-tools">
    Code-backed tools included with Comis
  </Card>
