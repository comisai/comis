...
Hook into lifecycle points to modify behavior, add audit logging, block actions, or inject context. The plugin system provides 5 modifying hooks (where your handler can alter data flowing through the pipeline) and 8 void hooks (where you observe events without modifying them).
...
The codebase spans 13 packages, ~1185 source files, and ~944 test files. There are 19 port interfaces, typed events across 4 domain subsystems, and lifecycle hooks for plugin integration.
