...
Connect Comis to any chat platform by implementing the `ChannelPort` interface.
Comis ships with adapters for the platforms listed in the
[package inventory](/developer-guide/packages), and you can add more without
modifying core code.
...
typed and composable across package boundaries.
...
The codebase spans focused packages, with port contracts, typed events, and
lifecycle hooks providing the main extension boundaries. See the
[package inventory](/developer-guide/packages) for the current list.
...
The primary validation command before any commit is `pnpm validate`; the root
`package.json` owns its exact gate chain.
...
<CardGroup cols={2}>
  ...
  <Card title="Packages" icon="cubes" href="/developer-guide/packages">
    Package roles and dependency graph
  </Card>
  <Card title="Event Bus" icon="tower-broadcast" href="/developer-guide/event-bus">
    Typed events across runtime subsystems
  </Card>
