...
The codebase spans 13 packages, ~1185 source files, and ~944 test files. There are 19 port interfaces, 163 typed events across 4 domain subsystems, and 13 lifecycle hooks for plugin integration.
...
- **`TypedEventBus`** -- The inter-module communication layer. Packages subscribe to typed events instead of importing each other directly. The `EventMap` interface provides compile-time safety for all 163 events.
...
<CardGroup cols={2}>
  ...
  <Card title="Event Bus" icon="tower-broadcast" href="/developer-guide/event-bus">
    Typed events across 4 domain subsystems (Messaging, Agent, Channel, Infra)
  </Card>
