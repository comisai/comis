...
pnpm build
```
Runs TypeScript compilation across all workspace packages, respecting project references.
</Step>
...
<Tip>
Run `pnpm validate` before every push. The root `package.json` owns the exact
gate chain, including the capability-protocol drift check.
</Tip>
...
- Analyze an existing JSON report without re-running the suite: `pnpm test:orchestrate --report-only`. The suite and its report come from whoever ran it (in CI, the sharded `integration` job); this reads `test/.test-results.json` and exits non-zero on a failure or a missing report. Without it, orchestrate re-runs the whole integration tier — the same command `pnpm test:integration` runs.
- Check only the E2E flow matrix, spawning no tests at all: `pnpm test:orchestrate --check-matrix`
...
Run `pnpm validate` before every push. The executable gate order lives in the
root `package.json`:
...
| Capability protocol | `pnpm capability-protocol:check` | Regenerates the language-neutral protocol bundle in check mode and rejects drift |
