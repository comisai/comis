...
- **Stage-C** (`describe.skipIf(!isLive)`, operator-gated) — the agent-content legs against a
  **real keyless model** served by ollama. A full-daemon reply, the
...
`model: "keyless"` is not pinned to one tag: the rig boots the **first**
`COMIS_LIVE_LOCAL_MODELS` entry at `COMIS_LIVE_OLLAMA_URL` — the same two knobs the local-model
tier reads, documented in `test/live/live.env.example` — and falls back to that file's stated
defaults when neither is set. Point it at a tag your box can actually hold: a model too large to
load boots the daemon fine but never authors a reply, so every Stage-C round-trip burns its
`waitForReply` budget instead of failing loudly.

```bash
# Stage-C — operator only (needs the keyless tag pulled and `ollama serve` up):
COMIS_LIVE=1 COMIS_LIVE_LOCAL_MODELS=llama3.2:3b pnpm vitest run -c test/live/vitest.config.ts \
  test/live/scenarios/channels/telegram-acceptance-vl.test.ts
```
...
`/rpc` over WebSocket (RPC passthrough + the curated `explain` / `system` + `trigger`), and the
...
| `tg explain` / `tg system` | The curated observability lenses for the rig's session. |
...
The emulator has a **webhook-POST mode** for testing its own secret-token gate,
but the product does not expose a Telegram webhook receiver. Product tests assert
that setting `webhookUrl` fails adapter startup before polling or external token
validation begins.

<Warning>
Comis is polling-only. A webhook update does not reach the agent, and adapter
startup rejects webhook configuration instead of reporting a connection that
cannot receive messages. A future webhook receiver must mount the platform
callback with secret-token verification before this harness mode can become a
product scenario.
</Warning>
...
COMIS_CHANLIVE_DIR=/tmp/my-rig node --import tsx test/live/bin/chan.ts --channel telegram rpc obs.system.health '{"since":1}' --json
