---
title: "Event Bus"
description: "163 typed events across 4 domain subsystems for cross-module communication"
...
The `TypedEventBus` wraps Node.js `EventEmitter` with a type-safe generic interface. All 163 events are defined in the `EventMap` interface, which is composed from 4 domain-specific sub-interfaces (`MessagingEvents`, `AgentEvents`, `ChannelEvents`, `InfraEvents`).
...
Events follow a `subsystem:action` naming pattern (for example, `message:received`, `tool:executed`, `graph:started`). The 163 events are organized into 4 subsystems:
