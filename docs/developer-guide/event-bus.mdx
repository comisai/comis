...
<Accordion title="AgentEvents (45 events)">

Skill lifecycle, tool execution, audit logging, observability (tokens and latency), cache break detection, model failover, security, execution graph, per-node sub-agent budget breaches, pipeline authoring telemetry, provider health, SEP (step-execution planning), and exec command blocking events.
...
**Pipeline authoring telemetry:** `pipeline:authored` fires once per `pipeline` authoring invocation -- `graph.define` and `graph.execute` -- carrying the `action` (`define` or `execute`), the calling model's server-resolved `capabilityClass` tier (`frontier` / `mid` / `small` / `nano`, or `unknown` when the tier cannot be resolved), whether the call parsed and validated against the graph schema (`schemaValid`, the real verdict -- `false` is still emitted, and the user-facing validation error still throws), and `repaired`. The payload is counts, ids, and enum labels only -- never a pipeline body, a node `type_config` value, a task or label string, or any secret. The tier is resolved daemon-side from the authoring agent, never tool-supplied. `repaired` is **always `false` until Phase 174** wires the small-model-authorable-DAG repair producer; the field exists now so the event shape is unchanged when that lands. The fleet aggregate of this event -- the small-model pipeline-authoring failure rate and the pre-committed Phase 174 build/defer gate -- is surfaced by [`obs.fleet.health`](/reference/json-rpc#obs-fleet-health).
