...
<Accordion title="AgentEvents (44 events)">

Skill lifecycle, tool execution, audit logging, observability (tokens and latency), cache break detection, model failover, security, execution graph, per-node sub-agent budget breaches, provider health, SEP (step-execution planning), and exec command blocking events.
