---
title: "Event Bus"
description: "160 typed events across 4 domain subsystems for cross-module communication"
...
The `TypedEventBus` wraps Node.js `EventEmitter` with a type-safe generic interface. All 160 events are defined in the `EventMap` interface, which is composed from 4 domain-specific sub-interfaces (`MessagingEvents`, `AgentEvents`, `ChannelEvents`, `InfraEvents`).
...
Events follow a `subsystem:action` naming pattern (for example, `message:received`, `tool:executed`, `graph:started`). The 160 events are organized into 4 subsystems:

| Subsystem | Event Count | Scope |
|-----------|-------------|-------|
| `MessagingEvents` | 35 | Message lifecycle, sessions, subagent lifecycle, compaction, context, commands, announcements |
...
<Accordion title="MessagingEvents (35 events)">
