---
title: "Event Bus"
description: "Typed events for cross-module communication"
...
The `TypedEventBus` wraps Node.js `EventEmitter` with a type-safe generic interface. All typed events are declared in the `EventMap` interface in `packages/core/src/event-bus/events.ts`, composed from domain-grouped sub-interfaces (`MessagingEvents`, `AgentEvents`, `ChannelEvents`, `InfraEvents`, and others). That declaration and its doc comment are the authoritative registry of the groups and each group's scope -- read them when you need the complete, current set.
...
Events follow a `subsystem:action` naming pattern (for example, `message:received`, `tool:executed`, `graph:started`). Each subsystem is owned by exactly one sub-interface of `EventMap`. The group list and each group's scope live in the `EventMap` doc comment in `packages/core/src/event-bus/events.ts` -- the single authoritative registry -- so this page does not restate them.
...
The accordions below describe the groups most often consumed outside the composition root, with the payload fields a subscriber usually needs. They are hand-maintained and **not exhaustive** -- `packages/core/src/event-bus/events*.ts` is authoritative for the complete event set and for every payload field.
...
|------------|-------------------|
| `announcement:dead_lettered` | runId, sessionKey, channelType, reason, timestamp |
| `announcement:dead_letter_delivered` | runId, channelType, attemptCount, timestamp |
| `announcement:quarantine_pending` | pendingCount, activeRecoveryCount, timestamp |
| `announcement:quarantine_read_failed` | timestamp |
...
| `compaction:flush` | sessionKey, memoriesWritten, trigger, success, failureReason?, timestamp |
...
| `session:sub_agent_spawned` | runId, parentSessionKey, agentId, rootRunId, parentRunId, parentLeaseId, caps, timestamp |
| `session:sub_agent_completed` | runId, parentSessionKey, agentId, success, runtimeMs, tokensUsed, cost, timestamp, cacheReadTokens, cacheWriteTokens, expectedOutputs, verifiedOutputs, attachmentsPrepared |
...
| `durable:resumed` | rootRunId, sessionKey, sourceCheckpointId, checkpointId, sourceTerminalReason, timestamp |
| `durable:suspended` | rootRunId, checkpointId, sessionKey, timestamp |
...
See [Execution Graphs: Small-model authoring](/agents/execution-graphs#small-model-authoring) for the behavior and [Pipelines: from_intent](/agent-tools/pipelines#from-intent) for the action.
...
**Completion delivery recovery:** `announcement:dead_lettered` fires after a failed completion or parent-decision fallback is durably retained. Its `sessionKey` attributes the degradation to `comis explain`, while `reason` distinguishes a failed delivery from a reserved parent decision. `announcement:quarantine_pending` reports both the daemon-wide operator-pending count consumed by `comis system-health` and the entries still eligible for automatic recovery; `announcement:quarantine_read_failed` says that standing count is unknown. `announcement:dead_letter_delivered` closes a successfully recovered delivery. These payloads carry ids, counts, closed reasons, and channel labels only -- never announcement text, child output, or an error string.
...
See [Resilience: Self-healing delivery retries](/agents/resilience#self-healing-delivery-retries) for the durable recovery and quarantine rules.
...
| `delivery:outward_ledger_transition` | rootRunId, runId, sessionKey, stepIndex, partId, transition, outcome, deliveryKind, platformMessageId, timestamp |
...
| `subagent:delivery_skipped` | runId, agentId, sessionKey, reason, timestamp |
...
<Accordion title="TerminalEvents">
