...
| `announcement:quarantine_pending` | pendingCount, timestamp |
...
**Completion delivery recovery:** `announcement:dead_lettered` fires after a failed completion or parent-decision fallback is durably retained. Its `sessionKey` attributes the degradation to `comis explain`, while `reason` distinguishes a failed delivery from a reserved parent decision. `announcement:quarantine_pending` is the daemon-wide standing count consumed by `comis system-health`; `announcement:dead_letter_delivered` closes a successfully recovered delivery. These payloads carry ids, counts, closed reasons, and channel labels only -- never announcement text, child output, or an error string.
