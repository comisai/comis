...
function. This function handles chunking, format
...
1. **Step 1** (before `setupChannels`): Create the queue adapter. Pass the
   `channelAdapters` map by reference (empty at this point).
2. **Step 2** (after `setupChannels`): Populate the adapters map, then run
...
Note that the in-process retry engine handles immediate retries
