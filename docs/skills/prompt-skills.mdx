...
<Tip>
Add an `argumentHint` under `metadata.comis` to show users what inputs your skill
expects. This hint appears in help listings and auto-complete suggestions.
</Tip>
...
- **Tool restrictions:** Use `allowed-tools` in the frontmatter -- a space-separated string in the six-field manifest -- to limit which tools the agent can use while this skill is active. For example, a research skill might set `allowed-tools: "web_search web_fetch"`. See [Manifest Reference](/skills/manifest) for the full field list.

- **Input validation:** Use `inputSchema` (under `metadata.comis`) to document the arguments the skill expects. It is advisory -- surfaced as documentation, not enforced at invocation time. See [Manifest Reference](/skills/manifest) for details.
...
- **Model invocation control:** Set `disableModelInvocation: true` under `metadata.comis` to hide a skill from the agent's automatic selection. The skill can still be invoked explicitly with `/skill:name`.
