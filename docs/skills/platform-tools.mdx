...
history (case-insensitive substring match, role-scoped). The DAG context engine's
`ctx_*` recall tools (`ctx_search`, `ctx_inspect`, `ctx_recall`, `ctx_expand`)
were removed pending the v2.12 "Lossless Context DAG" reimplementation and return
with the LCD engine. See [Context Tools](/skills/built-in-tools#context-tools) for
the `session_search` parameter reference.
