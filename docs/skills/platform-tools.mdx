...
like schedule tasks, operate on channel messages, manage sessions, analyze
...
| System Management | `agents_manage`, `obs_query`, `sessions_manage`, `memory_manage`, `channels_manage`, `tokens_manage`, `models_manage`, `providers_manage`, `skills_manage`, `mcp_manage`, `heartbeat_manage` | Supervisor-level administration tools | [Infrastructure Guide](/agent-tools/infrastructure) |
...
The `message` tool operates on an endpoint the runtime holds authority for --
see [endpoint authority](/agent-tools/messaging) for the exact resolution rule.
It supports sending new messages, replying to specific messages,
adding reactions, editing previous messages, and deleting messages without
collapsing channel instance, thread, or conversation-kind identity.

This is one of the most frequently used platform tools. Whenever an agent
needs proactive endpoint resolution rather than its current turn endpoint, it
uses `notify_user`.
...
- `sessions_list` -- list the caller conversation and directly delegated child sessions
...
System listing is available via `agents_manage` (action: `list`).
...
Agent administration tools are supervisor-level capabilities for managing the
...
- `obs_query` -- read-only observability queries via an action sub-system (some actions also take a `sub_action` for finer queries); the action list lives in the [Infrastructure Guide](/agent-tools/infrastructure#supervisor-tool-details)
...
- **`sessions_history`** -- retrieve history entries for a durable `tenant_id` + `agent_id` + `conversation_ref`, with pagination (`offset`, `limit`).
- **`sessions_list`** -- list the caller conversation and directly delegated child sessions, filterable by kind (`dm`, `group`, `sub-agent`) and recency (`since_minutes`).
...
quiet hours enforcement, and channel resolution guards before delivery. Every
route must resolve to a complete tracked endpoint; missing authority fails
closed.
...
| `channel_id` | `string` | No | Target conversation ID. Required with `channel_type`; the pair must resolve to a tracked endpoint. |
...
[`background_tasks` tool reference](/agent-tools/infrastructure)
for its conversation-scoped actions and long-running wait behavior.

## Regular vs Supervisor Tools

Most platform tools are available to any agent running with the `coding`,
`messaging`, or `full` tool policy profile. However, agent administration tools
