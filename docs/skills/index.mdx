...
- **File operations** -- read, write, edit, search files; apply patches; edit
  Jupyter notebooks
- **Shell commands** -- execute commands and manage background processes. Comis
  uses an OS sandbox when a supported provider is available, but ordinary
  `exec` falls back to host execution with a warning when it is not
...
- **Context tools** -- use the shipped `ctx_search`, `ctx_inspect`, and
  `ctx_expand` tools to recover detail from the current DAG session's lossless
  context store
...
system admin (8). See [Built-in Tools](/skills/built-in-tools) and
...
- **Agent administration** -- supervisor-level tools for managing the entire
  installation
...
Comis can also surface read-only learned Mental Model documents through the
same `<available_skills>` prompt channel. These are not equivalent to
operator-authored prompt skills. A read-only `candidate` can surface before it
is promoted to `active`, its `learned` label does not prove reliability, and
warning-level harmful language can pass static validation. Learned documents
are advisory Markdown only; they add no tool, credential, capability, approval,
or sandbox authority. Their store and recall boundary is `(tenant, agent)`, not
an individual chat or sender.
...
Skills can also be imported from repository directories, bounded `.skill`/ZIP
archives, operator-allowlisted well-known indexes, and configured registries.
Every source uses the same pre-write security gate and durable provenance. See
...
- **supervisor** -- system-management tools for administration agents
...
<CardGroup cols={3}>
  ...
  <Card title="Importing Skills" icon="file-import" href="/skills/importing">
    Import vetted prompt skills with trust and provenance controls
  </Card>
