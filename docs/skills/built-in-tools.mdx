...
remaining tools -- messaging, scheduling, sessions, media, system admin, etc. --
...
the bubblewrap sandbox that terminal tools require. Driven sessions preserve a
usable inherited `TERM`; when it is absent, blank, `dumb`, or `unknown`, Comis
uses `xterm-256color` so full-screen terminal applications can render correctly.
