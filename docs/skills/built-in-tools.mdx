...
Context tools help your agent search and recall information from the current conversation session. In the default **pipeline** mode the available tool is `session_search`. The DAG (LCD) context engine adds three **in-session** expansion tools -- `ctx_search`, `ctx_inspect`, and `ctx_expand` -- that recover detail the summary hierarchy compressed away within the current conversation. They are active only in DAG mode (`contextEngine.version: "dag"`), `never-export`, and distinct from cross-session recall (`memory_search`, `session_search`). See [Context expansion tools](/agent-tools/sessions#context-expansion-tools) and [Compaction](/agents/compaction) for how the engines work.
...
| `ctx_expand` | No | Yes | Recover a compressed region back to its underlying messages |
