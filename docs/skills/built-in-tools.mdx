...
description: "Reference for built-in file, shell, web, and browser tools"
icon: "wrench"
---
...
<Info>
**Where these fit.** The tools documented here (plus the context and terminal
driver tools below) are the framework-level "built-ins" that any coding-agent
host would have. The remaining tools -- messaging, scheduling, sessions, media,
system admin, etc. -- are [platform tools](/skills/platform-tools) layered on
top. The [Agent Tools](/agent-tools/index) page owns the complete inventory and
provides deeper per-tool guides.
</Info>
...
- If the primary provider fails, the tool automatically tries providers with
  configured API keys or endpoints. Providers without authority are skipped.
- Only after every eligible search provider fails does an enabled `browser`
  become the next recovery path. The agent starts it and opens a Google Search
  instead of retrying `web_search` again for that request.
...
- Uses `impit` with a Chrome-like TLS and HTTP profile; websites can still block automated requests
...
bubblewrap sandbox on Linux. See the [Terminal driver](/agent-tools/terminal-driver)
reference for the authoritative tool inventory and its configuration,
environment, sandbox, output, and recovery contracts.
