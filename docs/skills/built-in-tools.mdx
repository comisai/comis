...
- If every configured search provider fails and `browser` is enabled for the
  turn, the next recovery path starts the browser and opens a Google Search.
  The agent does not retry `web_search` again for that request.
