...
<Info>
A community skill hub is a **reserved** future acquisition channel -- the `clawhub` allowlist token names it, but there is no hub import flow yet. When it lands it will enter the pipeline at the same staging step every other source uses.
</Info>
