...
description: "Import a skill from a GitHub directory or a packaged archive through the staged, scan-first pipeline"
icon: "file-import"
---

Comis can install a skill you did not author yourself -- from a GitHub directory or a packaged archive. Every import runs through **one staged pipeline** and lands the skill at the `imported` trust tier: a deliberately cautious posture that treats the skill's contents as untrusted until you say otherwise.
...
<Info>
The registry-backed sources (a well-known index and a community hub) are modeled as future acquisition channels. They enter the pipeline at the same staging step; only `github` and `archive` (plus operator uploads) install today.
</Info>
