...
1. Anything outside the two contracted lines is stripped (the agent cannot inject `cluster`, schema changes, or any other field — D-5 in the design).
...
<AccordionGroup>
  <Accordion title="Daemon failed to read config file after sync-tooling --write">
    ...
    If owned by root, the file was written before Phase 26.1 shipped. Restore from the most recent backup or `chown comis:comis ~/.comis/config.yaml`.
  </Accordion>
