...
<Note>
The `group:context` / `group:context_expand` tool-policy groups bundled the DAG
context engine's `ctx_*` recall tools. Those tools and groups were removed pending
the v2.12 "Lossless Context DAG" reimplementation and return with the LCD engine.
In the default pipeline mode, use `session_search` (part of `group:sessions`).
</Note>
