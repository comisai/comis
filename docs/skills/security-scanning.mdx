...
| `community` | remote imports by default |
| `agent-authored` | skills an agent writes at runtime |

An operator-allowlisted well-known index or registry may explicitly set
`trust: operator`. That configured policy promotes imports from that source;
claims inside the bundle or registry response never do.
