...
For skills that expect structured input, you can define a JSON Schema in the `inputSchema` field. This validates the parameters before the skill runs.
...
If someone invokes this skill without specifying a `format`, the validation will reject the request before the skill runs.
