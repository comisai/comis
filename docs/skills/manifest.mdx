...
runtime keeps the turn going until that many **distinct successful** receipts
exist, and an answer produced without them is reported as partial. Set them only
on skills whose answers must be sourced -- `skills/deep-research/SKILL.md` ships
with `3` for both.
</Info>
