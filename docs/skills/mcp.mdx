...
`maxConcurrency` overrides the transport default. For stdio,
`supportsParallelToolCalls: true` opts into an implicit concurrency of 4 when
`maxConcurrency` is omitted; otherwise stdio remains serialized at 1. Keep it
serialized unless the server explicitly supports parallel calls. Both fields
stay attached to the persisted server entry across `comis mcp reconnect`.
Comis can infer `stdio` from `command` or `http` from `url` when `transport` is
omitted, but setting it explicitly makes shared configuration easier to review.
...
## Tool-call contracts and deadlines

Discovered MCP input schemas remain structured when Comis publishes them to the
model. Objects, arrays, primitive types, type unions, `oneOf` / `anyOf` /
`allOf`, enums, bounds, string formats, defaults, and descriptions are
preserved. An unresolved `$ref` is the exception: without the schema document
root, Comis exposes that node as unconstrained rather than pretending it
resolved the reference.

Every call registers an MCP progress handler, including background calls. A
progress notification refreshes the idle portion of the SDK timeout and does
not close the connection, but it never extends the call past the absolute
`integrations.mcp.callToolTimeoutMs` ceiling. The default is 120,000 ms. A
timeout error names the resolved value and tells the agent to narrow the request
instead of retrying the same call. The ceiling is measured from when the call is
made, so the per-server concurrency-slot wait counts against it; a call whose wait
exhausts the deadline is failed before it is sent, naming contention and the
server's `maxConcurrency` rather than blaming the request's scope. The timeout is runtime-immutable; an operator
must edit `config.yaml` and restart the daemon to change it. See the
[`callToolTimeoutMs` config contract](/reference/config-yaml#integrations).

## Image results

An MCP tool may return `image` content blocks beside or instead of text. Comis
attaches them to the tool result as model-visible images only after the same
sharp-backed sanitizer that processes browser screenshots has re-encoded them
(auto-rotate, fit within 1568 px, at most 5 MB, decompression-bomb guard). At
most four image blocks are considered per call; blocks beyond that limit,
blocks the sanitizer rejects, and blocks with unreadable data are dropped and
logged content-free at WARN. The text part of the result always comes first, followed
by a runtime-authored notice that names the server and states that text visible
in the images is untrusted data — the same posture as the `UNTRUSTED` wrapper
around MCP text. A result that carried only images reports what was attached or
dropped instead of "no text content".
