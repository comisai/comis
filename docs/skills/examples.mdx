...
Comis ships a set of prompt skills that seed into `~/.comis/skills/` on first
boot, and the easiest way to understand the format is to read a real one. Here
is the manifest and body of `log-troubleshooting`, the skill agents reach for
when the user asks "what went wrong?" or "why is the daemon slow?"

**Live at:** `~/.comis/skills/log-troubleshooting/SKILL.md`
...
<Tip>
Read the rest in [`log-troubleshooting/SKILL.md`](https://github.com/comisai/comis/tree/main/packages/daemon/bundled-skills/log-troubleshooting).
Its [sibling shipped skills](https://github.com/comisai/comis/tree/main/packages/daemon/bundled-skills)
...
- `allowedTools` records the intended restriction for invocation-capable
  consumers. Enforce the same restriction in the agent's tool policy; the
  manifest field alone is not an OS sandbox.
- The `userInvocable: true` setting is retained as registry metadata. The current
  daemon does not route `/skill:name` messages directly.
- The agent follows the tone, escalation, and response format rules for every reply

**Test it:**

Send an ordinary-language message such as: "Use the customer-support skill. I
ordered a product last week and it still has not arrived." Verify selection in
the execution trace when deterministic confirmation matters.
...
**Test it manually:** Ask: "Use the daily-briefing skill for US/Eastern."
...
payload_text: "Use the daily-briefing skill to prepare the morning briefing for US/Eastern."
```

This asks the model to select the daily briefing skill on the schedule. Selection
is model-driven, so inspect traces before relying on it for a critical workflow.
...
- `allowedTools` documents the intended read-only set. Mirror it in the agent's
  tool policy before treating this workflow as read-only.
...
```text
Use the code-review skill on src/api/handler.ts.
```
...
- `permissions.fsRead` declares the expected CSV path for review and custom
  consumers; it does not enforce kernel access by itself
- Use agent tool policy, safe-path controls, and host permissions to enforce the
...
```text
Use the csv-formatter skill on ./data/report.csv.
```
