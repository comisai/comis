...
- **Per-job efficiency** -- the [`comis system-health`](/reference/cli#comis-system-health) health report rolls up the skip-rate, the model turns saved, and the gate's own tool-call cost per agent (counts only, never the gate's gathered findings). A gate stuck at a 100% skip-rate on a monitor you expect to fire is the signal to inspect.
