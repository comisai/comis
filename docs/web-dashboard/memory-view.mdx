...
- `memory.ask(question)` -- grounded natural-language query over the memory store (default-on; disable per-agent with `dialectic.enabled: false`, or kill all cost features with `memory.enabled: false`). An abstain response carries a `reason` (`empty_recall`, `synthesis_abstained`, `dialectic_unavailable`, `no_agent_scope`) so an unwired dialectic is never mistaken for "no data".
