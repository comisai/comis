...
- `obs.channels.all` -- chat picker contents
- `message.fetch(channel_type, channel_id, limit?)` -- recent messages when the channel supports native history
- `session.list` and `session.history` -- stored-history fallback, matched by complete endpoint identity (channel instance, conversation, thread, and kind) rather than a display key
- `message.send`, `message.reply`, `message.react`, `message.edit`, `message.delete` -- compose and modify
...
You can attach files to messages with support for four attachment types: image, file, audio, and video. Select the attachment type, enter the attachment URL and optional caption, and send it to the current conversation.
