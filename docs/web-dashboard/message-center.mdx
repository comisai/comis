...
platform actions are available only when their RPC contract can represent the
selected target without losing endpoint identity.
...
- `message.fetch(channel_type, channel_id, limit?)` -- native recent history only when the selected target can be represented by those two fields
- `message.send`, `message.reply`, `message.react`, `message.edit`, `message.delete` -- native compose and modify operations when the selected target can be represented unambiguously
- `message.attach` -- URL-based attachment delivery
- `discord.action`, `telegram.action`, `slack.action`, `whatsapp.action` -- platform-specific actions
...
direct/shared kind. The native message RPCs currently accept only
`channel_type` and `channel_id`; they cannot carry that complete endpoint. The
Message Center therefore reads stored history through `session.history` and
disables native message mutations and platform actions for those rows instead
of collapsing the target to a bare conversation ID. See the
...
When the selected target is representable by the native RPC contract, platform
capabilities determine whether you can perform these actions:
