...
| Event | What you see |
|-------|--------------|
| `approval:requested` | The notification count increments and the Security view's Pending Approvals queue updates. |
| `approval:resolved` | The notification count decrements and the Pending Approvals queue updates. |
...
The sidebar is your primary way to navigate between views. It contains 22 items organized into four sections -- **Home** (1), **Operate** (7), **Observe** (6), and **Configure** (8) -- plus a **Logout** button. Each section header is a non-clickable label.

<Info>
Several detail pages are reached from their parent views rather than listed separately:
...
| Item | Badge | What It Does |
|------|-------|-------------|
| **Overview** | Delivery error count | System metrics summary with stat cards and 24-hour activity chart |
...
Stat cards refresh from their canonical RPC sources every 60 seconds and after relevant SSE events. The dashboard shows loading skeletons during initial data fetch and a retry button if an error occurs.
...
|----------|--------|---------|
| **Views** | 17 frequently used views | Dashboard, Agents, Billing, Context Engine, Security |
...
Each card below links to a dashboard feature or related detail view.
