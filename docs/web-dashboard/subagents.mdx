...
- `subagents.list` -- runs from the last 60 minutes (with status, depth, parent, cost, tokens, steps)
- `subagents.get(spawnId)` -- full execution tree for one run
- `subagents.kill(spawnId)` -- cancel a running or queued sub-agent
