...
Below the bundled tools, a **Platform Tools** section lists higher-level building blocks the agent uses to talk to the rest of Comis -- memory, sessions, agents, messaging, media, infrastructure, and agent administration groups (e.g., `memory_search`, `sessions_spawn`, `discord_action`, `image_analyze`, `cron`, `gateway`, `models_manage`).
