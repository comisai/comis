...
**Who it's for:** designers of multi-agent workflows (debate, vote, refine, map-reduce) who would rather draw the graph than write JSON, and operators watching live executions.
...
Saves go through `graph.save`; runs through [`graph.execute`](/reference/json-rpc#graph-execute), which the builder calls with the canvas's nodes inline rather than a saved graph id; live updates through SSE events `graph:node:status` and `graph:progress`. Saved graphs are stored on disk so they survive restarts.

For the underlying domain model (node types, barrier modes, retry policy, context modes) see [Execution Graphs](/agents/execution-graphs).
...
<Steps>
  ...
  <Step title="Wire them in series">
  ...
  <Step title="Configure node 1: research">
  ...
  <Step title="Configure node 2: vote">
  ...
  <Step title="Configure node 3: summarize">
