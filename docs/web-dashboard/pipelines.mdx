...
Saves go through `graph.save`; runs through `graph.run(graphId, variables?)`; live updates through SSE events `graph:node:status` and `graph:progress`. Saved graphs are stored on disk so they survive restarts.
