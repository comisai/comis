...
- `workspace.list(agentId, path?)` -- file tree
- `workspace.read(agentId, filePath)` -- file content
- `workspace.write(agentId, filePath, content)` -- save
- `workspace.delete(agentId, filePath)` -- delete
- `workspace.resetToDefault(agentId, filePath)` -- restore template content
- `workspace.gitStatus(agentId)` -- branch, changed files
- `workspace.gitDiff(agentId, filePath?)` -- diff
- `workspace.gitCommit(agentId, message)` -- commit
- `workspace.gitRestore(agentId, filePath)` -- discard local edits
