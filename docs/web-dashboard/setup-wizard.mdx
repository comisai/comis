...
Pick the LLM provider to wire up. Each option in the picker shows the provider name, a one-line description, and whether it needs an API key, base URL, or model id:

- **Anthropic** (recommended) -- Claude models
- **OpenAI** -- GPT-4o, o1, o3 families
- **Google** -- Gemini
- **Groq, Mistral, DeepSeek, xAI, Together, Cerebras, OpenRouter** -- third-party hosts and aggregators
- **Ollama** / **local** -- self-hosted endpoints

Enter the API key (saved to `~/.comis/.env` or the secrets store, never printed), the base URL if non-default, and the default model id.
