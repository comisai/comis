...
- `/chat/:conversationRef` -- searches the authorized tenant and agent partitions, selects the owning agent, and opens the matching stored conversation
...
On the left, the session sidebar lists conversations for the selected agent. Each session shows its label or shortened opaque conversation reference, channel kind, message count, and last activity.
...
For a routed conversation reference, the console switches to the owning agent.
The message input remains disabled unless that reference resolves to an
authorized tenant, agent, and conversation target.
