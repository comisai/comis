...
- `POST /api/chat` -- sends a new message; body is JSON `{ message, agentId?, sessionKey?, locale? }` (`locale` is a canonical BCP-47 tag; see the [HTTP gateway reference](/reference/http-gateway))
