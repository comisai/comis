...
- `/chat/:conversationRef` -- opens directly to the matching stored conversation in the selected agent's authorized session list
...
- `session.list` and `session.history` -- resolve and load stored conversations through explicit tenant, agent, and conversation-reference authority
...
For a routed conversation reference, the message input remains disabled unless
that reference resolves to an authorized tenant, agent, and conversation target.
