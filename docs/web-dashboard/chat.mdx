...
**Who it's for:** anyone testing an agent's behavior before exposing it on a real channel, plus developers debugging tool calls live.

## Routes

- `/chat` -- opens the console without a selected session
- `/chat/:conversationRef` -- searches every authorized tenant and agent partition without a session-kind filter, selects the owning agent, and opens the matching direct-message, group, or sub-agent conversation

## Backing API

- `POST /api/chat` -- sends messages for console-created web conversations; body is JSON `{ message, agentId?, sessionKey?, locale? }` (`locale` is a canonical BCP-47 tag; see the [HTTP gateway reference](/reference/http-gateway))
- `session.list` and `session.history` -- resolve a routed conversation across authorized agents, then load it through its exact tenant, agent, and conversation-reference authority
- `session.send` -- continues a selected stored conversation through the same scoped authority
...
On the left, the session sidebar lists direct-message conversations for the selected agent. Each session shows its label or shortened opaque conversation reference, channel kind, message count, and last activity.
...
- A **search bar** ("Filter sessions...") that filters the visible session list by conversation reference or label as you type

Click any session in the list to load its conversation in the message area.
Route resolution is separate from the direct-message sidebar query: a routed
group or sub-agent conversation can remain active even though it is not a
sidebar row. The console switches to the owning agent, loads history with the
resolved tenant, agent, and conversation reference, and uses that same exact
scope when continuing the conversation. The message input remains disabled
unless all three authority fields resolve.
...
At the top of the message area, a header bar lets you select the agent and shows the active conversation reference and message count.
...
<Tip>
The chat console uses the `web` channel for sessions it creates. Selecting an existing platform conversation resumes only that authorized stored target; its human-readable session key is never reused as transport identity.
</Tip>
...
- **Live streaming** -- Responses for console-created web conversations appear word by word. A stored-conversation continuation appears after its scoped session request completes.
