...
- `/chat/:conversationRef` -- opens directly to the stored conversation identified by its opaque conversation reference
...
On the left, the session sidebar lists all of your conversations. Each session shows its opaque conversation reference, the agent and channel it belongs to, and the last time it was active.
...
- A **search bar** ("Filter sessions...") that filters the visible session list by conversation reference, agent ID, or channel type as you type
...
At the top of the message area, a header bar shows the conversation reference, the agent handling the conversation, and the channel type.
