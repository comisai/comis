...
The Security view is the dashboard for reviewing agent security events and controlling what agents may do. Approvals, audit, tokens, secrets, and policy live on this page.
...
The notification bell opens this view on the Pending Approvals tab when action is required.
...
- Shows whether the gate is enabled, what happens to an action no rule matches, and the
  ordered rule list with each rule's mode and, for auto-approval, its trust floor
- `approvals` is runtime-immutable: edit it in the config file and restart the daemon
...
When an agent requests permission for a sensitive or destructive action, manage it from the **Pending Approvals** tab in Security.
...
The Rules section exposes editable action-confirmation, agent-to-agent, and Node
permission settings, plus the read-only approval policy. The rules decide requests
that reach the gate; they do not place a gate in front of a path that never submits
one, so the policy is not proof that an action is guarded. Use the Pending Queue to
verify that each critical path actually created a request.
...
<Steps>
  ...
  <Step title="Approve a pending action">
    Open **Security**, then select **Pending Approvals** (or use the notification bell). Review the request and click **Approve** or **Deny**.
  </Step>
</Steps>
