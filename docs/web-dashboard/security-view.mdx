...
The Rules section exposes action-confirmation, agent-to-agent, Node permission,
and approval-mode settings. Treat the approval-mode editor as configuration
surface, not proof that an action is guarded. The current runtime has no
general evaluator that applies a default approval mode to every action.
Operator-backed confirmation occurs only when a supported execution path calls
the approval gate. Use the Pending Queue to verify that each critical path
actually created a request.
