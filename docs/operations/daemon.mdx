...
3. **Sub-agent runner** -- Gives active tasks a bounded drain. If the grace period expires, checkpoint-admitted durable sub-agents suspend their local provider work and retain the running checkpoint for boot recovery; non-durable work is stopped with an attributed failure.
