...
- `agents.<id>.promptTimeout.promptTimeoutMs` is a **stall budget**, not a wall clock: the
  ...
  second), tool progress, and tool completions, so only a fully silent gap kills the turn. It needs to cover
...
Start deployment-wide: `comis system-health --since 24` surfaces the
...
[`obs.system.health`](/reference/json-rpc#obs-system-health) RPC), the degraded-by-cause
breakdown (`context_exhausted`, `output_starved`, `timeout`, `narration_stall`), and
breaker/cost rollups. See [`comis system-health`](/reference/cli#comis-system-health).
