...
```dockerfile
ARG COMIS_NODE_BOOKWORM_IMAGE="node:22-bookworm@sha256:..."
ARG COMIS_NODE_BOOKWORM_SLIM_IMAGE="node:22-bookworm-slim@sha256:..."
ARG COMIS_VARIANT="slim"
ARG COMIS_DOCKER_APT_PACKAGES=""
```

- **Digest pinning** -- the base image ARGs accept `@sha256:` digests for
  reproducible builds. Replace the placeholder digests with actual values from
  `docker inspect --format='{{.RepoDigests}}' node:22-bookworm`.
...
The repository includes `.github/workflows/docker-release.yml` for automated
Docker image builds. This workflow triggers on releases and handles building and
publishing the image. See the workflow file for configuration details.
