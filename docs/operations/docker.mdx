...
- The container starts cleanly. The daemon, gateway, dashboard, channels,
  memory, RAG, and all LLM features work normally.
- The agent's `exec` skill is **non-functional** -- every shell command the
  agent tries to run returns the kernel error and the inner command never
  executes. There is no leak (the malicious command also doesn't run), but
  the agent cannot drive scripts, file operations, or anything else that
  goes through `exec`.
- Without `exec`, the agent is significantly less capable. Production-grade
  testing of agent workflows that involve shell execution must happen on
  Linux.

On a real Linux host the kernel allows nested-namespace `procfs` mounts, the
sandbox runs fully, the agent has working `exec`, and the credential
isolation we describe below is actively enforced.

#### Recommendations for Mac / Windows users

| Use case | Recommendation |
|---|---|
| Local development of UI / channels / non-exec features | Docker Desktop is fine -- you'll see the dashboard, can chat with the agent, configure channels, and inspect logs. |
| Local development that exercises `exec` end-to-end | Use a Linux VM on your Mac (Lima, Colima with VZ, OrbStack with a Linux distro, UTM). The container kernel inside those is a real Linux kernel and bubblewrap runs unrestricted. |
| Production / multi-user deployment | Deploy to a Linux host (any major cloud VM, a self-hosted server, or a single-board computer running Linux). |
| Just trying Comis out before committing | Docker Desktop is the fastest path. You can always migrate the data directory (`~/.comis`) to a Linux host later. |

The startup logs include a clear WARN when bubblewrap is installed but
non-functional on the running kernel, so you don't have to guess whether
the sandbox is active. Look for `bwrap installed but smoke test failed`.
...
| **bwrap** | Each agent-issued shell command from the daemon's own data inside the container -- `/data/.env`, `/data/secrets.db`, `/etc/comis/config.yaml`, channel tokens, the master encryption key |
...
`/data` -- so a single `cat /data/.env` would exfiltrate `SECRETS_MASTER_KEY`,
...
(read-only), and a fresh `tmpfs` for `/tmp`. `/data`, `/etc/comis`, and
...
|---|---|---|
| **`.env` file (legacy mode -- default)** | Single-tenant deployments where credentials are managed by the host operator. The simplest setup, no encryption key to manage. | Provider keys and channel tokens come from compose's `environment:` pass-through (read from your repo `.env`) and from a `<data dir>/.env` file mounted into the container at `/data/.env`. The daemon merges both at startup; values already in `process.env` always win. The daemon does **not** create `secrets.db`. |
...
Storing both the encrypted database (`/data/secrets.db`) and the master
