...
<Steps>
  ...
  <Step title="Create the configuration">
    ...
    Create the environment file at `/etc/comis/env` with the config path. If
    the wizard initialized encrypted storage, also provide its master key to
    the service through your system secret-provisioning mechanism:

    ```bash
    sudo tee /etc/comis/env > /dev/null << 'EOF'
    COMIS_CONFIG_PATHS=/etc/comis/config.yaml
    SECRETS_MASTER_KEY=<copy-from-the-secure-backup-created-during-setup>
    EOF
    ...
    <Tip>
    The default encrypted backend keeps provider and channel credentials in
    `secrets.db`; the service still needs the master key. An owner-readable
    environment file reduces casual exposure but is not an encrypted secret
    manager. Never put credentials directly in `config.yaml`.
    </Tip>
  </Step>
...
Loads environment variables (such as `SECRETS_MASTER_KEY` and `COMIS_CONFIG_PATHS`) from the
