...
**`*.trajectory.jsonl`** — one file per agent session, with a 10 MB soft cap and a 50 MB hard cap applied in-session (BOUND-02). Accumulated session files across past sessions are pruned by the startup sweep against `maxFiles` and `maxAgeDays`.
