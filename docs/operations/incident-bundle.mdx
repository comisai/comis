...
2. **End-user / owner (slash command):** `/export-trajectory` in a Telegram DM with the bot.
...
1. The bot replies in the source group thread that export is available only in a direct message.
2. No bundle is created and no direct-message destination is inferred from the group sender ID.
3. The owner can open an authenticated DM with the bot and run the command there.
