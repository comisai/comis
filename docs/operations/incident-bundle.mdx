...
The bundle pipeline enforces four hard limits (BUNDLE-03). Exceeding a limit does not crash the
...
IDs but also legitimate counters and timestamps. This is the §8.3 design tradeoff: more aggressive
...
<Warning>
Owner gate is enforced at the slash-command handler. Non-owner invocations are silently ignored —
the command name `export-trajectory` is in `KNOWN_COMMANDS` so the text never reaches the LLM.
This means non-owners see no reply at all when they type the command.
</Warning>
