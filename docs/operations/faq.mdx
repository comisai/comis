...
<AccordionGroup>
  ...
  <Accordion title="What if an agent goes rogue?">
    ...
    The approvals schema defaults off; guided setup enables it when it creates
    an administrator mapping. The default tool policy is broad. Configure limits
    and permissions for your threat model; no control makes a highly privileged
    agent risk-free. See [Agent Safety](/agents/safety).
  </Accordion>
</AccordionGroup>
