...
observability — and v2.22 fixes all five with one script-class data table, one pure
normalization function, one trigram search lane, one shared prompt line, and one phrase
table, with zero new runtime dependencies. The result: a Hebrew, Arabic, Russian, or CJK
conversation gets honest token math, search that survives morphology, summaries and replies
in its own language, and fleet-visible script health — while a **Latin-script deployment is
byte-identical to before** (token factor 1.0, the same search SQL, the same English strings).
...
English-calibrated character-to-token ratio under-counts non-Latin text — which used to admit
prompts that actually overflow the model window (silent truncation for non-Latin users) and
arm the compaction triggers late. Comis applies a per-script multiplicative factor (always in
...
pre-v2.22 token count is corrected where it is load-bearing (at pre-flight budget-item
...
The bidi-and-security boundary is decided and deliberately unchanged by this milestone. The two
points an operator (and the next security review) needs to understand:
...
**The three fleet checks that prove a multilingual local stack works.** Run
