...
A proactive check can legitimately run and deliver nothing — the model may decline, or a visibility setting may withhold the reply. [`comis explain`](/reference/cli#comis-explain) reports that attempt's disposition, delivery counts, and the exact suppression reason in one call, so a silent check is never indistinguishable from a broken one.
