...
│   └── <agentId>/
│       ├── journals/<sessionId>.json
│       └── descriptors/<sessionId>.json # recoverable session identity
├── managed-runs/private/   # owner-private capability-service content (default path)
...
├── skills/                  # seeded bundled skill library plus operator-added skills
...
- `managed-runs/private/` — appears only after an enabled capability service creates owner-private managed-run content. A custom `capabilityServices.privateContentDirectory` replaces this default path.
...
Before copying, the daemon refuses configurations containing plaintext secrets. Valid 64-hex SHA-256 pins under `agents.*.skills.terminal.allow[].match.hash` are integrity metadata rather than credentials, so they remain in the recovery snapshot; secret-shaped or malformed values in the same field are still rejected.
...
Capability-service managed runs also keep their durable state transitions,
content-free report/evidence indexes, attention and operation records,
continuation claims, workspace leases, execution attachments, and managed
terminal bindings here. Each execution attachment retains its filesystem
identity and public relay identity so restart recovery and terminal launch use
the same run-bound authority. A continuation execution failure reduces its exact
accepted report interval to `unknown` and settles that interval without immediate
replay. The same SQLite transaction records whether that reduced interval must
settle as `completed` or `failed`, so restart recovery cannot recompute or change
the outcome. A later accepted report advances the cursor and schedules the next
continuation; an abandoned claim remains recoverable after restart.
...
Secret redaction preserves exact, fixed-shape runtime identities needed for replay,
including conversation references and Comis-minted managed-run/workspace-lease
handles. Those handles already appear in the owner-only launch-plan result and are
not service credentials; credential-shaped values under the same field names are
still redacted.
...
comis sessions reset <sessionKey> --tenant <tenantId> --agent <agentId> --yes
# Also clear RAG memories for this session (the full-forget path):
comis sessions reset <sessionKey> --tenant <tenantId> --agent <agentId> --memory --yes
# Nuclear forget — also purge consolidated observations derived from this session:
comis sessions reset <sessionKey> --tenant <tenantId> --agent <agentId> --memory --purge-derived --yes
```
...
### `managed-runs/private/`

Private capability-service activation descriptors, report bodies, evidence bodies,
and attention content. The path is relative to the data directory and is selected
by `capabilityServices.privateContentDirectory`; `managed-runs/private` is the
default. Comis creates the root and every scope directory with owner-only `0700`
permissions. Scope and body filenames are SHA-256-derived, while `memory.db`
stores the owner-scoped content-free index, content hash, byte length, retention,
and relative path.

**Written by:** linked capability-service activation, report, evidence, and
attention flows.

**Safe to delete?** No while managed runs remain active or recoverable. Missing or
hash-mismatched bodies fail closed as unavailable evidence, so preserve this tree
together with `memory.db` when backing up or restoring managed-run state.
...
Durable drive-state journals for the [terminal-driver](/agent-tools/terminal-driver), written **only** when an agent runs a durable drive (`drive.durable: true`). Each journal lives at `terminal-drive/<agentId>/journals/<sessionId>.json` — one small JSON file per driven session, beside (but distinct from) the agent's persistent `projects/` working directory under `workspace/`. A matching content-free reattach descriptor lives under `terminal-drive/<agentId>/descriptors/` only while the session remains recoverable. Descriptor updates use a flushed temporary file, atomic rename, and directory flush; a failed replacement therefore leaves the prior descriptor available for cleanup. A normal durable exit retires that descriptor immediately; a managed exit retains it until named tmux termination, local run/lease/terminal retirement, and descriptor deletion are all durably confirmed.

The journal is what lets a long autonomous drive **survive a daemon restart**: a durable drive launches the driven CLI inside a detached tmux server that outlives the daemon, and the journal records the compact drive state — the objective, the last screen classification, the prompts already answered, and the steps tried — so on restart the driver **re-attaches to the live tmux session and resumes** instead of starting over or re-answering prompts. Managed recovery also requires the live pane PID and process-start identity to exactly match the descriptor. Missing or mismatched identity is kept only for safe cleanup and is never re-attached as the original run. A genuinely-gone session is reported as failed with its journal preserved.
...
A JSONL **snapshot** of retained announcement-delivery failures, active producer ownership, parked parent-decision reservations, and bounded evidence for invalid rows. Active ownership records carry the authoritative session, tool-result, or graph producer identity so restart recovery can remove retired producers without inventing a completion. On provider recovery the delivery system replays only entries whose outcome is safe to retry. Exhausted, expired, ambiguous, and invalid records remain parked until an operator records a decision with `comis quarantine`.
...
Ledgerless replay guards are stored separately under
`dead-letters.jsonl.terminal-decisions/`. The `decisions/` and `retirements/`
subdirectories contain content-free JSON records, sharded by the first two
characters of their SHA-256 digest. Decision records suppress repeated delivery;
retirement records keep those guards until the authoritative session, tool
result, or graph producer can no longer replay. Records are written atomically,
and invalid, oversized, or misplaced records appear in the quarantine health
surface without disabling unrelated guards. Ledger-governed decisions use the
indexed SQLite outward terminal-decision store instead.

Back up and restore `dead-letters.jsonl` together with the complete
`dead-letters.jsonl.terminal-decisions/` directory. Omitting either side can
lose pending ownership or replay suppression. Do not edit digest records while
the daemon is running; after an offline repair, `quarantine list` reloads the
collections and reports any corruption that remains.

**Written by:** the announcement delivery subsystem.
**Safe to delete?** No. Deleting it discards delivery evidence and pending outcomes. Do not edit or remove it while the daemon is running; the live queue is the authority and may rewrite the snapshot. Use `node packages/cli/dist/cli.js quarantine list` and `quarantine release` for operator decisions.
...
A write interrupted by a crash can leave an `auth-profiles.json.<pid>.<random>.tmp` file behind. The next Comis process to open the store reclaims those leftovers — except when the PID in the name is still running, because that names a write still in progress (a refresh by the daemon while the CLI starts up, for example).
...
for path in \
  "$HOME/.comis/.env" \
  "$HOME/.comis/secrets.db" \
  "$HOME/.comis/managed-runs/private"; do
  [[ -e "$path" ]] && optional_state+=("$path")
done
...
If `capabilityServices.privateContentDirectory` uses a non-default relative
path, include that path beneath the data directory instead. Restore it together
with `memory.db`; the SQLite indexes and owner-private bodies are one durable
managed-run state set.
...
By default Comis resolves config from `~/.comis/config.yaml` and
`~/.comis/config.local.yaml`. Override those locations with
[`COMIS_CONFIG_PATHS`](/reference/environment-variables#comis_config_paths),
whose environment-variable reference owns the list syntax.
...
To reset one conversation while keeping configuration, credentials, unrelated
memory, and capability-service state:

```bash
comis sessions reset <sessionKey> --tenant <tenantId> --agent <agentId> --yes
```

Do not remove `memory.db` as a conversation-reset shortcut. It also contains
long-term memory, delivery state, managed runs, leases, attachments, and terminal
authority. If an intentional full workspace reset removes it, remove the
configured `capabilityServices.privateContentDirectory` in the same reset so its
owner-private bodies do not become orphaned.
