...
Admission is not reported until the protected task record has completed its
atomic write, rename, and applicable sync checks. Ordinary write, rename,
file-sync, or directory-sync failures reject promotion and surface the
persistence error to the foreground turn. If rejection happens after the
rename, Comis restores the previously accepted record; for a new task, it
syncs a non-recoverable placeholder before removing the file. Startup recovery
therefore cannot adopt a task whose admission was rejected. The one narrower
exception is Node's Permission Model reporting that its `fsync` API is disabled:
after the write and rename succeed, Comis accepts the record as committed
without `fsync`. Other persistence errors are never treated as that restriction.
