...
Ledgerless terminal decisions are stored separately as a bounded, content-free,
digest-keyed index in `dead-letters.jsonl.terminal-decisions.jsonl`. This index
suppresses replay without keeping the pending snapshot alive or creating one
filesystem entry per decision. Ledger-governed decisions use the indexed SQLite
outward terminal-decision store instead.
