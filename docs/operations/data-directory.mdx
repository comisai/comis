...
persistence error to the foreground turn. The one narrower exception is Node's
Permission Model reporting that its `fsync` API is disabled: after the write and
rename succeed, Comis accepts the record as committed without `fsync`. Other
