...
├── logs/
│   └── daemon.log           # structured JSON log (pino-roll → daemon.1.log)
...
| **RAG memory** | `memory_*` | **Durable.** Distilled facts learned from conversations. Cleared separately via `comis sessions reset-lcd --memory`. |
| **SDK session JSONL** | `~/.comis/workspace/sessions/.../<key>.jsonl` | **Disposable** — the SDK's per-session working transcript. Routinely deleted by the operator (housekeeping) and wiped on new-session or reset. |

<Note>
The SDK session JSONL is **disposable**. Deleting it (for any reason — operator housekeeping, `rm`, daemon restart) is always treated as "continue": the LCD store keeps its full history and appends the fresh transcript as a continuation. The daemon cannot distinguish disk-space housekeeping from a deliberate reset, so it never infers forget from a file deletion — it always continues.
</Note>

**Forgetting is explicit.** To clear a conversation's durable history so the model starts with a clean slate, use the explicit reset command:

```bash
comis sessions reset-lcd <sessionKey> --yes
# Also clear RAG memories (full-forget path):
comis sessions reset-lcd <sessionKey> --memory --yes
```
...
The daemon's own **structured JSON** log, written via pino-roll and rotated to `daemon.1.log`, `daemon.2.log`, … when it reaches 10 MB by default (configurable via `daemon.logging.maxSize` and `daemon.logging.maxFiles`). This is the canonical application log — leveled lines with `traceId`, `agentId`, `durationMs`, etc. — and the file you parse to diagnose behaviour. Its location is set by `daemon.logging.filePath` (default `~/.comis/logs/daemon.log`).
...
<Tip>
Under pm2, Comis skips the stdout transport to avoid duplicating entries. The canonical structured log is always `~/.comis/logs/daemon.log` (active rotated file: `daemon.1.log`).
</Tip>
