...
The `video_generate` tool (v2.24) persists generated videos to `media/videos/` inside the agent's workspace (`~/.comis/workspace/media/videos/` for the default agent) the same way — a UUID filename via the media-persistence service, with a raised size cap appropriate for video. The clip is downloaded to a buffer and persisted **before** any delivery decision (an expiring provider URL cannot orphan it), then delivered as a `video` attachment via `sendAttachment` from that durable path; a channel without attachment support (currently only IRC) degrades to a notice plus the persisted path, and for large clips the inline base64 fallback is size-capped (the handler returns the persisted workspace path instead of a multi-megabyte blob). Persisted videos are not deleted after delivery (see the [`video_generate` tool](/agent-tools/media)). _(The durable async job-store database for long-running renders that outlive the turn lands in a later v2.24 phase.)_
