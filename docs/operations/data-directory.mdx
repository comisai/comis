...
├── skill-index-cache/       # validated well-known indexes, keyed by URL hash (0o600 files)
├── identity/
...
- `skill-index-cache/` — created after the first well-known index fetch; entries expire according to `skills.import.indexCacheTtlMs`.
...
`source` records which install path produced the skill (`create`, `update`, `upload`, or a remote source). `ref` is a public locator and is absent for locally-authored skills — it never carries credentials. `trust` is **derived from the install call**, never declared by the skill itself: remote imports default to `community`, an operator-configured well-known index or registry may explicitly promote its imports to `operator`, local authoring by the default agent is `operator`, and only the daemon's own seeding is `first-party`. See [Security Scanning](/skills/security-scanning) for how the verdict is produced.
...
Validated projections of operator-allowlisted well-known skill indexes. The
filename is a SHA-256 digest of the index URL, so remote URL text does not become
a filesystem path. Files are daemon-private and contain the validated index
shape plus its fetch time, never downloaded skill contents.

An unexpired entry avoids a network request. If refresh fails after expiry,
Comis may use the stale validated projection and reports degraded coverage; it
still fetches and vets every selected skill file normally. Safe to delete while
the daemon is stopped—the next well-known import fetches the index again.
