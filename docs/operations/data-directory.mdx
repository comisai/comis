...
The seeded `media/` subdirectory is scratch space for agent file operations. The `image_generate` tool does **not** write generated images here today — it writes an ephemeral file under the OS temp directory purely to hand off to the channel adapter, then deletes it (or returns the image as base64 when no channel delivery is possible). Durable persistence of generated images under the data directory is planned for a later phase.
