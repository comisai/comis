...
#### Per-user profile — folded into the Mental Model store (v2.31)

The standalone **`user_representation`** table was **removed in v2.31**: the per-user profile is now a `kind:"profile"` document in the **`mental_models`** store (the same store the learned-skill surface reads), maintained by the one outcome-gated reflection cron. A profile **correction** **supersedes** the prior doc with a non-destructive **history-append** (the prior body is preserved in the doc's `history` column, never hard-deleted), and external-trust sources are excluded by the engine's two-axis anti-poison SELECT. Existing `~/.comis` DBs keep any old `user_representation` rows on disk (the table is simply no longer created or read); the profile rebuilds into `mental_models` on the next reflection run. The full `mental_models` schema detail lands in a later v2.31 docs pass.
