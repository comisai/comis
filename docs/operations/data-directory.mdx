...
A JSONL **snapshot** of retained announcement-delivery failures, parked parent-decision reservations, and bounded evidence for invalid rows. On provider recovery the delivery system replays only entries whose outcome is safe to retry. Exhausted, expired, ambiguous, and invalid records remain parked until an operator records a decision with `comis quarantine`.
