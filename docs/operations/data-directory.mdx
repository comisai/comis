...
- `cap.sock` — present only when an autonomy-bearing agent is configured (the loopback capability endpoint). Ephemeral — `0600` owner-only, deleted and recreated on each daemon start. The lease itself is **in-memory** (run-scoped), so there is no `cap.db` / lease table on disk. The [bounded-autonomy](/agents/autonomy) control-plane state (the per-root spawn-ceiling counters, the call-rate windows, the per-root budget meters, and the outward-quota windows) is likewise **in-memory only** — there is no on-disk autonomy state file, so the bounds gate a single live daemon run and reset on restart (the M1 scope; a durable budget/sent-ledger is future work). The data-dir layout is unchanged by autonomy.
