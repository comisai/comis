...
| **RAG memory** | `memory_*` | **Durable.** Distilled facts learned from conversations. Cleared separately via `comis sessions reset --memory` (reserved — not yet implemented). |
...
# Also clear RAG memories (full-forget path — reserved, not yet implemented):
