...
Ledgerless terminal decisions are stored separately as content-free, digest-keyed
records under `dead-letters.jsonl.terminal-decisions/`. This indexed store
suppresses replay without keeping the pending snapshot alive or making each
queue mutation rewrite the lifetime decision history. Ledger-governed decisions
use the indexed SQLite outward terminal-decision store instead.
