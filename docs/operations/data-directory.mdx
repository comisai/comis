...
├── terminal-drive/          # durable terminal-driver state (only with drive.durable: true)
...
- `terminal-drive/` — appears only when an agent runs a **durable** terminal drive (`drive.durable: true`). The per-agent durable drive-state journals live here; absent for non-durable (default) drives.
</Note>
...
**Safe to delete?** Yes, when the daemon is stopped. Deleting a journal means an in-flight durable drive cannot resume after the next restart — the driver treats the session as gone rather than resuming it. Non-durable (default) drives never write here.
