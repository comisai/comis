...
Comis correlates structured events, traces, and diagnostic artifacts so an
operator can investigate a deployment-wide failure from one workflow.

This page documents the observability foundations. It covers the trajectory layer, the event-bus bridge mapping, lifecycle envelopes, forensic INFO promotions, the dedup detector, boot invariants, the alert budget, `step:` discipline, log rotation, the memory & recall diagnostics (recall-trace artifact, recall events, degradation signals), and the bundle export entry point. If you are new to Comis observability, start here; then follow the cross-links for operator workflows.
...
### Token counts differ between lenses — by design

`obs.explain` and `obs.system.health` both report `cost.totalTokens`, and they count
**different things**. Each now declares which via a `tokenBasis` discriminator:

| lens | `tokenBasis` | counts |
|---|---|---|
| `obs.explain` (one session) | `input+output+cache` | every token the session moved, **including** cache reads and writes |
| `obs.system.health` (window) | `input+output` | billable input + output only, cache **excluded** |

One 27-minute session legitimately reported 6,043,245 on the first and 18,637 on the
second. Neither is wrong — read `tokenBasis` before comparing them, and never treat a
mismatch between the two as a bug on its own.

A third number exists: the per-session `_session-metadata.json` rollup is written per
**execution** (last write wins), so on a multi-turn session it reflects the most recent
execution rather than the whole session. Prefer `obs.explain` for a session total.
...
The on-wire field is `normalized.metadata.traceId`; the helper `getMessageTraceId()` reads it. Channel adapters establish the inbound request scope around their dispatch loops. The orchestrator inherits that exact scope, verifies that its channel and trace match the message, and establishes a fallback boundary only for an unscoped custom adapter.

<Info>
**Architecture invariant:** `test/architecture/trace-propagation.test.ts` asserts that every adapter handler fanout runs inside `runWithContext` and that execution stages do not shadow the inherited scope. This test is shrink-only — violations are caught at CI time, not at incident time.
</Info>
...
- **Bounded cache-prefix evidence** — each `stream:context` record carries `systemDigest`, `toolsDigest`, and cumulative `messagePrefixHashes` at exponentially spaced depths. Comparing adjacent calls distinguishes system/tool changes from message-prefix changes and bounds the first divergent message range even after the full fingerprint array is replaced by its persistence sentinel.
...
|---|---|---|---|
| `trace.metadata` | After `session.started` | harness, model, config, chunked plugin/skill/tool inventories, prompting, redaction snapshot | Yes (latch in `pi-event-bridge.ts`) |
...
Plugin, skill, and tool inventories use `{ count, chunks, truncated }`. Each inner chunk and the
outer chunk list stay within the persistence walker's 64-item array bound, so a live inventory is
not replaced wholesale by an array-limit sentinel. `count` is the unique pre-cap total and
`truncated` reports whether the 4,096-entry chunk grid omitted anything.
...
The trajectory bus-bridge maps typed event-bus events to trajectory events. The
`TRAJECTORY_BRIDGE_MAPPING` constant is the authoritative closed set.

<Info>
**Architecture invariant:** `test/architecture/trajectory-event-types-known.test.ts` asserts the disjoint-set invariant (no overlap between bridge-mapped types and direct-emit types) and a minimum bridge count.
</Info>
...
Seven forensic events that fire at most O(1) per turn are promoted from DEBUG to INFO so production daemons at `logLevel: info` retain the signal needed to diagnose the duplicate-adapter incident class.

At DEBUG, a production daemon at `logLevel: info` would silently discard a line like `Message enqueued` — making a duplicate-enqueue symptom invisible unless debug logging had been enabled before the incident.
...
**Why it matters:** without `step:` discipline, filtering by pipeline stage requires reading every log line. The target is ≥ 50% of `daemon.log` lines carrying a `step:` field.
...
Every row carries two optional provenance fields (readers that do not know
...
Users can also trigger export via the `/export-trajectory` slash command in an authenticated direct message. Group invocations are rejected before a bundle is created.
...
## 14. Worked example — duplicate adapter registration

This section shows how four observability signals surface duplicate adapter
registration.
...
All four signals are available in a single terminal workflow. Actual
investigation time depends on retention, deployment size, and the available
identifiers.
...
### 15.1 The recall-trace artifact

The recall trace is a per-recall JSONL artifact that records the ranking preview for each recall: which lanes matched, the fused order, the pre/post-rerank scores, the recency/temporal/proof/trust score components, and the include/exclude reason for every candidate. It is the sibling of the [cache trace](/agents/cache), with the same bounded writer family and rotation policy.

| Property | Value |
|---|---|
| Config key | `diagnostics.recallTrace.enabled` |
| Default | `true` — available from first boot; set `false` when local retention policy requires it |
...
It is enabled by default because the bounded, sanitized evidence must exist before a surprising recall occurs. You can override its cap or disable it explicitly:

```yaml
# config.yaml
diagnostics:
  recallTrace:
    enabled: true          # default — redacted, bounded JSONL
...
### 15.2 Recall & curation events

Typed events report recall, generation-quality, and curation activity on the event bus. Three of them — **`memory:recalled`**, **`memory:reranked`**, and **`memory:generation_quality`** — are part of the [§4 bridge mapping](#4-bridge-mapping): they cross onto the per-session **trajectory timeline** as `memory.recalled` / `memory.reranked` / `memory.generation_quality`. A recall shows up in `comis explain` (its lane/candidate/final counts and the rerank outcome) and aggregates into the report's [`recall` section](/reference/json-rpc#obs-explain). A memory-generation pass whose output diverged from its source fires `memory:generation_quality` (only on an issue) and rolls up into the `comis system-health` `generation_quality` finding. The remaining recall/curation events (`memory:entities_linked`, `memory:consolidated`, `memory:recall_used`, `memory:review_completed`) are emitted **directly** at a single canonical site each and are **not** bridge-mapped — `test/architecture/trajectory-event-types-known.test.ts` allowlists those four as not-trajectory-mapped, and asserts the disjoint-set invariant (no event is both bridged and direct-emit).

Every event payload is **counts, booleans, and IDs only** — never query text, memory bodies, or entity names (AGENTS.md §2.7). The per-recall ranking detail lives in the bounded recall-trace artifact above, not on the bus.
...
| `memory:generation_quality` | After a memory-generation pass (consolidation / reasoning / user-representation), **only on a detected issue** | `pass`, `sourceScript`, `outputScript`, `languageMismatch`, `emptyOutput`, `formatViolation` |
...
`memory:recalled.finalCount == 0` means recall returned nothing for that turn — a recall **miss**. When *every* recall in a **degraded** session missed (and no tool/context/breaker cause matched), `comis explain` root-causes it as `recall_miss` and points at the recall scope and (for non-Latin queries) the trigram-twin lanes. A turn that finalized as a **failure** is root-caused by that terminal failure instead (`execution_auth_failure` / `execution_dependency_failure` / `execution_terminal_failure`) — an empty memory store makes every recall a zero-hit, so the recall miss beside a dead turn is incidental. `memory:reranked.fellBack` / `.timedOut` flag the graceful-degradation paths (see below). `context:evicted` and `context:dag_compacted` both fire in DAG mode (the default engine) — `context:dag_compacted.durationMs` reports the real wall-clock of each compaction pass; `context:dag_expanded` reports an in-session zoom into the DAG (counts/`durationMs` only — never message or summary content). `context:mode_switched` fires only on a real engine-mode direction change and reports the one-time reconciliation `importedCount`; the mid-conversation `pipeline`⇄`dag` reconciliation that emits it is not yet implemented, so that one event is currently dormant.
...
For **aggregate** health (rather than a single recall), point at the `comis memory stats` recall-counter overlay: lane usage, rerank-fallback rate, consolidation throughput, and recall hit-rate. A climbing rerank-fallback rate, for instance, says the reranker is timing out across the system, not just on one turn. The overlay is best-effort — a daemon that has not wired the counters still renders base stats.
...
## 16. Spend Governance (Cost Kill-Switch)

The kill-switch turns the token/billing tracking of §17 into **enforcement**: a daemon-wide cost accumulator that can warn — or refuse new turns — once cumulative spend crosses a configured dollar ceiling. It ships **off and observe-only**, and is honest about what it cannot price. Configure it under `observability.spend` (config-yaml only, no env var — see the [config reference](/reference/config-yaml)).

### Ships off, opt-in to abort

All three ceilings default to `null` (off) and `action` defaults to `warn`. A deployment that does not opt in is never enforced and cannot be DoS-ed by a fat-fingered cap. Turning it on is two steps: set a ceiling (`perAgentUsd`, `perTenantUsd`, and/or `daemonGlobalUsd`) to observe in `warn` mode, then flip `action: abort` once you trust the numbers.

| Dimension | Key | Scope |
|-----------|-----|-------|
| Per `(tenant, agent)` | `perAgentUsd` | one agent's cumulative spend |
| Per tenant | `perTenantUsd` | all agents of one tenant — the cross-tenant isolation dimension; a breach aborts only that tenant, never a neighbour |
| Daemon-global | `daemonGlobalUsd` | every agent and tenant on the daemon |

### Restart-durable accumulator

There is one source of truth: an in-memory accumulator, **rehydrated at boot** from the persisted `obs_token_usage.cost_total` rows (a rolling per-agent `SUM`) and **incremented live** from the `observability:token_usage` event the persistence subscriber already consumes. The persisted rows are its durability — a restart resumes the running total rather than resetting the ceiling to zero. (If the observability store is unavailable at boot the accumulator starts at `$0` — honest degradation, not a silent reset of a live ceiling.)

### Cooperative abort

Abort is **cooperative**, not a hard mid-turn kill — and this is the contract, not a hidden detail:

- A breach **halts subsequent turns** (the next admission is rejected).
- **In-flight turns complete and bill** — a turn already running when the ceiling trips is not torn down.
- The **realized ceiling is therefore `configured + in-flight overshoot`**, never exactly the configured number.

Admission is an **atomic check-and-reserve**: at the start of each turn the bridge reserves a conservative `perTurnMax` (there is no pre-flight cost estimate at that point), then reconciles the reservation to the actual billed amount after the turn. Because the reserve is synchronous, concurrent callers serialize and each sees the prior reservation — so the in-flight overshoot is bounded by `concurrency × perTurnMax`, not unbounded. A breach routes through the existing budget path and surfaces as `execution:aborted { reason: "spend_exceeded" }` / an `ExecutionResult.finishReason` of `spend_exceeded`. The graph `maxCost` control interoperates through the same seam.

### Three-state honest pricing

The kill-switch is **fail-loud, not fail-open** about pricing. Every model resolves to one of three states:

| State | Meaning | Enforcement |
|-------|---------|-------------|
| `priced` | a catalog rate exists | enforced against the ceilings |
| `free` | a local / on-device model (genuine `$0`) | counts as honest `$0`, **never** trips a ceiling — a local-first deployment is never false-DoS-ed |
| `unknown` | a **remote** model with no catalog entry | fires `observability:spend_unpriceable`; trips the kill-switch only under `action: abort` **and** `onUnknownPricing: abort` |

A *transient* pricing-fetch failure is different from `unknown`: it falls back to the dated `pricingFallback: snapshot` rate and **still enforces** the ceiling — a flaky lookup never causes a false abort, and never silently stops counting. This is the guard against the chimeric-model class of incident, where a remote model burned tokens under a phantom price.

### Events (counts-only, content-free)

Three typed events fire on the bus. All are **content-free** per the [§5 bounding rules](#5-defense-in-depth-bounding): dollar amounts as numbers, scope as the closed `agent | tenant | global` enum, and ids only — never a message, prompt, or query body.

| Event | When |
|-------|------|
| `observability:spend_warning` | early — spend crosses `warnAtFraction` (default 0.8) of a ceiling, before it trips. Carries `{ scope, spentUsd, capUsd, fraction }`. |
| `observability:spend_exceeded` | a ceiling was reached — the kill-switch tripped for that scope. Carries `{ scope, spentUsd, capUsd, estUsd }`. Emitted under both `warn` and `abort`. |
| `observability:spend_unpriceable` | a remote model burned tokens with unknown pricing. Carries `{ provider, model }` (config ids, not user content). |

```bash
jq 'select(.event | startswith("observability:spend_"))' ~/.comis/logs/daemon.log
```

### System finding — `config_posture:pricing_gap`

`comis system-health` surfaces a `config_posture:pricing_gap` finding when one or more agents are burning tokens on **remote, unknown-priced** models — telling an operator their pricing coverage *before* they trust the ceiling. It is a **count plus a remediation hint only** (no model id or token in the body); free/local and priced agents are never counted. A newer healthy boot supersedes an older flag. If you see it, spend is being under-counted for those agents (honest `$0` applies only to local/free providers) — close the gap before relying on `action: abort`.

---

## 17. Additional Tracking Surfaces

<Note>
The following tracking surfaces operate continuously alongside the diagnostics above. They are referenced by the web dashboard at [/web-dashboard/observability](/web-dashboard/observability).
</Note>
