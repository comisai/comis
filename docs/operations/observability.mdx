...
`memory:recalled.finalCount == 0` means recall returned nothing for that turn. `memory:reranked.fellBack` / `.timedOut` flag the graceful-degradation paths (see below). `context:mode_switched` fires only on a real direction change — never on a brand-new DAG-default conversation — and reports the one-time reconciliation `importedCount`.
