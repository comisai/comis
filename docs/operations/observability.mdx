...
This page documents the observability foundations. It covers the trajectory layer, the 65-entry bridge mapping, lifecycle envelopes, forensic INFO promotions, the dedup detector, boot invariants, the alert budget, `step:` discipline, log rotation, the memory & recall diagnostics (recall-trace artifact, recall events, degradation signals), and the bundle export entry point. If you are new to Comis observability, start here; then follow the cross-links for operator workflows.
...
## 4. Bridge Mapping (65 entries)

The trajectory bus-bridge maps 65 event types from the typed event bus to trajectory events (up from 18 previously — a ~3.6× coverage improvement).

<Info>
**Architecture invariant:** `test/architecture/trajectory-event-types-known.test.ts` asserts the disjoint-set invariant (no overlap between bridge-mapped types and direct-emit types) and a bridge count ≥ 45. Current count: 65.
</Info>
...
Typed events report recall and curation activity on the event bus. Two of them — **`memory:recalled`** and **`memory:reranked`** — are part of the [§4 bridge mapping](#4-bridge-mapping-65-entries) (RECALL-01): they cross onto the per-session **trajectory timeline** as `memory.recalled` / `memory.reranked`, so a recall now shows up in `comis explain` (its lane/candidate/final counts and the rerank outcome) and aggregates into the report's [`recall` section](/reference/json-rpc#obs-explain). The remaining recall/curation events (`memory:entities_linked`, `memory:consolidated`, `memory:recall_used`, `memory:review_completed`) are emitted **directly** at a single canonical site each and are **not** bridge-mapped — `test/architecture/trajectory-event-types-known.test.ts` allowlists those four as not-trajectory-mapped, and asserts the disjoint-set invariant (no event is both bridged and direct-emit).
