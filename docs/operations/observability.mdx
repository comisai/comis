...
`memory:recalled.finalCount == 0` means recall returned nothing for that turn. `memory:reranked.fellBack` / `.timedOut` flag the graceful-degradation paths (see below). `context:mode_switched` fires only on a real engine-mode direction change and reports the one-time reconciliation `importedCount`. The `dag` (LCD) engine is available as an opt-in mode, but the mid-conversation `pipeline`⇄`dag` reconciliation that emits this event lands in a later phase, so the event is currently dormant.
