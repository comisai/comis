...
Read the trace with [`comis memory recall-trace <session>`](/reference/cli#comis-memory) — `--format json` for the full per-record ranking breakdown, the table view for correlation keys.
