...
<Note>
The OTLP transport ships as `http/protobuf` this release. Setting
`protocol: "grpc"` validates, but the daemon falls back to `http/protobuf` with a
logged warning (gRPC is a documented later addition — no silent wrong-transport).
</Note>
...
and a PII surface). Secrets and content are re-redacted at the exporter boundary
independently of any upstream scrubbing, so nothing leaks even with the GenAI
semantic-convention attributes enabled.
...
`@comis/observability-otel`; the shipped dashboards reference only metrics in that
catalog (a CI drift guard fails the build if a panel ever references a metric the
...
(outcome, cost, per-tool failures, breaker timeline, and — since the cost/audit
work — the cache-break and audit sections).
