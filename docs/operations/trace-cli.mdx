...
All subcommands support `--json` for machine-readable output (CLI-07).

---

## `--message-id <uuid>`

Search for all trace events associated with a specific inbound message (CLI-01). Use this when a
...
Search all log and trajectory events by traceId (CLI-02). Use this when you've found a traceId in
...
Stream trace events live for a specific chat (CLI-03). Use this when a user reports an issue
...
Scan the session index for recent failures (CLI-04). Use this when you need a quick overview of
...
Export a self-contained diagnostic bundle for a session (CLI-05). Invokes the Phase 4 bundle
...
This is the M3 acceptance gate — under 5 minutes from complaint to bundle.
...
Three commands. Under 5 minutes from complaint to bundle. This is the M3 acceptance gate.
