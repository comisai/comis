...
An agent attaches a gate when it creates or edits a job -- pass the gate source as **`wake_gate_script`** (and optionally **`wake_gate_language`**, `js` or `ts`) to the `cron` tool's `add` or `update` action.
...
```javascript
// wake-gate script (language: js)
const res = await tools.web_fetch({ url: "https://ci.example.com/api/queue" });
...
if (queue.pending.length === 0) {
  ...
  // Wake the agent with the pending items pre-gathered as context.
  console.log(JSON.stringify({ wake: true, context: `Pending builds: ${queue.pending.join(", ")}` }));
}
```
