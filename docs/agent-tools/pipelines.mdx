...
The `pipeline` tool lets agents define, execute, and manage multi-agent workflows as execution graphs (DAGs). Each node in a graph is a sub-agent task that runs independently. Nodes can execute in parallel when they have no dependencies, or wait for upstream nodes to complete before starting.

For conceptual background on execution graphs, see [Execution Graphs](/agents/execution-graphs).
