...
An invalid intent (for example `debate` with fewer than two agents) is rejected before any graph runs. `from_intent` is gated by the `orchestration.authoring.intentAction` config flag (default `false`); when the flag is off the daemon refuses the call. Each successful synthesis emits a `graph:synthesized_from_intent` audit event.
