...
| `sessions_list` | List the caller conversation and delegated children |
...
| `memory_manage` | Create, update, or delete memory entries (admin CRUD: stats, browse, delete, forget, flush, export, pin, unpin) |
...
The four session tools (`session_search`, `session_status`, `sessions_history`, `sessions_list`) let agents inspect their current conversation and directly delegated child sessions. Authenticated operator RPC calls use a separate control-plane authority.

<AccordionGroup>
  <Accordion title="session_status -- View current session">
    Shows the current session's agent name, model, token and cost totals, and step usage.
  ...
  <Accordion title="sessions_list -- List active sessions">
    Lists the caller conversation and directly delegated child sessions, with optional filters by session kind and recency.

    **Parameters:**

    | Parameter | Type | Required | Description |
    |-----------|------|----------|-------------|
    | `tenant_id` | string | Yes | Tenant that owns the sessions. |
    | `agent_id` | string | Yes | Agent whose sessions should be listed. |
    ...
    Returns the [`session.list`](/reference/json-rpc#session-list) rows unchanged,
    including the complete authoritative endpoint on endpoint-bound sessions.
  </Accordion>

  <Accordion title="sessions_history -- Browse conversation history">
    Retrieves history for the exact caller conversation or a directly delegated child using its durable conversation identity.
  ...
  <Accordion title="session_search -- Search/browse conversation history">
    ...
    **Parameters:**
    ...
    The tenant and agent scope is derived from the calling agent's authenticated context -- there are no `tenant_id` / `agent_id` parameters.
...
<AccordionGroup>
  <Accordion title="sessions_send -- Send a message to another session">
    ...
    **Parameters:**
    ...
    | Parameter | Type | Required | Description |
    |-----------|------|----------|-------------|
    | `tenant_id` | string | Yes | Tenant that owns the target conversation. |
    | `agent_id` | string | Yes | Agent that owns the target conversation. |
    | `conversation_ref` | string | Yes | Opaque durable reference returned by `sessions_list`. |
    ...
    **Example -- Send a notification:**
    ...
    tenant_id: "default"
    agent_id: "ops-bot"
    conversation_ref: "cv_DsMcSBa9dmYP7ptzs_Xd7CMHOi6tliEQVzCbN1oOD4q"
  ...
  <Accordion title="sessions_spawn -- Spawn a sub-agent session">
    ...
    **Parameters:**
    ...
    | `async` | boolean | No | Accepted for explicit asynchronous intent; every spawn runs in the background and returns an internal run handle immediately. |
    | `delegation_scope` | `"whole_request"` or `"partial"` | No | Declares ownership assigned to the child. Use `whole_request` only when it owns every requested outcome; use `partial` when the parent retains distinct work. Omitting the field does not transfer whole-request ownership. |
    ...
    | `expected_outputs` | string[] | No | File paths the sub-agent must produce. Validated after execution: a run that stops cleanly without writing every listed path is reported to the parent as **failed** (`errorKind: validation`), not completed, and the missing paths ride the announcement. Files that pass validation are scheduled as governed completion attachments when automatic channel delivery owns the result, including when another declared output is missing. Declaring outputs is therefore how you stop a child from silently returning nothing and request delivery of its verified files. |
    | `max_steps` | integer | No | Maximum execution steps (floor of 30, default 50, capped at config default) |
    `sessions_spawn` never blocks for the child result. Its tool result returns an internal run handle and a background-running marker immediately; a queued spawn additionally returns `queued: true`. The handle is for later `subagents` calls and is not copied into the user-facing launch acknowledgement. Absent an active parent wait, the completion is delivered automatically to the immutable originating conversation, so the model-facing tool cannot select an announcement channel. When that automatic completion route is available, accepting the spawn ends the foreground workflow with a neutral pending acknowledgement instead of repeating the delegated work or emitting a second terminal completion. Use `subagents` with `action: wait` to make the current parent turn own the result without polling.
  ...
  <Accordion title="subagents -- Manage running sub-agents">
    ...
    **Parameters:**
    ...
    | `timeout_ms` | integer | No | Requested wait deadline from 0 to 300000 ms. The model-facing tool caps the effective interval at one third of the owning agent's `promptTimeout.promptTimeoutMs` (minimum 1 ms), so an unfinished wait yields control before the parent reaches its no-progress deadline. |
    ...
    **Actions:**
    ...
    | `wait` | Await selected owned runs, or all active direct children when `run_ids` is omitted. Each result is `completed`, `denied_unknown`, `timeout`, or `cancelled`; terminal completions retain `completed`, `failed`, `killed`, `watchdog_timeout`, or `ghost_sweep` as the end reason. Requested and effective wait budgets are recorded so `comis explain` can distinguish a bounded wait from the parent deadline. |
...
<AccordionGroup>
  ...
  <Accordion title="memory_manage -- Admin CRUD">
    Admin-only memory CRUD operations. Requires admin trust level. The `delete`, `forget`, and `flush` sub-actions require user approval. `forget` searches the authorized memory scope and removes every matching explicit or paired entry. `pin`/`unpin` mark a single entry as always-injected in recall.

    **Parameters:**

    | Parameter | Type | Required | Description |
    |-----------|------|----------|-------------|
    | `manage_action` | string | Yes | One of `stats`, `browse`, `delete`, `forget`, `flush`, `export`, `pin`, `unpin` |
    ...
    | `query` | string | For forget | Natural-language memory to remove from every matching scoped entry |
...
```yaml
# Spawn three researchers; every call returns an internal run handle immediately
# Then use subagents wait with those handles to collect terminal results
```
