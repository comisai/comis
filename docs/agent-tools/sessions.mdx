...
<AccordionGroup>
  ...
  <Accordion title="sessions_spawn -- Spawn a sub-agent session">
    ...
    **Parameters:**
    ...
    | `expected_outputs` | string[] | No | File paths the sub-agent must produce. Validated after execution: a run that stops cleanly without writing every listed path is reported to the parent as **failed** (`errorKind: validation`), not completed, and the missing paths ride the announcement. Declaring outputs is therefore how you stop a child from silently returning nothing. |
