...
<Warning>
**Endpoint authority (security).** Every `message` call resolves to one complete
endpoint -- channel type, configured channel instance, conversation ID, optional
thread, and direct/shared kind -- before the adapter runs. The turn's own
endpoint is used whenever it matches the requested `channel_type` /
`channel_id`. When it does not, the daemon may substitute a complete endpoint
that ingress **already observed** for this agent, and only when that
`(agent, channel type, conversation)` triple resolves to exactly one such
endpoint; zero matches or an ambiguous set fails closed. The tool's
`channel_type` and `channel_id` fields still only select among endpoints the
runtime has authority for -- they never mint one from raw coordinates, and an
explicit `endpoint` that conflicts with the authenticated turn is rejected.
Cross-channel operations additionally require admin trust. This prevents a
prompt-injected agent from crossing account, thread, or conversation boundaries.
</Warning>
...
<AccordionGroup>
  <Accordion title="send -- Send a new message">
    Sends a new message to an endpoint the runtime holds authority for. An inbound turn uses its source endpoint; a scheduled or heartbeat turn sends once the run is bound to an exact endpoint, or to a uniquely observed endpoint resolved under the authority rules above.
  ...
  <Accordion title="attach -- Send a message with a file">
    ...
    **Parameters:**
    ...
    | `attachment_url` | string | Yes | URL or workspace path of the file to send: `http://`, `https://`, `file://`, an absolute path, or a path relative to the workspace root (the form `find` returns). Relative paths resolve against the calling agent's workspace and cannot escape it. |
...
sufficient authority — the **Endpoint authority** warning at the top of this
page states the exact resolution rule. Keeping the complete endpoint (channel instance, thread,
direct/shared kind) authoritative is what keeps separate accounts, threads, and
conversations distinct even when a platform reuses the same visible ID.

Use `notify_user` when delivery should resolve a configured or tracked endpoint
rather than one this turn already has authority for. Notification resolution
accepts only complete endpoints recorded from authoritative session activity and
