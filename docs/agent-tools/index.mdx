...
In the default **pipeline** mode, the `session_search` tool searches the raw
session history (see [Sessions](/agent-tools/sessions)). The in-session context
inspection/recall tools (`ctx_search`, `ctx_inspect`, `ctx_recall`, `ctx_expand`)
belong to the DAG (LCD) context engine. That engine is available as an opt-in mode
(`contextEngine.version: "dag"`), but these on-demand recall tools land in a later
phase and are **not** present yet — the current release does lossless verbatim
assembly without recall tools. See
[Context Management](/agents/context-management#dag-mode-opt-in).
