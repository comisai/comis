...
description: "Guide to the agent-facing tool families"
icon: "toolbox"
---

Comis ships with **72 built-in agent tools** spanning file operations, web access, messaging, scheduling, media processing, browser automation, context management, interactive terminal sessions, and administration. This page is the master reference -- every tool listed with its category, description, and link to detailed documentation.
...
The tools listed below are the complete built-in surface. Each agent sees the subset enabled by its runtime dependencies, security boundary, and tool policy. Each tool is linked to its detailed guide page where you will find parameters, examples, and tips for effective use.
...
|------|-------|-------------|
| **Built-in tools** | 72 | Core capabilities shipped with Comis -- file operations, shell commands, web search, browser automation, messaging, scheduling, sessions, media, infrastructure, context, interactive terminal sessions, supervisor admin, and platform actions. Registered when their dependencies and feature gates are enabled, then filtered by tool policy. |
...
| **Custom skills** | Unlimited | Markdown-defined "mini-programs" with optional tool restrictions and bin/env requirements. Comis ships bundled skills, and you can author your own. See [Skills](/skills/index). |

Tool access is controlled by [tool policy](/skills/tool-policy). By default, policy does not further restrict the tools enabled by the runtime, but you can narrow access per agent using profiles (like `coding` or `messaging`) or fine-grained allow/deny lists.
...
### Process & Execution (3 tools)

| Tool | Description | Guide |
|------|-------------|-------|
| `exec` | Shell execution with restricted-binary policy and OS sandboxing when a supported provider is available. Otherwise it warns and runs on the host | [Built-in Tools](/skills/built-in-tools) |
| `process` | Background process lifecycle (spawn, kill, status, logs) | [Built-in Tools](/skills/built-in-tools) |
| `sleep` | Pause once for a bounded duration while background work finishes | [Built-in Tools](/skills/built-in-tools) |
...
| `terminal_session_read` | Read the redacted rendered screen and settled exit detail. | [Terminal Driver](/agent-tools/terminal-driver) |
...
| `terminal_session_status` | Inspect classifier state, confidence, reason, and cursor. | [Terminal Driver](/agent-tools/terminal-driver) |
...
The message tool provides one interface across supported platforms for sending,
replying, reacting, editing, deleting, fetching history, and attaching files at
the current authoritative conversation endpoint. Use `notify_user` for
proactive delivery that must resolve a configured or recently tracked endpoint.
...
| `sessions_list` | List the caller conversation and delegated children | [Sessions](/agent-tools/sessions) |
...
| `sessions_spawn` | Start a background sub-agent with artifact refs and an optional objective | [Sessions](/agent-tools/sessions) |
| `subagents` | Manage sub-agents (`list`, `wait`, `kill`, `steer`) | [Sessions](/agent-tools/sessions) |

### Pipelines & Orchestration (2 tools)
...
| `orchestrate` | Run a capability-scoped tool script in a jailed child process | [Orchestrate](/agent-tools/orchestrate) |
...
| `video_generate` | Submit an asynchronous video-generation job | [Media](/agent-tools/media) |
| `video_status` | Check the progress and result of a video-generation job | [Media](/agent-tools/media) |

### Memory

Memory tools give agents access to long-term memory. The review and reflection
jobs can build memory over time, and these tools let an agent explicitly search,
read, and store information. Memory uses hybrid text and vector search for
semantic matching.

The recall boundary is `(tenant, agent)`, not an individual user, chat, or
sender. An agent shared by several people can recall a fact learned from one
person while speaking to another. The `learned` trust label controls policy and
ranking; it does not prove that content is reliable. Governed-learning
documents are advisory Markdown, and read-only candidates can surface before
promotion. Warning-level harmful language can pass their static validator.
An explicitly trusted owner correction in `single_owner` mode immediately stales
the attributed procedure; other correction sources retain corroboration and
trend gates and do not guarantee demotion.
...
<Warning>
Supervisor tools can modify agents, delete sessions, flush memory, and manage gateway tokens. Grant these tools only to trusted agents that need deployment-wide control. See [tool policy](/skills/tool-policy) for how to restrict access.
</Warning>

| Tool | Description | Guide |
|------|-------------|-------|
| `agents_manage` | Agent agent administration (create, get, update, delete, suspend, resume) | [Infrastructure](/agent-tools/infrastructure) |
| `obs_query` | Read-only observability queries -- diagnostics, cost, delivery, channel health, incident reports, and the security audit log | [Infrastructure](/agent-tools/infrastructure) |
...
<CardGroup cols={3}>
  ...
  <Card title="Infrastructure" icon="server" href="/agent-tools/infrastructure">
    Configuration, gateway control, and agent administration
  </Card>
