...
|------|-------|-------------|
| **Built-in tools** | 70 | Core capabilities shipped with Comis -- file operations, shell commands, web search, browser automation, messaging, scheduling, sessions, media, infrastructure, context, interactive terminal sessions, supervisor admin, and platform actions. Available on every agent unless restricted by tool policy. |
...
Interactive terminal sessions for driving allowlisted binaries (added in v2.11). Requires an operator-configured `allowEntries` allowlist -- without a matching entry the tool rejects. Requires bubblewrap sandbox on Linux. See [exec sandbox](/security/exec-sandbox).
...
| `memory_ask` | Grounded natural-language answer over the agent's recall pipeline. Opt-in: requires `dialectic.enabled: true` in agent config. | [Sessions](/agent-tools/sessions#memory-tools-long-term-memory) |
...
In pipeline mode, four individual tools expose the same underlying operations. In DAG mode, `context_tool` is used instead.

| Tool | Description | Guide |
|------|-------------|-------|
| `context_tool` | Unified context DAG management with 4 actions: `search`, `recall`, `inspect`, `expand` | [Context Management](/agents/context-management) |
| `ctx_expand` | Expand context entries (pipeline mode). In DAG mode, use `context_tool` instead. | [Context Management](/agents/context-management) |
| `ctx_inspect` | Inspect individual context entries (pipeline mode). In DAG mode, use `context_tool` instead. | [Context Management](/agents/context-management) |
| `ctx_recall` | Recall context by semantic query (pipeline mode). In DAG mode, use `context_tool` instead. | [Context Management](/agents/context-management) |
| `ctx_search` | Search context by keyword (pipeline mode). In DAG mode, use `context_tool` instead. | [Context Management](/agents/context-management) |
