...
<Warning>
Every platform action is also bound to the complete endpoint for the current
turn. Include the platform's conversation coordinate (`channel_id`, `chat_id`,
or `group_jid`) even for an action whose platform API primarily targets a
server, user, or bot setting. The coordinate must match the authoritative
channel instance, conversation, optional thread, and direct/shared kind already
resolved for the turn; missing or conflicting authority fails before the
adapter action runs.
</Warning>
