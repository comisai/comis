...
Platform action tools give agents moderation and management capabilities specific to each chat platform. Each platform has its own tool (`discord_action`, `telegram_action`, `slack_action`, `whatsapp_action`) with actions tailored to that platform's features.

## Quick Reference

| Platform | Tool Name | Actions |
|----------|-----------|---------|
| Discord | `discord_action` | 19 actions |
| Telegram | `telegram_action` | 12 actions |
| Slack | `slack_action` | 12 actions |
| WhatsApp | `whatsapp_action` | 11 actions |

All platform action tools follow the same pattern: call the tool with an `action` parameter specifying what to do, plus any additional parameters that action requires.

## Discord Actions

The `discord_action` tool provides 19 actions for managing Discord servers, channels, threads, roles, and member moderation.
...
The `telegram_action` tool provides 12 actions for managing Telegram groups and channels.
...
The `slack_action` tool provides 12 actions for managing Slack workspaces and channels.
...
The `whatsapp_action` tool provides 11 actions for managing WhatsApp groups.
