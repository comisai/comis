...
- An authority-backed linked Git worktree receives lease-private worktree administration, refs, config, and writable objects. Git resolves existing objects through the repository's read-only shared object database, while its effective common directory contains no shared hooks or sibling-worktree metadata. Commits therefore persist inside the leased workspace without advancing a shared branch or changing another worktree.
