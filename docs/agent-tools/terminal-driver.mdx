...
**Per-project folders.** Pass `project: "<name>"` to `terminal_session_create` and the session opens in a dedicated, auto-created folder `<workspace>/projects/<name>`. Pass the **same** name to keep working on, fix, or extend an existing project; **different** names give isolated folders — that is how multiple projects run **in parallel** (each its own session + folder, no collisions). An explicit `cwd` is also honored, but only if it resolves *within* the session workspace (otherwise it is clamped to the workspace — the jail-escape guard), so `project` is the simpler, path-free way to get isolation.
