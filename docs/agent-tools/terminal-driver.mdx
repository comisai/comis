...
- Every managed terminal end-of-life path uses the same barrier: explicit kill, reaper eviction, and unattended never-tasked cleanup confirm backend termination, durably retire the exact terminal binding, and only then delete registry authority or cap state. Capability-service notification remains best-effort; endpoint loss never skips the authoritative local retirement, and a local-store failure retains the handle for retry.
- Managed launch fails closed unless its descriptor is durably written before backend creation and updated with the resolved root-process identity. A natural exit retains that descriptor until the exact run, lease, and terminal binding is durably retired.
