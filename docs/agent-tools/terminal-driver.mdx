...
- The session working directory is `<agent-workspace>/projects` — `~/.comis/workspace/projects` for the default agent, `~/.comis/workspace-<agentId>/projects` for a named agent — with each named project under `projects/<name>`. A full, milestone-length run (e.g. driving Claude Code to build a project) leaves its output there after the session ends.
