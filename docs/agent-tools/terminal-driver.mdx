...
| `terminal_session_read` | Read the rendered screen + cursor (`text`/`ansi`/`html`, scrollback). After process exit, the result also carries the known exit code and a bounded final output tail. Screen and exit-tail output are ANSI-cleaned where applicable, redacted, and wrapped as untrusted content. |
  ...
  default:
    ...
    skills:
      terminal:
        ...
        redactSecrets: true
        audit: { enabled: true }
        # unsafeDisableSandbox: false   # DANGEROUS opt-out of the bwrap jail (see Security model).
                                        # Only for hosts that cannot run bwrap; leave false otherwise.
...
| `ephemeralWritablePaths` | `string[]` — existing directories replaced by empty, jail-private tmpfs mounts (`~` expands to home). Use this for narrowly scoped runtime state beneath a read-only credential directory. Each host target must already exist as a real directory; an unavailable target rejects the session before bubblewrap starts and names this config field. | `[]` |
...
- **`unsafeDisableSandbox` (default `false`, DANGEROUS):** the operator opt-out of the jail — the peer of `browser.noSandbox`, for constrained hosts that genuinely cannot run bwrap (a container without unprivileged user-namespaces, a locked-down CI box). When `true`, `session_create` runs the driven CLI **directly** instead of failing closed, with **no filesystem / network / uid confinement** (the `scope` dimensions are unenforceable without the jail). The **env-scrub is still enforced and never optional**: it strips daemon secrets (gateway token / master key) from the child env. A **durable `backend: "tmux"` drive is preserved** (session persistence survives a daemon restart even with the sandbox off): the tmux server env stays clean without the jail's `--unsetenv` because the server is only ever started by `new-session` running the already-scrubbed env, and **each drive gets its own tmux server** on its own socket — so a rotated secret (e.g. an ADO PAT) reaches each new drive rather than the value an earlier drive's server booted with. Env values are never passed on the tmux command line, where `/proc/<pid>/cmdline` would make them readable by any local user. It is surfaced at boot in `comis system-health` config-posture (`terminalUnsafeDisableSandbox`), and it is **operator-file-only**: no runtime RPC can set it — neither `config.patch` (the whole `agents.*` prefix is immutable there) **nor** `agents_manage` / `agents.update` (which otherwise writes agent config) — so an admin-trust agent can never self-enable it; only editing the config file and restarting the daemon can. The same file-only rule covers `skills.execSandbox` (the exec OS-sandbox switch), `skills.terminal.allow` (the command allowlist), and the sender-authority keys under `elevatedReply` ([Capability model](/security/capability-model#deny-by-origin-on-the-control-plane)). Leave it `false` on any host where bwrap is available.
...
- **Env-scrub:** interpreter-control vars (`NODE_OPTIONS`, `BASH_ENV`, `PYTHONSTARTUP`, function-export) and nested-CLI markers (`CLAUDE_CODE_*`, `CLAUDECODE`) are stripped. The daemon's `NODE_ENV` is stripped too — not as a security vector but so a driven `pnpm`/`npm`/build tool starts from its own defaults instead of inheriting the daemon's `production` mode (which prunes devDependencies in the workspace the user is developing in).
- **Managed execution attachments:** when exactly one run-bound socket is authorized, Comis mounts its relay at the fixed attachment path and injects `COMIS_EXECUTION_ATTACHMENT`, `COMIS_EXECUTION_ATTACHMENT_TARGET_NAME`, and the public authentication value `COMIS_EXECUTION_ATTACHMENT_IDENTITY`. All three keys are removed from the inherited environment first, and remain absent when the host did not resolve exactly one attachment. For a durable managed terminal, the relay runs as a detached session-bound helper: it survives terminal-worker replacement during daemon restart and retires when the owning tmux session disappears.
...
- An authority-backed linked Git worktree receives lease-private worktree administration, refs, config, and writable objects. The writable source directories live beneath the daemon-controlled Git worktree administration; paths inside the worker-writable workspace are mount targets only. A concurrent terminal therefore cannot replace a validated bind source before bubblewrap resolves it. Git resolves existing objects through the repository's read-only shared object database, while its effective common directory contains no shared hooks or sibling-worktree metadata. Commits therefore persist without advancing shared refs, retain the leased branch ancestry, and remain fetchable by an explicit host-side candidate handoff. After verifying that the private administration belongs to the linked worktree, Comis adds its root-anchored directory name to the repository-local exclude file so host Git status ignores the runtime metadata while continuing to report unrelated files.
- Before every authority-backed launch, the daemon materializes and validates lease-private Git administration after approval and before reserving a terminal. The sandboxed worker re-validates those paths before mounting them but does not need write access to an arbitrary leased worktree. Symlinked or escaped paths fail before reservation; failures after reservation confirm termination and retire that binding before the workspace can be released.
- Every managed terminal end-of-life path uses the same barrier: explicit kill, reaper eviction, unattended never-tasked cleanup, and failed launch admission issue the named tmux termination and require `has-session` to confirm absence, durably retire the exact terminal binding, and only then delete registry authority or cap state. End-of-life capability-service notification remains best-effort; endpoint loss never skips the authoritative local retirement, and a local-store failure retains the handle for retry.
- A managed create succeeds only after the owning capability service acknowledges both the `created` and `running` transitions. A service can reject admission because its task posture, dependency graph, or worker scheduling capacity does not permit the launch. Comis then confirms terminal termination, retires the binding, and returns a tool error instead of exposing an unauthorized session. The warning records the protocol `reasonCode` as a `precondition` failure and directs the operator to those service-owned controls; transport and endpoint failures remain `dependency` failures with socket and instance guidance.
- Managed launch fails closed unless its descriptor is durably written before backend creation and atomically replaced with the resolved root-process identity. Descriptor replacement uses a flushed temporary file, atomic rename, and parent-directory flush, so a failed update keeps the prior cleanup-capable descriptor. Descriptor deletion is also confirmed before registry authority is discarded.
- Managed restart recovery re-queries the detached tmux pane PID and its process-start identity. Comis re-attaches only when both exactly match the persisted identity; a missing or mismatched identity is retained as cleanup-only authority and is never driven as the original run.
...
- **`listed-hosts`** -- a no-secret host-allowlist CONNECT proxy bounds the child to named `hosts`. It works for clients that honor `HTTPS_PROXY`. For a durable tmux drive, the proxy runs as a detached session-bound helper: it survives replacement of the terminal worker during a daemon restart and retires itself when the tmux session disappears, so the surviving jail keeps usable provider egress without widening the allowlist. Driving a CLI whose HTTP client ignores `HTTPS_PROXY` through `listed-hosts` is still being hardened (a transparent-egress path) -- use `full` for those today.
...
A drive's notifications -- the "running in the background" promotion, the heartbeat, the `done`/`failed` outcome, the completion notice, and every escalation -- go back to **the conversation the drive was started from**. The session records that origin at `terminal_session_create` (from the resolved turn scope) and keeps it for the drive's whole life: a **durable** drive persists it with its descriptor, so a drive re-attached after a daemon restart still reports to the same thread hours later.

This matters most when several drives run at once. Delivery resolves in the order **origin -> `notification.primaryChannel` -> most recent session**, so without a recorded origin a drive started in one chat would report to whichever conversation happened to be active when it finished -- and an escalation delivered to the wrong thread is an escalation nobody answers, leaving the drive parked.

A drive with **no channel origin** -- one started from the HTTP API or a cron turn -- has nothing to pin, and falls through to `notification.primaryChannel`, then the most recent session. Set `primaryChannel` for the agent if you run unattended drives and want their outcomes somewhere predictable.

The origin is a **delivery destination only**. It is captured server-side from the request context and is never a tool parameter, so a prompt-injected agent cannot redirect its own escalations into another conversation; it grants no privilege, and the delivery authority is still minted by the daemon's notification boundary rather than by the originating chat.
