...
**Nested sandboxes (Claude Code's Bash tool).** Some AI CLIs sandbox their _own_ subprocesses. Claude Code, on Linux, wraps each Bash-tool command in a second bubblewrap that remounts `$HOME` read-only — and nested inside this driver's jail that inner sandbox can't provision its own state, failing with `EROFS … mkdir ~/.claude/session-env/…`. The symptom is a claude that can **author** files but cannot **run** them (its Bash tool is dead while Read/Write/Edit work). The driver avoids this by telling the driven CLI it is **already** sandboxed — it sets `CLAUDE_CODE_BUBBLEWRAP=1` in the child environment — so claude skips the redundant inner sandbox and runs Bash commands directly inside this jail, which is already the enforced security boundary (the filesystem/network scope above, with `~/.comis` always masked). This is automatic; no configuration is required.
