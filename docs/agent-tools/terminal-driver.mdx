...
- Every managed terminal end-of-life path uses the same barrier: explicit kill, reaper eviction, and unattended never-tasked cleanup issue the named tmux termination and require `has-session` to confirm absence, durably retire the exact terminal binding, and only then delete registry authority or cap state. Capability-service notification remains best-effort; endpoint loss never skips the authoritative local retirement, and a local-store failure retains the handle for retry.
- A capability service can reject a terminal transition because its task posture, dependency graph, or worker scheduling capacity does not permit the launch. The warning records the protocol `reasonCode` as a `precondition` failure and directs the operator to those service-owned controls; transport and endpoint failures remain `dependency` failures with socket and instance guidance.
