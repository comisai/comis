...
The web tools give your agents access to the internet. `web_search` queries search engines (with 8 provider options), and `web_fetch` retrieves and reads web page content. Both tools include security protections against accessing internal networks.

Web tools are among the most commonly used agent capabilities. They let your agent look up current information, read documentation, check prices, verify facts, and stay informed about topics that go beyond its training data.

Both tools are enabled by default in the `full` tool policy profile.
...
| `provider` | string | No | Specific provider to use (defaults to the first available in your configuration) |
