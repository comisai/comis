...
in-jail `.read(offset?, limit?)` / `.grep(pattern)` / `.jq(expr)` extraction methods. The
`results/` directory is bounded (a per-file cap, a per-run aggregate cap, and a TTL) and
auto-cleaned when the run ends.
