...
is **off in every default profile** — the one genuinely new default-off capability of this surface
(contrast the write/message surfaces below, whose capabilities are already on in `standard`).
...
<Warning>
**What is new here is the typed surface, not the capabilities.** `orch:write` and `orch:message`
are **floor capabilities** — on by default in `standard` and up (only the `assistant` profile
withholds them). What ships new is the **typed `comis_tools` methods** (`write`,
`message_send`/`reply`/`react`) that let a script reach those already-held capabilities directly
in one turn. The methods are inert for an agent that does not hold the underlying capability: an
`assistant`-profile agent, or one you gate with `write: false`, cannot call them. Only `orch:mcp`
above is a genuinely new default-off capability — do not read "typed surface is new" as "the
write/message capabilities are off by default".
</Warning>

`write` is **path-confined to the run's workspace** and **run-ephemeral**: every path resolves
under the agent's jailed workspace root (a `../` or absolute path is refused), and the workspace is
cleaned when the run ends, exactly like the files the read tools see. It is not a persistent store.
`message_send`/`reply`/`react` target the agent's **own origin channel** by default — a send to a
new channel escalates for approval (see [Autonomy](/agents/autonomy#what-standard-turns-on)) — and
the [`message.*`](/reference/json-rpc#message-send) edit/delete/fetch subset stays admin-only, not
part of `orch:message`.
