...
(the write/message *capabilities* are already on in `standard`, though the `write` surface itself is
a separate opt-in — see below).
...
capability (an `assistant`-profile agent, or one you set `write: false`, cannot call `write`). Only
`orch:mcp` above is a genuinely new default-off *capability*.
</Warning>
