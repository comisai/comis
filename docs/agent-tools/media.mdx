...
<AccordionGroup>
  ...
  <Accordion title="tts_synthesize -- Generate spoken audio from text" icon="volume-high">
    Use `tts_synthesize` to convert text into spoken audio. The generated audio file is automatically sent as a message attachment in the chat.
    ...
    | `format` | string | No | Audio format: `"mp3"`, `"opus"`, or `"wav"` (default: from config) |
    ...
    **Example usage:**
    ```
    "Read this summary aloud using the 'alloy' voice."
    ```

    The agent calls `tts_synthesize` with the summary text and the requested voice, then sends the audio file as a chat attachment.
  </Accordion>
  ...
  <Accordion title="describe_video -- Describe video content" icon="film">
    ...
    | Parameter | Type | Required | Description |
    |-----------|------|----------|-------------|
    | `attachment_url` | string | Yes | The URL of the video to describe |
