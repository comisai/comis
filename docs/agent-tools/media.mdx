...
| `video_generate` | Generate a short video from a text prompt and deliver it to the current channel | Video gen (follows the agent's main provider by default — Google→Veo, xAI→Grok Imagine; `fal` + `FAL_KEY` is the wired baseline this release) |
...
<AccordionGroup>
  ...
  <Accordion title="video_generate -- Generate videos from text prompts" icon="clapperboard">
    Use `video_generate` to create a short video clip from a text prompt. The generated clip is delivered directly to the current channel via the daemon. This is the v2.24 baseline: an inline submit -> poll -> download against the **FAL** queue API (`fal-ai/veo3.1/fast` by default), proving the full loop end-to-end.
    ...
    **Provider following (`integrations.media.videoGeneration.provider: auto`, the default).** Video generation follows the agent's main provider and reuses its credentials, mirroring image generation: a `google` main resolves to **Google Veo** and an `xai` main resolves to **xAI Grok Imagine**, each with the key the agent already uses (`GOOGLE_API_KEY` / `XAI_API_KEY`) -- no video-specific key. Where the main provider has no video API (e.g. `openai`, `anthropic`), the tool returns an honest "unavailable" with a hint to set `integrations.media.videoGeneration.provider` (e.g. `fal` + `FAL_KEY`) -- it never silently routes to a different paid provider.
    ...
    <Note>
    In **this release (v2.24 Phase 188)** the FAL path is fully wired and is the loop-prover: set `provider: fal` + `FAL_KEY` to generate today. The native **Veo** and **Grok Imagine** adapters land in **Phase 190** -- under `provider: auto` a `google`/`xai` main currently resolves the selection but reports an honest "unavailable" naming Phase 190 (use `provider: fal` + `FAL_KEY` in the meantime). The `video_status` tool (for long-running renders that outlive the turn) lands in **Phase 189**.
    </Note>
    ...
    | `image_url` | string | No | A workspace file path, URL, or data-URI of a reference image for image-to-video. SSRF-guarded + workspace-confined (full image-to-video variant selection lands in a later phase; the security floor is enforced now). |
    ...
    **Delivery, persistence, and degradation.** A generated clip is downloaded to a buffer and persisted to the agent's confined workspace under `~/.comis/workspace/media/videos/` (a UUID filename, size-capped) **before** any delivery decision -- an expiring provider URL cannot orphan it. Delivery is **capability-driven**: a channel whose adapter implements attachment sending receives the persisted file directly (as a `video` attachment with `durationSecs`); a channel without it (currently only **IRC**) degrades to a notice + the persisted path. For large clips the inline base64 fallback is **size-capped** -- above the cap the handler returns the persisted workspace path instead of a multi-megabyte base64 blob.

    **Observability.** Each generation logs a single INFO completion line carrying the executing video provider, the agent's main provider, the model, `estimatedCostUsd`, `costUsd`, `sizeBytes`, `durationMs`, and the mime type; every failure branch logs an `errorKind` + an actionable `hint` (never the raw provider message, a key, or a bearer). The richer `comis explain` video reconstruction lands with the trajectory bridge in a later phase.
  </Accordion>
</AccordionGroup>
...
| Video generation (`video_generate`) | Follows the agent's main provider (`auto`): **Google Veo** for a `google` main, **xAI Grok Imagine** for an `xai` main (native adapters land Phase 190); explicit **`fal`** (FAL queue, `fal-ai/veo3.1/fast`) is the wired baseline this release | `auto` = the agent's main provider (reuses its key); else the explicit `provider` (`fal` / `google` / `xai`) | `FAL_KEY` for explicit `fal`; in the `auto` case reuses the main provider's key (`GOOGLE_API_KEY` for Veo, `XAI_API_KEY` for Grok — wired Phase 190). Honest "unavailable" (not silent) if the main provider can't generate video and no `provider` is set. |
