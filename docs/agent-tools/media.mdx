...
| `image_generate` | Generate an image from a text prompt and deliver it to the current channel | Image gen (follows the agent's main provider by default; OpenRouter / fal opt-in) |
...
| Image generation (`image_generate`) | Follows the agent's main provider (`auto`); explicit `openrouter` / `fal` / `openai` | `auto` = the agent's main provider (reuses its key); else the explicit `provider` | None in the `auto` case (reuses the main provider's key); `OPENROUTER_API_KEY` for the OpenRouter opt-in; `FAL_KEY` for explicit `fal`. Honest "unavailable" (not silent) if the main provider can't generate images and no `provider` is set. |
