...
<AccordionGroup>
  ...
  <Accordion title="image_generate -- Generate images from text prompts" icon="image">
    ...
    **Codex (ChatGPT-login) agents generate images with no API key.** When the agent's main provider is `openai-codex` (`provider: auto`), or you set `provider: openai-codex` explicitly, `image_generate` **reuses the agent's existing ChatGPT/Codex OAuth credentials** -- there is no image-specific API key to set. The OAuth bearer is resolved per call, so an expired token refreshes automatically; if the agent is not logged in, the tool returns an honest "unavailable" with a `comis auth login --provider openai-codex` hint (it never silently routes elsewhere). The native OpenAI and Google image-API paths (key-auth) arrive in a later phase.
...
| Image generation (`image_generate`) | Follows the agent's main provider (`auto`); explicit `openrouter` / `openai-codex` / `fal` / `openai` | `auto` = the agent's main provider (reuses its key, or OAuth bearer for a Codex main); else the explicit `provider` | None in the `auto` case (reuses the main provider's key); a Codex (ChatGPT-login) main reuses its **OAuth bearer** -- no image key; `OPENROUTER_API_KEY` for the OpenRouter opt-in; `FAL_KEY` for explicit `fal`. Honest "unavailable" (not silent) if the main provider can't generate images and no `provider` is set. |
