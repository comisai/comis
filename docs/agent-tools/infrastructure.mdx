...
**What it does:** Lets agents manage the Comis daemon itself -- read and write configuration, restart, store secrets, inspect long-running tasks, and (for supervisor agents) manage the entire system.
...
The `background_tasks` tool lets agents inspect and manage the daemon's durable queue for eligible long-running tool calls that were automatically promoted out of a foreground turn. Spawned sub-agents use the `subagents` tool, while execution graphs use `pipeline`.

| Action | Description |
|--------|-------------|
| `list` | List this conversation's background tasks with status and timestamps |
| `get` | Get status metadata for a single task by ID |
| `cancel` | Cancel a running task |
| `read_output` | Wait for a running promoted tool call and return its terminal output for acknowledgement after successful session journaling |

Use `background_tasks` to inspect a promoted tool call, consume its eventual
result with one `read_output` call, or cancel a runaway job. Do not poll
`read_output`: when the task is still live, the call waits on the original
execution and emits bounded progress heartbeats so prompt stall detection does
not mistake a healthy long-running wait for an unresponsive turn. A successful
`read_output` is acknowledged only after its tool result crosses the session
journal boundary at turn end. If the journal or protected consumption receipt
cannot be persisted, the pending result remains eligible for continuation
delivery; an existing delivery owner remains authoritative.

The tool is conversation-scoped rather than admin-gated. Every action requires
an active request context matching both the owning agent and the exact
originating conversation; tasks owned by another conversation are not exposed.

**Pending approvals are reported beside status.** A task whose tool call is waiting at an approval
gate is genuinely `running` — its execution is alive, holding on the gate — so status alone cannot
separate "working" from "waiting for a human", and those need opposite actions from the user
("wait" versus "approve, or nothing happens"). When this conversation has pending approvals,
`list`, `get`, and `read_output` report them alongside the task, naming each short id and what it
authorizes. `read_output` returns that immediately instead of waiting out its heartbeat, since the
work cannot progress until someone acts. The approvals are read from the gate at query time and
scoped by conversation, so the answer cannot go stale and never leaks another conversation's
requests. With approvals disabled there are none by construction and reporting is unchanged.
...
|------|---------|-----------------|
| `agents_manage` | create, get, update, delete, suspend, resume | Agent system -- create new agents, modify settings, suspend or resume them |
| `obs_query` | diagnostics, billing, delivery, channels, explain, trace, session_report, system_health, audit | Observability -- system diagnostics, cost tracking, delivery metrics, incident reports, and the security audit log |
| `sessions_manage` | delete, reset, export, compact | Session lifecycle -- clean up, export, and compact conversation data |
| `memory_manage` | stats, browse, delete, forget, flush, export, pin, unpin | Memory lifecycle -- view stats, remove every scoped match for a natural-language forget request, clean up memory, and pin/unpin always-inject entries |
...
<AccordionGroup>
  <Accordion title="agents_manage -- Agent agent administration" icon="users-gear">
  ...
  <Accordion title="obs_query -- Observability and metrics" icon="chart-line">
    The agent-facing read surface onto the same observability data as the [`comis explain` / `comis system-health` / `comis security audit-log`](/reference/cli) commands and the [`obs.*` RPCs](/reference/json-rpc). Read-only -- no approval gate, but every action requires admin trust.

    **Actions:**
    - **diagnostics** -- Recorded diagnostic events by family; `category` accepts `usage`, `webhook`, `message`, or `session` (it is not a failure filter -- use `system_health` for degraded or failure counts)
    - **billing** -- AI provider cost tracking; `sub_action` selects `currentRoot` (default; includes spawned descendants), `byProvider`, `byAgent`, `bySession` (excludes descendants), or `total` (daemon lifetime)
    - **delivery** -- Message delivery traces (`sub_action`: `recent`, `stats`)
    - **channels** -- Per-channel activity and staleness (`sub_action`: `all`, `stale`, `get`)
    - **explain** -- The assembled root-cause `IncidentReport` for a session or trace; defaults to the current session
    - **trace** -- Search trace rows
    - **session_report** -- The session rollup (the same `IncidentReport` as `explain`)
    - **system_health** -- Cross-session triage over a window (`since_hours`, default 24): degradation rate, recurring errors, model and config health
    - **audit** -- The durable [security audit log](/security/audit): secret access, injection detection, command blocks, filterable by `kind`, `classification`, `agent_id`, `tenant`, `outcome`, and time

    Cost and latency evidence is qualified at the boundary. The `billing`, `explain`, `session_report`, and `system_health` results carry an `evidenceLimits` field marking their cost figures as a **runtime estimate** derived from reported tokens and configured pricing, with the provider invoice `unverified`; the three report actions also mark cross-execution duration ranking `unavailable`. An agent quoting these numbers therefore cannot present them as a billed amount, nor its worst degraded execution as the slowest one.
  </Accordion>
  ...
  <Accordion title="sessions_manage -- Session lifecycle" icon="comments">
    ...
    **Actions:**
    ...
    Every action requires `tenant_id`, `agent_id`, and the opaque
    `conversation_ref` returned by `sessions_list`.
  </Accordion>
