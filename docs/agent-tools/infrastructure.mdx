...
Use `background_tasks` to inspect a promoted tool call, consume its eventual result with one `read_output` call, or cancel a runaway job. Do not poll `read_output`: when the task is still live, the call waits on the original execution and emits bounded progress heartbeats so prompt stall detection does not mistake a healthy long-running wait for an unresponsive turn. Permissions are gated to admin-trust agents because cancellation can affect other users' work.
