...
| `read_output` | Wait for a running promoted tool call, then return its terminal output |
...
not mistake a healthy long-running wait for an unresponsive turn. The tool is
agent-scoped rather than admin-gated: every action is restricted to tasks owned
