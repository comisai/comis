...
| Action | Description |
|--------|-------------|
| `list` | List this agent's background tasks with status and timestamps |
| `get` | Get status metadata for a single task by ID |
| `cancel` | Cancel a running task |
| `read_output` | Wait for a running promoted tool call, then atomically claim and return its terminal output |
...
not mistake a healthy long-running wait for an unresponsive turn. Before a
completed or failed result is returned, `read_output` durably claims delivery
ownership; a persistence failure or competing delivery returns a conflict
instead of permitting a contradictory terminal notice. The tool is agent-scoped
rather than admin-gated: every action is restricted to tasks owned by the
calling agent.
