...
The `broker:egress_blocked` event deliberately carries `targetHostHash` rather than the plaintext hostname. The SHA-256 hash is computed inside the emit helper — there is no call path that emits the raw host value into the event payload (redaction-by-construction, OBS-01).
...
WebSocket credential injection is not supported in this release (WS-01 is a future capability). The broker returns 501 with the `ws_upgrade_not_supported` reason code so the error is actionable rather than a silent connection hang.
...
- **One broker-issued exec per agent assembly (Phase 8):** The session token is issued once per
  ...
  **Workaround (Phase 8):** Limit each agent turn to a single exec call when `executor.broker` is
  ...
  **Resolution:** Per-command token issuance (threading `sessionManager` into `ExecToolDeps` so
  `issueToken()` is called inside `execute()`) is planned as Phase 9 follow-on work.

## Non-Goals (This Release)

- **OAuth / subscription auth** — OAUTH-01 (future)
- **AWS SigV4 signing execution** — FINAL-02; the `awsSigV4` finalizer interface ships as a tested no-op pass-through (deferred)
- **Full WebSocket credential injection** — WS-01 (future); the fail-closed guard (501) is shipped
- **External vault / on-demand secret fetch** — VAULT-01 (future); SecretManager is the only resolution path today
