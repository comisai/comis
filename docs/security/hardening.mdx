...
<Steps>
  ...
  <Step title="Enable the approval workflow">
    ...
    there is no general rule evaluator that classifies arbitrary actions (such as a
    file deletion) and forwards them to the gate, and the `approvals.rules` /
    `approvals.defaultMode` fields are accepted by the schema without being
    evaluated. After enabling, verify each path that matters to you -- a pending
