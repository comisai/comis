...
is intentionally usable rather than maximally restrictive: the approvals schema
default is off (guided setup enables it when it creates an administrator),
...
<Steps>
  ...
  <Step title="Enable the approval workflow">
    Wire the operator-backed approval gate into the execution paths that implement it:
    ...
    Enabling the gate makes the paths that submit approval requests -- such as the
    `orchestrate` pre-flight, which submits one request for a script's statically
    extracted capability footprint -- pause for an operator decision, and deny the
    action if nobody responds within the timeout. Setting `batchApprovalTtlMs: 0`
    disables the approval cache so each request is decided on its own.

    Enabling approvals does **not**, by itself, gate every destructive operation:
    nothing classifies an arbitrary action (such as a file deletion) and forwards it
    to the gate. `approvals.rules` and `approvals.defaultMode` decide requests that
    already reach the gate -- they cannot add one to a path that never submits a
    request. After enabling, verify each path that matters to you -- a pending
    request is the only proof that a path reached the gate. See
    [Approvals](/security/approvals) for which fields take effect.

    **Why:** Prevents agents from taking irreversible actions without your knowledge
    on the paths that implement the gate. For operations outside those paths, rely on
    the tool-policy profile and the workspace/sandbox boundary instead.
  </Step>
