...
<Steps>
  ...
  <Step title="Configure browser sandboxing">
    ...
    **What the installer does for you:** When the daemon is installed with `--with-browser`, `--with-xvfb`, or `--with-cloakbrowser`, the systemd unit is written with two layers of filesystem isolation around the browser binary:
    ...
    For the `--with-xvfb` variant, the companion `comis-xvfb.service` unit runs as the same user, joins the daemon's `/tmp` namespace via `JoinsNamespaceOf=`, and exposes Xvfb on a Unix socket only (`-nolisten tcp`) — no network surface added.
