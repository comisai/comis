...
| `orch:mcp` | Call an allowlisted connected MCP server's tool from a jailed script (inbound; off in every default profile) |
...
This is the **one genuinely new default-off capability** on this surface: unlike
the floor capabilities below, `orch:mcp` is in **no** default profile. It is
granted only by an explicit operator opt-in (`autonomy.mcp.enabled: true`), and
even then a call is reachable only if the specific `{server, tool}` pair is on
that agent's [inbound allowlist](/agents/autonomy#connected-mcp-tools).

Reachability is therefore **default-deny at two independent layers**:

<Steps>
  <Step title="The capability grant">
    Without `orch:mcp` in the resolved capability set, the gate denies every
    `mcp` call outright -- the whole surface is dark. A fresh agent has no
    `autonomy.mcp` block, so it never holds the capability.
  </Step>
  <Step title="The per-agent inbound allowlist">
    Even holding `orch:mcp`, a `{server, tool}` pair absent from
    `autonomy.mcp.allow` -- or on a server with no explicit inbound
    classification -- is denied at the daemon-side executor. An unlisted server
    or tool is unreachable by absence.
  </Step>
</Steps>
...
is often described alongside `orch:mcp`, but its posture is different and worth
stating plainly: **`orch:write` and `orch:message` are floor capabilities**, on
by default in `standard` and up (only `assistant` withholds them). What is new is
only the **typed SDK methods** that reach those already-granted capabilities in
one jailed turn. Calling the write/message *capabilities* "default-off" would be
inaccurate -- the honest statement is that the **typed surface** is new (and
inert for an agent that does not hold the underlying floor capability), while
**only `orch:mcp` is a new default-off capability**.
