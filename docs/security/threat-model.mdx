...
| Agent bypassing a revoke (re-leasing / continuing after stop) | `lease.revoke` / `run.kill` are **admin-scoped and deny-by-origin** — an agent-origin call is rejected at the dispatch chokepoint, so an agent can neither revoke others nor un-revoke itself. A revoked lease's `validate` AND `renew` are both denied (no unbounded re-lease), and `run.kill` cascades to grandchildren via `parentLeaseId` |
