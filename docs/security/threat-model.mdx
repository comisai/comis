...
- **Resource exhaustion / DoS from the operator's own agents, across a daemon restart** — the [bounded-autonomy](/agents/autonomy) guards (spawn ceiling, per-root budget, call-rate limit, outward quota) are real hard bounds *within* a daemon run, but their state is **in-memory** (per the M1 scope): the counters/meters reset on restart, so they bound a single live run, not cumulative spend across restarts. A durable sent-ledger and resumable budgets are future work.
