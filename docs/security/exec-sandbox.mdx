...
On Linux, the `broker-only` network mode combines with `secureCredentialHome` so neither the key material nor a direct network path to the key provider is reachable inside the sandbox. Validation of network-level enforcement on the Linux production host class is in progress.
