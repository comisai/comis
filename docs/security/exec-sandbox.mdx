...
The daemon records this opt-out as a warning-level `config_posture` signal, and `comis system-health` names `skills.execSandbox.enabled` so the relaxed posture cannot appear protected.
</Warning>
