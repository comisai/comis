...
Two storage backends are supported, selected by [`oauth.storage`](/reference/config-yaml#oauth):

| Mode | Path | Hot-reload | Threat posture |
|------|------|------------|----------------|
| `file` (default) | `~/.comis/auth-profiles.json` | Yes -- chokidar 100ms debounce | Filesystem perms only (`0o600`) |
| `encrypted` | `oauth_profiles` table inside `secrets.db` | No -- SQLite WAL masks events | AES-256-GCM ciphertext per row |

### File mode (`oauth.storage: file`)
...
- **Atomic write** -- the daemon writes to a temp file, `fsync`s, then renames into place. Crashes mid-write cannot leave a half-written file visible.
...
### Encrypted mode (`oauth.storage: encrypted`)
