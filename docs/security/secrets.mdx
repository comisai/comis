...
Comis uses an encrypted SQLite credential store by default. The setup wizard
writes encrypted values to `~/.comis/secrets.db` and stores the master key as
`SECRETS_MASTER_KEY` in the owner-readable `~/.comis/.env` file. Back up that
master key securely: without it, encrypted credentials cannot be recovered.

## Three Ways to Store Secrets

The `security.storage` setting selects the credential backend. Secret references
can additionally resolve through environment, file, or credential-helper
providers.

| Method | Security | Best For | Setup |
|--------|----------|----------|-------|
| Encrypted store (default) | Strong at-rest protection | Normal and production deployments | Configured by the wizard |
| Environment variables | Plain text in process/file environment | Controlled development environments | Manual |
| File store | Plain text with owner-only permissions | Compatibility with external secret provisioning | Manual |

### Environment Variables (.env)

Environment storage is an explicit alternative to the encrypted default. Comis
loads `~/.comis/.env` at startup, but any credential stored there is readable
by anyone who can read that file.

```bash title="~/.comis/.env"
OPENAI_API_KEY=<your-provider-key>
ANTHROPIC_API_KEY=<your-provider-key>
DEEPGRAM_API_KEY=<your-provider-key>
```
...
the database alone does not reveal the secrets without the master key. This
does not protect against compromise of a running daemon or a host where both
the database and master key are accessible.
...
**Backup obligation:** Losing the master key makes `secrets.db` permanently
unreadable; Comis has no key escrow. Back up the key separately in a secure
credential manager immediately after setup.
...
<Steps>
  ...
  <Step title="Scope it to one agent">
    ...
    Restart the daemon after editing configuration: `comis daemon stop`, then
    `comis daemon start`.
  </Step>
  ...
  <Step title="Verify the scope">
    ...
    Use `comis daemon logs` and the security audit sink to inspect access
    outcomes. Audit records contain the secret name and outcome, never the
    secret value.
...
| **Platform** | All platforms | Kernel-enforced broker-only egress requires Linux; TLS injection works everywhere |
...
The redaction is performed at the structured-log call site, not by Pino's redaction list, so it survives even if the log call accidentally interpolates the email into the message string.
...
The single `security.storage` key selects the credential storage backend for all stores:
...
If choosing `encrypted`, ensure `SECRETS_MASTER_KEY` is set in `.env`. If it is not set, run `comis secrets init` to generate one. Because `AppConfigSchema` uses `z.strictObject`, any unrecognized key under `security:` triggers a `VALIDATION_ERROR` and the daemon refuses to start -- there is no silent fallback.

After changing the mode, verify boot:

```bash
node packages/cli/dist/cli.js secrets list
```

If the daemon starts and returns a list (empty or with keys), the switch is complete.

**Check for stranded credentials:**
