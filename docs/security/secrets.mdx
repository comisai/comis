...
|---------|---------|-------------|
| `security.secrets.enabled` | `true` | Enable the encrypted secrets store (auto-enabled since v1.1) |
...
<Info>
To disable the encrypted store entirely, set `COMIS_DISABLE_ENCRYPTED_SECRETS=1`.
The daemon then boots in envfile-only mode and emits a startup WARN. Use this only
if you need to manage secrets exclusively via `.env` files.
</Info>
