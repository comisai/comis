...
Comis can require human approval before agents perform destructive or
sensitive operations. **The approval workflow is disabled by default.** Set
`approvals.enabled: true` and configure the applicable rules before relying on
it. Action classification continues while approvals are disabled, but it does
not create a human-in-the-loop pause by itself.

<Warning>
The default unmatched mode is `auto`, and the default agent tool-policy profile
is `full`. Enable and test approvals from the same agent, tool, and channel path
you will expose to untrusted users.
</Warning>
...
<Steps>
  ...
  <Step title="Comis classifies the action">
    Actions are classified as read, mutate, or destructive. Registered actions
    have explicit classifications; an unknown action defaults to destructive.
  </Step>

  <Step title="Confirmation is checked">
    If approvals are enabled and the first matching approval rule requires
    confirmation, the request enters the approval workflow. With approvals
    disabled, no request is created.
  </Step>
...
<Tip>
For the current registered actions and their classifications,
