...
/**
 * Shrink-only arch invariant (D9): appendSessionIndexEntry must THROW when a
 * VITEST/NODE_ENV=test process writes under the real `~/.comis`. This is the
 * static regression lock for the Phase-149-02 leak class — a test run silently
...
describe("no-prod-datadir-in-tests invariant (D9)", () => {
  it("append.ts has the VITEST+real-homedir throw-guard (regression lock)", () => {
    ...
    if (!hasGuard) {
      violations.push(
        "append.ts is missing the VITEST+real-homedir throw-guard (D9)",
      );
    }
    expect(
      violations,
      "appendSessionIndexEntry must throw when a VITEST/NODE_ENV=test process writes under the real ~/.comis (D9). " +
