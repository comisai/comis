...
describe("orchestrate comis_tools SDK drift gate", () => {
  ...
  it("emits the orch:message outward triplet (send/reply/react) as DIRECT callCapSocket methods, not tool.invoke", () => {
    ...
    // load-bearing for at-most-once dispatch with explicit reconciliation: the outward-step ledger
    ...
    expect(js).toContain('async message_send(args, operationId) {\n    return callCapSocket("message.send", args, operationId);\n  }');
    expect(js).toContain('async message_reply(args, operationId) {\n    return callCapSocket("message.reply", args, operationId);\n  }');
    expect(js).toContain('async message_react(args, operationId) {\n    return callCapSocket("message.react", args, operationId);\n  }');
    ...
    expect(dts).toContain("message_send(args: Record<string, unknown>, operationId: string): Promise<unknown>;");
    expect(dts).toContain("message_reply(args: Record<string, unknown>, operationId: string): Promise<unknown>;");
    expect(dts).toContain("message_react(args: Record<string, unknown>, operationId: string): Promise<unknown>;");
  });
