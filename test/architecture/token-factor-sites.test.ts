...
/**
 * TOK-01 factored-or-marked invariant — estimation-constant division sites.
 *
 ...
 * that the design's two-site list missed — silently undoing the script
 * factor right after tool deferral (179-RESEARCH Pitfall 3). With this
 ...
 * sites (the planning-time inventory found ~30 across both estimator
 * families) — a rotted regex or a broken walk that finds near-zero sites
 ...
 * one optional opening paren) by the constant name. Matches the divisor
 * forms `/ CHARS_PER_TOKEN...` and `/ (CHARS_PER_TOKEN... * scriptTokenFactor(...))`.
 * Multiplications never match (no `/` before the constant).
 */
const DIVISION_BY_ESTIMATION_CONSTANT =
  /\/\s*\(?\s*CHARS_PER_TOKEN(?:_RATIO)?(?:_STRUCTURED)?\b/;

/**
 * Anti-rot floor for the matcher self-check. Planning-time inventory
 * (179-06, @ ad6aa89e) counted ~30 matched lines across packages/agent/src;
 * a scan finding fewer than this means the regex or the walk rotted —
 * fix the matcher, never lower the floor to pass.
 */
const MIN_EXPECTED_MATCHES = 25;
...
function scanFile(file: string): MatchedSite[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!DIVISION_BY_ESTIMATION_CONSTANT.test(line)) continue;

    // SYMMETRIC two-line lookback: the matched line itself OR either of
    // the TWO preceding non-empty lines may carry `scriptTokenFactor(`
    // (factored) or `flat-by-design` (marked). Comment lines count as
    // preceding non-empty lines — that is what makes marker comments work.
    ...
    const pass = window.some(
      (w) => w.includes("scriptTokenFactor(") || w.includes("flat-by-design"),
    );
    sites.push({ file, line: i + 1, text: line.trim(), pass });
  }
  return sites;
}
...
describe("token-factor-sites — TOK-01 factored-or-marked invariant", () => {
  it("finds at least 25 estimation-constant division sites (matcher anti-rot self-check)", () => {
  ...
  it("reports zero unfactored and unmarked estimation-constant divisions in packages/agent/src", () => {
    ...
    expect(
      ...
      formatViolations({
        description:
          "Every char->token division by an estimation constant in packages/agent/src must be FACTORED (scriptTokenFactor on the division line or within its two preceding non-empty lines) or explicitly MARKED flat-by-design (TOK-01) — a new flat site silently under-counts dense scripts (the #190 recurrence class).",
        ...
        designRef:
          "design/multilingual-excellence.md §4 TOK-01; 179-RESEARCH Pitfall 3 (#190 third-site recurrence)",
      }),
    ).toEqual([]);
  });
});
