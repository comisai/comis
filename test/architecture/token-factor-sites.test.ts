 ...
 * Review WR-03 hardening: `scriptTokenFactor(` counts as FACTORED only on
 ...
 *
 * Matched divisor shapes (review WR-03 — strengthen, never weaken):
 ...
 * (markers only — see the WR-03 factored-token comment exclusion above).
 *
 ...
 * vacuously. The floor sits ABOVE the pre-WR-03 matcher's count so a
 ...
 * the family-2 root shape the pre-WR-03 matcher was blind to).
...
/**
 * Anti-rot floor for the matcher self-check. The WR-03-strengthened scan
 * finds 38 sites (2026-06-13); the PRE-WR-03 matcher found 32, so a floor
...
function scanFile(file: string): MatchedSite[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Logical-line join (review WR-03): prettier may wrap the divisor onto
    ...
    // `scriptTokenFactor(` (factored — NON-comment lines only, review
    // WR-03: prose quoting the call must not classify a flat site) or
...
describe("token-factor-sites — TOK-01 factored-or-marked invariant", () => {
  ...
  it("sees the family-2 root divisor shapes: ratio-variable, wrapped ternary, and split-line divisors (WR-03 guard-strength pin)", () => {
    // Review WR-03 false-negative vector: the pre-WR-03 matcher required the
    ...
    expect(
      ...
      "ratio-variable divisor site in token-estimator.ts must be matched (WR-03 vector 1)",
    ).toBe(true);
    ...
    expect(
      ...
      "wrapped ternary divisor site in token-estimator.ts must be matched (WR-03 vector 1)",
    ).toBe(true);
    ...
    expect(
      ...
      "split-line divisor site in llm-compaction.ts must be matched (WR-03 vector 1)",
    ).toBe(true);
  });
