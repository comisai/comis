...
/**
 * Stage-A test for the Signal `SignalEnvelope` builders.
 *
 ...
 * drift is a COMPILE error — the compile-drift discipline. This test asserts the RUNTIME
...
describe("signal-payloads — makeMessageEnvelope (text round-trip)", () => {
  beforeEach(() => {
    resetSignalTimestampCounter();
  });
  ...
  it("emits a strictly-increasing timestamp by default (the long-poll/react ordering invariant)", () => {
    ...
    // timestamp field (signal-client.ts:37-58), so the typed builder cannot
...
describe("signal-payloads — makeReactionEnvelope (the react FLOW)", () => {
  beforeEach(() => {
    resetSignalTimestampCounter();
  });
...
describe("signal-payloads — makeSignalAttachment (the compile-time attachment tripwire)", () => {
...
describe("signal-payloads — wire-type source discipline (the wire-type import is type-only + return-annotated)", () => {
  ...
  it("returns a SignalEnvelope from each builder via an explicit return annotation (the compile-drift guarantee)", () => {
    const src = readFileSync(PAYLOADS_SOURCE, "utf8");
    expect(src).toMatch(/: SignalEnvelope/);
    // The reaction builder exists (the verb Signal supports).
