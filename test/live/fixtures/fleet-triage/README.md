# Fleet-triage corpus (the fleet-triage baseline)
...
`obs.fleet.health` lens must reproduce, plus the manual cost/effort to
...
| File | Shape |
|------|-------|
| `triage-corpus.json` | `{ "signals": FleetSignal[] }` — the by-hand findings as structured data. Each entry: `{ signal, byHandCount, location, alreadyStructured, errorKind? }`, where `signal`/`location` are CLOSED string unions (an out-of-enum value fails the loader's shape-guard). Counts + typed enums only, **never raw log bodies**. |
