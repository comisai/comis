...
- **`fleet-marathon-campaign.md`** — a **pinned CAMPAIGN** target (an autonomous **hours-to-days marathon**): a web-researched use-case backlog + a whole-capability `COVERAGE-MATRIX.md`, then the 00-MISSION loop repeated per use case over a REAL credentialed MCP (ituran-mcp), in Hebrew, with per-cycle obs audits (non-negotiable #4), per-UC memory/recall/learning audits (#5), and durable resume state (`CAMPAIGN-STATE.md`). Use this shape when the target is "the entire system".
