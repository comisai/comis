...
> `artifact-to-action` is validated offline by its per-variant selftest and contract suite. Neither has a
> successful model-transfer result on record (see the validation-status notes in
