# targets/hebrew/ — the pinned MARATHON CAMPAIGN targets (Hebrew-first)

The pinned hours-to-days whole-system CAMPAIGN targets live here — target shape 1 (a use case)
sized for an autonomous marathon run, each written to a professional end-to-end tester's bar: a
researched real-world use-case backlog + a whole-capability coverage matrix extracted from the
build, the `../../00-MISSION.md` loop repeated per use case, a severity model (S1–S4) with
reproducible defect reports, durable resume state (`CAMPAIGN-STATE.md`), and explicit entry/exit
...
| --- | --- | --- |
| `fleet-marathon-campaign.md` | B2B fleet-ops over the real credentialed ituran-mcp, single-operator, in Hebrew | read-only external MCP (a claimed-but-unperformed write is an S1) |
...
| `sre-oncall-marathon-campaign.md` | bilingual on-call SRE copilot over a real shell + coding-CLI + ops MCPs + a live webhook pager, rotation RBAC | blast-radius / production-safety confinement |
