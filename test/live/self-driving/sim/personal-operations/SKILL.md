...
5. After probing each source, record what it actually returned with `report_source_status` (`delivered` or
   `unavailable`). The review is graded on that record, so a source you probed but never recorded counts as
   unrecorded.
