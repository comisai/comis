...
- **Comis runtime (redriven).** `<repo>/test/live/self-driving/scripts/artifact-to-action-runtime-drive.mjs`
  boots an isolated daemon (own data root, own loopback gateway), registers the workload through
  `integrations.mcp.servers`, and answers every completion request from a local OpenAI-compatible server
  whose next tool call is computed from the prior tool results — no model, no network. Reproduce with
  `node test/live/self-driving/scripts/artifact-to-action-runtime-drive.mjs --variant A`.

| runtime drive | variant `A` | variant `A-degraded` |
|---|---|---|
| tools discovered over MCP | 13 | 13 |
| distinct tools dispatched / calls | 13 / 16 | 7 / 10 |
| `tool.result` records in the durable trajectory | 16 | 10 |
| session rollup `endReason` / `degraded` | `success` / `false` | `success` / `false` |
| `obs.explain` severity / `likelyRootCause` | `ok` / none | `ok` / none |
| terminal grade | `success`, score 1, 1 staged, 1 committed, readback true | `success`, score 1, 0 staged, 0 committed |
| `traceId` / `runId` | `4cec6fac-abf1-42b3-a3ad-33b315b2cba9` / `f2bcd6e6-ff03-4993-85fe-be423c798877` | `e7842718-d415-4171-8a5d-fc620ad30bc5` / `d2fb7a2b-30f6-48b6-be00-01d7c9322d6a` |

Both runtime drives ran on this branch's head under the same session key —
`test:agent:default:gateway-7062520c…:control-plane:gateway:["cli",null]:peer:gateway-7062520c…` — which is
derived from the drive's fixed loopback token and is therefore stable across reruns; the `traceId`/`runId`
pair identifies the recorded run and is regenerated by each rerun. Verified from the runtime's own durable
output (`workspace/sessions/**/_session-metadata.json`, the `*.trajectory.jsonl` beside it, and `obs.explain`
over the gateway), not from the model reply.
