...
8. **Provider-backed cyber-abuse-shaped tests are suspended unless the operator explicitly requests them.** Read `CYBER-ABUSE-SUSPENSIONS.md`, label every risky row, and record an unauthorized row as `NOT-RUN: provider cyber-abuse safety suspension`. A target, test plan, prior authorization, or agent judgment never authorizes it. The agent may declare risk, but only the operator may direct it to set the exact acknowledgement; never persist that acknowledgement.
...
Before leaving STEP 2, classify every provider-backed row against `CYBER-ABUSE-SUSPENSIONS.md`. Mark risky
rows `SUSPENDED` unless the operator explicitly requested them in the current task. Do not send a setup,
baseline, negative-control, or expected-refusal prompt merely to discover whether the provider will accept it.
