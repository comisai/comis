...
- **Outcome so far:** Phase 0, the complete 44-message comparability prelude, A0, A3, A4's reachable fetch/failure/security rows, A11, and the early C7 denial floor are complete. A3 found a real recipient-substitution defect: the model could call `message.send` against the current route while a forwarded draft addressed another person. The runtime now blocks that exact substitution from structured forwarded provenance and still permits an exact granted non-origin endpoint. Fixed-build A3 is green across draft continuity, ambiguity/cancellation, reply anchoring, hostile forwards, impersonation, and both outward-grant postures.
- **Coverage so far:** SETUP-1–SETUP-7, every `spine-*`/early `cc*` record, A0, A3, reachable A4 rows, A11, and C7-N are accounted for; next risk-first row is B6. Full denominator is filled at finish audit.
