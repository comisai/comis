...
- **Outcome so far:** Phase 0, the complete 44-message comparability prelude, A0, A4's reachable fetch/failure/security rows, A11, and the early C7 denial floor are complete. The signed-response replay failure found by A0-N is fixed test-first and verified against the original durable session; `explain` now counts the successful replay recovery. A4 exposed frozen-workload gaps rather than an open product failure: the comparison turn contains no pair of options, the corpus contains no slow source, and this isolated secret inventory has no keyed search-provider credential.
- **Coverage so far:** SETUP-1–SETUP-7, every `spine-*`/early `cc*` record, A0, reachable A4 rows, A11, and C7-N are accounted for; next risk-first row is A3. Full denominator is filled at finish audit.
