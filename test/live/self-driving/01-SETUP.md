...
- **Provider/model:** `provider:` + `model:` on the agent's line (the `integrations.media` block also has `provider:` — sed only the agent's). The model must be a **valid catalog model** for the provider — enumerate with **`comis models list`** and filter to the provider (the CLI is the live catalog surface). ⚠ Do NOT use `getModels()` from `@earendil-works/pi-ai`: that export no longer exists at HEAD (`m.getModels is not a function`), so the one-liner previously documented here fails before it can tell you anything. An invalid/mistyped model is the cause of the `effectiveWindow:8192` viable-floor WARN below, so verify the id against the CLI list before blaming a window bug.
- **Budget — the per-root meter has THREE limbs; know all of them + the one gotcha:** the per-root `BoundedAutonomy` meter (`schema-agent-autonomy-bounds.ts`) bounds the whole spawn tree on `agents.<id>.autonomy.budget.{ aggregateUsd, tokens, wallClockMs }` — raise **all three** for heavy/long turns, not just `aggregateUsd`:
  - `aggregateUsd` (schema default **200**; the flat `aggregateBudgetUsd` is its alias) — the priced $-limb.
  - `tokens` (schema default **200,000,000**) — bites even on a $0/subscription model (BUDGET-02). **Counts cache-read tokens**, so a multi-tool turn on a cache-heavy provider (Codex/OpenAI, 30–50K cacheRead/call) burns them faster than the actual $ suggests.
  - `wallClockMs` (schema default **172,800,000** = 48 h) — backstop on a stuck tree.
  ⚠ **Read the defaults off the schema, not off this doc.** These are `schema-agent-autonomy-bounds.ts` (`aggregateUsd` :45, `tokens` :47, `wallClockMs` :49) and the **resolved** value is whatever the agent's `autonomy.profile` yields (`schema-agent-autonomy.ts` — `profile` defaults `standard`), so a per-agent config or a non-default profile can land anywhere. Confirm the live numbers with `config.read` before deciding a limb is "too low" — an earlier revision of this doc quoted $2 / 2M / 30 min, two orders of magnitude below HEAD, which sends you tuning limbs that were never the constraint.
...
5. **Baselines recorded** — `memory.db` counts, `system`, log offsets — so you can tell *your* incidents from pre-existing ones.
...
- **Severing the LCD takes the DURABLE IDENTITY TRIPLE, not a session key.** At HEAD
  `session.reset_conversation` requires `{tenant_id, agent_id, conversation_ref}` (+ optional
  `memory`, `purge_derived`) — `packages/core/src/api-contracts/sessions.ts:605`. Passing
  `{session_key}` is **rejected with a schema error**, so the old "silently returns `lcdRowsDeleted:0`"
  trap no longer applies: a wrong shape now fails loudly, which is the improvement, but a runner
  following the old advice loses a cycle on the rejection. Get the opaque `conversation_ref` from
  `db.mjs pick sessions tenant_id,agent_id,conversation_ref` (or the `conversation_ref` that
  `subagents`/session listings expose). The formatted `sessionKey` — which still looks like
  `default:agent:default:platform_<digest>:telegram:peer:platform_<digest>` under privacy-principal
  routing — is a human-readable **projection** and cannot recover the durable reference. Drive one
  sever per `conversation_ref` — **measured on this rig: ONE chat owned 26 refs**, with the LCD spread
  across 21 of them (one held 1386 rows, the rest 2–54 each; 1601 total). Severing only "the" ref cleared
  **2 of 1601 rows (0.1%)** and left the conversation fully intact, so any "cross-session recall" test
  after it is invalid. Enumerate every ref (`db.mjs pick sessions conversation_ref 30`) and sever each,
  then assert `count lcd_messages` ≈ 0 while `count memories` is unchanged (301 survived here — LTM must
  not be collateral) and verify `lcdRowsDeleted>0` plus
  `runtimeSessionDestroyed:true`; then confirm `count lcd_messages` is 0 while `count memories` is
  unchanged, or a "cross-session" recall test is really an LCD read.
