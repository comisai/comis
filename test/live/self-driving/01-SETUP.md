...
- **Prove you're on the new code** every confirm: a live daemon holds its `dist/` in memory; `pnpm build` alone does not hot-reload it.
...
- **Budget — the per-root meter has THREE limbs; know all of them + the one gotcha (codex-30uc run 2026-06-25 burned cycles here):** the per-root `BoundedAutonomy` meter (`schema-agent-autonomy-bounds.ts`) bounds the whole spawn tree on `agents.<id>.autonomy.budget.{ aggregateUsd, tokens, wallClockMs }` — raise **all three** for heavy/long turns, not just `aggregateUsd`:
  ...
  - Mitigation when a long session starts aborting `spend_exceeded` with no limb actually near its cap: **clean-restart per ~5-turn group** (resets the in-memory per-root meter). The per-root abort logs `"Per-root <token|wall-clock> budget exceeded"` (with the numbers) — grep that to see WHICH limb, since the bridge's user-facing abort is content-free.
...
- **`pkill -f "daemon.js"` self-matches your ssh shell** (its argv contains the pattern) → kills the shell → ssh exit 255. Always anchor: `pkill -9 -f "^node .*daemon\.js"`.
