...
# 1) selected daemon process alive + the systemd unit healthy (the production install runs under
# comis.service). A local host can carry several isolated daemons, so a process-wide pgrep is not
# evidence that this DATA/SERVICE tuple is running.
daemon_pid="$(rig_daemon_pid)"
if [ -n "$daemon_pid" ]; then
  pass "daemon-process" "selected daemon process is running (pid $daemon_pid)"
else
  fail "daemon-process" "no daemon owned by the selected DATA/SERVICE tuple — start it with restart-daemon.sh"
fi
