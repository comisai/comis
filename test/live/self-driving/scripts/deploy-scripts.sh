...
# (helpers like clean-restart.sh + a models-sweep.sh can go MISSING on the box, and stale one-offs
# linger — cost archaeology + per-run scp's). This keeps /root +
...
# /root — ALL driver/oracle helpers. GLOB every *.mjs + the root-run *.sh (a
...
# silently DROPPED new .sh helpers, e.g. drive-sim-workload.sh).
