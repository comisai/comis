...
| **`targets/`** | How to specify a target + worked examples: `EXAMPLE-nvda-dag.md` (channel/orchestrate), `EXAMPLE-verified-learning.md` (offline/DB/event-resident), and `EXAMPLE-cron-wake-gate.md` (cron/scheduler-gate + jail, driven with `scripts/wg.mjs`). Also `MEMORY-LEARNING-STRESS-CATALOG.md` (12 complex memory/learning workloads), `adaptive-threat-hunting.md` (a pinned spec) + the two pinned hours-to-days whole-system CAMPAIGNs: `fleet-marathon-campaign.md` (B2B fleet-ops over the real ituran-mcp, single-operator, read-only gate) and `chief-of-staff-marathon-campaign.md` (Hebrew-first household chief-of-staff over the live web + a real mailbox + personal-stack MCPs, multi-sender trust, third-party-confinement gate). |
