...
| **`scripts/`** | Copy-paste helpers (`install-vps.sh` (installer-first (re)install of THIS checkout), `deploy-dist.sh`, `deploy-scripts.sh`, `deploy-emu.sh`, `setup-vps.sh`, `restart-daemon.sh`, `clean-restart.sh`, `wire-emu.mjs`, `drive.mjs`, `revoke.mjs`, `db.mjs` (DB oracle), `models-sweep.sh`, `config.example.yaml`). See `scripts/README.md`. |
