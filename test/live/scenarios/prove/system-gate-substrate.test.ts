...
/**
 * SYSTEM-gate substrate — system-triage baseline.
 *
 ...
 * `system-recurrence-gate.test.ts` / `system-triage-corpus.test.ts`,
 ...
 *     test/live/scenarios/prove/system-gate-substrate.test.ts
 *
 * @module
 */
...
import {
  loadCorpus,
  type SystemSignal,
} from "../../support/system-triage-corpus.js";
import {
  ...
  type GapGateRow,
} from "../../support/system-recurrence-gate.js";
...
describe("SYSTEM-gate substrate — the frozen corpus loads and is well-formed", () => {
  it("loadCorpus returns all 6 SystemSignal members with the by-hand reference counts", () => {
    const corpus = loadCorpus(FIXTURES_DIR);
    expect(corpus.signals.length).toBe(6);

    const byName = new Map<SystemSignal["signal"], SystemSignal>(
      corpus.signals.map((s) => [s.signal, s]),
    );

    // The flagship signal: LCD-ingest-skipped, ×7 by hand, log-only.
  ...
  it("returns a well-formed manual cost-to-beat verification bar", () => {
    const corpus = loadCorpus(FIXTURES_DIR);
...
describe("SYSTEM-gate substrate — the scorers render every verdict class over the frozen corpus", () => {
  /**
   * Build a synthetic measured map exercising each verdict branch deterministically:
   *  - lcd-ingest-skipped: realCount >= threshold + exercised → INSTRUMENT
  ...
  it("recordSignalRecurrence + buildGapGateTable produce the expected per-signal verdicts", () => {
    ...
    const verdictOf = (s: SystemSignal["signal"]) =>
      rows.find((r) => r.signal === s)?.verdict;

    expect(verdictOf("lcd-ingest-skipped")).toBe("INSTRUMENT");
  ...
  it("the rendered markdown carries the CAVEAT, the table header, every verdict class, and passes the secret sweep", () => {
    ...
    expect(md).toContain("INSTRUMENT");
  ...
  it("a FLAGSHIP signal at realCount 0 on an un-exercised path is INCONCLUSIVE, never a confident SKIP", () => {
    // The load-bearing guard: a 0 on the flagship LCD signal — when the
