...
- **The always-on scenario** (`fleet-reprove.test.ts`, **keyless, in `pnpm
  ...
  `assembleFleetHealthReport` call over a **seeded** store reproduces the corpus's
...
- **The operator's live RUN** (this runbook, **NOT in CI**) proves the
  **end-to-end loop** against REAL, freshly-written rows: the new instrumented
  ...
  one `node packages/cli/dist/cli.js fleet` call surfaces them.
...
The contrast against `test/live/fixtures/fleet-triage/manual-cost-to-beat.json` is
...
### 3. Surface the fleet in ONE call (0 log-greps)

```bash
node packages/cli/dist/cli.js fleet --since 24
# JSON for transcription / tooling:
node packages/cli/dist/cli.js fleet --since 24 --format json
```
...
log to answer "what's wrong across the fleet right now" — that is the whole point.

---

## Skip ≠ fail + NO new env var

- The always-on `fleet-reprove.test.ts` stays **GREEN with no operator action** —
...
- **No new environment variable.** The fleet proof introduces none — no live-gate
...
`node packages/cli/dist/cli.js fleet --since 24` call, the A-track rates are
...
`1 call / 0 grep` = confirm the whole fleet picture came from a single
`node packages/cli/dist/cli.js fleet --since 24` invocation with no `daemon.log`
