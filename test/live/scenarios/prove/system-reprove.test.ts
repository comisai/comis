...
/**
 * SYSTEM-reprove — obs.system.health reproduces the frozen
 * corpus in ONE call, ZERO log-greps.
 *
 * The milestone's reason-to-exist proof. The system health view replaces
 ...
 * proves that a SINGLE `assembleSystemHealthReport` call reproduces that corpus's
 ...
 * entire Stage-C judged apparatus DROPPED. The system proof is a COUNT/STRUCTURE
 ...
 * read of any kind (the same anti-cargo-cult discipline `system-gate-substrate.
 * test.ts:5-12` documents). The costed LIVE RUN (boot the new instrumented daemon
 * → it writes real I-track rows → `comis system-health` surfaces them) is the operator's —
 * see the co-located `system-reprove-runbook.md`.
 *
 ...
 * (system-health.ts:316,328,331-333). The findings asserted below all came from the
...
import { loadCorpus } from "../../support/system-triage-corpus.js";
...
import { assembleSystemHealthReport } from "@comis/daemon";

// fileURLToPath(import.meta.url) is robust across vitest pool modes (the
// system-gate-substrate.test.ts:59 idiom) — preferred over a bare __dirname.
const __dirnameLocal = dirname(fileURLToPath(import.meta.url));
const FIXTURES_DIR = join(__dirnameLocal, "../../fixtures/system-triage");
...
/** A `session_summary` `details` JSON payload (mirror system-health.test.ts:62-84). */
...
/**
 * Seed the store so a single `assembleSystemHealthReport` call reproduces the
 * corpus's signal classes. Mirrors system-health.test.ts:150-200, ADDING
...
function seedStore(store: ObservabilityStore, now: number): void {
  ...
  // The `signal` key drives the finding code via healthSignalLabel (system-health.ts:106-114).
  store.insertDiagnostic({
    timestamp: now - 2_100,
    category: "health_signal",
    severity: "warning",
...
describe(
  "system verification: obs.system.health reproduces the frozen corpus in one call without grep",
  () => {
    it("ONE assembleSystemHealthReport call reproduces both flagships + model/config (I-track) + the A-track rates", async () => {
      ...
      seedStore(store, now);
      const dataDir = fs.mkdtempSync(join(os.tmpdir(), "system-reprove-"));
      ...
      // day-files, so it is NOT asserted (system-health.ts:365-372).
      ...
      expect(() => assertNoSecrets(JSON.stringify(report), "system-reprove-report")).not.toThrow();
    });
    ...
    it("CONTRASTS the manual cost the lens replaces — narrative coverage, NO hard-coded number", () => {
      ...
      // scrape) the ONE-call system report REPLACES. The contrast is QUALITATIVE —
    ...
    // data (the assembler reads the injected clock ONCE, system-health.ts:309). That
    // invariant is proven directly, with two assemblies, in the unit suite
    // (system-health.test.ts "is DETERMINISTIC: same data + same fakeClock ->
    // byte-identical reports"). This scenario stays focused on the
