...
/**
 * Pure recurrence/gate scorer — THE TDD CORE of Phase 158 (PROVE & gate:
 * fleet-triage baseline, requirement M2).
 *
 * THE deliverable that gates Phase 160 (the I-track reorder/trim signal). The Plan-03
 * measurement reads the real `~/.comis` (read-only) and counts how often each
 * currently-log-only signal recurs; this module turns those counts into a per-signal
 * INSTRUMENT/SKIP verdict table: a signal that recurs enough is worth instrumenting in
 * Phase 160; one that never fires is not ("don't event a WARN that never fires").
 *
 ...
 * analog of Phase-149's diagnosis-gating-report.ts (GatingRow + buildGatingTable +
 * renderGatingMarkdown), MINUS the LLM-judge apparatus: 158 spends zero tokens, so
 * there is NO CostGovernor, NO judgeAnswer, NO COMIS_LIVE gate.
 *
 * THE LOAD-BEARING DISTINCTION (RESEARCH Pitfall 4 / Anti-Pattern): a `realCount === 0`
 ...
 * — NOT evidence it never recurs). A false confident-SKIP on the milestone's flagship
 * signal (LCD ×7) would silently gut Phase 160 — so the two cases produce DIFFERENT
 * verdicts. The measured dev `~/.comis` is light (LCD=0, budget=0), so this guard is
 * what stops the gate from trimming the whole I-track on a false negative.
 *
 * SECURITY (T-158-01-01): renderGapGateMarkdown runs `assertNoSecrets` over its output
 ...
 *   - INSTRUMENT-160     — recurs enough in real data → worth instrumenting in Phase 160.
...
/**
 * One row of the gap-gate table — a per-signal verdict on whether Phase 160 should
 * instrument it. Carries only counts / a signal name / a typed verdict — no raw WARN
 * bodies — so JSON.stringify(row) and the rendered markdown pass assertNoSecrets
 * (T-158-01-01).
 */
export interface GapGateRow {
  signal: FleetSignal["signal"];
  /** What THIS session's v2.14 review found by hand — from the M1 corpus. */
  byHandCount: number;
  /** Measured recurrence in the REAL `~/.comis` (Plan 03). */
  realCount: number;
  ...
  /** Did the emitting code path run on the measured machine? (the Pitfall-4 discriminator). */
 ...
 * occurrences in the measured window. A LOW named constant (RESEARCH A2) because this
...
/**
 * The deterministic verdict function — the load-bearing Pitfall-4 distinction.
 *
...
/** True for the signals deferred OUT of the milestone (native node-llama-cpp stdout). */
function isOutOfScope(signal: FleetSignal["signal"]): boolean {
  return signal === "model-health-deferred";
}

/**
 * Map M1 corpus signals + measured recurrence counts into raw {@link GapGateRow}s.
 *
 ...
 * crash — the tolerant-counter rule, Security V5, mirroring diagnosis-harness.ts
 ...
 * logic is exercised on a row-in → row-out boundary (mirroring the 149
 ...
 * signal the Phase-160 planner reads).
 *
 * The CAVEAT block (the WARNING-PARTIAL-GATE analog, diagnosis-gating-report.ts:180-188)
 ...
 * (Pitfall 4).
 *
 * Runs `assertNoSecrets` over the output before returning (T-158-01-01,
 * defense-in-depth — the residency rule applied to every persisted-bound string).
 */
export function renderGapGateMarkdown(
  ...
  if (inconclusiveCount > 0) {
    // Pitfall 4: an INCONCLUSIVE is NOT a confident SKIP. Surface the count so the
  ...
  // T-158-01-01: never persist/return a string that carries a credential shape.
