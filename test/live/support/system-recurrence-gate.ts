...
/**
 * Pure recurrence/gate scorer — the TDD core of the system-triage baseline gate.
 *
 ...
 * system-recurrence-gate.test.ts (TDD mode) BEFORE the script consumes it — the EXACT
 ...
 *   - INSTRUMENT         — recurs enough in real data → worth instrumenting.
 ...
 * count). Only then does recurrence decide: >= threshold → INSTRUMENT; otherwise a 0
...
function verdict(r: {
  ...
  if (r.realCount >= RECURRENCE_THRESHOLD) return "INSTRUMENT";
...
function isOutOfScope(signal: SystemSignal["signal"]): boolean {
 ...
 * per signal, and a summary line counting INSTRUMENT verdicts (the reorder/trim
...
export function renderGapGateMarkdown(
  ...
  const instrumentCount = rows.filter((r) => r.verdict === "INSTRUMENT").length;
  const inconclusiveCount = rows.filter((r) => r.verdict === "INCONCLUSIVE").length;

  const lines: string[] = [];
  lines.push("# System observability gap gate");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    `**Summary:** ${instrumentCount} INSTRUMENT verdict(s) of ${rows.length} signal(s) — ` +
      "these are the log-only signals that should become structured health data.",
  );
  if (inconclusiveCount > 0) {
    ...
    lines.push(
      `**NOTE — INCONCLUSIVE:** ${inconclusiveCount} signal(s) showed 0 occurrences on an ` +
        ...
        "daemon before trimming these from the instrumentation scope.",
    );
  }
  lines.push("");
