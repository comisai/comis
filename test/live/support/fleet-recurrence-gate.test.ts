...
describe("buildGapGateTable — per-signal recurrence verdict (the gate for Phase 160)", () => {
  it("buildGapGateTable returns INSTRUMENT-160 for a recurring log-only signal", () => {
    ...
    expect(table[0]!.verdict).toBe("INSTRUMENT-160");
...
describe("recordSignalRecurrence — maps a corpus + measured counts into GapGateRow[]", () => {
  ...
  it("recordSignalRecurrence carries byHandCount from the corpus + realCount from the measurement", () => {
    const measured = new Map<FleetSignal["signal"], { realCount: number; pathExercised: boolean }>([
...
describe("renderGapGateMarkdown — the gate artifact (markdown table + caveat + summary)", () => {
  ...
  it("renderGapGateMarkdown emits a markdown table, a data-scoped CAVEAT, and an INSTRUMENT-160 summary", () => {
