...
/**
 * Pure system-triage corpus loader + contract types.
 *
 ...
 * loader is RED→GREEN unit-tested in system-triage-corpus.test.ts BEFORE the
 ...
 * system-health review, NOT a reconstruction from a rotating `daemon.1.log`. `loadCorpus`
...
/**
 * One system-triage signal from the by-hand review — the reference the
 * `obs.system.health` lens must reproduce. Carries counts + typed enums only, NEVER
 ...
 * as the verification bar. The `obs.system.health` lens is proven when it
 ...
 * shape-guard's membership check (it must stay in lockstep with the {@link SystemSignal}
 ...
 */
const SIGNAL_VALUES = new Set<SystemSignal["signal"]>([
  "lcd-ingest-skipped",
  "mcp-reconnect",
  "budget-exceeded",
  "config-posture",
...
const LOCATION_VALUES = new Set<SystemSignal["location"]>([
  "daemon.log",
  "trajectory.jsonl",
  "obs_diagnostics",
  "native-stdout",
...
function parseJsonFile(dir: string, file: string): unknown {
  ...
  try {
    ...
    throw new Error(`system-triage-corpus: malformed JSON in fixture file ${file}`);
  }
}

/**
 * Validate that a parsed `triage-corpus.json` has the load-bearing SystemSignal[]
 ...
 */
function assertSignals(parsed: unknown): SystemSignal[] {
  if (parsed === null || typeof parsed !== "object") {
    // Guard before dereferencing — a `null`/primitive root must not leak a raw TypeError.
    throw new Error("system-triage-corpus: malformed triage-corpus.json — not an object");
  }
  const signals = (parsed as { signals?: unknown }).signals;
  if (!Array.isArray(signals) || signals.length === 0) {
    // Path only — never echo the parsed body.
    throw new Error("system-triage-corpus: malformed triage-corpus.json — signals[] missing or empty");
  }
  for (const entry of signals) {
    if (entry === null || typeof entry !== "object") {
      // A `null` / primitive entry must not leak a raw TypeError on `entry.signal`.
      throw new Error(
        "system-triage-corpus: malformed triage-corpus.json — a signal entry is not an object",
      );
    }
    const s = entry as Partial<SystemSignal>;
    if (
      ...
      throw new Error(
        "system-triage-corpus: malformed triage-corpus.json — a signal entry lacks a load-bearing field",
      );
    }
    if (
      !SIGNAL_VALUES.has(s.signal as SystemSignal["signal"]) ||
      !LOCATION_VALUES.has(s.location as SystemSignal["location"])
    ) {
      // The closed-union contract (README + SystemSignal JSDoc): a typo'd / stale signal
      // or location name is a corrupt artifact, not a new gate row.
      throw new Error(
        "system-triage-corpus: malformed triage-corpus.json — signal/location out of the closed enum",
      );
    }
  }
  return signals as SystemSignal[];
}
...
function assertManualCost(parsed: unknown): ManualCostToBeat {
  if (parsed === null || typeof parsed !== "object") {
    ...
    throw new Error(
      "system-triage-corpus: malformed manual-cost-to-beat.json — not an object",
    );
  }
  ...
  if (
    ...
    // Path only — never echo the parsed body.
    throw new Error(
      "system-triage-corpus: malformed manual-cost-to-beat.json — a load-bearing field is missing or mis-typed",
    );
  }
 ...
 * `manual-cost-to-beat.json`) into a {@link SystemCorpus}.
 *
 ...
 */
export function loadCorpus(dir: string): SystemCorpus {
  return {
