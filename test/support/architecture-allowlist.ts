...
export const fileSizeAllowlist: readonly FileSizeAllowlistEntry[] = [
  ...
  },
  {
    ...
    lines: 1636,
    reason: "Executor-adjacent afterTurn finally-block module (1,636L re-measured after Phase 169 CWF-05 addition). Already over the 800L cap; Phase 129-06 added a THIN gated call (runLeafPassAfterTurn — body in lcd-compaction-trigger.ts) and Phase 130-02 added a SECOND THIN gated call (runCondensePassAfterTurn — body in lcd-condense-trigger.ts) + its import comment, both inside the existing if (deps.contextStore) block, per the plan's hard constraint that the pass bodies live elsewhere. Phase 169 added ~20 lines of degraded-reply wiring at the effectiveFinishReason chokepoint (import + CWF-05 gate block). The afterTurn write-path bodies (ingest, leaf trigger, condense trigger) all live in sibling modules; this file remains an orchestration shell of per-turn cleanup steps. Defer pending global-removal shrinkage or a focused post-run-cleanup/metrics helper extraction",
    removedIn: "deferred",
  },
  ...
  // api/*-handlers.ts split into per-domain modules)
  {
    ...
    reason: "daemon.ts composition root after stage decomposition collapse. Single file holds main() + a few small helpers (DEFAULT_CONFIG_PATHS / applyInspectDefaultsForLogging / hardenDataDirPermissions) + inlined foundation/agents/channels/gateway/shutdown bodies + 30 ex-stage-helper functions. `runPreflightDoctor` was extracted to wiring/preflight-doctor.ts (Phase 196) — re-exported from daemon.ts so its surface is unchanged — to keep this file under its 3000L hard cap (__tests__/architecture.test.ts:173) after the v2.25 audio wiring. The 5-stage decomposition was removed because its enforcement test (200-LOC-per-stage rule at __tests__/architecture.test.ts:174-382) created more structural overhead than the per-stage cap mitigated. The bootstrap-order assertion (5-stage call sequence + handle chaining contract) was deleted entirely; its runtime invariant is now carried by integration tests — daemon-lifecycle.test.ts:89-99 asserts the 5 startup log lines emit in source order. Composition-root cap raised to 3000L (350L headroom over re-measured 2,700L baseline).",
    removedIn: "deferred",
  },
...
export const testNamingAllowlist: readonly TestNamingAllowlistEntry[] = [
  ...
  { file: "packages/skills/src/tools/browser/config.test.ts", line: 60, kind: "it", text: "accepts valid port", reason: "Captured(min-length=18); shrink in follow-on work"},
  { file: "packages/skills/src/tools/browser/config.test.ts", line: 113, kind: "it", text: "overrides headless", reason: "Captured(min-length=18); shrink in follow-on work"},
  { file: "packages/skills/src/tools/browser/config.test.ts", line: 118, kind: "it", text: "overrides noSandbox", reason: "Captured(min-length=19); shrink in follow-on work"},
