...
export const testNamingAllowlist: readonly TestNamingAllowlistEntry[] = [
  ...
  { file: "packages/core/src/config/schema-agent.test.ts", line: 1389, kind: "it", text: "rejects empty id", reason: "Captured(min-length=16); shrink in follow-on work; line re-synced after Phase 95 rag.lanes + rag.lanes.temporal inserts, then Phase 96 rag.lanes.causal insert, then Phase 100 rag.lanes.graphSpread insert, then Phase 102 rag.mmr + rag.queryUnderstanding inserts, then Phase 112 rag.forget + rag.scoring.forgetAlpha inserts"},
