...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      // CI shards this tier (`vitest --shard=N/4 --coverage --reporter=blob`), so a
      // shard step carries only its own slice of the suite and can never reach a
      // run-level floor. It sets VITEST_SHARD_COLLECT_ONLY=1 to collect WITHOUT
      // enforcing; the merge job (`--merge-reports --coverage`) and any local run
      // with the env unset enforce on the FULL coverage. Same protocol as the unit
      // tier in the root config — leaving the threshold live per-shard reds every
      // shard at once and reads like a coverage drop rather than a config error.
