 ...
 *   1. The apiRoot redirection flows through @discordjs/rest
 ...
 * mock supports it, but discord.js's gateway client expects intricate
 * timing that is brittle inside vitest's 60s timeout. The REST wire
 * surface proven here is the load-bearing surface for outbound dispatch.
 *
 * @module
 */
...
describe("E2E: discord × dm — REST wire roundtrip against the 127.0.0.1 mock", () => {
  ...
  it("validates a bot token by hitting /api/v10/users/@me through the redirected REST client", async () => {
    ...
    // refactor actually flows through @discordjs/rest.
