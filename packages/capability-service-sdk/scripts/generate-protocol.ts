...
const METHOD_CATALOG = [
  ...
  },
  {
    ...
    direction: "service-to-comis",
    callerClass: "capability-service",
    requiredServiceScope: "approval_receipt",
    classification: "mutation",
    operationIdRequired: true,
    maxRequestBytes: CAPABILITY_SERVICE_LIMITS.maxRequestBytes,
    maxResponseBytes: CAPABILITY_SERVICE_LIMITS.maxResponseBytes,
    semanticInvariants: [
      "operation-id-must-match-envelope-id",
      "owning-service-instance-and-managed-run-only",
      "approval-request-and-mcp-operation-must-match-host-binding",
      "grant-must-be-unexpired-and-approved",
      "identical-consume-replay-returns-the-original-receipt",
      "altered-or-second-consume-is-rejected",
    ],
    requestSchema: "schemas/consumeApproval.request.schema.json",
    responseSchema: "schemas/consumeApproval.response.schema.json",
  },
  {
...
const SCHEMAS: ReadonlyArray<{ readonly path: string; readonly schema: ZodType }> = [
  ...
  { path: "schemas/consumeApproval.request.schema.json", schema: CapabilityConsumeApprovalRequestSchema },
  { path: "schemas/consumeApproval.response.schema.json", schema: CapabilityConsumeApprovalResponseSchema },
  ...
  { path: "schemas/mcp-managed-run-group-result.schema.json", schema: McpManagedRunGroupResultSchema },
