...
/**
 * The contract for the single metric catalog.
 *
 ...
 * set). The OTLP push surface and Prometheus pull surface BOTH read it —
 * one mapping, two readers, so the two surfaces can never drift.
 *
 * These pins guard the three load-bearing invariants the metric catalog + the
 * no-high-cardinality requirement demand:
 *
 ...
 *       ride as EXEMPLARS only; defining the union closed HERE means a
 *       high-cardinality label cannot be introduced.
 *   (b) the FULL metric set is present (a minimum count + the
 *       load-bearing names the dashboards/rules key on).
 *   (c) `comis_build_info` carries ONLY a `version` label — NO `commit` (no
 *       `git rev-parse` runs at daemon boot, so a commit label would be a
 *       runtime-unavailable phantom).
 *
 * @module
 */
...
// The high-cardinality labels that must NEVER appear (the PII/DoS guard).
...
describe("metric-catalog", () => {
  it("is a frozen, non-empty catalog of MetricDef entries", () => {
    ...
    // The metric catalog enumerates ~24 distinct series (cost/tokens/cache/
  ...
  it("(b) contains the full metric set — the load-bearing Prometheus names", () => {
    ...
    const required = [
      ...
      // No comis_cache_break_cost_usd_total — the cache_break bus event carries no
      // cost field; estCostUsd is computed downstream, not emitted, so the metric
      // would be unsourced. The $-lost-by-reason view lives in `comis explain`, not
      // on the pull surface.
  ...
  it("(b') instrument types match the design (cost/tokens/breaks = counter; spend = gauge; durations + read-ratio = histogram)", () => {
    ...
    // via meter.createObservableGauge).
    ...
    // comis_up + comis_build_info are observableGauge: a constant series
  ...
  it("(c) comis_build_info carries ONLY a `version` label — never `commit`", () => {
