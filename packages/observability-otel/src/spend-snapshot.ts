 ...
 * `SpendAccumulator` (`@comis/agent`) into this extension's `comis_spend_*`
 * observable gauges.
 *
 ...
 * byte-identical to this). Mirrors `SpendAccumulator.getSnapshot()`:
 * content-free dollar counts keyed by the `${tenantId} ${agentId}` /
 * `tenantId` scope keys.
 *
 ...
 * kill-switch counters. `perAgent` reflects BOTH billed spend and
