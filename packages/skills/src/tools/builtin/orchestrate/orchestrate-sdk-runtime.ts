 ...
 * `{ bearer, method, params, operationId? }` newline-JSON line over the lease-authenticated
...
/** Direct platform side effects that may retry only with one stable identity. */
const OUTWARD_MESSAGE_METHODS = new Set(["message.send", "message.reply", "message.react"]);
...
/** Distinguishes a complete endpoint rejection from a transport/response loss. */
class CapApplicationError extends Error {}

function callCapSocketOnce(socketPath: string, payload: string): Promise<unknown> {
  return new Promise<unknown>((resolve, reject) => {
    ...
    socket.on("data", (chunk: string) => {
      ...
      try {
        ...
        finish(() => reject(new Error("malformed response from cap socket")));
        return;
      }
      if (reply.error !== undefined) {
        const message =
          typeof reply.error === "string" ? reply.error : "capability call failed";
        finish(() => reject(new CapApplicationError(message)));
        return;
      }
...
/**
 * Send one `{ bearer, method, params, operationId? }` line over the cap socket for an ARBITRARY
 ...
 * @param operationId - Stable caller identity required for outward operations
 *   and preserved across one transport-loss retry.
...
export function callCapSocket(
  ...
  const isOutwardMessage = OUTWARD_MESSAGE_METHODS.has(method);
  if (isOutwardMessage && operationId === undefined) {
    return Promise.reject(new Error("outward operation identity is required"));
  }
  if (operationId !== undefined && (operationId.length === 0 || operationId.length > 256)) {
    return Promise.reject(new Error("outward operation identity must contain 1 to 256 characters"));
  }
  const payload =
    JSON.stringify({
      bearer,
  ...
  const firstAttempt = callCapSocketOnce(socketPath, payload);
  if (!isOutwardMessage) return firstAttempt;
  return firstAttempt.catch((error: unknown) => {
    if (error instanceof CapApplicationError) throw error;
    return callCapSocketOnce(socketPath, payload);
  });
}
