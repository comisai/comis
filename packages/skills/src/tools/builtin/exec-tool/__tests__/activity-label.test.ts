...
/**
 * exec-tool activity-label integration — Phase 78 WS-A + WS-C / LBL-02 /
 * SPEC-§6.3.
 *
 ...
 * shell summarizer landed in Phase 70 / STRAT-09). The hook lives on the spec
 * at registration time (78-01 added the `transform?` field to
 ...
 *
 * The fourth case is the SEC-07 belt-and-braces redaction lock: a command that
 ...
 *   2) `applyTemplate` step 4 (78-01) pipes the transform output through
...
describe("exec-tool activity label (Phase 78 WS-A + WS-C / SPEC-§6.3)", () => {
  beforeAll(async () => {
  ...
  it("Case A — registers a LabelSpec for exec with a transform hook present", () => {
    ...
    // BOTH the fallback label ("running command", which 78-04 falls through to
    // when the transform returns "") AND a defined transform function (the
    // parseShellCommand bridge — LBL-02).
  ...
  it("Case B — applyTemplate for {command: 'npx tsc'} renders the parseShellCommand output verbatim", () => {
    ...
    // the live equality check (LBL-02 runtime gate).
  ...
  it("Case D — SEC-07 belt-and-braces: a secret-shape token in the command renders as <redacted>", () => {
