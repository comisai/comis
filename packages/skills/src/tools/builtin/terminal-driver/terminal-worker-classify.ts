...
/**
 * terminal-worker-classify -- the worker's per-session classify-and-emit glue,
 * extracted from `terminal-worker-entry.ts` so that file
 ...
 * snapshot), runs the pure {@link classifyFrame}, and hands the
 * {@link Classification} to the per-session {@link AttentionEmitter} —
 * which writes a {@link TerminalEventFrame} to fd3 ONLY on a state TRANSITION. This is
 * the no-poll mechanism: the worker is driven by the settle it already runs,
 * NEVER by a timer.
 *
 * PROGRESS tracking (stuck-by-progress, no clock inside the classifier): the
...
export interface ObserveSettledFrameArgs {
  ...
  state: SessionState;
  /** The per-session attention emitter over the worker's injected `writeFd3`. */
  ...
  /**
   * Suppress the fd3 ATTENTION write for this frame while still updating the
 ...
 * first (the stability flush) so the snapshot reflects every emitted byte —
...
export async function observeSettledFrame(args: ObserveSettledFrameArgs): Promise<void> {
  ...
  // the progress clock; otherwise noProgressMs grows toward stuckMs. The very
  ...
  // The selected platform profile (by the session's operator-declared allowId
  // — never content-sniffed). The classifier stays the sole owner of `activity`: it
  // consumes the profile's perception + the dialog `detect` patterns. Absent ⇒ generic path.
  ...
  // `diffEmpty` (the prerequisite for awaiting-input) means the SETTLED screen
  ...
  // Edge-triggered fd3 emit (the emitter writes only on a state transition). On a
 ...
 * in the worker (no daemon-side grid duplication). A point-in-time
 ...
 * are the attention emitter's edge-trigger anchors — a read-only status
 ...
 *
 * The reply also carries the classifier's `confidence` + `reason`
...
export async function statusReplyFromState(args: StatusReplyArgs): Promise<WorkerStatusPerception> {
  ...
  // The selected platform profile (by allowId). Same generic fallback
  // as the settle path — absent ⇒ unchanged.
  const profile = state.allowId !== undefined ? getPlatformProfile(state.allowId) : undefined;
  const perception = profile?.perception;
  const dialogDetects = profile?.dialogs?.map((d) => d.detect);
  ...
  return {
    ...
    screenDiffEmpty,
    interactions: state.interactions,
    // Surface the classifier's own confidence + reason (computed above at
    // `classification`) — never a hardcoded constant; this is the WHY/HOW-SURE the
    // status tool + the autonomous policy read.
