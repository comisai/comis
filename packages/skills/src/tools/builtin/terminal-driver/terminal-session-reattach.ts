...
/**
 * The recover-on-boot SCAN orchestrator + the injected descriptor-store port. This is
 * the SIBLING that keeps the registry under the 800-line cap: the registry's
 * recover-on-boot is a thin call into {@link recoverSessionDescriptors}, never an
 * inlined scan loop.
 *
 * WHY THIS EXISTS (the load-bearing gap, framed in terminal-reattach-match.ts).
 ...
 * persisted {@link SessionDescriptor}s, run each through the pure
 * {@link reattachDecision}, and yield a typed list the registry switches on.
 *
 * THE DECISION VOCABULARY (mirrors {@link reattachDecision}, NOT a second copy). Each
 * recovered descriptor maps to exactly one {@link RecoveredAction}:
 ...
 *     frame (the worker's `has-session`-gated backend re-attaches the surviving
 *     pane on the NEXT read; a fresh create would double-drive). The descriptor's
 *     persisted identity (allowId/owner/scope) is carried VERBATIM.
 ...
 *     discriminant keeps {@link reattachDecision}'s internal name `failed`, but there
 *     is NO `failed` session_state member — the registry EMITS `lost`; the user-facing
 *     `failed` OUTCOME is layered downstream. This module NEVER touches
 ...
 *     out of the result) so the registry's existing lost floor stands unchanged.
 *
 ...
 *     port, NOT here — a testability seam, NOT because skills→observability
 *     is forbidden (it is allowed; this module simply needs no I/O).
 *   - TOTAL / NEVER throws: a corrupt descriptor (`deserializeDescriptor`
 ...
 * (the consumer side); the DAEMON IMPLEMENTS it as a sibling of the drive-journal
 * store (the fs-safe write + recover-on-boot scan over the confined durable
 ...
 * lost floor handles it), so the registry only ever sees `reattach`/`failed`.
 *
 * The `failed` arm carries the `owner` (NOT just the sessionId from the decision)
 * so the registry can fire its content-free unrecoverable hook with the `agentId`
 * (a failure branch is reconstructable per-agent from the bus alone) WITHOUT a
 * second descriptor lookup. The owner is the persisted identity, never widened.
 */
...
export interface RecoverSessionDescriptorsDeps {
  /** The durable descriptor store (daemon impl in production; a fake in unit tests). */
 ...
 * the pure {@link reattachDecision}. PURE (through the injected port), TOTAL,
 * NEVER throws.
 *
 ...
 *     reason is carried for the content-free hint/errorKind.
 ...
 * recovered", identical to today's empty-Map-on-boot behavior.
 *
...
export function recoverSessionDescriptors(deps: RecoverSessionDescriptorsDeps): RecoveredAction[] {
  ...
  try {
    ...
    // the reaper/operator reaps by name, never a daemon-boot crash.
    return [];
  }
...
/**
 * Rebuild a `running` {@link SessionHandle} from a recovered descriptor (WHERE
 ...
 * plus the two durability handle fields `durable`/`tmuxName` (the durable marker the
 ...
 * the next read (never double-drive).
 *
...
/**
 * The durable-aware lost gate (pure). `true` iff `handle` must NOT be
 * flipped `lost` on a worker close/crash: a DURABLE drive whose detached tmux server is
 * still alive outlives the worker (the lazy respawn re-attaches the surviving pane).
 ...
 * The registry consults it at BOTH lost sites (the worker-close flip + the
 ...
 * EXCEPT a durable session whose tmux is still alive ({@link staysRecoverable} — its
 * detached server outlives the worker; the lazy respawn re-attaches the surviving
 * pane on the next read). The registry passes its map + injected probe; the loop body
 * lives here (cap headroom). The non-durable / no-probe case keeps today's
 * blanket lost flip.
 */
...
export interface DurableCreateInputs {
  sessionId: string;
  tmuxName?: string;
  /** The per-boot `-S` socket this durable session's tmux server is bound to (persisted
 ...
 * (persist BEFORE the create frame so a SIGKILL mid-create cannot orphan
...
/**
 * The durability seams the registry bundles as one nested `durability?` dep (so the
 ...
 * ⇒ a no-op (today's empty-Map-on-boot). The hooks are injected (NOT a value-imported bus)
 * so the registry stays infra-decoupled; the fs-safe descriptor write lives in the daemon impl.
 *   - `descriptorStore` — persist-at-create + recover-on-boot (the daemon's fs impl).
 ...
 *     `terminal:session_state(state:"lost")` + a content-free reason (NO `failed` member).
 */
...
/**
 * The minimal worker round-trip the recover-on-boot re-attach drives:
...
/**
 * Drive ONE recovered session's worker re-attach + gate the obs hooks
 ...
 * On `ok:true` → fire `onReattached` (the confirmed re-attach is the obs record). On
 ...
 * by the daemon holder). Best-effort/never-throws (a faulting round-trip resolves the
 * safe `lost` direction).
 */
async function driveWorkerReattach(
  ...
  // hook + drop the now-dead DESCRIPTOR (its tmuxName no longer resolves, so keeping
...
/**
 * The recover-on-boot APPLICATION: scan the persisted descriptors and, for each,
 * either rehydrate a `running` handle into the registry's `sessions` map (a live tmux,
 * NO create frame) and DRIVE the worker re-attach (gating the obs hooks on the
 * worker's confirmation), or — for a session gone at the boot probe — fire the
 * content-free unrecoverable hook + remove the dead DESCRIPTOR (the daemon maps the
 ...
 * descriptor is skipped (today's lost floor). The `descriptorStore`-absent case is a
 * no-op. The BULK lives here (not the registry) to protect the 800-line cap;
 * the registry's recover-on-boot is ONE call.
 *
 ...
 * @param reattachWorker - The registry's `reattach` worker round-trip.
  ...
  for (const r of recoverSessionDescriptors({ store: deps.descriptorStore, isTmuxAlive })) {
    if (r.action === "reattach") {
      ...
      if (reattachWorker !== undefined) {
        // Ask the worker to re-attach the surviving pane; gate the obs hooks on its
