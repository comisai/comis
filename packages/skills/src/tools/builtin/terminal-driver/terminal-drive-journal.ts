...
/**
 * The pure bounded content-free drive-state journal.
 *
 ...
 * reads/updates) and the durable persistence beside `terminal-workspace.ts` live
 * elsewhere — which is exactly why this shape is cleanly serializable and
 * {@link deserializeJournal} is defensive enough to read a corrupted-after-crash file.
 * NO fs write happens here.
 *
 * The three load-bearing properties:
 *   - BOUNDED: {@link appendAnswered}/{@link appendStep} oldest-trim
 ...
 *     woken-turn driver) runs `scrubSecretsFromText` before handing a tag/digest
 *     in; this module treats every tag as an OPAQUE short string and never re-expands it.
 *   - TOTAL (de)serialize (the crash-recovery contract): {@link serializeJournal} →
...
// ---------------------------------------------------------------------------
// Caps — documented constants, like terminal-loop-guard's window/max.
// ---------------------------------------------------------------------------
 ...
 * `objective`, or `lastScreenDigest`. The caps above bound the array COUNT;
...
/** The safe default classification for a fresh / unreadable journal (a shipped state). */
...
// ---------------------------------------------------------------------------
// The shape — the MINIMAL rolling set; content-free.
// ---------------------------------------------------------------------------
 ...
 * bytes, command output, secrets, or keystrokes. This is the resume substrate
 * the durability layer persists; keep it cleanly serializable.
 */
export interface DriveJournal {
  /** A short content-free label for the drive (NOT raw TUI bytes). */
  objective: string;
  /** The last classifier verdict — the SHIPPED state union (never an invented state). */
  lastClassification: ClassifierState;
  /**
   * A REDACTED one-line digest of the last rendered screen (`terminal-read-digest.ts`
   * produces it; the caller runs `scrubSecretsFromText` before it lands here). Never the grid.
   */
  lastScreenDigest: string;
  /** Normalized prompt-hashes/tags already answered (no raw prompt text). Capped. */
  answeredPrompts: string[];
  /** Enum/short-tag steps already tried (no raw command output). Capped. */
  stepsTried: string[];
  ...
  costUsd: number;
  /**
   * The truncation breadcrumb — the run-total count of tag entries dropped by oldest-trim
   * across BOTH arrays. A non-zero value means the drive exceeded a cap; it is NEVER a
   * silent drop.
   */
  truncations: number;
}
...
export function emptyJournal(objective: string): DriveJournal {
  return {
    // Clamp the objective to the per-entry byte bound (a degenerate/over-long
 ...
 * the appended tag to {@link TAG_MAX} bytes (the per-entry SIZE bound). Returns
 ...
 * `truncations` breadcrumb (never a silent full-size keep). Pure — never mutates `arr`.
 ...
 * journal (immutable). The caller supplies an already-normalized/redacted tag; this
...
export function updateJournal(
  ...
  return {
    ...
    // Clamp the digest to the per-entry byte bound (the caller hands an ≤80-char
...
// ---------------------------------------------------------------------------
// (De)serialize (the durable-persistence-ready shape) — pure, total, defensive.
// ---------------------------------------------------------------------------

/** Serialize a journal to a JSON string. Pure; no fs write (the durability layer persists it). */
 ...
 * cannot blow the count cap) AND clip each entry to {@link TAG_MAX} bytes (a
...
/** True iff `v` is one of the SHIPPED classifier states. */
 ...
 * parses, defaulting to `{}` on invalid JSON) or an already-parsed value (the durable
 * recover-on-boot path may hand a parsed object). This is the read-side of the durable resume contract:
...
export function deserializeJournal(raw: string | unknown): DriveJournal {
  ...
  const r =
    parsed !== null && typeof parsed === "object" && !Array.isArray(parsed)
      ...
      : {};
  return {
    // Clip the scalar string fields too — a corrupted file with a multi-kilobyte
