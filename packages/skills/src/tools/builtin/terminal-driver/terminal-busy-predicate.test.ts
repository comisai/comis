...
/**
 * Unit tests for the pure busy-vs-hung predicate
 * (terminal-busy-predicate.ts).
 *
 * `busyOrHung({ alive, noProgressMs, stuckMs })` is the SINGLE liveness/endurance
 * signal the daemon backstop and the reaper's alive-busy idle exclusion
 ...
 *
 * The doctrine under test: the worst outcome is a false death — a legitimately
 ...
 * waiting), never a false "hung". It reads NO screen: the caller passes the
...
// ---------------------------------------------------------------------------
// Recent progress is BUSY (a CPU-busy / output-trickling compile
// is never killed). The defining contract of the whole long-drive regime.
// ---------------------------------------------------------------------------

describe("busyOrHung — recent progress is busy (a working drive is never killed)", () => {
...
// ---------------------------------------------------------------------------
// No progress past the stuck window is HUNG (the genuinely-idle,
// no-transition case the backstop synthesizes `stuck` for).
// ---------------------------------------------------------------------------

describe("busyOrHung — no progress past the stuck window is hung", () => {
...
// ---------------------------------------------------------------------------
// A dead backend is HUNG regardless of timing (tmux has-session false).
// ---------------------------------------------------------------------------

describe("busyOrHung — a dead backend is hung regardless of timing", () => {
...
// ---------------------------------------------------------------------------
// The strict-`>` boundary (mirrors checkWallClock's strict >):
// AT the window the budget is not yet exceeded ⇒ still busy.
// ---------------------------------------------------------------------------

describe("busyOrHung — the stuck window is a strict > boundary", () => {
...
// ---------------------------------------------------------------------------
// TOTAL / never-throws: a degenerate input yields the SAFE direction
// ("busy", keep waiting) — NEVER a false "hung".
// ---------------------------------------------------------------------------

describe("busyOrHung — degenerate input is busy (the safe direction, never throws)", () => {
