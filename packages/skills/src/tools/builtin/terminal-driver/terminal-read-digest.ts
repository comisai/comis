...
/**
 * The pure bounded digest/diff read selector.
 *
 ...
 * trim. The byte cap is {@link READ_DIGEST_BYTE_CAP} (documented like
 ...
 * `lastScreenDigest`: `"<rows>r <cols>c, <changed> changed,
 * cursor@(x,y)"` plus a SHORT excerpt of the first non-empty line — content-free BY
 * CONSTRUCTION (counts/coords; it never dumps the whole grid). The CALLER
 * (the woken-turn driver) runs `scrubSecretsFromText` over the excerpt
 * before it reaches the journal/notification — the excerpt this module emits is
 * deliberately short, not pre-redacted (redaction is the wiring layer's job;
 * `scrubSecretsFromText` is the canonical redactor — don't hand-roll another).
 *
...
// ---------------------------------------------------------------------------
// Bounds (documented constants, the anti-unbounded guarantee).
// ---------------------------------------------------------------------------
 ...
 * read is clipped with a `truncations` breadcrumb — never a silent trim.
 */
...
// ---------------------------------------------------------------------------
// Types — the read-mode selector contract.
// ---------------------------------------------------------------------------
 ...
 * `full`) AFTER the byte cap. `truncated` is the breadcrumb flag — `true` iff
...
export interface ReadDigest {
  ...
  screen: string;
  /** The truncation breadcrumb — `true` when the byte cap clipped the result. */
  truncated: boolean;
 ...
 * bytes — the anti-silent-trim breadcrumb. Pure; never throws (a non-positive cap
...
/**
 * Select + bound the text a woken drive turn should see this wake.
 *
 ...
 * breadcrumb — never a silent trim.
 *
...
// ---------------------------------------------------------------------------
// The content-free one-line screen digest (the journal's lastScreenDigest).
// ---------------------------------------------------------------------------
 ...
 * excerpt, NEVER the whole grid. The CALLER (the woken-turn driver) runs
