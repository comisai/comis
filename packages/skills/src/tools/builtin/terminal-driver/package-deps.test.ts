...
/**
 * Supply-chain assertion for the terminal-driver native-dep install posture.
 *
 ...
 *     FAILING the install — spec §2.2), exact-pinned `1.1.0`;
 *   - `@xterm/headless` + `@xterm/addon-serialize` are regular dependencies
 *     (the worker owns the emulator; they are declared so the dep graph is
 *     stable, even though rendering happens later), exact-pinned;
 ...
 * node-pty) is proven by the injected-thrower RED test; this is the
...
describe("terminal-driver supply-chain (install posture)", () => {
