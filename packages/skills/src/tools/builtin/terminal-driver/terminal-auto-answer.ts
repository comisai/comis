...
/**
 * The pure `safe-only` auto-answer policy.
 *
 ...
 *   3. ESCALATE-ALWAYS VETO (structural) → when a safe pattern matched, if that SAME
 ...
 *
 * OPERATOR-DIALABLE, NEVER model-dialable: `mode` + `hintPatterns` come
 ...
 *     {@link AutoAnswerDecision}; the woken turn acts on it (an `answer`
 *     keystroke rides the `enforceSendCapsThenAudit` path; an `escalate` raises
...
/**
 * The typed auto-answer verdict. An `answer` carries the
...
// ---------------------------------------------------------------------------
// The escalate-always structural matchers (the veto gate)
// ---------------------------------------------------------------------------
//
...
/** Auth/login/credential cues — escalate-always (the credential-phishing guard). */
 ...
 * dialog path) — AUTH (credential-phishing) and DESTRUCTIVE (data-loss) cues ONLY,
 ...
 *
 * (Observed live: claude 2.1.x reworded its trust gate to add an "Enter
...
/** Does ANY matching dialog carry `destructive:true`? Order-independent: a destructive
...
/**
 * Decide whether to auto-answer a settled prompt or escalate it. Pure, total,
 ...
 * @param dialogs - The SELECTED platform profile's dialogs (selected by operator allowId,
 ...
 *   WINS over any dialog safeAnswer. Empty ⇒ exactly the hintPattern-only behavior.
...
export function decideAutoAnswer(
  ...
  //    under mode `all`, even if it declares a safeAnswer (the safety floor). Order-
  //    INDEPENDENT: ANY matching destructive dialog escalates, so a destructive entry can
  ...
  // 4. The ESCALATE-ALWAYS gate is a VETO that fires ONLY when we WOULD otherwise send a
  ...
  //    escalated (a CLI cannot phish a safe affordance beneath an auth/destructive prompt —
  //    the phish BY DEFINITION renders a safe affordance, so it ALWAYS matches here, leaving
  ...
  //    that wedge the drive — observed live); gating the veto on an actual safe match removes
