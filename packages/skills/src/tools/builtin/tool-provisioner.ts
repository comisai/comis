...
const TOOLS_DIR = safePath(homedir(), ".comis", "bin");
...
export function getToolPath(tool: "rg" | "fd"): string | null {
  ...
  const localPath = safePath(TOOLS_DIR, config.binaryName);
...
async function downloadTool(tool: "rg" | "fd"): Promise<string> {
  ...
  // 5. Download tar.gz
  const tempTarPath = safePath(TOOLS_DIR, `.${config.binaryName}-download.tar.gz`);
  const tempExtractDir = safePath(TOOLS_DIR, `.${config.binaryName}-extract`);
  ...
  try {
    ...
    const possiblePaths = [
      safePath(tempExtractDir, assetBaseName, config.binaryName),
      safePath(tempExtractDir, config.binaryName),
    ];
    ...
    const finalPath = safePath(TOOLS_DIR, config.binaryName);
