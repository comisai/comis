...
export function createCronTool(rpcCall: RpcCall): AgentTool<typeof CronToolParams> {
  ...
  return {
    ...
    description:
      ...
      'RUN OUTCOMES: status reports execution health while deliveryStatus reports the independent delivery result. deliveryStatus="accepted" means the final response reached the bound conversation, even when status="failed" because one or more tools failed. Report both facts; never describe an accepted delivery as missing or not run. ' +
