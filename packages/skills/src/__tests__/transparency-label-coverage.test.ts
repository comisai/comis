 ...
 * humanized fallback label and bypass the §17.6 review.
 *
 ...
 * passes for every tool and is a no-op gate (RESEARCH Pitfall 3). The coverage
...
describe("transparency label-coverage gate", () => {
  it("fails when a platform tool lacks both a LabelSpec and a suppressActivity flag", () => {
    ...
    for (const descriptor of createPlatformToolRegistry()) {
      ...
      const hasSpec = hasRegisteredLabelSpec(name); // NOT resolveLabelSpec (Pitfall 3)
      if (!suppressed && !hasSpec) offenders.push(name);
    }
    expect(offenders).toEqual([]);
  });

  it("binds the §17.6 notify spec on the EMITTED name notify_user, not the descriptor name notify", () => {
