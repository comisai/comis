...
/**
 * Validate working directory is within workspace bounds via safePath.
 * Returns an error message if outside bounds, null if valid.
 */
function validateCwd(workspacePath: string, cwd: string): string | null {
  try {
    safePath(workspacePath, cwd);
    return null;
  } catch (error) {
    if (error instanceof PathTraversalError) {
      return `Working directory outside workspace bounds: ${cwd}`;
    }
    throw error;
  }
}
  ...
  return {
    ...
    async execute(
      ...
      try {
        ...
        const cwd = cwdParam || workspacePath;

        // Validate cwd through safePath (only when user provides explicit cwd)
        if (cwdParam) {
          const cwdError = validateCwd(workspacePath, cwd);
          if (cwdError) {
            throwToolError("invalid_value", cwdError);
          }
        }
