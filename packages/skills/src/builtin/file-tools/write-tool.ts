  ...
  return {
    ...
    async execute(
      ...
      const resultData = await withFileMutationQueue(absolutePath, async () => {
        // Declared outside the `exists` branch so the !exists path can still
        // return them uniformly (always false/undefined for new files).
        let coldRead = false;
        let auditNotice: string | undefined;
        if (exists) {
          ...
          // --- V6: Cold-register-on-write ---
          // Prior behavior threw a read-before-write error when the tracker did
          // not contain the path. Root cause: the per-turn tracker was empty at
          // session start, so files the LLM legitimately knows (system-prompt
          // templates, cross-turn memory) got blocked. Now that the tracker is
          // session-lifetime (see session-tracker-registry.ts), the safer
          // behavior is: if tracker is missing the path but the file exists,
          // read it under the same mutex, register it, and let the downstream
          // staleness check (V7) run. Safety is preserved: if an external
          // writer mutated the file between our stat above and the cold-register
          // read, the staleness compare will fire [stale_file].
          if (tracker && !tracker.hasBeenRead(absolutePath)) {
            const currentContent = await fs.readFile(absolutePath);
            tracker.recordRead(absolutePath, stat.mtimeMs, 0, undefined, currentContent);
            coldRead = true;
            logger?.debug?.(
              "Cold-registered existing file during write (no prior tracker entry)",
              { path: absolutePath, sizeBytes: stat.size, reason: "cold-register-on-write" },
            );
            // Build audit notice appended to the tool result so the LLM observes
            // that it just overwrote a file it hadn't explicitly read this session.
            const PREVIEW_MAX = 500;
            const previewBuf = currentContent.subarray(0, PREVIEW_MAX);
            const truncatedBytes = currentContent.length - previewBuf.length;
            const previewText = previewBuf.toString("utf-8");
            const truncatedSuffix = truncatedBytes > 0
              ? `\n... (truncated, ${truncatedBytes} more bytes)`
              : "";
            auditNotice =
              `Note: wrote to existing file without a prior read in this session. ` +
              `Previous content (${formatSize(currentContent.length)}) overwritten. ` +
              `First ${Math.min(PREVIEW_MAX, currentContent.length)} bytes were:\n` +
              `---\n${previewText}${truncatedSuffix}\n---`;
          }

          // --- V7: Staleness check (runs even after cold-register to catch external racers) ---
          // IMPORTANT: re-stat the file so checkStaleness compares recorded
          // mtime against the CURRENT on-disk mtime, not the pre-cold-register
          // mtime captured above. Without re-stat, an external writer that
          // mutated the file between V5 (initial stat) and here would slip
          // past the safety gate because we'd feed the stale mtime in as the
          // "current" mtime. Cold-register records `stat.mtimeMs` from V5;
          // checkStaleness must see the fresh mtime to detect the race.
          if (tracker) {
            const freshStat = await fs.stat(absolutePath);
            const currentContent = await fs.readFile(absolutePath);
            const staleness = tracker.checkStaleness(absolutePath, freshStat.mtimeMs, currentContent);
            if (staleness.stale) {
              throw new Error(
      ...
      // Build the multi-block tool result. A second "text" block carries the
      // cold-register audit notice when present, so the LLM sees it inline
      // with the success result instead of buried in a warning log.
