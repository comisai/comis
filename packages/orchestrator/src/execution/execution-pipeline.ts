...
export async function executeAndDeliver(
  ...
  // Stage 2: LLM execution with timeout, thinking filter, abort signal
    ...
    const strictDeliveryOutcome = await deliverExecutionResponse(
      ...
      undefined,
      // A background hand-off is NOT a runtime failure. The coordinator-outcome branch above
      // already treats `background_pending` as non-terminal UI state rather than a failed activity;
      // keep the delivery origin aligned with that same authoritative classification.
