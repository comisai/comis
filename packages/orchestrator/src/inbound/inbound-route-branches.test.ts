...
// ---------------------------------------------------------------------------
// Command queue routing + priority lane assignment
// ---------------------------------------------------------------------------

describe("routeInboundMessage command-queue routing", () => {
  it("assigns followup lane when message metadata flags isFollowup", async () => {
    const enqueue = vi.fn(async () => ok(undefined));
    const eventBus = {
      emit: vi.fn(() => true),
      on: vi.fn().mockReturnThis(),
    ...
    const deps = makeMinimalDeps({
      commandQueue: { enqueue } as never,
      eventBus: eventBus as never,
      priorityScheduler: {} as never,
      queueConfig: {
        defaultMode: "queue",
        perChannel: {},
        priorityEnabled: true,
        laneAssignment: {
          followupLane: "lane-followup",
          scheduledLane: "lane-scheduled",
          dmLane: "lane-dm",
          mentionLane: "lane-mention",
          defaultLane: "lane-default",
        },
      } as never,
    });
    const adapter = makeAdapter();
    const msg = makeMsg({ metadata: { isFollowup: true } });
    ...
    expect(eventBus.emit).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ lane: "lane-followup", reason: "followup" }),
    );
  });

  it("assigns scheduled lane when message metadata flags isHeartbeat", async () => {
    const enqueue = vi.fn(async () => ok(undefined));
    const eventBus = {
      emit: vi.fn(() => true),
      on: vi.fn().mockReturnThis(),
    ...
    const adapter = makeAdapter();
    const msg = makeMsg({ metadata: { isHeartbeat: true } });
    ...
    expect(eventBus.emit).toHaveBeenCalledWith(
      "priority:lane_assigned",
      expect.objectContaining({ lane: "lane-scheduled", reason: "heartbeat" }),
    );
  });
