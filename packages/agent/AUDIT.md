...
| renderAnnouncementFailureNotice | optional | failed completions use the generic deterministic failure notice when no per-agent locale renderer is wired | packages/agent/src/spawn/sub-agent-runner.ts:465 |
...
- **Final count:** 32 (8 required + 24 optional)
...
- The architecture test asserts the interface body's actual field count via bidirectional set equality. The current 32-field shape includes clock + timers as port-typed dependencies.
