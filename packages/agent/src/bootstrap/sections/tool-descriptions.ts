...
export const LEAN_TOOL_DESCRIPTIONS: Record<string, string | ((ctx: ToolDescriptionContext) => string)> = {
  ...
  web_fetch: "Fetch content from URL (HTML, PDF, JSON auto-detected). If a supplied URL appears in the current request or relevant conversation context, call this before making claims about its contents; do not rely on prior knowledge. Use this, not exec curl/wget.",
  ...
  skills_manage: (ctx: ToolDescriptionContext): string => {
    const base = "Manage prompt skills: list installed entries; import from an immutable commit-pinned GitHub directory URL; create, update, or delete. Use local scope for this agent's workspace; shared scope is default-agent only. Mutations use the approval gate.";
    return ctx.trustLevel === "admin" ? base : base + " Admin required.";
  },
...
## Sandbox-Forbidden Paths
The exec sandbox denies writes to ~/.comis/skills/, global node_modules (e.g. ~/.nvm/.../lib/node_modules), ~/.gitconfig, and /var/folders/ system temp. Installing a skill via \`cp\` or \`npm install -g\` WILL fail with EPERM. To install a skill, call discover_tools({query: "skills_manage"}) and then use skills_manage with scope: "local" — it writes into the agent's own workspace/skills. To install packages, run the install inside the agent workspace (cwd is already the workspace root) so artifacts land under ./node_modules there.
## Passing API Tokens to Subprocesses
