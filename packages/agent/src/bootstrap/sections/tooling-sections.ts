...
export function buildTaskDelegationSection(
  ...
  const lines: string[] = [
    ...
    "2. If the task explicitly requires a named tool, pass it in `required_tools` and include a matching `tool_groups` profile; task prose cannot grant tools",
    ...
    "7. Acknowledge the launch in natural user-facing language. The returned run handle is internal; never expose it",
    "8. Do not repeat the delegated work in the parent. Stop after the acknowledgement; automatic completion will re-enter the originating conversation when the result is ready",
    "",
  ...
  if (subAgentToolNames && subAgentToolNames.length > 0) {
    ...
    if (parentOnly.length > 0) {
      lines.push(
        ...
        "handle that step yourself before or after the spawn, as the dependency requires,",
        "and pass only the bounded, non-sensitive result when the child needs it.",
...
export function buildPrivilegedToolsSection(
  ...
  const lines: string[] = [
    ...
    "- memory_manage: delete, forget, flush",
    "- channels_manage: enable, disable, restart",
    "- tokens_manage: create, revoke, rotate",
    "- providers_manage: create, delete",
    "- skills_manage: import, create, update, delete",
    "- mcp_manage: connect, disconnect, reconnect",
    "",
    "**No approval pause (some actions below mutate state):**",
    "These actions still require a current admin request; lack of an approval pause does not grant independent authority.",
    ...
    "- memory_manage: stats, browse, export, roundtrip, pin, unpin",
    "- channels_manage: list, get",
    "- tokens_manage: list",
    "- providers_manage: list, get, update, enable, disable",
    "- skills_manage: list",
    "- mcp_manage: list, status",
    "- heartbeat_manage: get, update, status, trigger",
    "",
    "**Read-only examples:** obs_query, models_manage, and the get/list/status actions above.",
    "",
