...
export function buildMemoryRecallSection(
  ...
  return [
    ...
    "Recalled memories and `memory_search` results come from PAST conversations and may be OUTDATED. When a recalled fact conflicts with what the user has stated in the CURRENT conversation, the current conversation always takes precedence — never override a fresh statement with a stale memory. Treat recalled memories as background context, not as ground truth: if the user gave a value (a codeword, a name, a preference, a number) earlier in THIS conversation, use that value even if memory recalls a different one from before.",
    "",
    "### Tools",
