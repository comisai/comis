...
describe("stripDiscoverySchemas", () => {
  ...
  // ----- Cache fence awareness -----

  it("skips entries at or below the cache fence index", () => {
    const text1 = functionsBlock([{ name: "fenced_tool" }]);
    const text2 = functionsBlock([{ name: "unfenced_tool" }]);
    const sm = createMockSessionManager([
      toolResultEntry("discover_tools", text1, "tc_1"),
      toolResultEntry("discover_tools", text2, "tc_2"),
    ]);

    // Fence at index 0 — first entry (message index 0) is protected
    stripDiscoverySchemas(sm, undefined, 0);

    // tc_1 at message index 0 should NOT be stripped (at fence)
    const msg1 = (sm.fileEntries[0] as any).message;
    expect(msg1.content[0].text).toBe(text1);

    // tc_2 at message index 1 should be stripped (above fence)
    const msg2 = (sm.fileEntries[1] as any).message;
    expect(msg2.content[0].text).toBe(
      "[Discovery loaded: 1 tool(s) are now callable]\n- unfenced_tool",
    );
  });

  it("strips all entries when no fence is provided", () => {
    const text = functionsBlock([{ name: "tool_a" }]);
    const sm = createMockSessionManager([
      toolResultEntry("discover_tools", text, "tc_1"),
    ]);
  ...
  it("counts only message entries for fence index (skips non-message entries)", () => {
    const text1 = functionsBlock([{ name: "tool_a" }]);
    const text2 = functionsBlock([{ name: "tool_b" }]);
    const sm = createMockSessionManager([
      { type: "summary", content: "some summary" },
      toolResultEntry("discover_tools", text1, "tc_1"),
      { type: "summary", content: "another summary" },
      toolResultEntry("discover_tools", text2, "tc_2"),
    ]);

    // Fence at message index 0 — first message entry is protected
    stripDiscoverySchemas(sm, undefined, 0);

    // tc_1 is message index 0 → protected
    const msg1 = (sm.fileEntries[1] as any).message;
    expect(msg1.content[0].text).toBe(text1);

    // tc_2 is message index 1 → stripped
    const msg2 = (sm.fileEntries[3] as any).message;
    expect(msg2.content[0].text).toBe(
      "[Discovery loaded: 1 tool(s) are now callable]\n- tool_b",
    );
  });
