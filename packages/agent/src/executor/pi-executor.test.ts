...
describe("PiExecutor", () => {
  ...
  // -------------------------------------------------------------------------
  // 260428-kvl: tool-definitions drift wiring
  // -------------------------------------------------------------------------

  describe("260428-kvl: getActiveToolDefinitions wiring", () => {
    it("passes getActiveToolDefinitions to createPiEventBridge that returns full tool definitions filtered by active names", async () => {
      // Construct full ToolInfo-shaped fixtures so the closure returns full
      // definitions, not just names. This is exactly the bug 260428-k8d's
      // names-only check missed: same names, different definitions.
      const allToolDefs = [
        { name: "bash", description: "Execute bash commands", parameters: { cmd: "string" } },
        { name: "read", description: "Read a file", parameters: { path: "string" } },
        { name: "deferred_only", description: "Inactive tool", parameters: {} },
      ];
      mockGetAllTools.mockReturnValue(allToolDefs);
      // Active subset is bash + read (deferred_only excluded).
      mockGetActiveToolNames.mockReturnValue(["bash", "read"]);
      ...
      // Verify createPiEventBridge received getActiveToolDefinitions
      // (not getActiveToolNames) -- this is the kvl wiring contract.
      const bridgeCall = (createPiEventBridge as Mock).mock.calls[0][0];
      expect(bridgeCall.getActiveToolDefinitions).toBeTypeOf("function");
      expect(bridgeCall.getActiveToolNames).toBeUndefined();

      // Invoke the closure: it should return full tool DEFINITIONS
      // (objects with name/description/parameters) for active tools only.
      const defs = bridgeCall.getActiveToolDefinitions();
      expect(Array.isArray(defs)).toBe(true);
      expect(defs).toHaveLength(2);
      expect(defs.map((d: { name: string }) => d.name).sort()).toEqual(["bash", "read"]);
      // Full definitions (NOT just names) -- this is the kvl primitive swap.
      const bashDef = defs.find((d: { name: string }) => d.name === "bash") as { description: string };
      expect(bashDef.description).toBe("Execute bash commands");
    });

    it("description change on a tool produces different definitions (drift would fire) even when names match -- the bug 260428-k8d missed", async () => {
      // Names identical, but one tool's description changed between calls.
      // The kvl closure returns full defs each time, so a diff in description
      // shows up directly in the returned array (and would hash differently).
      const turnOneDefs = [
      ...
      const turnTwoDefs = [
        { name: "bash", description: "Execute bash commands", parameters: {} },
        // Same name, different description -- this is the JIT-guide expansion bug.
        { name: "read", description: "Read a file (with expanded schema)", parameters: { newField: "string" } },
      ];
      // Same name set both turns -- this is what 260428-k8d's names-only
      // check would have considered "no drift". The kvl closure exposes the
      // diff so the hash will differ.
      mockGetActiveToolNames.mockReturnValue(["bash", "read"]);

      mockGetAllTools.mockReturnValueOnce(turnOneDefs).mockReturnValueOnce(turnTwoDefs);
      ...
      const bridgeCall = (createPiEventBridge as Mock).mock.calls[0][0];
      // Note: mockGetAllTools is consumed by setupTools introspection inside
      // execute(); the closure stored on bridgeCall reads getAllTools() each
      // time it is invoked. So invoke the closure twice -- it pulls the next
      // mock return per invocation.
      const defs1 = bridgeCall.getActiveToolDefinitions();
      const defs2 = bridgeCall.getActiveToolDefinitions();
      // Same names, different descriptions:
      expect(defs1.map((d: { name: string }) => d.name).sort()).toEqual(
        defs2.map((d: { name: string }) => d.name).sort(),
      );
      const read1 = defs1.find((d: { name: string }) => d.name === "read") as { description: string };
      const read2 = defs2.find((d: { name: string }) => d.name === "read") as { description: string };
      expect(read1.description).not.toBe(read2.description);
    });
  });
