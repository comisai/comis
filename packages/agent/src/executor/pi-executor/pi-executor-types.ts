...
export interface PiExecutorDeps {
  ...
  /** Daemon data dir (COMIS_DATA_DIR / config.dataDir). Used as the cache-trace
   *  containment base and threaded to prompt assembly via ToolAssemblyDeps so
   *  the recall-trace recorder resolves from the same source as its reader.
   *  Absent ⇒ ~/.comis. */
  dataDir?: string;
  // Tools
   ...
   * `resolveCacheTraceFilePath`; otherwise the recorder writes below dataDir,
   * falling back to `~/.comis/logs/cache-trace.jsonl` when dataDir is absent.
   */
  cacheTraceConfig?: {
    readonly enabled?: boolean;
