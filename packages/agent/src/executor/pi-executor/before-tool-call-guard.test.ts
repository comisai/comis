...
describe("createBeforeToolCallGuard", () => {
  ...
  it("blocks explicit invocation reads for a prompt skill outside the current registry", async () => {
    const { stepCounter, budgetGuard, circuitBreaker } = passThroughSafety();
    const guard = Reflect.apply(createBeforeToolCallGuard, undefined, [
      stepCounter,
      budgetGuard,
    ...
    await expect(guard({
      toolCall: { name: "read" },
      args: { path: "/workspace/operator-skills/operator-procedure-fixture/SKILL.md" },
    })).resolves.toEqual({
      block: true,
      reason: expect.stringMatching(
  ...
  it("allows invocation reads for a prompt skill in the current registry", async () => {
    const { stepCounter, budgetGuard, circuitBreaker } = passThroughSafety();
    ...
    await expect(guard({
      toolCall: { name: "read" },
      args: { path: activePath },
    })).resolves.toBeUndefined();
  });

  it("allows ordinary inspection of an unregistered prompt skill file", async () => {
    const { stepCounter, budgetGuard, circuitBreaker } = passThroughSafety();
    const guard = Reflect.apply(createBeforeToolCallGuard, undefined, [
      stepCounter,
      budgetGuard,
      ...
      undefined,
      undefined,
      { activeLocations: new Set<string>() },
    ]);
