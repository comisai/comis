...
export function applyToolDeferral(
  ...
  const discoverTool = remainingDeferred.length > 0
    ? createDiscoverTool(remainingDeferred, logger, embeddingPort)
    : null;
    ...
    async execute(_toolCallId: string, params: unknown) {
      ...
      if (structuredResults.length > 0) {
        ...
        if (topResults.length === 0) {
          // Warn on no matches
          logger.warn({ query }, "discover_tools: no good matches found");
          return {
            content: [{ type: "text" as const, text: "No matching tools found. Try different keywords." }],
            isError: false,
            details: undefined,
