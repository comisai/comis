    ...
    async execute(
      ...
      const lockResult = await sessionAdapter.withSession(
        ...
        async (sm) => {
          ...
          // Phase 14: capture origin at wrap-time (D-02 explicit threading).
          ...
          if (deps.backgroundTaskManager && config.backgroundTasks?.enabled !== false) {
            ...
            const originResolver = (): BackgroundTaskOrigin | undefined => {
              ...
              // a complete origin per SPEC AC-3.
              ...
              // Phase 14 SPEC R4 / D-01: read incoming hop count off msg.metadata
              // so plan 14-04's runner can enforce the recursion bound. Top-level
              // user messages have no metadata.backgroundHopCount -> default to 0.
