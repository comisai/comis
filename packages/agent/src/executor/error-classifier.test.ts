...
describe("classifyError", () => {
  ...
  it("classifies Anthropic invalid_request_error (400) as non-retryable client_request", () => {
    const error = new Error(
    ...
    // And it should mention a reset hint
    expect(result.userMessage.toLowerCase()).toContain("reset");
  });

  it('classifies "cannot be modified" errors (thinking blocks bug) as client_request', () => {
  ...
  it("client_request userMessage is safe, human-readable, and never leaks internals", () => {
    const error = new Error(
    ...
    expect(result.userMessage).toMatch(/request|conversation|formatting/i);
  });
