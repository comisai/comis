...
/**
 * Index of the assistant whose thinking must be PRESERVED this call, or -1.
 *
 * The newest assistant in a trailing tool-use cycle is the one window where the provider forbids
 * altering the thinking block, so {@link stripReplayThinking} leaves it alone. That makes it the one
 * message guaranteed to change shape between calls: sent WITH thinking now, and WITHOUT it as soon
 * as a newer assistant makes it historical.
 */
export function findPreservedThinkingIndex(messages: Array<Record<string, unknown>>): number {
  const latestAssistant = findLatestAssistantIndex(messages);
  return latestAssistant >= 0 && isUnclosedToolUseCycle(messages, latestAssistant) ? latestAssistant : -1;
}

/** True when any block on this message carries a cache marker (either provider shape). */
function hasCacheMarker(msg: Record<string, unknown>): boolean {
  const content = msg.content;
  if (!Array.isArray(content)) return false;
  return (content as Array<Record<string, unknown>>).some(
    b => b.cache_control != null || blockKind(b) === "cache_marker",
  );
}

/** Drop every cache marker from a message; returns how many were removed. */
function removeCacheMarkers(msg: Record<string, unknown>): number {
  const content = msg.content;
  if (!Array.isArray(content)) return 0;
  const blocks = content as Array<Record<string, unknown>>;
  let removed = 0;
  const kept = blocks.filter((b) => {
    if (blockKind(b) === "cache_marker") { removed++; return false; }
    return true;
  });
  for (const b of kept) {
    if (b.cache_control != null) { delete b.cache_control; removed++; }
  }
  if (removed > 0) msg.content = kept;
  return removed;
}

/**
 * Keep the preserved-thinking window OUT of the cached prefix.
 *
 * The cached prefix must be byte-stable, and the preserved-thinking assistant is the one message
 * that provably is not: the provider requires its thinking block while its tool cycle is open, and
 * `stripReplayThinking` removes that block the moment a newer assistant arrives. Caching through it
 * therefore invalidates its zone once per tool cycle, forever.
 *
 * Live (comis-moshe 2026-08-02): `cache_read` pinned at exactly 80,865 across every call with ~18k
 * re-created each time — only the zones BEFORE that assistant ever hit, while the marker on the
 * trailing message re-paid its whole zone every cycle. Comis's own breakpoints already land before
 * it; the marker that reaches past it arrives on the trailing message.
 *
 * The trade is deliberate and matches {@link deferRecallToUncachedTail}: the newest turn goes
 * uncached for ONE call and is cached on the next, once its shape has settled — instead of a zone
 * that can never hit. Only applied when a marker survives EARLIER in the prefix, so a payload whose
 * only marker is the trailing one keeps it rather than caching nothing at all.
 *
 * Mutates messages in place. Returns the number of markers removed.
 */
export function deferPreservedThinkingToUncachedTail(
  messages: Array<Record<string, unknown>>,
): number {
  const preserved = findPreservedThinkingIndex(messages);
  if (preserved < 0) return 0;
  let earlierMarker = false;
  for (let i = 0; i < preserved; i++) {
    if (hasCacheMarker(messages[i]!)) { earlierMarker = true; break; }
  }
  if (!earlierMarker) return 0;
  let removed = 0;
  for (let i = preserved; i < messages.length; i++) removed += removeCacheMarkers(messages[i]!);
  return removed;
}
