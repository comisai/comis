 ...
 * derive ALL paths from `workspaceDir`. Closes RC-7 (15-s pip install in
 * the chart hot path; matplotlib Fontconfig error from a non-writable
 * default cache dir).
 *
 * Per CONTEXT D-W1 / T0.17 source-grep: this file contains no host-env
 * reads. The motivation is that subprocesses inheriting the daemon's
 * full environment can pick up host paths (e.g. `/root/.cache/matplotlib`)
 * that fail under read-only systemd hardening (`ProtectSystem=strict`)
 * and that leak host PATH into the agent's tool execution.
 *
 * Per CONTEXT D-W2: matplotlib + python deps install into the agent
 * workspace's `venv/`. The Dockerfile pre-warms the default workspace's
 * venv at image build time so first-chart latency drops; subsequent
 * chart calls reuse the venv. Full Docker image size goes up
 * (acceptable trade-off).
 *
 * Mirrors workspace-resolver.ts:18-30's safePath + workspaceDir
 * precedent (no path.join, no host-env reads).
 *
 * @module
 */
