...
/**
 * Fresh-tail size bounding for the LCD `dag` assembler.
 *
 * The fresh tail ships UNCONDITIONALLY and the dag path runs neither
 ...
 * message (the session brick: one over-window message rides the fresh
 ...
 *
 * Invariant reconciliation (the lcd-assembler call site enumerates the full
 * list): the guards are referential no-ops below the cap; only text CONTENT
 * shrinks (no message removed/reordered, no `toolCallId` touched, non-text
 * blocks pass through); the LCD store keeps the full content losslessly so the
...
/**
 * The single shared tool-result size guard for the dag assembler's fresh-tail
...
/**
 * The sibling guard for oversized
...
/**
 * The fraction of the turn's `availableHistoryTokens` (H = W − S − O −
 ...
 * `0.8 × effectiveWindowChars` alone would NOT be: on a 32K small
 * window, 0.8 × W ≈ 26K tokens + S ≈ 7K still exceeds the window.)
 */
const FRESH_TAIL_MESSAGE_CAP_HISTORY_FRACTION = 0.8;

/**
 * The cap FLOOR. When H collapses to ~0 (everything reserved — a tiny
 ...
 * at the 3.5 ratio) is already over ANY window where H ≈ 0 — so for sub-floor
 * messages the floor changes nothing.
 */
 ...
 * `max(FLOOR, min(absolute per-result constant, 0.8 × H in chars))`. For
 * frontier/mid H is huge, so the cap degrades to the absolute 100K-char
 * {@link LCD_FRESH_TAIL_MAX_TOOL_RESULT_CHARS} constant.
 */
...
/**
 * Bound oversized messages inside the unconditional fresh tail.
 ...
 *  - `user` / `assistant` (the session-brick class — one pasted
 ...
 * still re-pairs every result with its `tool_use`.
 *
...
export function boundFreshTailMessages(
  ...
  const bounded = freshTail.map((m) => {
    ...
    if (isTextMessage && typeof content === "string") {
      ...
      if (!result.truncated) return m; // fits below the cap — byte-identical.
      boundedMessages++;
    ...
    // verbatim. The guard only bounds array-of-blocks content.
...
/**
 * Bound the PROTECTED fresh tail's TOTAL tokens to
 ...
 * trim it), and {@link boundFreshTailMessages} bounds only individual oversized
 ...
 * the window (the observed nano failure mode: freshTail grows 1438→2369→3488…→exhaust).
 * This enforces the total directly.
 *
 ...
 * nothing is dropped — a verbatim no-op for everything that fits).
 *
...
export function boundFreshTailTotalToResidual(
  ...
  // Measure with the SAME factored estimator the pre-flight uses
  ...
  // With mismatched estimators the 4:1↔3.5:1 gap scales with the residual: on a small S
  // (large residual) the bounded tail still measures over the bound at the pre-flight's
  // ratio → exhaustion.
  ...
  if (s === 0) return freshTail; // nothing dropped — verbatim no-op (same reference).
  return freshTail.slice(stepStarts[s]!);
}

/**
 * Compute the protected-fresh-tail residual and trim the tail to it.
 *
 ...
 * reasoningStyle — the IDENTICAL value the pre-flight throws against (a native model
 ...
 * pre-flight uses (estimator parity, no fudge). Frontier/mid: an Infinite window → the
 * residual is undefined → returns the tail UNCHANGED (no-op → byte-identical, the locked
 * frontier/mid contract).
 *
 ...
 * sizes) are one-read visible from a single log line.
 *
 * @param freshTail - the per-message-bounded fresh-tail messages.
