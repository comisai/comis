 ...
 * (structural precedent: recall-dag-budget-partition.test.ts).
 *
 ...
 *      (summaryReserve = min(budget.outputReserveTokens,
 ...
 *      prevTokens)` (the ACTUAL threaded previousSummary tokens
 ...
 * "compaction intermittently fails" with no knob-named diagnosis.
 * The arithmetic grid below is window-independent algebra over
 ...
 *  degenerate-window and skip-honesty fixtures). */
const GRID_WINDOWS = [8_000, 16_000, 32_000, 131_072, 200_000] as const;

describe("compaction span invariant: window-independent arithmetic grid", () => {
  const cfg = ContextEngineConfigSchema.parse({});
  // The previousSummary dimension — the budget subtracts the
  ...
  it("LEAF: clampedLeafChunk + leafTargetTokens + OVERHEAD + prevTokens ≤ W for every grid window × previousSummary size", () => {
    for (const W of GRID_WINDOWS) {
      for (const prev of PREV_TOKENS) {
        ...
        // the deterministic floor instead — the inequality deliberately
  ...
  it("CONDENSE: any kept child prefix (Σ ≤ childTokenBudget) + condensedTargetTokens + OVERHEAD + prevTokens ≤ W for every grid window × previousSummary size", () => {
    for (const W of GRID_WINDOWS) {
      for (const prev of PREV_TOKENS) {
        ...
        // infeasible-window skip is the condense trigger's skip-honesty fixture).
...
describe("compaction span invariant: structural source-locks across the three clamp sites", () => {
  ...
  it("both LCD triggers key their clamp to the RESOLVED summarizer (resolveSummarizerWindowTokens — never the getModel() session-primary snapshot)", () => {
    // A clamp keyed to the session-primary snapshot would pass a 131K
...
// ---------------------------------------------------------------------------
// The SERVED-window dimension. The boot probe discovers the window the
...
// clamp sites size spans against a configured fiction and the provider
// silently truncates the summarize input.
// ---------------------------------------------------------------------------

describe("compaction span invariant: served-window arithmetic dimension", () => {
  const cfg = ContextEngineConfigSchema.parse({});
  const PREV_TOKENS = [0, 1_200, 2_000] as const;
  /** configured × served grid: the incident pair (131_072 / 8_192), a mid
  ...
  it("LEAF and CONDENSE clamps computed on min(configured, served) always fit the window the provider will ACTUALLY serve", () => {
    for (const { configured, served } of SERVED_GRID) {
      // The candidate-bound served window min()s the
...
describe("compaction span invariant: served-window source-locks", () => {
  ...
  it("resolveSummarizerWindowTokens consumes the CANDIDATE-BOUND served fields (deps.primaryServedWindow / deps.overrideModel?.servedWindow)", () => {
    ...
    // regression the binding prevents).
  ...
  it("the wiring site provider-gates the served pair in CONFIG space (providers.entries keys — never model.provider)", () => {
    ...
    // the executor reconcile's already-gated windowProvenance.served (the
    // registry alias fallback can rename model.provider, so config-space
