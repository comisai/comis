...
/**
 * Unit tests for the SHARED factored per-message estimator.
 *
 ...
 * These tests pin: (1) the ASCII 3.5:1 form, (2) multi-part/tool-result text
 * extraction (text ?? content per block, NOT summed), (3) dense-script inflation,
...
describe("factoredMessageTokens — the shared fresh-tail estimator", () => {
  ...
  it("array content sums each block's text ?? content (NOT text+content), via messageText", () => {
    const m = {
      role: "assistant",
      content: [
  ...
  it("a dense (Hebrew) message counts MORE tokens/char than the bare 3.5:1 form", () => {
