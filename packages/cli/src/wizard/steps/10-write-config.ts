...
const CUSTOM_PROVIDER_API_KEY_NAME = "CUSTOM_PROVIDER_API_KEY";
...
/** Resolve the SecretManager name selected by the provider credential step. */
function getSelectedProviderSecretName(state: WizardState): string | undefined {
  const provider = state.provider;
  if (!provider?.apiKey) return undefined;
  if (provider.id === "custom") return CUSTOM_PROVIDER_API_KEY_NAME;
  if (provider.id === "anthropic" && provider.authMethod === "oauth") {
    return "ANTHROPIC_OAUTH_TOKEN";
  }
  return PROVIDER_ENV_KEYS[provider.id];
}
...
function buildConfigObject(state: WizardState): Record<string, unknown> {
  ...
  if (state.provider?.id === "custom") {
    const { customEndpoint, compatMode, apiKey } = state.provider;
    const model = state.model;
    if (!customEndpoint || !compatMode || !apiKey || !model) {
      throw new Error(
        "Custom provider setup is incomplete. Run `comis init` interactively and provide an endpoint, compatibility mode, API key, and model.",
      );
    }
    providerEntries.custom = {
      type: compatMode,
      name: "Custom endpoint",
      baseUrl: customEndpoint,
      apiKeyName: CUSTOM_PROVIDER_API_KEY_NAME,
      enabled: true,
      models: [{ id: model }],
    };
  } else if (state.provider?.id === "anthropic" && state.provider.apiKey) {
    providerEntries.anthropic = {
      type: "anthropic",
      apiKeyName: state.provider.authMethod === "oauth"
        ? "ANTHROPIC_OAUTH_TOKEN"
        : "ANTHROPIC_API_KEY",
    };
  }
  if (Object.keys(providerEntries).length > 0) {
    config.providers = { entries: providerEntries };
  }
...
function buildEnvLines(
  ...
  // Anthropic accepts both a direct API key and a setup token. Keep only the
  // mode selected in this wizard run so a stale opposite credential cannot be
  // mistaken for the active one by tools that inspect the env file directly.
  const selectedProviderSecretName = getSelectedProviderSecretName(state);
  if (state.provider?.id === "anthropic" && selectedProviderSecretName) {
    const inactiveName = selectedProviderSecretName === "ANTHROPIC_OAUTH_TOKEN"
      ? "ANTHROPIC_API_KEY"
      : "ANTHROPIC_OAUTH_TOKEN";
    merged.delete(inactiveName);
  }
