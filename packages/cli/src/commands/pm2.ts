...
import { parseConfigPaths, safePath, systemGetEnv } from "@comis/core";
...
/** Normalize the config-path environment contract persisted into pm2. */
export function resolvePm2ConfigPaths(
  rawPaths: string | undefined = systemGetEnv("COMIS_CONFIG_PATHS"),
  homeDir: string = os.homedir(),
): string {
  const configured = parseConfigPaths(rawPaths);
...
async function handleSetup(options: { enableBoot?: boolean }): Promise<void> {
  ...
  const configPaths = resolvePm2ConfigPaths();
...
async function enableBoot(): Promise<void> {
  ...
  if (!sudoCmdMatch) {
    // pm2 can configure startup without sudo when the current account has permission.
