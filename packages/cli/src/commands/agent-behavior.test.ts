...
/**
 * Agent data matching what config.get returns for the routing section.
 * The list command calls config.get for both "agents" and "routing" sections
...
describe("agent create sends correct RPC", () => {
  ...
  it("sends config.set RPC with section routing, key agents.<name>, value containing defaultProvider and defaultModel", async () => {
    const program = createTestProgram();
    registerAgentCommand(program);
...
describe("agent create initializes workspace", () => {
  ...
  it("calls ensureWorkspace and resolveWorkspaceDir to initialize agent workspace", async () => {
    ...
    // config.set value should only have name (no provider/model since not specified)
    expect(callSpy).toHaveBeenCalledWith("config.set", {
      section: "routing",
      key: "agents.basic-agent",
      value: { name: "basic-agent" },
    });
...
describe("agent configure sends only specified fields", () => {
  ...
  it("sends only defaultProvider when only --provider specified", async () => {
    const program = createTestProgram();
    registerAgentCommand(program);
  ...
  it("sends both fields when both --provider and --model specified", async () => {
    ...
    expect(callSpy).toHaveBeenCalledWith("config.set", {
      section: "routing",
      key: "agents.my-agent",
      value: { defaultProvider: "openai", defaultModel: "gpt-4o" },
    });
  });
});
...
describe("agent delete with --yes sends null", () => {
  let consoleSpy: ReturnType<typeof createConsoleSpy>;
  let exitSpy: ReturnType<typeof createProcessExitSpy>;
  let callSpy: ReturnType<typeof vi.fn>;
  ...
  it("sends config.set with null value when --yes flag provided", async () => {
    const program = createTestProgram();
    registerAgentCommand(program);
