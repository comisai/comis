...
describe("reset sessions via RPC", () => {
  ...
  it("reports success when no sessions exist", async () => {
    const mockCall = vi.fn()
  ...
  it("propagates RPC method errors instead of falling through to file deletion", async () => {
    vi.mocked(withClient).mockImplementation(async (fn) => {
      return fn({
        call: vi.fn().mockRejectedValue(new Error("Admin trust level required")),
        close: vi.fn(),
      });
    });
    ...
    expect(errOutput).toContain("Admin trust level required");
  });
});
