...
| eventBus | optional | graph handlers cannot emit authoring telemetry and sub-agent waits cannot emit their content-free terminal observations; the handlers still function, but the affected `explain` diagnostics are unavailable | packages/daemon/src/api/types.ts:377 |
