...
| deadLetterQueue | optional | `obs.quarantine.list` returns an empty listing and `obs.quarantine.release` reports `released:false`; an operator cannot inspect or clear a quarantined announcement through the RPC and must fall back to stopping the daemon and editing the dead-letter JSONL | packages/daemon/src/api/types.ts:769 |
