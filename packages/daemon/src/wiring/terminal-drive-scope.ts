...
/**
 * The pure daemon-wiring drive-scope helpers (DRIVE-01; design §7.1.1, 164-06 Task 1) —
 ...
 * resolves.
 *
 * The load-bearing nuance (objective of 164-06): a woken turn keeps two owners distinct.
 ...
 *      `status` would return the empty `alive:false` not-found view and drive nothing (the
 *      I9-class silent strand — T-164-19/T-164-23). The same `allowId`/`scope`/`uid` (stamped
 *      ONCE at create, never re-derived on the wake path) therefore apply → I5 (WHERE not
 *      WHAT) by construction.
 ...
 * `sub-agent:<uuid>`, so a real owner key never starts with `drive:` — A4 / T-164-20). The
 ...
 * cannot collide with a real subagent owner (A4 / T-164-20). The trailing colon makes the
 ...
 * dedicated drive scope, NOT the primary `sessionKey:""` conversation), else `""` (today's
 * inline path — byte-identical, I1).
 *
