...
export async function setupSingleAgent(
  ...
  const executor = createPiExecutor(effectiveConfig, {
    ...
    entityStore: deps.entityStore, temporalStore: deps.temporalStore, causalStore: deps.causalStore, tripleStore: deps.tripleStore, embeddingStore: deps.embeddingStore, usefulnessStore: deps.usefulnessStore, pinnedStore: deps.pinnedStore, provenanceStore: deps.provenanceStore, userRepresentationStore: deps.userRepresentationStore, mentalModelStore: deps.learnedSkillStore, relationshipStore: deps.relationshipStore,  // rag.entityLane + rag.lanes.temporal + rag.lanes.causal + rag.lanes.graphSpread + rag.mmr + rag.feedback + rag.pinned (R6 — the pinned-first lane; pinnedStore is the same memoryAdapter cast as MemoryPinnedStore) + DIST-03 provenance down-weighting (Phase 173; provenanceStore is the LcdProvenanceReadStore from buildProvenanceReadStore — the built-but-not-wired carry-in activation) + mentalModelStore is the v2.31 kind:"profile" read source for the rewired <user_profile> block (FOLD-01, Phase 225 Plan 02 — replaces userRepresentationStore; the SAME MentalModelStorePort already wired for the learned-skill surface) + socialModeling standing-block -> createMemoryRecall/prompt-assembly read (default-OFF; JSDoc on AgentSetupDeps). (rag.onlineTuning's tuned-vector read was deleted in Phase 224.)
