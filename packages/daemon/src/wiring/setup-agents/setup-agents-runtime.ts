...
export async function setupSingleAgent(
  ...
  const executor = createPiExecutor(effectiveConfig, {
    ...
    entityStore: deps.entityStore, temporalStore: deps.temporalStore, usefulnessStore: deps.usefulnessStore,  // P83/rag.entityLane + P95·LANES-02/rag.lanes.temporal + P93·FEED-03/rag.feedback -> createMemoryRecall read (default-OFF; JSDoc on AgentSetupDeps).
    secretManager: scopedManager,
