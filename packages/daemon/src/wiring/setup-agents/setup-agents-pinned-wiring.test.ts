...
// Shaped as a minimal MemoryPinnedStore stub (listPinned is the only recall-path method).
const SENTINEL_PINNED_STORE = {
  pin: vi.fn(async () => ({ ok: true as const, value: true })),
  unpin: vi.fn(async () => ({ ok: true as const, value: true })),
  listPinned: vi.fn(async () => ({ ok: true as const, value: [] })),
} as any;
...
function makeDeps(container: AppContainer): SingleAgentDeps {
  ...
  return {
    ...
    pinnedStore: SENTINEL_PINNED_STORE,
...
describe("setupSingleAgent forwards pinnedStore into createPiExecutor", () => {
  ...
  it("passes the SingleAgentDeps.pinnedStore through to the executor deps so the pinned-first recall lane fires at runtime", async () => {
    ...
    expect(mockCreatePiExecutor).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ pinnedStore: SENTINEL_PINNED_STORE }),
    );
  });
});
