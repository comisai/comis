 ...
 *
 * Mirrors setup-agents-userrep-wiring.test.ts 1:1 — the established analog for
 * asserting a deps field reaches the mocked createPiExecutor. RED on pre-fix code
 * (the :466 line omits relationshipStore), GREEN once the forward is added.
 *
 * @module
 */
...
const SENTINEL_RELATIONSHIP = { read: vi.fn(), upsert: vi.fn() } as any;
...
function makeDeps(container: AppContainer): SingleAgentDeps {
  ...
  return {
    ...
    relationshipStore: SENTINEL_RELATIONSHIP,
...
describe("setupSingleAgent forwards relationshipStore into createPiExecutor (gap closure)", () => {
  ...
  it("passes the SingleAgentDeps.relationshipStore through to the executor deps", async () => {
    // RED on pre-fix code: the createPiExecutor object literal omits relationshipStore,
    // so the executor (and thus prompt-assembly's <channel_relationships> read) never sees
    // the store → the LLM-free standing block is a silent no-op in the live daemon. GREEN
    ...
    expect(mockCreatePiExecutor).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ relationshipStore: SENTINEL_RELATIONSHIP }),
    );
  });
});
