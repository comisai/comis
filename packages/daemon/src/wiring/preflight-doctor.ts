 ...
 * composition root under its 3000-line architecture cap (adding audio wiring
 ...
 * otherwise trip the `globals` architecture gate. The helper
...
// ─────────────────────────────────────────────────────────────────────────────
// Legible resolved-profile boot logging + the honest-degrade
...
// lives elsewhere — here it is an input, so neither
 ...
 * posture (CLAUDE.md "promote load-bearing evidence to INFO").
 * Carries exactly the four legibility facts (profile, enabled caps, the budget
 * ceiling, the one field to change it) plus the over-grant notice.
 */
 ...
 * projects the four legibility facts + the over-grant notice. One record per agent, in
 ...
 * message + hint + errorKind). The namespace-downshift finding is
...
/**
 * Build the namespace-preflight downshift `doctor` finding.
 *
 * PURE — driven by the preflight-RESULT INPUT (the probe lives elsewhere). Returns
 ...
 * `errorKind`, no drift.
 *
 * @param preflight the host preconditions (a caller-supplied input; filled by the namespace probe when wired).
 */
