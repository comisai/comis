 ...
 * to keep that leaf under the 600L setup-channels cap. This file holds NO
 ...
 * the KEYLESS __ONLINE_TUNING__ bandit, the __USEFULNESS_JUDGE__ +
 * __MEMORY_TRIPLE_EXTRACTION__ dormant crons, and the __SOCIAL_MODELING__
 * directional-relationship builder (the LAST LLM-backed intercept here) with
 * the rest of the social-modeling subsystem. The surviving sentinels (the KEYLESS
 * __MEMORY_LIFECYCLE__ sweep + the __REFLECT__ engine) live in
 ...
 *
 * The `__SOCIAL_MODELING__` intercept — the last LLM-backed sentinel that
 ...
 * socialModeling config key). The surviving wired sentinels (the KEYLESS
...
export async function handleMemoryCronSentinel(
  ...
  // NOTE: the __ONLINE_TUNING__ bandit sentinel was DELETED (the UCB
  // tuned-alpha bandit is gone; recall scoring is the fixed config.rag.scoring alphas),
  // and the __SOCIAL_MODELING__ directional-relationship sentinel too (the
