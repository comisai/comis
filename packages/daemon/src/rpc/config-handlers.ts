 ...
 * LLMs often send "true"/"false" as strings in tool call parameters, causing
 * Zod validation failures when the schema expects boolean/number types.
 * Only coerces unambiguous cases: "true"/"false" -> boolean, numeric strings -> number.
 * Recurses into plain objects. Arrays and non-plain values pass through unchanged.
 */
function coerceConfigValue(value: unknown): unknown {
  if (typeof value === "string") {
    ...
    // Number coercion (only for strings that are purely numeric, not empty)
    if (value !== "" && !isNaN(Number(value)) && isFinite(Number(value))) {
      return Number(value);
    }
    // JSON array/object coercion — LLMs often stringify arrays/objects in tool call params
    if (value.startsWith("[") || value.startsWith("{")) {
      try {
        const parsed = JSON.parse(value);
        // Recursively coerce values inside the parsed structure
        return coerceConfigValue(parsed);
      } catch {
        // Not valid JSON — fall through and return as plain string
      }
    }
    return value;
  }
  if (Array.isArray(value)) {
    return value.map(coerceConfigValue);
  }
  if (value !== null && typeof value === "object") {
    const result: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(value as Record<string, unknown>)) {
      result[k] = coerceConfigValue(v);
    }
    return result;
  }
  return value;
...
export function createConfigHandlers(deps: ConfigHandlerDeps): Record<string, RpcHandler> {
  ...
  return {
    ...
    "config.apply": async (params) => {
      ...
      const coercedValue = coerceConfigValue(value) as Record<string, unknown>;
      const ctx = params._context as { agentId?: string; userId?: string; traceId?: string } | undefined;
