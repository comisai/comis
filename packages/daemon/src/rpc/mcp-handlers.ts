...
export interface McpHandlerDeps {
  /** The MCP client manager instance (undefined if no servers configured at startup). */
...
export function createMcpHandlers(deps: McpHandlerDeps): Record<string, RpcHandler> {
  function requireManager(): McpClientManager {
    if (!deps.mcpClientManager) {
      throw new Error("MCP client manager not initialized. No MCP servers were configured at startup.");
    }
    return deps.mcpClientManager;
  }
