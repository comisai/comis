...
async function bootShutdown(
  ...
  // 9.2. Config-posture SNAPSHOT (one-shot boot record, NOT an event) — the
  // posture derivations live in recordBootConfigPosture (wiring/main-helpers.ts).
  recordBootConfigPosture(boot, gateway.defaultAgentId, posture.findings);
