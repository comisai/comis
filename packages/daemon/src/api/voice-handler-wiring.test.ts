...
/**
 * `wireVoiceForHandler` honest fallback rung.
 *
 * On the selector-less boot path (`deps.voiceSelection` absent — a boot mode
 * without the audio selector / a harness that omits it) the shim derives the
...
/** A no-op logger — wireVoiceForHandler's log lines only fire on completed/failed. */
const noopLogger = {
...
describe("wireVoiceForHandler — honest fallback rung (no voiceSelection)", () => {
  it("labels an UNPINNED `auto` config provider source:'fallback' (not 'explicit')", () => {
    ...
    expect(wired.source).toBe("fallback"); // an unpinned provider is a fallback, never a dishonest "explicit"
