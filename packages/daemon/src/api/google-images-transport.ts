...
/**
 * Google (Gemini) Images transport (PRV-02).
 *
 ...
 *
 * Call shape (RESEARCH CRITICAL #4, VERIFIED against `@google/genai@1.52.0`):
 ...
 *   `{inlineData:{data,mimeType}}` part per IMAGE reference, IN-01). The image
 ...
 * `inlineData.data` (A2 — it is not guaranteed to be at index 0).
 *
...
/**
 * Hand-built `ImagesModel` for the Gemini image path (Pitfall 4 — pi-ai's image
 * catalog is openrouter-only). The 8 fields tsc requires for `ImagesModel<TApi>`
 ...
 * `Model_2` union) and is overridable by config/tool. `cost: 0` is the
...
/**
 * The `google-images` transport (PRV-02).
 *
...
export const generateImagesGoogle: ImagesApiFunction = async (model, context, options) => {
  ...
  try {
    ...
    // inlineData part per IMAGE reference (IN-01), preserving order.
    ...
    // A2: scan ALL parts for the first inline image (not guaranteed at index 0).
