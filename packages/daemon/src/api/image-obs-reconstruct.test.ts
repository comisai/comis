...
/**
 * Offline reconstruction test for the image.* trajectory lifecycle.
 *
 ...
 * from the `image.generated` trajectory record. This is the
 ...
 *  empty — an image turn carries no executor `sessionEnd` rollup. */
...
describe("offline reconstruction — comis explain surfaces an image turn's costUsd", () => {
  it("assembleIncidentReportFromSources reconstructs provider/model/costUsd/outcome from image.generated", async () => {
    ...
    // The binding bar: the image turn's cost is reconstructable from the
    // trajectory. The report surfaces a dedicated `image` block built
    // from the image.generated record (mirrors the `recall` block —
    // NOT the executor-only cost.costUsd).
  ...
  it("a persist-failure-but-delivered turn reconstructs outcome=ok with costUsd (not failed)", async () => {
    // The handler emits image.generated{outcome:ok, persisted:false, costUsd} on
    // the persist-failure-but-base64-delivered path. `comis explain` must
  ...
  it("out-of-order image records fold by seq — a lower-seq failed does not flip a higher-seq generated", async () => {
    // Guards the regression where the fold relies on ARRAY (file) order: an
    // image.failed appearing AFTER image.generated in the array unconditionally
    // flips outcome to "failed". The fold is seq-aware so only a record with a
    // HIGHER seq can overwrite the terminal outcome. Here the array order is reversed vs the
