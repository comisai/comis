...
import { ConversationRefSchema, createConversationRef } from "@comis/core";
...
function makeOriginTarget(agentId = "agent-1") {
...
describe("createNotificationHandlers", () => {
  it("notification.send calls notifyUser with correct options mapping", async () => {
    ...
    const originTarget = makeOriginTarget();

    await handler({
      _agentId: "agent-1",
      _deliveryTarget: originTarget,
      message: "Hello user",
      priority: "high",
    ...
    expect(service.resolveDestination).not.toHaveBeenCalled();
    expect(service.notifyUser).toHaveBeenCalledWith({
      ...
      authority: {
        tenantId: "default",
        agentId: "agent-1",
        conversationRef: originTarget.conversation.conversationRef,
      },
      destinationEndpoint: originTarget.destinationEndpoint,
    });
  });
  ...
  it("includes _agentId from params as options.agentId", async () => {
    ...
    await handler({
      ...
      _deliveryTarget: makeOriginTarget("custom-agent"),
      message: "test",
    });
  ...
  it("notifyUser returning ok produces an exact-route success receipt", async () => {
    ...
    const result = await handlers["notification.send"]!({
      _agentId: "a",
      _deliveryTarget: makeOriginTarget("a"),
      message: "hello",
    });
  ...
  it("rejects an agent notification without its trusted request route", async () => {
    const service = makeMockService();
    const handlers = createNotificationHandlers({ notificationService: service });
