 ...
 *      §6.3 report → stamp `likelyRootCause` (with the `session_not_found`
 *      not-found marker) → bound to the depth budget.
 *   5. dev-mode `response.parse` (catches field regressions in dev only).
 *
...
export async function assembleIncidentReportFromSources(
  ...
  // Step 3: canonicalize a traceId to its sessionKey FIRST, so by-trace
  ...
  // A traceId that resolves to "" (no row in today/yesterday's session
  ...
  // The 5th source — the session's audit events (persisted via SQLite, NOT a
  // trajectory record, so they are read HERE,
  ...
  // Read the latest config_posture obs_diagnostics row and populate
  // signals.proxyPosture — a boot-time fact not in the trajectory, so the handler
  // (not toIncidentSignals) is the right injection point. Mirrors how
  // signals.endReason and signals.degraded are threaded by the handler.
  // Absent obsStore or no config_posture rows → signals.proxyPosture stays
  // undefined → assembler omits the field (zero-config byte-identical invariant).
  if (obsStore !== undefined) {
    const postureRows = obsStore.queryDiagnostics({ category: "config_posture" });
    if (postureRows.length > 0) {
      // Latest row wins (standing state — same as fleet-findings.ts pattern).
      let latestRow = postureRows[0]!;
      for (const row of postureRows) {
        if (row.timestamp > latestRow.timestamp) latestRow = row;
      }
      try {
        const d = JSON.parse(latestRow.details ?? "{}") as Record<string, unknown>;
        const ps = d["proxyInstallerStatus"] as Record<string, unknown> | undefined;
        // Only surface when proxyInstallerStatus is present (proxy was configured).
        if (ps !== null && typeof ps === "object") {
          signals.proxyPosture = {
            configured: true,
            // maskedUrl: not in proxyInstallerStatus (only installerError + loopbackMode);
            // maskedUrl lives on the ProxyBootPosture that is NOT persisted here.
            // Only what the posture row carries — no field reconstruction.
            installerOk: ps["installerError"] === null || ps["installerError"] === undefined,
            ...(typeof ps["installerError"] === "string" && ps["installerError"].length > 0
              ? { installerError: ps["installerError"] as string }
              : {}),
            ...(typeof ps["effectiveLoopbackMode"] === "string"
              ? { loopbackMode: ps["effectiveLoopbackMode"] as string }
              : {}),
          };
        }
      } catch {
        // Malformed details JSON — leave signals.proxyPosture undefined (defensive).
      }
    }
  }
...
/**
 * Aggregate the tenant-scoped `obs_audit_events` rows for ONE session into the
 * content-free `audit?` section — `{ total, byKind }`, counts by
 ...
 * capped regardless of row volume (no per-row growth) — the bound holds
...
export function bindObsExplainHandlers(
  ...
  return {
    [ObsExplainContract.method]: async (rawParams) => {
      ...
      // assemble → rootCause / not-found marker → bound → dev-mode parse). The
      // body is identical to the former inline pipeline; the obs_explain MCP path
      // runs the SAME function under daemon authority.
      // Pass obsStore so the assembler can read the latest config_posture
      // row and populate signals.proxyPosture.
      return assembleIncidentReportFromSources(reader, dataDir, params, deps.obsStore);
    },
  };
}
