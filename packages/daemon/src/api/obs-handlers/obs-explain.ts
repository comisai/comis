...
export interface AssembleIncidentReportParams {
   ...
   * in-process root by its validated encoded session suffix, a real socket/spawned
   * root by the session-index scan. An unresolvable rootRunId soft-fails to "" →
   * the not-found marker (it never masquerades as a clean session). Lets the
