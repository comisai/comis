...
/**
 * `obs.system.health` RPC handler — the cross-session SystemHealthReport assembler.
 ...
 *
 * Bounded (digest-only): `findings[]` is capped at {@link SYSTEM_FINDINGS_CAP}
 ...
 * co-located, split to `system-health-heuristics.ts` / `system-health-bound.ts`
 ...
 * the obs-explain progressive-shed caps; the system report is small (the schema
 ...
 */
const SYSTEM_FINDINGS_CAP = 8;

/**
 * The degraded-rate threshold above which the system-degradation heuristic fires.
 * A window where most sessions degraded is a system-level signal (distinct from a
...
// details parsers) is extracted to ./system-findings.ts to keep this module under
...
// no globals — same system signals always yield the same verdict.
// ---------------------------------------------------------------------------

/** A deterministic system root-cause verdict. Shape-identical to the schema field. */
interface SystemRootCause {
  code: string;
  detail: string;
...
/**
 * The ordered system root-cause registry. First predicate to return a non-null
 * `SystemRootCause` wins. Order is the determinism contract — a deployment-wide
 * degradation outranks a single recurring WARN class.
 */
const SYSTEM_HEURISTICS: ReadonlyArray<(s: SystemSignals) => SystemRootCause | null> = [
  ...
  //    Clones the system_acute_degradation shape (a named-cause acute event).
  ...
  (s) => {
    ...
    return {
      code: "system_autonomy_degradation",
      detail:
  ...
  // 1) High system degradation — most sessions in the window HARD-degraded (the
  ...
  //    are excluded so a system of self-healed hiccups does not false-alarm.
  (s) => {
  ...
  //    first. The live system verdict pointed at TLS-off (chronic, known) while
  ...
  (s) => {
    ...
    return {
      code: "system_recurring_health_signal",
      detail: `${s.healthSignalCount} recurring health WARN signal(s) across the system`,
      suggestedNextSteps: [
...
/** Run the ordered registry; first non-null verdict, or null (a clean system). */
function systemRootCause(s: SystemSignals): SystemRootCause | null {
  for (const h of SYSTEM_HEURISTICS) {
    const r = h(s);
    if (r !== null) return r;
  }
  return null;
...
/**
 * Cap `findings[]` to {@link SYSTEM_FINDINGS_CAP} highest-count-first, pushing an
...
function boundFindings(findings: readonly Finding[], truncations: TruncationEntry[]): Finding[] {
  if (findings.length <= SYSTEM_FINDINGS_CAP) return [...findings];
  truncations.push({
    field: "findings",
    reason: `capped at ${SYSTEM_FINDINGS_CAP} highest-count findings (had ${findings.length})`,
    pointer: "obs.diagnostics",
  });
  return [...findings].slice(0, SYSTEM_FINDINGS_CAP);
}
...
export async function assembleSystemHealthReport(
  ...
  // Session rollup — per-session rollups -> cross-session system rollup. `rows` is
  ...
  // pre-exclusion.)
  const degraded = system.degradedCount;
  ...
  const topErrorKinds = Object.entries(system.topErrorKinds).map(([kind, count]) => ({ kind, count }));
  ...
  // the synthetic-excluded `system` read-back — NO Date.now()/new Date(),
  ...
  const autonomy = computeAutonomySlice({
    ...
    // (summed from per-session breakerTripCount in reduceSystemWindow) — NEVER re-derived over
  ...
  const likelyRootCause = systemRootCause({
    ...
    deliveredWithToolErrorsCount: system.deliveredWithToolErrorsCount,
...
/**
 * Bind the `obs.system.health` handler — the admin gate + strip-before-parse +
 * the read fan-in. COMPUTED-KEY form (`[ObsSystemHealthContract.method]`) is
 * MANDATORY — `api-contracts-bidirectional.test.ts` only recognizes the computed
 * key, not a `"obs.system.health":` string literal.
 *
 ...
 */
export function bindSystemHealthHandlers(deps: ObsHandlerDeps): Record<string, RpcHandler> {
  const dataDir = deps.dataDir ?? defaultDataDir();

  return {
    [ObsSystemHealthContract.method]: async (rawParams) => {
      ...
      const params = ObsSystemHealthContract.request.parse(stripInternalFields(rawParams));
      ...
      if (IS_DEV) ObsSystemHealthContract.response.parse(report);
      return report;
    },
  };
}
