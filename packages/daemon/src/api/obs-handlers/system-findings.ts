...
/**
 * System findings derivation: turn the ingested diagnostic rows (`health_signal` /
 * `model_health` / `config_posture`) into `{code, detail, count, hint}` findings.
 *
 * Lives separately from `system-health.ts` to keep that module under the
...
import { buildAutonomyFindings } from "./system-autonomy.js";

// `Finding` is declared in the leaf `system-findings-extractors.ts` (so the
 ...
 * (counted in neither cohort, never throws). Exported so system-health.ts
 * can feed the gate.
 */
 ...
 *  summed system savings while the run still counts (degraded). A COMPLETED run —
...
export function buildFindings(
  ...
  // passes (a regression class surfaced as a system count instead of an offline
  ...
  // the `system-autonomy.ts` sibling (the obs-handlers 500-line subdir cap). Each has
  // its own zero-traffic guard inside the helper; the returned findings inherit the
  // SYSTEM_FINDINGS_CAP bound + the highest-count-first sort below.
  ...
  // present) is NOT degradation; counting every row inflated the system view —
  ...
  if (configPosture.length > 0) {
    ...
    // healthy one must not keep flagging the system. Used for BOTH the named-keys rollup and
  ...
  // the window (the system-visible savings + suppression signal). Counts ONLY (fire count
  ...
  // NOT inflate the system degrade count, the benign-reason discipline), so the hint says
  ...
  // per-agent breakdown lives in the SystemHealthReport.cronWakeGate block.
