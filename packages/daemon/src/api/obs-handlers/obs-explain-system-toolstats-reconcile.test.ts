 ...
 *
 * `obs.explain` and `obs.system.health` must NEVER report contradicting per-tool
 ...
 *   - `obs.system.health` `toolStats` = the per-session rollup, LATEST-execution-
 *     wins (`aggregateSessionsInWindow` collapses a session's many
 *     `session_summary` rows to `MAX(id)`, then `reduceSystemWindow` sums across
 ...
 *      each tool where the persisted rollup (= system's number) differs from the
 *      trajectory, with BOTH count pairs — so a consumer cross-referencing
 *      `comis explain` vs `comis system-health` understands WHY the numbers differ
 ...
 *      (explain) AND `reduceSystemWindow` (system) and assert the two lenses
...
import { reduceSystemWindow } from "@comis/memory";
 ...
 * (system's source) carries, because both are spread from the one
...
/** Build the system-side per-session rollup row carrying the SAME rollup toolStats. */
...
describe("obs.explain ↔ obs.system.health toolStats reconciliation", () => {
  it("surfaces a transparent coverage.toolStats note when the latest-execution rollup is a SUBSET of the whole-session trajectory (multi-execution)", () => {
    ...
    // failed total. The persisted rollup (= system's number) reflects only the
    // LAST execution → 2 ok / 3 failed. This is the dominant production case;
    // without the note, explain shows 6/9 and system shows 2/3 with
    ...
    // whole session — that is its job and its advantage over system).
    ...
    // referencing `comis explain` (trajectory) vs `comis system-health` (rollup) sees
  ...
  it("treats a rollup-only tool (in the rollup, absent from the trajectory) as an OVERcount — reconciled=false", () => {
    ...
    // backing while system counts it.
  ...
  it("end-to-end: ONE seeded session feeds BOTH lenses and they reconcile under the bounded rule (rollup ⊆ trajectory, never contradicting)", () => {
    // The single source of truth for system: the per-session rollup row. Its
    ...
    // SYSTEM lens: reduce the window over the one session.
    const system = reduceSystemWindow([makeSystemRow(rollupToolStats)], { excludeSynthetic: true });
    ...
    // The two lenses are RECONCILED: for every tool system reports, the count is
    ...
    expect(report.coverage!.toolStats!.reconciled).toBe(true);
    for (const [tool, systemStat] of Object.entries(system.toolStats)) {
      const trajStat = report.toolStats[tool]!;
      expect(systemStat.ok).toBeLessThanOrEqual(trajStat.ok);
      expect(systemStat.failed).toBeLessThanOrEqual(trajStat.failed);
    }

    // And the divergence (web_fetch: system 2/3 vs explain 6/9) is explicitly
