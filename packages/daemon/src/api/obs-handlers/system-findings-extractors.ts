...
/**
 * System-findings row extractors: the defensive per-row `details`-JSON parsers
 * `buildFindings` (./system-findings.ts) folds into `{code, detail, count, hint}`
 * findings.
 *
 * Extracted from `system-findings.ts` to keep that module under the obs-handlers
...
/**
 * One report finding. Shape-identical to `SystemHealthReport.findings[number]`.
 * Declared in this leaf module (no back-imports) so both `system-findings.ts` and
 * the `system-autonomy.ts` sibling can import it without a cycle; `system-findings.ts`
...
export function flaggedPostureKeys(row: DiagnosticRow): string[] {
  ...
  try {
    ...
    // and flips to `warning`, but the system health view omitted it, so an operator triaging via
    // `comis system-health` saw the browser + no-downgrade relaxations named and this one silent.
...
/** health_signal labels that get a DEDICATED system finding
...
export const DEDICATED_SCRIPT_SIGNALS: ReadonlySet<string> = new Set([
  ...
  // surfaced ONLY as the structured `autonomy.resumed` COUNT (system-health.ts),
 ...
 *  DISTINCT signal label is the ONLY way the system health view separates killed from revoked
