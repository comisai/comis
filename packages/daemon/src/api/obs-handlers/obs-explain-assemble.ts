...
/**
 * `assembleIncidentReport` — the pure assembler.
 *
 ...
 *      the `sessionEnd` rollup (endReason / durationMs / totalTokens /
 ...
 * signals/metadata in a unit test (the clean TDD seam; the handler wires the
 * reader→normalize→assemble→heuristics→bound steps linearly).
 *
 * Two fields are deliberately left for downstream steps:
 *   - `likelyRootCause` stays `null` — the deterministic heuristic registry
 *     populates it.
 *   - `truncations` starts `[]` and `suggestedNextSteps` starts `[]` — the
 *     bounding pass records its lossiness ledger and the report-level guidance.
 *
 * @module
 */
 ...
 * no I/O, no LLM. `likelyRootCause` stays `null` (the heuristic registry fills
...
export function assembleIncidentReport(
  ...
  return {
    ...
    // subagent.budget_exceeded records — capSource names WHICH knob bound
    ...
    // The terminal per-call budget equation (absent for older sessions).
    ...
    // The per-reason cache breaks (absent when the session had none). Bounded to
    // CACHE_BREAKS_CAP highest-count-first; the bound pass (obs-explain-bound.ts)
    // records a truncations[] breadcrumb when it sheds the tail. Content-free
    // (counts + closed reason label + a number).
    ...(signals.cacheBreaks !== undefined && signals.cacheBreaks.length > 0
      ? { cacheBreaks: signals.cacheBreaks }
      : {}),
    // The spend kill-switch breach (scope + the two dollar numbers) reconstructed
    // from the terminal spend.exceeded record. Absent when the session was not
    // spend-killed (additive; schemaVersion 1). The verdict stays amount-free;
    // this section carries the numbers the Incident view renders.
    ...(signals.spend !== undefined ? { spend: signals.spend } : {}),
    // The image-generation turn reconstructed from the trajectory's image.*
    // records (absent when the session generated no image). The cost rides here so
    // `comis explain` shows it (Route a — NOT cost.costUsd, which reads the
    // executor sessionEnd, a different path).
    ...(signals.image !== undefined ? { image: signals.image } : {}),
    // The vision turn reconstructed from the trajectory's media.vision.* records
    // (absent when the session ran no vision). The vision cost rides here too
    // (Route a) — the image/vision folds are independent.
    ...(signals.vision !== undefined ? { vision: signals.vision } : {}),
    // The VIDEO turn reconstructed from the trajectory's video.* records (absent
    // when the session generated no video). Reconstructs a background-completed
    // job too — the in-turn video.submitted ties the later off-turn
    // video.generated via jobId/traceId on one sessionKey. Cost rides here
    // (Route a). The offline assembler is the binding oracle.
    ...(signals.videoGenerated !== undefined ? { videoGenerated: signals.videoGenerated } : {}),
    // The VOICE turn reconstructed from the trajectory's media.stt.* / media.tts.*
    // records (absent when the session ran no voice). Wholly in-turn (the daemon
    // voice RPC handlers direct-emit). Cost rides here (Route a): keyless
    // `costUsd:0` is VISIBLE, keyed cost is omitted. The offline assembler is the
    // binding oracle.
    ...(signals.voice !== undefined ? { voice: signals.voice } : {}),
    // The Verified-Learning outcome signal reconstructed from the trajectory's
    // learning.outcome_observed records (absent when the session recorded no
    // outcome). Counts/ids/closed-enums only; drives outcome_unresolved.
    ...(signals.learning !== undefined ? { learning: signals.learning } : {}),
    // The daemon-boot proxy posture read from the latest config_posture
    // obs_diagnostics row by the handler. Absent when no proxy configured (the
    // zero-config gate — the voice/learning spread mold). maskedUrl +
    // booleans/closed-enum strings ONLY — never a raw proxy URL.
    ...(signals.proxyPosture !== undefined ? { proxyPosture: signals.proxyPosture } : {}),
    summary,
    // The heuristic registry fills likelyRootCause; the bounding pass fills
    // truncations + the report-level suggestedNextSteps.
