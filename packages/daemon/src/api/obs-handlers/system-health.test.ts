...
/**
 * `obs.system.health` handler + assembler acceptance tests.
 *
 ...
 *      deterministic verdict fires when a system signal is present.
 *
 * @module
 */
...
import { systemDateFrom, systemNowMs, SystemHealthReportSchema } from "@comis/core";
import type { SystemHealthReport } from "@comis/core";
import { pipelineAuthoringGate } from "@comis/observability";
import { pipelineAuthoringAggregateFromRows } from "./system-findings.js";
import { initSchema, createObservabilityStore } from "@comis/memory";
import type { ObservabilityStore } from "@comis/memory";
import { createFakeClock } from "../../../../../test/support/fake-clock.js";
import { assembleSystemHealthReport, bindSystemHealthHandlers, pickWorstDegradedExecution } from "./system-health.js";
...
/** A session_started JSONL object (mirror system-session-index.test.ts). `ts`
...
/** A session_ended JSONL object (mirror system-session-index.test.ts). */
...
function makeDataDirWithActivity(): string {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "system-health-"));
...
describe("assembleSystemHealthReport (bounded read fan-in)", () => {
  ...
  it("surfaces the memory_lifecycle finding from forget-sweep rows (handler wiring)", async () => {
    ...
    expect(ml, "system-health must query category:memory_lifecycle + pass it to buildFindings").toBeDefined();
    expect(ml!.detail).toContain("evicted=1");
  });

  it("does NOT root-cause the system to recurring-health-WARNs from severity-info benign signals (session_rebase)", async () => {
    ...
    // healthSignalCount counted every row regardless, so a healthy system whose
    // only health rows were benign rebases root-caused to "recurring health
    // WARN signal(s)" — chronic noise ranked as the system's root cause.
    const now = systemNowMs();
    const store = makeStore();
    store.insertDiagnostic({
  ...
  it("still root-causes to recurring-health-WARNs from severity-warning signals (genuine divergence)", async () => {
    ...
    const report = await assembleSystemHealthReport({ obsStore: store, dataDir: makeDataDirWithActivity(), clock: createFakeClock(now) }, 24);
    expect(report.likelyRootCause?.code).toBe("system_recurring_health_signal");
  });
  ...
  it("does NOT root-cause to degradation when ALL degraded sessions merely delivered-with-tool-errors", async () => {
    ...
    expect(report.likelyRootCause?.code, "self-healed hiccups must not root-cause to high-degraded").not.toBe("system_high_degraded_rate");
    expect(report.likelyRootCause?.code, "delivered-with-tool-errors is not an acute event").not.toBe("system_acute_degradation");
  ...
  it("STILL root-causes to acute degradation on a genuine HARD failure amid delivered-with-tool-errors", async () => {
    ...
    expect(report.likelyRootCause?.code, "a genuine hard failure must still root-cause").toBe("system_acute_degradation");
  ...
  it("surfaces the learning_health finding from reflection-funnel rows (handler wiring)", async () => {
    ...
    const report = await assembleSystemHealthReport({ obsStore: store, dataDir: makeDataDirWithActivity(), clock: createFakeClock(now) }, 24);
    expect(report.findings.find((f) => f.code === "learning_health"), "system-health must query category:learning_health").toBeDefined();
  });

  // WIRING GUARD: assembleSystemHealthReport must QUERY category:cron_wake_gate,
  // compute the slice onto report.cronWakeGate, AND fold the benign rollup finding. The
  // reducer/finding unit tests prove they are BUILT from rows; this proves system-health
  ...
  it("surfaces the cronWakeGate slice + a benign cron_wake_gate_efficiency finding from wake-gate rows (handler wiring)", async () => {
    ...
    const report = await assembleSystemHealthReport({ obsStore: store, dataDir: makeDataDirWithActivity(), clock: createFakeClock(now) }, 24);
    // The slice is present (system-health queried the category + computed it).
    expect(report.cronWakeGate, "system-health must query category:cron_wake_gate + compute the slice").toBeDefined();
    ...
    expect(f, "system-health must pass cron_wake_gate rows to buildFindings").toBeDefined();
    expect(f!.count).toBe(2);
    // BENIGN: an info-severity gate fire does NOT inflate the system degrade count
  ...
  it("merges the session rollup + activity index + diagnostic findings onto SystemHealthReport (digest-only, with coverage)", async () => {
    const now = systemNowMs();
  ...
  it("is DETERMINISTIC: same data + same fakeClock -> byte-identical reports", async () => {
    ...
    const a = await assembleSystemHealthReport({ obsStore: store, dataDir, clock: createFakeClock(now) }, 24);
    const b = await assembleSystemHealthReport({ obsStore: store, dataDir, clock: createFakeClock(now) }, 24);
  ...
  it("CLOCK-INDEPENDENT: the session-index window follows the INJECTED clock, not real Date.now()", async () => {
    ...
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "system-health-fixedclock-"));
    const logsDir = path.join(dataDir, "logs");
    fs.mkdirSync(logsDir, { recursive: true });
    ...
    const report = await assembleSystemHealthReport({ obsStore: makeStore(), dataDir, clock: fixedClock }, 24);
  ...
  it("BOUNDS findings to the cap and records the drop in truncations[]", async () => {
    const now = systemNowMs();
    const store = makeStore();
    // Seed many DISTINCT health_signal labels so findings exceed SYSTEM_FINDINGS_CAP
    ...
    const dataDir = makeDataDirWithActivity();
    const report = await assembleSystemHealthReport({ obsStore: store, dataDir, clock: createFakeClock(now) }, 24);
  ...
  it("EMPTY: no store rows + no day-files -> a self-evidently empty report (not a clean zero)", async () => {
    const now = systemNowMs();
    const emptyDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "system-health-empty-"));
    const report = await assembleSystemHealthReport(
    ...
    // No system signal fired -> null verdict (a clean session has no root cause).
  ...
  it("HEURISTIC: acute named degradation outranks chronic config posture below the rate threshold", async () => {
    ...
    expect(report.likelyRootCause?.code).toBe("system_acute_degradation");
    expect(report.likelyRootCause?.detail).toContain("context_exhausted");
  ...
  it("HEURISTIC: a high degraded rate yields a deterministic likelyRootCause verdict", async () => {
    const now = systemNowMs();
    const store = makeStore();
    // Three sessions, all degraded -> degradedRate 1.0 (> the system threshold).
...
function emptyDataDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "system-served-below-"));
}

// ---------------------------------------------------------------------------
// The pipelineAuthoringGate verdict surfaced on the
// SystemHealthReport. Seeds `pipeline_authoring` health_signal rows, asserts the
...
// SystemHealthReportSchema.parse() (guards against schema/daemon drift).
// ---------------------------------------------------------------------------
...
describe("assembleSystemHealthReport — pipelineAuthoringGate verdict", () => {
  it("surfaces buildAuthor:true when >= 20 small-tier rows are materially below frontier — and the verdict round-trips .parse()", async () => {
    ...
    const report = await assembleSystemHealthReport({ obsStore: store, dataDir: emptyDataDir(), clock }, 24);
...
describe("buildFindings — config_posture:served_below_configured", () => {
  ...
  it("reads the LATEST row only (standing state, not cumulative) — a newer 0 suppresses an older 3; a newer 1 emits count 1", async () => {
    ...
    const reportSuppressed = await assembleSystemHealthReport(
    ...
    const reportLatest = await assembleSystemHealthReport(
...
// ---------------------------------------------------------------------------
// The two dedicated multilingual system findings.
...
// only — never raw WARN bodies (the system digest-only rule).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// The AUTONOMY block on the SystemHealthReport.
...
// breaker subset reads back from the synthetic-excluded reduceSystemWindow
...
describe("assembleSystemHealthReport — autonomy block", () => {
  ...
  // INVISIBLE to
  // `comis system-health`: its trip is never a session endReason and never a breakerTripCount,
  ...
  it("round-trips SystemHealthReportSchema.parse() with denialBreakerTrips (additive-optional, no drift)", async () => {
    const now = systemNowMs();
    const store = makeStore();
  ...
  it("names the worst autonomy run's rootRunId AND a verdict suggests `comis explain <rootRunId>`", async () => {
    ...
    // AND a SYSTEM verdict names it in a `comis explain <id>` suggestion (likelyRootCause
  ...
  it("admin gate + report shape unchanged: the report still round-trips SystemHealthReportSchema.parse() with the autonomy block", async () => {
    const now = systemNowMs();
    const store = makeStore();
...
describe("bindSystemHealthHandlers (admin dual-layer gate)", () => {
  it("admin gate: missing _trustLevel:admin throws", async () => {
    const handlers = bindSystemHealthHandlers(makeDeps({ obsStore: makeStore(), clock: createFakeClock(systemNowMs()) }));
    await expect(handlers["obs.system.health"]!({ sinceHours: 24 })).rejects.toThrow(/Admin/i);
    await expect(handlers["obs.system.health"]!({ sinceHours: 24, _trustLevel: "user" })).rejects.toThrow(/Admin/i);
  });

  it("stripInternalFields: _trustLevel never reaches the parsed params / report", async () => {
    const dataDir = makeDataDirWithActivity();
    const handlers = bindSystemHealthHandlers(
      makeDeps({ obsStore: makeStore(), dataDir, clock: createFakeClock(systemNowMs()) }),
    );
    const r = (await handlers["obs.system.health"]!({
      sinceHours: 24,
      _trustLevel: "admin",
    })) as SystemHealthReport & { _trustLevel?: unknown };
  ...
  it("applies the 24h default when sinceHours is omitted (handler-body default)", async () => {
    ...
    const r = (await handlers["obs.system.health"]!({ _trustLevel: "admin" })) as SystemHealthReport;
...
describe("bindSystemHealthHandlers (boot.clock wiring is load-bearing)", () => {
  it("the clock-wired handler returns a SystemHealthReport (NOT a clock-undefined throw)", async () => {
    const dataDir = makeDataDirWithActivity();
    const store = makeStore();
  ...
  it("throws when deps.clock is UNWIRED (proves the buildRpcDispatchDeps clock wiring is load-bearing)", async () => {
    ...
    await expect(
      handlers["obs.system.health"]!({ sinceHours: 24, _trustLevel: "admin" }),
    ).rejects.toThrow();
  });
});

describe("pickWorstDegradedExecution", () => {
  const row = (o: Partial<{ sessionKey: string; traceId: string; degraded: boolean; endReason: string; lastTs: number; source: string }>) => ({
    sessionKey: "s", traceId: "trace-s", degraded: false, endReason: "success", lastTs: 0, source: "runtime", ...o,
  });
  ...
  it("prefers a degraded session whose endReason matches the dominant cause", () => {
    ...
    expect(pickWorstDegradedExecution(rows, "context_exhausted")).toEqual({ sessionKey: "s-match", traceId: "trace-s", endReason: "context_exhausted" });
  });
  ...
  it("breaks ties on most-recent (lastTs desc) among matching causes", () => {
    ...
    expect(pickWorstDegradedExecution(rows, "context_exhausted")?.sessionKey).toBe("s-new");
  });
  ...
  it("falls back to the most-recent degraded session when none match the cause", () => {
    ...
    expect(pickWorstDegradedExecution(rows, "context_exhausted")?.sessionKey).toBe("s-b");
  });
  ...
  it("ignores synthetic and clean sessions; undefined when none degraded", () => {
    ...
    expect(pickWorstDegradedExecution(rows, "context_exhausted")).toBeUndefined();
