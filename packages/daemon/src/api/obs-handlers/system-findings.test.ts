...
// ---------------------------------------------------------------------------
// The dedicated multilingual system advisory (standing state).
//
...
describe("buildFindings — config_posture rollup names the flagged keys", () => {
  ...
  it("names skills.terminal.unsafeDisableSandbox when the bwrap jail is opted out", () => {
    ...
    // row to `warning` (build-config-posture-record.ts), but the system health view must NAME the
    // knob too — an operator triaging via `comis system-health` should not have to grep daemon.log
...
// ---------------------------------------------------------------------------
// The voice_health system finding.
//
...
// ---------------------------------------------------------------------------
// Orchestration observability — three dedicated system findings for the
...
describe("buildFindings — health_signal rollup counts only degraded (warning) rows", () => {
  it("does NOT surface a severity-info session_rebase row as an lcd_divergence finding (benign continuation, not degradation)", () => {
    ...
    // the system health view — but the findings rollup ignored severity and folded
...
// ---------------------------------------------------------------------------
// The config_posture:pricing_gap system finding.
//
...
// ---------------------------------------------------------------------------
// The dedicated orchestrate_efficiency system finding + its defensive extractor.
//
...
// auth flood becomes a counted `comis system-health` finding, symmetric with the
