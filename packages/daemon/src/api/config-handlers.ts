 ...
 * (`[ConfigReadContract.method]:`) so the bidirectional 1:1
 ...
 * admin trust check reads `rawParams._trustLevel` BEFORE the strip
 * step (the gate stays separate from the contract schema per D-04).
 *
 * The bespoke pre-Zod validation (admin gates, rate-limit, immutable
 * path detection, env-ref validation, suspicious-env warnings, MCP
 * duplicate-name detection, agent-credential resolution) is
 * intentionally retained for user-friendly error UX. The contract
 * parse runs AFTER and serves to (a) narrow params types for the
 * rest of the handler body and (b) provide a defense-in-depth gate
 * against future drift between the contract schema and the bespoke
 * checks. The dev-mode `Contract.response.parse(...)` gate before
 * each return doubles as a contract-drift canary.
 *
 * @module
 */
