...
export interface OrchestratorApiDeps {
  ...
  /** Phase 217-05 (BREAK-02): the per-rootRunId denial breaker the dispatch chokepoint drives — recordDenial ONLY on a CapabilityDeniedError floor-block, recordAllow on the allow branch; on the Nth consecutive floor-block it trips → execution:aborted{denial_breaker} + killByRootRun. Optional; absent ⇒ no breaker (pre-217 deny-without-abort, byte-identical). */ denialBreaker?: import("../autonomy/denial-breaker.js").DenialBreaker;
  /** Phase 217-05 (EVICT-01/03): the daemon-wide evicted-rootRunId set. The chokepoint READS isEvicted at the next gate decision to demote the run's mode to "default"; the autonomy.evict handler WRITES mark (it flows here via the dispatch deps, then into createAutonomyHandlers through the `...deps` spread). Optional; absent ⇒ no evict (the evict handler is not registered, byte-identical). */ evictRegistry?: import("../autonomy/evict-registry.js").EvictRegistry;
  /** Phase 217-05 (UNATT-03): the content-free escalate NotifyFn the chokepoint fires (NEVER awaited) on an unattended would-ask deny + on a breaker trip — out-of-band + auditable. Optional; absent ⇒ deny-without-escalate (pre-217, byte-identical). */ escalate?: import("../autonomy/durable-resume-engine.js").NotifyFn;
