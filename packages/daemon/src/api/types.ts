...
export interface SessionsApiDeps {
  ...
  /** Phase 69 SERVE-06: optional DeliveryQueuePort for the session.history
   *  handler's deliveryStatus join. Absent in deployments that have no
   *  delivery queue (no channel adapters) -- the handler then reports every
   *  message as confirmed (nothing pending to mark). The MCP resources/read
   *  surface (Plan 05) consumes the derived field to filter to CONFIRMED-only
   *  messages, but the field is also useful to the dashboard / observers. */
