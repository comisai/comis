...
import type { MemoryVisibilityRequest } from "@comis/core";
...
import { resolveRequestMemoryRecallScope, resolveRequestMemoryWriteScope } from "./memory-authority.js";
...
function parseMemoryVisibility(value: string): MemoryVisibilityRequest {
  switch (value) {
    case "conversation":
    ...
    case "agent-shared":
      return { kind: "agent-shared" };
    default:
      throw new ValidationError("Memory visibility must be conversation, principal, or agent-shared");
  }
}
...
export function createMemoryHandlers(deps: MemoryHandlerDeps): Record<string, RpcHandler> {
  return {
    [MemorySearchFilesContract.method]: async (rawParams) => {
      ...
      const tenantId = (rawParams._tenantId as string | undefined) ?? params.tenantId;
      const agentId = (rawParams._agentId as string | undefined) ?? params.agentId;
      if (!tenantId) {
        throw new AuthorizationError("Memory search requires explicit tenant authority");
      }
      if (!agentId) {
    ...
    [MemoryGetFileContract.method]: async (rawParams) => {
      ...
      const fileAgentId = rawParams._agentId as string | undefined;
      if (!fileAgentId) throw new AuthorizationError("Memory file read requires explicit agent authority");
      const fileWorkspaceDir = deps.workspaceDirs.get(fileAgentId);
      if (!fileWorkspaceDir) throw new AuthorizationError("Memory file read has no workspace for the authorized agent");
    ...
    [MemoryStoreContract.method]: async (rawParams) => {
      ...
      const storeTenantId = (rawParams._tenantId as string | undefined) ?? params.tenantId;
      if (!storeTenantId) {
        throw new AuthorizationError("Memory store requires explicit tenant authority");
      }
      const storeAgentId = (rawParams._agentId as string | undefined) ?? params.agentId;
      if (!storeAgentId) {
        throw new AuthorizationError("Memory store requires explicit agent authority");
      }
      ...
      const visibility = parseMemoryVisibility(params.visibility);
      const writeScope = resolveRequestMemoryWriteScope(
        storeTenantId,
        storeAgentId,
        visibility,
        isAdminCaller,
      );
      if (!writeScope.ok) throw new AuthorizationError(writeScope.error.message);
    ...
    [MemoryChangeVisibilityContract.method]: async (rawParams) => {
      if (rawParams._trustLevel !== "admin") {
        throw new AuthorizationError("Admin access required to change memory visibility");
      }
      const params = MemoryChangeVisibilityContract.request.parse(stripInternalFields(rawParams));
      const visibility = parseMemoryVisibility(params.visibility);
      const identity = resolveInternalTurnIdentity({
        tenantId: params.tenantId,
        agentId: params.agentId,
        originKind: "control-plane",
        instanceId: "memory-visibility",
        conversationId: `memory-visibility-${params.id}`,
        principalId: `control-plane-memory-visibility-${params.agentId}`,
      });
      if (!identity.ok) throw new AuthorizationError(identity.error.message);
      const changed = await deps.memoryApi.changeVisibility(params.id, {
      ...
      if (changed.value !== undefined) {
        deps.embeddingQueue?.enqueue(changed.value.id, changed.value.content);
      }
      const result = {
        changed: changed.value !== undefined,
        ...(changed.value !== undefined ? { id: changed.value.id } : {}),
      };
      if (systemGetEnv("NODE_ENV") !== "production") {
        MemoryChangeVisibilityContract.response.parse(result);
      }
      return result;
    },
    ...
    [MemoryStatsContract.method]: async (rawParams) => {
      ...
      const result = deps.memoryApi.stats(params.tenant_id, params.agent_id);
      if (systemGetEnv("NODE_ENV") !== "production") {
    ...
    [MemoryDeleteContract.method]: async (rawParams) => {
      ...
      let failCount = 0;
      for (const id of ids) {
        const result = await deps.memoryAdapter.delete(id, {
          tenantId: params.tenant_id,
          agentId,
        });
    ...
    [MemoryFlushContract.method]: async (rawParams) => {
      ...
      const count = deps.memoryApi.clear({ tenantId: params.tenant_id, agentId });

      const result = {
        flushed: true as const,
        entriesRemoved: count,
        scope: { tenantId: params.tenant_id, agentId },
      };
      if (systemGetEnv("NODE_ENV") !== "production") {
    ...
    [MemoryExportContract.method]: async (rawParams) => {
      ...
      const entries = deps.memoryApi.inspect({
        tenantId: params.tenant_id,
        agentId,
        limit,
        offset,
    ...
    // every query scoped to the request's explicit tenant and agent authority.
    ...
    [MemoryObservationsContract.method]: async (rawParams) => {
      ...
      const obsResult = await deps.consolidationStore.listObservations(
        agentId,
        params.tenant_id,
        limit,
      );
    ...
    [MemoryEntitiesContract.method]: async (rawParams) => {
      ...
      const entResult = await deps.entityStore.listEntities(agentId, params.tenant_id, limit);
      if (!entResult.ok) {
    ...
    [MemoryRecallStatsContract.method]: async (rawParams) => {
      ...
      const userParams = stripInternalFields(rawParams);
      const params = MemoryRecallStatsContract.request.parse(userParams);
      void params.tenant_id;
      void params.agent_id;
    ...
    [MemoryRecallTraceContract.method]: async (rawParams) => {
      ...
      // A bare `{records: []}` makes a disabled recorder indistinguishable
      // from "no recalls happened". Report the recorder gate, and when empty,
      // say why and name the knob that enables tracing.
      ...
      if (records.length === 0) {
        result.hint = tracingEnabled
          ...
          : "recall tracing is disabled for this runtime — no traces are being recorded; set diagnostics.recallTrace.enabled: true and re-run the session";
      }
      if (systemGetEnv("NODE_ENV") !== "production") {
