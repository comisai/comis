  ...
  async function fetchJson<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      const sanitized = safeBody
        ...
        .replace(/[A-Za-z0-9_-]{20,}/g, "[REDACTED]"); // eslint-disable-line no-restricted-syntax -- web-api-client error-body sanitization (predates Plan 45-02; not the Pino censor literal)
