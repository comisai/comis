 ...
 *   3. Scope columns — the twin row carries the conversation_ref + agent_id
...
function seedMessage(db: Database.Database, id: string, conversationRef: string, agentId: string): void {
  db.prepare(
  ...
  ).run(id, conversationRef, agentId);
}

/** Seed a minimal lcd_summaries row, return its summary_id. */
function seedSummary(db: Database.Database, summaryId: string, conversationRef: string, agentId: string, content: string): void {
  db.prepare(
  ...
  ).run(summaryId, conversationRef, agentId, content);
}

function triMatch(db: Database.Database, table: string, matchExpr: string, conversationRef: string, agentId: string): unknown[] {
  return db
    .prepare(`SELECT rowid FROM ${table} WHERE ${table} MATCH ? AND conversation_ref = ? AND agent_id = ?`)
    .all(matchExpr, conversationRef, agentId);
}
...
describe("createFtsPopulator — normalized trigram twin inserts", () => {
  ...
  it("populateMessageTri stores the FOLDED content so a folded MATCH hits and the RAW token does not", () => {
    ...
    pop.populateMessageTri("m1", textParts(HE_HASFARIM_RAW), { conversationRef: "conv", agentId: "agent" });
  ...
  it("insertSummaryTri stores the FOLDED summary content (folded MATCH hits, raw does not)", () => {
    ...
    const pop = createFtsPopulator(db);
    pop.insertSummaryTri("sum1", HE_HASFARIM_RAW, { conversationRef: "conv", agentId: "agent" });
  ...
  it("the twin row carries the conversation_ref + agent_id from the scope (a different agent's MATCH finds nothing)", () => {
    seedMessage(db, "m1", "conv", "agent-a");
    const pop = createFtsPopulator(db);
    pop.populateMessageTri("m1", textParts(HE_HASFARIM_RAW), { conversationRef: "conv", agentId: "agent-a" });
  ...
  it("populateMessageTri at the base rowid keeps the twin row joinable to lcd_messages", () => {
    ...
    const twinRow = db
      .prepare("SELECT rowid FROM lcd_messages_fts_tri WHERE conversation_ref = 'conv' AND agent_id = 'agent'")
...
describe("createFtsPopulator — null-handle no-op on a trigram-less host", () => {
  ...
  beforeEach(() => {
    ...
    noTri.exec(`
      ...
      CREATE VIRTUAL TABLE lcd_messages_fts USING fts5(
        content, conversation_ref UNINDEXED, agent_id UNINDEXED, message_id UNINDEXED, tokenize='porter unicode61'
      );
    `);
  });
  ...
  it("populateMessageTri / insertSummaryTri are clean no-ops on a trigram-less host (null handles)", () => {
    ...
    expect(() => pop.populateMessageTri("m1", textParts(HE_HASFARIM_RAW), { conversationRef: "conv", agentId: "agent" })).not.toThrow();
    expect(() => pop.insertSummaryTri("sum1", HE_HASFARIM_RAW, { conversationRef: "conv", agentId: "agent" })).not.toThrow();
  ...
  beforeEach(() => {
    ...
    partial.exec(`
      ...
      CREATE VIRTUAL TABLE lcd_messages_fts_tri USING fts5(
        content, conversation_ref UNINDEXED, agent_id UNINDEXED, message_id UNINDEXED, tokenize='trigram'
      );
