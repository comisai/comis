...
import { createManagedRunExternalReferenceLookup } from "./managed-run-external-reference.js";
...
export function createSqliteManagedRunStore(db: Database.Database): ManagedRunStorePort {
  ...
  const externalReference = createManagedRunExternalReferenceLookup(db);
  ...
  return Object.freeze({
    ...
    getByExternalRunRef: (scope, serviceInstanceId, externalRunRef, availability) => boundary(
      () => externalReference.get(scope, serviceInstanceId, externalRunRef, availability),
    ),
