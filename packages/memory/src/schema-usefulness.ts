 ...
 * outcome-attributed task-failure signal (FORGET-02) read by the memory
 ...
 *
 * Relocated here (Phase 224, v2.31) OUT of `schema-tuned-alpha.ts` — that file
 * is the bandit `tuned_alpha` migration and is DELETED when the bandit is cut.
 * This column is a KEEPER (the FORGET-02 source the lifecycle JOIN reads), so it
 ...
 * reverse DDL, no branch on an old shape (design §9, additive).
 *
