...
/**
 * Typed mapper for the `{ reason, count }` GROUP BY projection. `json_extract`
 * can return NULL (a row whose details lacks `$.reason`); coalesced to "" in SQL
 * but kept nullable here for defense-in-depth (a malformed row degrades to an
 * empty result via the untyped-sqlite-gate-mandated mapper, never an `as` cast).
 */
...
/**
 * PERSIST-01: "rate by reason over time" for detected prompt-cache breaks — a
 * clean `GROUP BY json_extract(details,'$.reason')` over the EXISTING
 * `obs_diagnostics` table + `idx_obs_diag_category` (no new table, no new index —
 * §14). Scoped to `category='cache_break'`; an optional `since`/`until` epoch-ms
 * window narrows the scan. Parameterized (the reason value is read out of the JSON
 * `details`, never interpolated into SQL — T-176-14).
 *
 * Standalone (not on the `ObservabilityStore` interface) — a focused analytics
 * read consumed by the fleet/explain surfaces; mirrors the `aggregate*` family's
 * GROUP-BY shape without bloating the interface.
 */
