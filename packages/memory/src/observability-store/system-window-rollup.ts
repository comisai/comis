...
/**
 * `reduceSystemWindow` — the PURE cross-session window-rollup reducer.
 *
 * Folds the per-session rollups (`SessionSummaryRollup[]`, produced by
 * `ObservabilityStore.aggregateSessionsInWindow`) into ONE system
 ...
 * inflate the operator-facing system metric. The filter acts on the REAL
 ...
 * errorKinds map is hard-capped at {@link SYSTEM_TOP_ERROR_KINDS_CAP}, sorted by
...
/**
 * How many merged ErrorKinds the system rollup keeps — hard cap, DoS-bounded.
 * Mirrors the per-session `TOP_ERROR_KINDS_CAP` (session-health-rollup.ts) so the
 * system aggregate cannot grow with failure volume across sessions.
 */
const SYSTEM_TOP_ERROR_KINDS_CAP = 3;

/**
 * How many distinct degradation CAUSES (`endReason` buckets) the system rollup's
 ...
 * because the named causes are the headline of the system detector ("N degraded by
 * context_exhausted, M by output_starved") and an operator wants the full spread.
 */
const SYSTEM_DEGRADED_BY_CAUSE_CAP = 10;
...
/**
 * The cross-session system aggregate produced by {@link reduceSystemWindow}.
 *
 ...
 * {@link SYSTEM_TOP_ERROR_KINDS_CAP}; `toolStats` sums each tool's ok/failed
 * across sessions.
 */
export interface SystemWindowRollup {
   ...
   * the numerator of {@link degradedRate}. Exposed so the system handler's
   ...
   * (the system-level degradation detector: "N degraded by context_exhausted, M
   * by output_starved"). ONLY degraded rows contribute; a missing/blank cause
   * folds into `"unknown"`. Capped at {@link SYSTEM_DEGRADED_BY_CAUSE_CAP},
   ...
   * {@link degradedByCause}), broken out so the system detector can report the HARD
   ...
   * with real failures (a system of delivered-with-tool-errors turns otherwise reads
...
/**
 * Reduce per-session A1 rollups into the system window aggregate.
 *
 ...
 * @returns the bounded, deterministic {@link SystemWindowRollup}.
 */
export function reduceSystemWindow(
  ...
  for (const r of kept) {
    if (r.degraded) {
      ...
      // than a hard failure; broken out so the system detector reports the hard rate.
    ...
    for (const [kind, n] of Object.entries(r.topErrorKinds)) {
      ...
      // the system handler, so it must not assume its caller validated. A
  ...
  const topErrorKinds: Record<string, number> = Object.fromEntries(
    [...mergedKinds.entries()]
      ...
      .slice(0, SYSTEM_TOP_ERROR_KINDS_CAP),
  );
  ...
  const degradedByCause: Record<string, number> = Object.fromEntries(
    [...causeCounts.entries()]
      .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
      .slice(0, SYSTEM_DEGRADED_BY_CAUSE_CAP),
  );

  const sessionCount = kept.length;
  return {
    sessionCount,
    // The absolute degraded count over the kept rows — the system handler
