...
/**
 * `reduceSystemWindow` — pure cross-session window-rollup reducer.
 *
 * The reducer folds the per-session rollups (`SessionSummaryRollup[]`) into a
 * single system aggregate: session count, degraded rate, merged +
...
import type { SystemWindowRollup } from "./system-window-rollup.js";
...
describe("reduceSystemWindow", () => {
  it("excludes synthetic rows from every aggregate — and the two flag-branches DIFFER (no-op-trap guard)", () => {
    ...
    // operator-facing system metric if it leaked through.
    ...
    const excluded = reduceSystemWindow([runtimeRow, syntheticRow], { excludeSynthetic: true });
    const included = reduceSystemWindow([runtimeRow, syntheticRow], { excludeSynthetic: false });
  ...
  it("excludes every non-runtime source value (test AND bench), not just one", () => {
    ...
    const out = reduceSystemWindow([runtimeRow, testRow, benchRow], { excludeSynthetic: true });
    // Only the single runtime row survives the filter.
    expect(out.sessionCount).toBe(1);

    // Without the filter, all three are counted.
    const all = reduceSystemWindow([runtimeRow, testRow, benchRow], { excludeSynthetic: false });
  ...
  it("merges topErrorKinds across sessions and caps the merged map to the top-N by summed count", () => {
    ...
    const out = reduceSystemWindow(rows, { excludeSynthetic: true });
  ...
  it("exposes the ABSOLUTE degradedCount over the KEPT (synthetic-excluded) rows, reconciling with degradedRate", () => {
    ...
    // so the system handler's `sessions.degraded` shares the synthetic-excluded
  ...
  it("returns degradedRate 0 (no divide-by-zero) when sessionCount is 0", () => {
    ...
    const empty = reduceSystemWindow([], { excludeSynthetic: true });
  ...
  it("coerces non-finite / non-number nested values to finite numbers (NaN/string corruption guard)", () => {
    ...
    // public export reachable directly by the system handler, so it must not
    ...
    const out = reduceSystemWindow([malformed, valid], { excludeSynthetic: true });
  ...
  it("emits toolStats keys in a deterministic (name-asc) order independent of input ordering", () => {
    ...
    const outA = reduceSystemWindow(rowsA, { excludeSynthetic: true });
    const outB = reduceSystemWindow(rowsB, { excludeSynthetic: true });
  ...
  it("is deterministic: same input rows → deeply-equal output, with ties broken by kind name", () => {
    ...
    const first: SystemWindowRollup = reduceSystemWindow(rows, { excludeSynthetic: true });
    const second: SystemWindowRollup = reduceSystemWindow(rows, { excludeSynthetic: true });
    ...
    const reversed = reduceSystemWindow([...rows].reverse(), { excludeSynthetic: true });
    expect(reversed).toEqual(first);
  });

  // ------------------------------------------------------------------------
  // degradedByCause: the system-level detector ("N sessions degraded
  ...
  it("counts `completed_with_tool_errors` sessions in deliveredWithToolErrorsCount (a SUBSET of degradedCount) — the user still got a reply", () => {
    // The live friction: a system of turns that DELIVERED a final answer despite a
    ...
    // failure (no reply). Split it so the system finding reports the HARD rate.
    const rows: SessionSummaryRollup[] = [
  ...
  it("deliveredWithToolErrorsCount is 0 when no session finished completed_with_tool_errors", () => {
    ...
    expect(reduceSystemWindow(rows, { excludeSynthetic: true }).deliveredWithToolErrorsCount).toBe(0);
  });
  ...
  it("excludes synthetic rows from degradedByCause (the metric-integrity filter)", () => {
    ...
    const excluded = reduceSystemWindow(rows, { excludeSynthetic: true });
    const included = reduceSystemWindow(rows, { excludeSynthetic: false });
