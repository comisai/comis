 ...
 *    this file): CREATE a `_new` table without it, `INSERT … SELECT` every other
...
export function initSchema(db: Database.Database, embeddingDimensions: number): { vecAvailable: boolean } {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS obs_token_usage (
      ...
      cache_saved REAL NOT NULL DEFAULT 0,
      latency_ms INTEGER NOT NULL,
      -- PERSIST-02 cost-correctness columns (already emitted on observability:token_usage;
      -- the tokenUsageEventToRow mapper + insertTokenUsageStmt write-path persist them).
      -- A fresh DB gets them here; ensureObsTokenColumns ALTERs an existing DB. The dead
      -- cache_retention column (never written by any event) is intentionally ABSENT (OQ3 DROP).
      warmup_turn INTEGER,
    ...
    CREATE INDEX IF NOT EXISTS idx_obs_token_session ON obs_token_usage(session_key, timestamp);
  `);
  // PERSIST-02: forward-only upgrade for EXISTING DBs (the CREATE above handles a
  // fresh DB). Drops the dead cache_retention column (guarded table-rebuild) and
  // adds the 5 cost-correctness columns (guarded ALTERs). Idempotent — a no-op on
  // an already-migrated DB. Called HERE, after the obs_token_usage CREATE, because
  // it operates on that table (not in the ensure-call sequence above, which runs
  // before this block).
  ensureObsTokenColumns(db);

  // --- Security audit events (AUDIT-01/02) ---
  // A DEDICATED table (not obs_diagnostics) per the §14 ruling — distinct retention
  // + actor/outcome/severity attribution columns + a regulatory grep surface. The
  // sink (Plan 03) resolves tenant_id from the trace context else persists the
  // '' system-scope sentinel (tenant_id is NOT NULL); agent_id stays NULL when
  // absent. refs is a scrubbed JSON blob. This plan owns the DDL + the row type;
  // the insert/query methods + JSONL writer land in Plan 03.
  db.exec(`
