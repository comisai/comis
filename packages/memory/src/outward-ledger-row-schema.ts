...
/**
 * Zod row schema for the `outward_send_ledger` table — the Phase-216 three-state
 * exactly-once outward-send ledger (ONCE-01..ONCE-04). SSOT for the file-internal
 ...
 *
 * SECURITY — CONTENT-FREE (T-216-11, mirrors durable_runs T-216-05 / video_jobs
 * T-189-02): the column set is `content_digest` (sha256) + routing/lifecycle +
 * the reconcile verdict ONLY. There is deliberately NO `body`/`text`/`message`
 * column and NO secret/token/bearer column — the strictObject REJECTS one if the
 * DDL ever adds it. The reconcile matches on the digest, never the message text.
 *
 * The `state` column re-enforces the closed five-member OutwardSendState union at
 * READ (belt-and-suspenders the SQL CHECK constraint + the Plan-01 type). Columns
 * MUST match the `ensureOutwardLedgerTable` DDL exactly.
 *
 * @module
 */
...
export const OutwardLedgerDbRowSchema = z.strictObject({
  id: z.string(),
  // The (root_run_id, step_index) idempotency key — the UNIQUE index pair (ONCE-02).
  ...
  // 'unresolved' — the SQL CHECK belt-and-suspenders the closed Plan-01 union.
  state: z.string(),
  // Set only once state='committed'; NULL on every in-flight / failed row.
  platform_message_id: z.string().nullable(),
  // sha256 ONLY — NEVER the message body (T-216-11). NOT NULL in the DDL.
