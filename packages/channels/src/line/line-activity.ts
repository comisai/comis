...
/**
 * LINE AppendOnly activity renderer (§7.2 / §18.3 row "AppendOnly").
 ...
 *      Pitfall 4; no invented rich classifier). Per §19.3, the wrapped "Failed
 ...
 *      finalize table. This is the signature the daemon wiring constructs (plus
 *      the optional `signCallbackData` injected later).
 *
 * Quick-Reply approval chips (§7.7): a `kind:"approval"` frame's opening
 ...
 * (Pitfall 5). When the signer is absent, an approval frame degrades to a
 ...
 * only and is never rendered or logged as activity text (§19.3).
 */
 ...
 * for elapsed display. The optional `deps.clock` (now FORWARDED per §8.5;
 * previously it was destructured-but-unused) feeds the strategy's elapsed-time
 ...
 * `timer` is still accepted-but-unused to stay structurally assignable to the
 ...
 * imports the orchestrator package (Pitfall 5). When omitted, an approval frame
 * degrades to a send-only status (the signer is wired in a later plan; the rest
 * of the renderer is unaffected).
 */
export function createLineActivityRenderer(
  ...
  return createAppendOnlyRenderer({
    ...
    // Elapsed-fallback wiring (§8.5): forward the daemon-injected ClockPort into
    // AppendOnly so the strategy can capture startedAtMs on the first apply() and
    // compute elapsedMs for renderFrameText's "(running N s)" fallback. Previously
    // this `clock` was declared on the deps shape but destructured-but-unused —
    // silent inert.
    clock,
