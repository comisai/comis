 ...
 */
export async function validateSignalConnection(
  opts: SignalValidateOpts,
): Promise<Result<SignalBotInfo, Error>> {
  if (!opts.baseUrl || opts.baseUrl.trim() === "") {
    return err(new Error("Invalid Signal credentials: baseUrl must not be empty"));
  }
  // Step 1: Health check
