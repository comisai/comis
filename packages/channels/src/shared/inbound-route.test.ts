...
//
// T0.27 cross-check (B30): inbound-route uses BackgroundSessionResolver for
// active-session lookup, NOT a direct activeRunRegistry.get(formattedKey).
//
// RED until Plan 15-03 (Phase 3) rewires inbound-route.ts:202 to call the
// resolver. The check is a source-grep: post-Phase-3, the file imports
// `resolveActiveSession` (or `BackgroundSessionResolver`) AND does NOT
// contain a literal `activeRunRegistry.get(`.
...
describe("inbound-route uses BackgroundSessionResolver for active-session lookup (B30 / T0.27)", () => {
  it("source-grep: imports the resolver and does NOT directly call activeRunRegistry.get(", () => {
    ...
    const stripped = src
      ...
      .join("\n");
    // Post-Phase-3: the file imports the resolver (one of two acceptable
    ...
    // Post-Phase-3: NO literal activeRunRegistry.get( in the production source.
