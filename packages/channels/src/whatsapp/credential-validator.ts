 ...
 */
export async function validateWhatsAppAuth(
  opts: WhatsAppValidateOpts,
): Promise<Result<{ authDir: string; isFirstRun: boolean }, Error>> {
  if (!opts.authDir || opts.authDir.trim() === "") {
    return err(new Error("Invalid WhatsApp credentials: auth directory must not be empty"));
  }
