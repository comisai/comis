 ...
 *
 * The live source-of-truth exports 3 values: `createAcpAgent`,
 * `startAcpServer` (from `acp-server.ts`) plus `createAcpSessionMap` (from
 * `acp-session-map.ts`). Type-only exports (`AcpServerDeps`, `AcpSessionMap`,
 * `AcpSessionKey`) do not appear in `Object.keys(acp)` at runtime and are
