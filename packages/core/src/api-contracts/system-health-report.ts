...
/**
 * The `obs.system.health` wire shape — the cross-session
 * system-health digest. DISTINCT from the per-session `IncidentReport`
 ...
 * `coverage` honesty block (which system sources were read). Same window → same
 ...
 * {@link ObsSystemHealthContract} in `OBSERVABILITY_CONTRACTS`.
 *
 * @module
 */
import { z } from "zod";
...
/**
 * The `obs.system.health` response — a bounded cross-session system digest.
 *
...
export const SystemHealthReportSchema = z.object({
  ...
  sessions: z.object({
     ...
     * deliveredWithToolErrors`) so a system of self-healed tool hiccups does not
  ...
  /**
   * The system-level degradation detector: degraded session COUNTS
   ...
   * and deterministic, computed by `reduceSystemWindow` from the per-session row's
  ...
  /**
   * READ-coverage breadcrumb (meta-observability): which SYSTEM sources the
   * handler actually located + read. DISTINCT from `truncations[]`
   * (SIZE-drops): `coverage` records whether the INPUTS were read, so a
   * silently-empty system report ("0 rows / N days missing") is self-evident
   ...
   *
   * The shape maps to the system aggregates: `sessionSummary` (session-summary
...
/**
 * `obs.system.health` — the cross-session system-health triage RPC.
 * Admin-only; the daemon handler fans the cross-session
 * aggregation + log-derived diagnostics into the {@link SystemHealthReport}
 ...
 */
export const ObsSystemHealthContract = defineContract({
