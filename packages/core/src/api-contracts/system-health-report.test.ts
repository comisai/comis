...
/**
 * Schema-shape contract test for the system-health lens.
 *
 * Pins the bounded/digest-only/deterministic `SystemHealthReportSchema` shape so
 * the `obs.system.health` handler (bounding pass + heuristic registry +
...
import { SystemHealthReportSchema, ObsSystemHealthContract } from "./system-health-report.js";
import type { SystemHealthReport } from "./system-health-report.js";
import { OBSERVABILITY_CONTRACTS } from "./observability.js";

/**
 * A fully-populated, well-formed system report (every required field present,
 * including the optional `coverage` honesty block). The canonical valid fixture.
 */
function validReport(): SystemHealthReport {
  return {
    schemaVersion: 1,
    windowHours: 24,
...
describe("SystemHealthReportSchema (bounded/deterministic system wire shape)", () => {
  it("parses a fully-populated, well-formed system report", () => {
  ...
  it("rejects a fixture missing a required field on that field's path", () => {
    ...
    const result = SystemHealthReportSchema.safeParse(fixture);
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it("treats `coverage` as optional (additive — schemaVersion stays 1)", () => {
    const withoutCoverage = validReport() as Partial<SystemHealthReport>;
    delete withoutCoverage.coverage;
    const parsed = SystemHealthReportSchema.parse(withoutCoverage);
    expect(parsed.coverage).toBeUndefined();
    expect(parsed.schemaVersion).toBe(1);

    // A well-formed coverage block still parses (the honesty block round-trips).
    const withCoverage = SystemHealthReportSchema.parse(validReport());
  ...
  it("the pipelineAuthoringGate verdict SURVIVES .parse() (the round-trip proof)", () => {
    ...
    // decision). Declaring it in SystemHealthReportSchema is what makes .parse()
    ...
    const parsedBuild = SystemHealthReportSchema.parse(buildReport);
  ...
  it("treats `pipelineAuthoringGate` as optional (additive — schemaVersion stays 1)", () => {
    ...
    const parsed = SystemHealthReportSchema.parse(without);
  ...
  it("the populated `autonomy` block SURVIVES .parse() (additive block round-trip)", () => {
    ...
    // SystemHealthReportSchema is what makes .parse() PRESERVE it.
    const withAutonomy = {
      ...validReport(),
      autonomy: {
    ...
    const parsed = SystemHealthReportSchema.parse(withAutonomy);
  ...
  it("treats `autonomy` as optional (additive — a report WITHOUT it still parses, schemaVersion stays 1)", () => {
    ...
    const parsedNoWorst = SystemHealthReportSchema.parse(noWorstId);
  ...
  it("the populated `cronWakeGate` block SURVIVES .parse() (round-trip; a 100%-skip agent is visible + net-cost legible)", () => {
    ...
    const parsed = SystemHealthReportSchema.parse(withGate);
  ...
  it("carries degradedByCause — a required bounded Record<cause, count> of named causes", () => {
    // The system-level degradation detector. Required (the assembler always emits
  ...
  it("is DISTINCT from IncidentReport (mirror, not extend): a per-session shape is rejected", () => {
    // A shape resembling a per-session IncidentReport (no system `sessions`
    // rollup; carries the session-only `failures`/`breakerTimeline`/`offloads`).
    // Because `sessions` (required) is absent, it must NOT parse as a system report —
    // pinning that SystemHealthReport is a DISTINCT shape, not an IncidentReport alias.
    const sessionShaped = {
      schemaVersion: 1,
    ...
    expect(SystemHealthReportSchema.safeParse(sessionShaped).success).toBe(false);

    // The inferred type exposes the SYSTEM keys, NOT the session-only ones.
...
/**
 * Contract-shape pins: the `obs.system.health` RPC contract is the
 * admin-scoped sibling of `obs.explain`. Without `ObsSystemHealthContract`
 ...
 */
describe("ObsSystemHealthContract (admin-scoped system RPC contract)", () => {
  it("declares the method `obs.system.health`", () => {
    expect(ObsSystemHealthContract.method).toBe("obs.system.health");
  });

  it("is admin-scoped (scopes deep-equals [\"admin\"])", () => {
    expect(ObsSystemHealthContract.scopes).toEqual(["admin"]);
  });
  ...
  it("has a refine-free, default-free request: `{}` and `{sinceHours:24}` parse, `{sinceHours:-1}` fails", () => {
    ...
    expect(ObsSystemHealthContract.request.parse({})).toEqual({});
    expect(ObsSystemHealthContract.request.parse({ sinceHours: 24 })).toEqual({ sinceHours: 24 });
    // .positive() rejects a non-positive window — proves the field validates without
    // a cross-field .refine (the system request has one optional field, no neither-id guard).
    expect(ObsSystemHealthContract.request.safeParse({ sinceHours: -1 }).success).toBe(false);
  });

  it("is registered in OBSERVABILITY_CONTRACTS (an entry whose method is obs.system.health)", () => {
    const methods = OBSERVABILITY_CONTRACTS.map((c) => c.method);
    expect(methods).toContain("obs.system.health");
  });
});
