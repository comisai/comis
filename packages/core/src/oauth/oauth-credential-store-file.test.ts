 ...
 * FileLockPort adapter). Core cannot depend on @comis/scheduler under
...
describe("createOAuthCredentialStoreFile", () => {
  ...
  describe("restart-survives-write", () => {
    ...
    it("a new adapter instance reads what the previous instance wrote", async () => {
      const store1 = createOAuthCredentialStoreFile({ dataDir: tmp, fileLock });
      ...
      const store2 = createOAuthCredentialStoreFile({ dataDir: tmp, fileLock });
  ...
  describe("profile-ID validation", () => {
    ...
    it.each([
      ...
      { name: "null byte", id: "openai-codex:bad\0identity" },
