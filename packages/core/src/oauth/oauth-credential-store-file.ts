...
export interface OAuthCredentialStoreFileConfig {
  ...
  dataDir: string;
  /**
   * Cross-process filesystem mutex used for per-profile-ID locking.
   * Injected by the composition root (`@comis/scheduler`'s `createFileLock()`
   * in production daemon + CLI wiring). The port is stateless, so a single
   * shared instance is safe across multiple store handles in the same
   * process.
   */
 ...
 *
 * `fileLock.withLock` uses `retries: 0` (the LockOptions default when
 * unspecified maps to proper-lockfile's retry budget; LOCK_OPTIONS for this
 * adapter does not set retries, so contention fast-fails) so two concurrent
 * same-profile writes within the SAME Node process would race the cross-
 * process file lock and the second caller would get `err({ kind: "locked" })`.
 * The in-process mutex serializes same-profile writes BEFORE they reach the
 * file lock — different profiles still proceed in parallel (each gets its own
 * queue), preserving per-profile lock granularity. Mirrors cron-store.ts:181.
 */
