...
**Referenced from:** the leading comment of `packages/daemon/src/rpc/secrets-handlers.ts` (load-bearing: the architecture test asserts the reference exists per MEM-CTX-PORTS-14 part 3).
...
- [ ] No closure captured by `Promise.all([...])` references a `/secret|decrypted|plaintext/i` binding from an outer scope (RES-PIT-31-1)
...
- [ ] Logger calls at INFO/WARN/ERROR/FATAL contain the secret NAME only (do NOT rely on Pino redaction as the primary defense — see RES-PIT-31-2)
...
- [ ] The positive-control deliberate-leak fixture (gated on `COMIS_RESIDENCY_TEST_DELIBERATE_LEAK=1`) still asserts the test would CATCH a leak (RES-PIT-31-2)
...
- [ ] `disableRedaction: true` does NOT appear in production source outside `test/integration/secret-rpc-residency.test.ts` (architecture test enforces — RES-PIT-31-4)
- [ ] No new admin-scope drift: every `/^secrets\./` method registered at `"admin"` (architecture test enforces — RES-PIT-31-3)

## G. Documentation alignment
- [ ] `AGENTS.md §2.7` (logging) is consistent with this handler's logging shape
- [ ] CLI help text matches the MEM-CTX-PORTS-10 pattern: `"Requires the comis daemon to be running."` on every store-backed secrets subcommand
