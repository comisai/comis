...
/**
 * The per-agent `learningOutcome` config.
 *
 * Like the surrounding `memory*` cost features, learningOutcome defaults ON
 * (`enabled:true`) — the outcome-learning loop works out of the box (opt-out).
 * The master kill switch `memory.enabled` (default true) is the real
 * gate; flipping it off force-disables the loop at the cron registration site.
 * `judge.enabled` also defaults ON (opt-out) — the LLM outcome judge is the
 ...
 * and derive no learning). It is STILL force-disabled by `memory.enabled:
 * false` and only runs on an `unknown` resolve (bounds its cost). The
...
describe("LearningOutcomeConfigSchema — per-agent outcome-signal config (default ON, opt-out)", () => {
  it("parse({}) yields the documented default-ON block (enabled:true, tool/pipeline sources, judge ON)", () => {
    ...
    // judge defaults ON (opt-out) — the conversational-turn fallback source.
    expect(cfg.judge.enabled).toBe(true);
    expect(cfg.minConfidenceToLearn).toBe(0.6);
    expect(cfg.retentionDays).toBe(90); // best-out-of-box: larger outcome corpus for reflection
  ...
  it("correction is a default-ON cost-gate sub-block (enabled:true — opt-out, gated by the master cost switch)", () => {
    // The correction detector runs an LLM over untrusted follow-up
    // text. It defaults ON (opt-out) so the loop works out of the box; the
    // master cost switch `memory.enabled` is the real gate.
    const cfg = LearningOutcomeConfigSchema.parse({});
...
/**
 * The `learning:outcome_observed` event payload.
 *
 * Counts/ids/closed-enums ONLY — never message bodies, alpha values, or
 * procedure contents. The strict event type is the compiler-level
