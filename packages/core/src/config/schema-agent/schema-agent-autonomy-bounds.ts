...
/**
 * Agent config — Autonomy BOUNDS sub-blocks (213-03).
 *
 * The four NESTED `z.strictObject` bounding blocks the daemon-side
 * `BoundedAutonomy` service (Plans 04/05/06/07) reads, split out of
 ...
 * (RESEARCH §D — A2): a `z.strictObject` per limb group, every field
 ...
 * the resolver merges the nested + the prior-flat fields per-field.
 *
...
/**
 * The §3.8 `standard` BOUNDS defaults — ON under every autonomy-bearing profile.
...
export interface AutonomyBoundsConfigInput {
  ...
  /** Prior-flat alias of `budget.aggregateUsd` (folded into it). */
  readonly aggregateBudgetUsd?: number;
  /** Prior-flat alias of `spawn.maxConcurrentSelfAgents` (folded into it). */
  readonly maxConcurrentSelfAgents?: number;
}

/**
 * Merge the nested BOUNDS blocks per-field against the profile `base` (the
 * `cfg?.lease?.leaseMaxTtlMin ?? base.leaseMaxTtlMin` model, RESEARCH §D). Each
 * limb reads the explicit nested field first, then — for the two aliased limbs —
 * the prior-flat field (`aggregateBudgetUsd` → `budget.aggregateUsd`;
 ...
 * alias, not a deprecated migration path — no compatibility shim). PURE.
 */
