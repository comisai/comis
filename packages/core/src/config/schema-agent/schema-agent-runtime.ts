...
export const ActivityConfigSchema = z.strictObject({
     ...
     *  A missing entry is treated as disabled — the rollout barrier inside the
     *  one-milestone ship. `enabled` defaults false so every renderer is off
     *  until explicitly enabled (kill-switch safety). */
    channels: z
