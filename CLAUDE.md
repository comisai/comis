...
- **Every bundled copy ships with no dependency fields, and everything it imports is a top-level `comisai` dependency.** npm counts a dependency of a bundled package as part of that bundle whenever it resolves into the bundling package's own `node_modules/` (arborist `Node#getBundler()` follows `edgesIn`). A **global** install nests every `comisai` dependency exactly there, so npm skips unpacking those packages — expecting the tarball to carry them — while still scheduling their lifecycle scripts; a script whose package was never unpacked runs with a missing cwd, which npm reports as the opaque `spawn sh ENOENT`. A **local** install hoists the same packages to the project root, outside the bundler, and succeeds — so this class is invisible to `pnpm validate`, to `npm install comisai`, and to the tarball smoke (which packs but never installs). `prepack.js` enforces both halves via `scripts/bundled-manifest.js` (fails the pack when a bundled package needs something the umbrella does not pin); `test/architecture/umbrella-bundled-metadeps.test.ts` is the test-time guard. Bundling `@earendil-works/pi-ai` with its dependency list intact is what broke every v1.0.55 global install.
- **Pinned versions must satisfy the peer ranges of the packages that declare them.** pnpm only *warns* on an unsatisfied peer, so the workspace stays green while `npm install` fails the same conflict with `ERESOLVE`. That breaks the installer's `repair_comisai_bundled_deps` pass — a plain `npm install` inside the installed package — which is what repairs npm's long-standing pruning of a non-bundled direct dep's transitive deps (the `@earendil-works/pi-tui` class). `test/architecture/npm-peer-consistency.test.ts` catches it whenever one manifest pins both the dependent and the peer; `@hono/node-server` bumped to 2.0.11 against `@hono/node-ws@1.3.1`'s `^1.19.11` peer is the case it was written for.
